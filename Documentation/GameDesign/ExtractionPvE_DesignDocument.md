...
Альтернативная реальность, где СССР не распался, а застыл в вечном "золотом застое" — технократической системе контроля. Игрок — Курьер, мародёр-паразит в теле мёртвого города Почтовый-47, выживающий за счёт **демонтажа компонентов** из автоматизированных систем. Атмосфера вдохновлена "Бегущим по лезвию 2049": вечный снег, созерцательная меланхолия, красота в распаде.

**Ключевая концепция: ЭКОСИСТЕМА РАСПАДА**
Город работает по инерции. Роботы выполняют последние приказы 40-летней давности — строят, чинят, охраняют — без смысла и без хозяев. Они не враги, они — функции. Конфликт возникает, когда цели игрока (извлечь компоненты для выживания) противоречат их программе.

**Мотивация игрока: ФИЗИОЛОГИЯ, НЕ ГЕРОИЗМ**
Курьер — модифицированный человек с экзо-каркасом, био-фильтрами, нейро-интерфейсом. Всё это требует обслуживания. Рейд — не приключение, а техобслуживание собственного тела. Если не найдёшь био-фильтр — умрёшь через 48 часов.

Задача: проникать в зоны, где работают автоматизированные системы, **демонтировать** компоненты из роботов и инфраструктуры, выбираться до истечения ресурса фильтра. Каждый рейд — выживание, не героизм.
...
│                            RAID (Рейд)                                  │
│  ┌──────────────────────────────────────────────────────────────────┐  │
│  │ INFILTRATION ──► OBSERVE ──► SALVAGE ──► EXTRACTION              │  │
│  │      │             │           │             │                    │  │
│  │      ▼             ▼           ▼             ▼                    │  │
│  │  Skill XP    Route Plan   Components   Filter +48hrs             │  │
│  └──────────────────────────────────────────────────────────────────┘  │
│                                                                         │
│  ПРЕПЯТСТВИЯ (не враги!):           ДОБЫЧА (для выживания):             │
│  - Укладчики (строят, игнорируют)   - Био-фильтры (критично!)           │
│  - Сервиторы (патрулируют, сканят)  - Батареи экзо-каркаса              │
│  - Утилизаторы (чистят, опасны)     - Охладители нейро-чипа             │
│  - Охранники (защита периметра)     - Сервоприводы                      │
│  - "Гнёзда" роботов (лут-зоны)      - Процессоры и чипы                 │
└───────────────────────────────┬─────────────────────────────────────────┘
                                │
                    ┌───────────┴───────────┐
                    ▼                       ▼
...
## Типы машин (Экосистема распада)

### Философия: Функция, не враг

**Критическое правило:** Роботы НЕ враги. Они — функции, выполняющие последние приказы 40-летней давности. Конфликт возникает, когда игрок мешает их программе. Они не "атакуют" — они "устраняют помеху" или "очищают зону".

### Типы машин и их поведение

| Тип | Функция | Отношение к игроку | Компоненты внутри |
|-----|---------|-------------------|-------------------|
| **Укладчик** | Тянет кабели, строит | Безразличие. Раздавит, если под ногами | Сервоприводы, батареи |
| **Сервитор** | Обход, мониторинг | Любопытство. Сканирует, отмечает "аномалию" | Процессоры, датчики |
| **Утилизатор** | Уборка, переработка | Равнодушие. Игрок в зоне очистки = "мусор" | Охладители, фильтры |
| **Охранник** | Защита периметра | Программа. Нарушение = устранение | Батареи, броня |
| **Строитель** | Возводит конструкции | Игнорирование. Строит поверх трупов | Сервоприводы, инструменты |

### "Гнёзда" — техногенные аномалии

Роботы за 40 лет без обновлений накопили **глитчи в программах**. Они создают странные структуры:
- Строители "чинят" стены, которые не сломаны — наращивая слои
- Утилизаторы собирают "мусор" в кучи — включая ценные детали
- Охранники патрулируют периметр, которого больше нет

**Визуальный эффект:** Органические наросты из бетона, металла, проводов. Красиво. Жутко. Источник редких компонентов.
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                      │
│  НЕПРАВИЛЬНО: Игрок входит → роботы "просыпаются" и атакуют        │
│                                                                      │
│  ПРАВИЛЬНО:   Игрок входит в цех. Укладчик тащит балку.            │
│               Сервитор сканирует стену. Строитель заваривает дыру. │
│               Никто не обращает на игрока внимания —               │
│               пока он не мешает.                                   │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```

### Живые противники (редко!)

| Тип | Оружие | HP | Поведение | Локации |
|-----|--------|----|-----------| ---------|
| **Смотритель** | Pistol/SMG | 100 | Патрулирует парами, вызывает подкрепление | Окраина — редко |
| **Охранник** | Pistol | 80 | Стоит на посту, может принять взятку | КПП, важные объекты |

**Примечание:** Живые люди — редкость. Большинство "гармонизированы" и работают как биороботы. Настоящие противники — исключение, не правило.

---

## Механика демонтажа (Salvage System)

### Почему "Демонтаж", а не "Лутинг"
...
│                                                                      │
│  ДЕМОНТАЖ (наш подход):                                             │
│  Игрок видит поверженного/спящего робота                           │
...
| **Спящий робот** | То же + редкие (био-помпы) | Высокая | Пробуждение! |
...
```cpp
// 1. ОСМОТР: UI подсвечивает доступные компоненты
// 2. ВЫБОР: Игрок решает, что извлекать
// 3. ИНСТРУМЕНТ: Правильный = быстро и тихо; Неправильный = долго, шумно
// 4. ИЗВЛЕЧЕНИЕ: Анимация, время, уязвимость
// 5. ПОСЛЕДСТВИЯ: Система фиксирует "падение узла" — реакция других машин
```

### Физиологические компоненты (критичные для выживания)

| Компонент | Функция | Таймер без него | Где искать |
|-----------|---------|-----------------|------------|
| **Био-фильтр** | Очистка крови от токсинов | 48 часов до смерти | Сервиторы-медики, "гнёзда" |
| **Батарея экзо** | Питание каркаса | Потеря скорости/силы | Укладчики, зарядные станции |
| **Охладитель нейро** | Охлаждение чипа | Глитчи зрения, галлюцинации | Утилизаторы, серверные |
| **Сервопривод** | Точность движений | Дрожание рук, потеря точности | Строители, краны |
...
*Created: 2025-12-27*
*Updated: 2026-01-20 — Философская переработка: "Экосистема распада" + механика демонтажа*

**Ключевые изменения v5.0:**
- **Новая философия:** "Экосистема распада" — роботы как инерция, не враги
- **Мотивация игрока:** Физиологическая зависимость (био-фильтры, экзо-каркас)
- **Демонтаж вместо лутинга:** Хирургическое извлечение компонентов
- **"Гнёзда":** Техногенные аномалии — глитчи роботов за 40 лет
- **Мир живёт без игрока:** Машины работают независимо от присутствия игрока
- Живые противники — редкость, большинство — автоматизированные системы
- Визуальная эстетика полностью в стиле "Бегущего по лезвию 2049"

**Ключевые изменения v4.0:**
- Сеттинг: "Почтовый-47" — закрытый город
- Hub переименован в "Убежище" (теплотрассы)
...
*Вдохновение: Blade Runner 2049, Escape from Tarkov, S.T.A.L.K.E.R.*
