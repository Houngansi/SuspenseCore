# Extraction PvE: "Сектор-17"

## Концепция

Альтернативная реальность, где СССР не распался, а трансформировался в технократическую диктатуру. Игрок — подпольщик-контрабандист, действующий в закрытых городских секторах, контролируемых тоталитарным режимом. Мир напоминает атмосферу Half-Life 2: мрачные мегаполисы, повсеместная слежка, механизированные силы подавления и бесчеловечная машина контроля.
...
**Сеттинг**: Пост-советская технократическая дистопия. Сращивание советской эстетики с киберпанковыми элементами. Бетонные жилмассивы под неоновыми экранами пропаганды, заброшенные индустриальные районы, подземные убежища сопротивления. Атмосфера Half-Life 2 + Escape from Tarkov + S.T.A.L.K.E.R. в альтернативном СССР.
...
│                                                                         │
│  УГРОЗЫ:                            НАГРАДЫ:                            │
│  - Полиция режима (метрокопы)       - Редкое оружие                    │
...
```cpp
// Типы заданий для Окраины — Сектор-17В
UENUM(BlueprintType)
...
|-----|--------|----|-----------| ---------|
| **Полицейский (Метрокоп)** | Pistol/SMG | 100 | Патрулирует парами, вызывает подкрепление | Окраина — везде |
...
|-----------------|-----------------|------------|
| **Green** | Патрули метрокопов (2 чел.) | 2-3 патруля |
...
{
    Patrol,          // Ходит по точкам (метрокопы, роботы)
...
// - Метрокоп увидел → "Стоять!" + 2 сек на сдачу, потом огонь
...
namespace SuspenseCoreTags
{
    ...
    namespace Enemy
    {
        UE_DECLARE_GAMEPLAY_TAG_EXTERN(Police);      // Метрокоп
...
| Basic AI (Полицейский-метрокоп) | TODO | P0 |
...
- Новый сеттинг: "Сектор-17" — закрытый город под контролем режима
- Убраны мутанты и аномалии
- Добавлены силы режима: полиция (метрокопы), ОМОН, охранники
