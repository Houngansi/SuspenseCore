...
void UMedComInventoryGASIntegration::Initialize(UAbilitySystemComponent* InASC)
{
    // Store reference
    ASC = InASC;
...
FActiveGameplayEffectHandle UMedComInventoryGASIntegration::ApplyItemEffect(FName ItemID, TSubclassOf<UGameplayEffect> EffectClass, float Level)
{
    if (!ASC || !EffectClass)
    {
...
FGameplayAbilitySpecHandle UMedComInventoryGASIntegration::GiveItemAbility(FName ItemID, TSubclassOf<UGameplayAbility> AbilityClass, int32 Level)
{
    if (!ASC || !AbilityClass)
    {
...
bool UMedComInventoryGASIntegration::RemoveItemEffect(FName ItemID, TSubclassOf<UGameplayEffect> EffectClass)
{
    if (!ASC || !EffectClass)
    {
        return false;
...
bool UMedComInventoryGASIntegration::RemoveItemAbility(FName ItemID, TSubclassOf<UGameplayAbility> AbilityClass)
{
    if (!ASC || !AbilityClass)
    {
        return false;
...
TArray<FActiveGameplayEffectHandle> UMedComInventoryGASIntegration::GetActiveItemEffects(FName ItemID) const
{
...
TArray<FGameplayAbilitySpecHandle> UMedComInventoryGASIntegration::GetActiveItemAbilities(FName ItemID) const
{
...
FActiveGameplayEffectHandle UMedComInventoryGASIntegration::ApplyWeightEffect(float MaxWeight, float CurrentWeight)
{
    if (!ASC)
    {
        return FActiveGameplayEffectHandle();
...
bool UMedComInventoryGASIntegration::UpdateWeightEffect(float NewCurrentWeight)
{
...
void UMedComInventoryGASIntegration::ClearAllItemEffects()
{
    if (!ASC)
    {
        return;
