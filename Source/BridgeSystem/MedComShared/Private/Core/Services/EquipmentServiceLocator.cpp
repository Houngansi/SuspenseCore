...
UEquipmentServiceLocator* UEquipmentServiceLocator::Get(const UObject* WorldContext)
{
    if (!WorldContext) return nullptr;
    const UWorld* World = WorldContext->GetWorld();
    if (!World) return nullptr;
    UGameInstance* GI = World->GetGameInstance();
    return GI ? GI->GetSubsystem<UEquipmentServiceLocator>() : nullptr;
}

void UEquipmentServiceLocator::Initialize(FSubsystemCollectionBase& Collection)
{
    ...
    if (UWorld* World = GetWorld())
    {
        World->GetTimerManager().SetTimer(
            CleanupTimer,
            this,
            &UEquipmentServiceLocator::PerformAutomaticCleanup,
            CleanupInterval,
            true);
    }
...
void UEquipmentServiceLocator::Deinitialize()
{
...
bool UEquipmentServiceLocator::RegisterServiceClass(const FGameplayTag& ServiceTag,
...
bool UEquipmentServiceLocator::RegisterServiceClassWithInjection(const FGameplayTag& ServiceTag,
                                                                 TSubclassOf<UObject> ServiceClass,
                                                                 const FServiceInitParams& InitParams,
...
bool UEquipmentServiceLocator::RegisterServiceInstance(const FGameplayTag& ServiceTag,
...
bool UEquipmentServiceLocator::RegisterServiceFactory(const FGameplayTag& ServiceTag,
...
bool UEquipmentServiceLocator::UnregisterService(const FGameplayTag& ServiceTag, bool bForceShutdown)
{
    FScopeLock _( &RegistryLock );
...
UObject* UEquipmentServiceLocator::GetService(const FGameplayTag& ServiceTag)
{
...
UObject* UEquipmentServiceLocator::TryGetService(const FGameplayTag& ServiceTag) const
{
    FScopeLock _( &RegistryLock );
...
bool UEquipmentServiceLocator::IsServiceRegistered(const FGameplayTag& ServiceTag) const
{
    FScopeLock _( &RegistryLock );
    return Registry.Contains(ServiceTag);
}

bool UEquipmentServiceLocator::IsServiceReady(const FGameplayTag& ServiceTag) const
{
    FScopeLock _( &RegistryLock );
    const FServiceRegistration* Reg = Registry.Find(ServiceTag);
...
int32 UEquipmentServiceLocator::InitializeAllServices()
{
    FScopeLock _( &RegistryLock );
...
int32 UEquipmentServiceLocator::ShutdownAllServices(bool bForce)
{
    FScopeLock _( &RegistryLock );
...
void UEquipmentServiceLocator::ResetAllServices()
{
    FScopeLock _( &RegistryLock );
...
EServiceLifecycleState UEquipmentServiceLocator::GetServiceState(const FGameplayTag& ServiceTag) const
{
    FScopeLock _( &RegistryLock );
    const FServiceRegistration* Reg = Registry.Find(ServiceTag);
...
FString UEquipmentServiceLocator::BuildDependencyGraph() const
{
    FScopeLock _( &RegistryLock );
...
bool UEquipmentServiceLocator::ValidateAllServices(TArray<FText>& OutErrors)
{
    OutErrors.Empty();
...
TArray<FGameplayTag> UEquipmentServiceLocator::GetRegisteredServices() const
{
    FScopeLock _( &RegistryLock );
...
TArray<FGameplayTag> UEquipmentServiceLocator::GetAllRegisteredServiceTags() const
{
...
bool UEquipmentServiceLocator::InitializeService(FServiceRegistration& Reg)
{
    ...
    if (IEquipmentService* Svc = static_cast<IEquipmentService*>(
...
bool UEquipmentServiceLocator::InjectServiceDependencies(FServiceRegistration& Reg)
{
...
bool UEquipmentServiceLocator::ShutdownService(FServiceRegistration& Reg, bool bForce)
{
    ...
    {
        IEquipmentService* Svc = static_cast<IEquipmentService*>(Reg.ServiceInstance->GetInterfaceAddress(UEquipmentService::StaticClass()));
...
UObject* UEquipmentServiceLocator::CreateServiceInstance(const FServiceRegistration& Reg)
{
    if (Reg.Factory)
    {
...
FServiceRegistration* UEquipmentServiceLocator::FindReg(const FGameplayTag& Tag)
{
    return Registry.Find(Tag);
}
const FServiceRegistration* UEquipmentServiceLocator::FindReg(const FGameplayTag& Tag) const
{
    return Registry.Find(Tag);
}

FGameplayTagContainer UEquipmentServiceLocator::GetRequiredDeps_NoLock(const FServiceRegistration& Reg) const
{
    ...
    if (Reg.ServiceClass && Reg.ServiceClass->ImplementsInterface(UEquipmentService::StaticClass()))
    {
        ...
        if (CDO)
        {
            IEquipmentService* Iface =
                static_cast<IEquipmentService*>(CDO->GetInterfaceAddress(UEquipmentService::StaticClass()));
            if (Iface)
            {
...
TArray<FGameplayTag> UEquipmentServiceLocator::TopoSort(const TArray<FGameplayTag>& Services) const
{
...
bool UEquipmentServiceLocator::HasCircular(const FGameplayTag& Tag, TSet<FGameplayTag>& Visited) const
{
...
void UEquipmentServiceLocator::PerformAutomaticCleanup()
{
    int32 Invalid = 0;
...
int32 UEquipmentServiceLocator::CleanupUnusedServices()
{
    int32 Removed = 0;
...
bool UEquipmentServiceLocator::ValidateServiceInstance(const UObject* ServiceInstance) const
{
