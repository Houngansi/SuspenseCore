...
FEquipmentEventBus::FEquipmentEventBus()
...
void FEquipmentEventBus::InitializeAutomaticCleanup()
{
    if (UWorld* World = GWorld)
    {
        ...
        World->GetTimerManager().SetTimer(
            CleanupTimerHandle,
            [WeakThis = TWeakPtr<FEquipmentEventBus>(AsShared())]()
            {
...
FEventSubscriptionHandle FEquipmentEventBus::Subscribe(
...
TArray<FEventSubscriptionHandle> FEquipmentEventBus::SubscribeMultiple(
...
bool FEquipmentEventBus::Unsubscribe(const FEventSubscriptionHandle& Handle)
{
    if (!Handle.IsValid())
    {
        return false;
...
int32 FEquipmentEventBus::UnsubscribeAll(UObject* Owner)
{
    if (!Owner)
    {
        return 0;
...
void FEquipmentEventBus::Broadcast(const FEquipmentEventData& EventData)
{
...
void FEquipmentEventBus::BroadcastDelayed(const FEquipmentEventData& EventData, float Delay)
{
    if (Delay <= 0.0f)
    {
        Broadcast(EventData);
    ...
    TArray<FEquipmentEventData>& EventsAtTime = DelayedEvents.FindOrAdd(ExecutionTime);
...
void FEquipmentEventBus::QueueEvent(const FEquipmentEventData& EventData)
{
    FScopeLock Lock(&EventQueueLock);
...
int32 FEquipmentEventBus::ProcessEventQueue(int32 MaxEvents)
{
    if (bProcessingQueue)
    {
...
void FEquipmentEventBus::ClearEventQueue(const FGameplayTag& EventType)
{
    FScopeLock Lock(&EventQueueLock);
    ...
    else
    {
        ...
        TArray<FEquipmentEventData> RemainingEvents;
        
        // Извлекаем все события из очереди
        while (EventQueueCount > 0)
        {
            FEquipmentEventData EventData = EventQueueArray[EventQueueHead];
            EventQueueHead = (EventQueueHead + 1) % MaxQueueSize;
            EventQueueCount--;
        ...
        for (const FEquipmentEventData& Event : RemainingEvents)
        {
            if (EventQueueArray.Num() <= EventQueueTail)
            {
...
void FEquipmentEventBus::SetEventFilter(const FGameplayTag& EventType, bool bAllow)
{
    FScopeLock Lock(&BusLock);
...
bool FEquipmentEventBus::SetSubscriptionEnabled(const FEventSubscriptionHandle& Handle, bool bEnabled)
{
    FScopeLock Lock(&BusLock);
...
FEquipmentEventBus::FEventBusStats FEquipmentEventBus::GetStatistics() const
{
    FScopeLock Lock(&BusLock);
...
void FEquipmentEventBus::ResetStatistics()
{
    FScopeLock Lock(&BusLock);
...
bool FEquipmentEventBus::ValidateBusIntegrity() const
{
    FScopeLock Lock(&BusLock);
...
int32 FEquipmentEventBus::CleanupInvalidSubscriptions()
{
    FScopeLock Lock(&BusLock);
...
void FEquipmentEventBus::PerformAutomaticCleanup()
{
...
void FEquipmentEventBus::DispatchEvent(const FEquipmentEventData& EventData, const TArray<FEventSubscription>& Subscriptions)
{
...
void FEquipmentEventBus::ExecuteHandler(const FEventSubscription& Subscription, const FEquipmentEventData& EventData)
{
...
void FEquipmentEventBus::ProcessDelayedEvents()
{
    FScopeLock Lock(&BusLock);
    ...
    for (auto& Pair : DelayedEvents)
    {
        ...
        {
            for (const FEquipmentEventData& EventData : Pair.Value)
            {
                Broadcast(EventData);
            }
...
void FEquipmentEventBus::SortByPriority(TArray<FEventSubscription>& Subscriptions) const
{
...
bool FEquipmentEventBus::PassesFilter(const FGameplayTag& EventType) const
{
    const bool* FilterValue = EventFilters.Find(EventType);
    return !FilterValue || *FilterValue; // Default to allow if not filtered
}

bool FEquipmentEventBus::IsOwnerAtSubscriptionLimit(UObject* Owner) const
{
    if (!Owner)
    {
        return false;
...
void FEquipmentEventBus::UpdateOwnerSubscriptionCount(UObject* Owner, int32 Delta)
{
    if (!Owner)
    {
        return;
...
FString FEquipmentEventBus::FEventBusStats::ToString() const
{
