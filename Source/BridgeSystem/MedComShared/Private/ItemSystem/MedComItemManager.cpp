...
void UMedComItemManager::Initialize(FSubsystemCollectionBase& Collection)
{
...
void UMedComItemManager::Deinitialize()
{
...
bool UMedComItemManager::LoadItemDataTable(UDataTable* ItemDataTable, bool bStrictValidation)
{
    ...
    if (RowStruct != FMedComUnifiedItemData::StaticStruct())
    {
        ...
        UE_LOG(LogMedComItemManager, Error, TEXT("  Expected: %s"), *FMedComUnifiedItemData::StaticStruct()->GetName());
        UE_LOG(LogMedComItemManager, Error, TEXT("  Got: %s"), RowStruct ? *RowStruct->GetName() : TEXT("nullptr"));
        UE_LOG(LogMedComItemManager, Error, TEXT("  Please ensure your DataTable uses FMedComUnifiedItemData row structure"));
        return false;
    }
...
bool UMedComItemManager::GetUnifiedItemData(const FName& ItemID, FMedComUnifiedItemData& OutItemData) const
{
    // Use helper method for cache statistics tracking
    const FMedComUnifiedItemData* FoundItem = GetCachedItemData(ItemID);
    if (FoundItem)
    {
...
bool UMedComItemManager::HasItem(const FName& ItemID) const
{
    return UnifiedItemCache.Contains(ItemID);
}

TArray<FName> UMedComItemManager::GetAllItemIDs() const
{
    TArray<FName> Result;
...
bool UMedComItemManager::CreateItemInstance(const FName& ItemID, int32 Quantity, FInventoryItemInstance& OutInstance) const
{
    ...
    OutInstance = FInventoryItemInstance::Create(ItemID, Quantity);
...
int32 UMedComItemManager::CreateItemInstancesFromSpawnData(const TArray<FPickupSpawnData>& SpawnDataArray, TArray<FInventoryItemInstance>& OutInstances) const
{
    OutInstances.Empty();
...
TArray<FName> UMedComItemManager::GetItemsByType(const FGameplayTag& ItemType) const
{
    TArray<FName> Result;
...
TArray<FName> UMedComItemManager::GetItemsByTags(const FGameplayTagContainer& Tags) const
{
    TArray<FName> Result;
...
TArray<FName> UMedComItemManager::GetItemsByRarity(const FGameplayTag& Rarity) const
{
    TArray<FName> Result;
...
TArray<FName> UMedComItemManager::GetEquippableItemsForSlot(const FGameplayTag& SlotType) const
{
    TArray<FName> Result;
    
    for (const auto& Pair : UnifiedItemCache)
    {
        const FMedComUnifiedItemData& ItemData = Pair.Value;
...
TArray<FName> UMedComItemManager::GetWeaponsByArchetype(const FGameplayTag& WeaponArchetype) const
{
    TArray<FName> Result;
...
TArray<FName> UMedComItemManager::GetCompatibleAmmoForWeapon(const FName& WeaponItemID) const
{
    TArray<FName> Result;
    
    // First get the weapon data to find its ammo type
    const FMedComUnifiedItemData* WeaponData = GetCachedItemData(WeaponItemID);
...
int32 UMedComItemManager::ValidateAllItems(TArray<FString>& OutErrorMessages) const
{
...
bool UMedComItemManager::ValidateItem(const FName& ItemID, TArray<FString>& OutErrorMessages) const
{
    OutErrorMessages.Empty();
    
    const FMedComUnifiedItemData* ItemData = GetCachedItemData(ItemID);
    if (!ItemData)
    {
...
void UMedComItemManager::GetCacheStatistics(FString& OutStats) const
{
    float HitRate = 0.0f;
...
bool UMedComItemManager::RefreshCache()
{
    if (!ItemTable)
    {
...
bool UMedComItemManager::CreateInventoryItemData(const FName& ItemID, int32 Quantity, FInventoryItemInstance& OutInstance) const
{
...
bool UMedComItemManager::TryLoadFallbackTable()
{
...
bool UMedComItemManager::BuildItemCache(bool bStrictMode)
{
    UnifiedItemCache.Empty();
    ValidItemCount = 0;
    ...
    {
        FMedComUnifiedItemData* ItemData = ItemTable->FindRow<FMedComUnifiedItemData>(RowName, TEXT("ItemManager::BuildItemCache"));
        if (ItemData)
        {
...
bool UMedComItemManager::ValidateItemInternal(const FName& ItemID, const FMedComUnifiedItemData& ItemData, TArray<FString>& OutErrors) const
{
    OutErrors.Empty();
...
void UMedComItemManager::LogCacheStatistics(int32 TotalItems, int32 WeaponItems, int32 ArmorItems, int32 ConsumableItems, int32 AmmoItems) const
{
...
const FMedComUnifiedItemData* UMedComItemManager::GetCachedItemData(const FName& ItemID) const
{
    const FMedComUnifiedItemData* FoundItem = UnifiedItemCache.Find(ItemID);
...
void UMedComItemManager::InitializeItemRuntimeProperties(FInventoryItemInstance& Instance, const FMedComUnifiedItemData& ItemData) const
{
