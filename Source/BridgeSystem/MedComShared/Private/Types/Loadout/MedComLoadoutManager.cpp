...
void UMedComLoadoutManager::Initialize(FSubsystemCollectionBase& Collection)
{
...
void UMedComLoadoutManager::Deinitialize()
{
...
int32 UMedComLoadoutManager::LoadLoadoutTable(UDataTable* InLoadoutTable)
{
    if (!InLoadoutTable)
    {
    ...
    if (UEventDelegateManager* EventManager = GetEventDelegateManager())
    {
...
void UMedComLoadoutManager::ReloadConfigurations()
{
    if (!LoadedDataTable)
    {
...
const FLoadoutConfiguration* UMedComLoadoutManager::GetLoadoutConfig(const FName& LoadoutID) const
{
    if (LoadoutID.IsNone())
    {
        return nullptr;
...
const FInventoryConfig* UMedComLoadoutManager::GetInventoryConfig(const FName& LoadoutID, const FName& InventoryName) const
{
...
bool UMedComLoadoutManager::GetLoadoutConfigBP(const FName& LoadoutID, FLoadoutConfiguration& OutConfig) const
{
...
bool UMedComLoadoutManager::GetInventoryConfigBP(const FName& LoadoutID, const FName& InventoryName, FInventoryConfig& OutConfig) const
{
    const FInventoryConfig* ConfigPtr = GetInventoryConfig(LoadoutID, InventoryName);
    if (ConfigPtr)
    {
        OutConfig = *ConfigPtr;
        return true;
    ...
    // Возвращаем дефолтную конфигурацию если не найдена
    OutConfig = FInventoryConfig();
    return false;
}

TArray<FName> UMedComLoadoutManager::GetInventoryNames(const FName& LoadoutID) const
{
...
TArray<FEquipmentSlotConfig> UMedComLoadoutManager::GetEquipmentSlots(const FName& LoadoutID) const
{
    const FLoadoutConfiguration* LoadoutConfig = GetLoadoutConfig(LoadoutID);
    if (!LoadoutConfig)
    {
...
bool UMedComLoadoutManager::IsLoadoutValid(const FName& LoadoutID) const
{
    const FLoadoutConfiguration* LoadoutConfig = GetLoadoutConfig(LoadoutID);
    return LoadoutConfig && LoadoutConfig->IsValid();
}

TArray<FName> UMedComLoadoutManager::GetAllLoadoutIDs() const
{
    FScopeLock Lock(&CacheCriticalSection);
...
TArray<FName> UMedComLoadoutManager::GetLoadoutsForClass(const FGameplayTag& CharacterClass) const
{
...
bool UMedComLoadoutManager::ApplyLoadoutToInventory(
    ...
    const FName& InventoryName) const
{
    ...
    IMedComInventoryInterface* InventoryInterface = Cast<IMedComInventoryInterface>(InventoryObject);
    if (!InventoryInterface)
    {
        UE_LOG(LogLoadoutManager, Error, TEXT("ApplyLoadoutToInventory: Object does not implement IMedComInventoryInterface"));
        return false;
    }

    const FInventoryConfig* Config = GetInventoryConfig(LoadoutID, InventoryName);
    if (!Config)
    {
...
bool UMedComLoadoutManager::ApplyLoadoutToEquipment(
    ...
    const FName& LoadoutID) const
{
    ...
    // Path 1: Check if object implements IMedComLoadoutInterface (preferred)
    if (EquipmentObject->GetClass()->ImplementsInterface(UMedComLoadoutInterface::StaticClass()))
    {
        FLoadoutApplicationResult Result = IMedComLoadoutInterface::Execute_ApplyLoadoutConfiguration(
            EquipmentObject, LoadoutID, const_cast<UMedComLoadoutManager*>(this), false);
    ...
    // Path 2: Check if object implements IMedComEquipmentInterface
    else if (EquipmentObject->GetClass()->ImplementsInterface(UMedComEquipmentInterface::StaticClass()))
    {
...
bool UMedComLoadoutManager::ApplyLoadoutToObject(
    ...
    bool bForceApply) const
{
    ...
    if (!LoadoutObject->GetClass()->ImplementsInterface(UMedComLoadoutInterface::StaticClass()))
    {
        UE_LOG(LogLoadoutManager, Error, TEXT("ApplyLoadoutToObject: Object does not implement IMedComLoadoutInterface"));
        return false;
    }

    // Cast to interface and apply
    TScriptInterface<IMedComLoadoutInterface> LoadoutInterface;
    LoadoutInterface.SetObject(LoadoutObject);
    LoadoutInterface.SetInterface(Cast<IMedComLoadoutInterface>(LoadoutObject));

    FLoadoutApplicationResult Result = IMedComLoadoutInterface::Execute_ApplyLoadoutConfiguration(
        LoadoutObject, LoadoutID, const_cast<UMedComLoadoutManager*>(this), bForceApply);
...
FName UMedComLoadoutManager::GetDefaultLoadoutForClass(const FGameplayTag& CharacterClass) const
{
    const FName* DefaultLoadout = ClassDefaultLoadouts.Find(CharacterClass);
    return DefaultLoadout ? *DefaultLoadout : NAME_None;
}

bool UMedComLoadoutManager::ValidateAllConfigurations(TArray<FString>& OutErrors) const
{
    OutErrors.Empty();
    bool bAllValid = true;
...
float UMedComLoadoutManager::GetTotalWeightCapacity(const FName& LoadoutID) const
{
    const FLoadoutConfiguration* LoadoutConfig = GetLoadoutConfig(LoadoutID);
    if (!LoadoutConfig)
    {
...
int32 UMedComLoadoutManager::GetTotalInventoryCells(const FName& LoadoutID) const
{
    const FLoadoutConfiguration* LoadoutConfig = GetLoadoutConfig(LoadoutID);
    if (!LoadoutConfig)
    {
...
void UMedComLoadoutManager::SetDefaultDataTablePath(const FString& Path)
{
...
void UMedComLoadoutManager::BroadcastLoadoutChange(const FName& LoadoutID, APlayerState* PlayerState, bool bSuccess) const
{
    // Broadcast through EventDelegateManager for global C++ listeners
    if (UEventDelegateManager* EventManager = GetEventDelegateManager())
    {
    ...
    const_cast<UMedComLoadoutManager*>(this)->OnLoadoutManagerChanged.Broadcast(LoadoutID, PlayerState, bSuccess);
}

int32 UMedComLoadoutManager::CacheConfigurationsFromTable()
{
    if (!LoadedDataTable)
    {
        return 0;
...
bool UMedComLoadoutManager::ValidateConfiguration(const FLoadoutConfiguration& Config, TArray<FString>& OutErrors) const
{
    bool bIsValid = true;
...
void UMedComLoadoutManager::ClearCache()
{
    CachedConfigurations.Empty();
}

void UMedComLoadoutManager::TryLoadDefaultTable()
{
    if (DefaultLoadoutTablePath.IsEmpty())
    {
        return;
...
void UMedComLoadoutManager::LogLoadoutStatistics() const
{
    FScopeLock Lock(&CacheCriticalSection);
...
UEventDelegateManager* UMedComLoadoutManager::GetEventDelegateManager() const
{
