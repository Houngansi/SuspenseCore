...
public:
	 ...
	 */
	FEquipmentCacheManager(int32 MaxEntries = 100)
	 ...
	 * @param MaxEntries Maximum number of cache entries before LRU eviction
	 */
	FEquipmentCacheManager(float InDefaultTTL, int32 MaxEntries = 100)
		: MaxCacheEntries(MaxEntries)
	...
	bool Set(const KeyType& Key, const ValueType& Value, float TTLSeconds = -1.0f)
	{
		...
		if (!CheckValueSize(Value))
		{
			...
			UE_LOG(LogTemp, Warning,
				TEXT("FEquipmentCacheManager: Rejected cache set due to size constraint (KeyHash=%u)"),
		...
		if (EnablePoisoningProtection)
		{
			...
			if (FreqData.IsExcessiveUpdateRate(MaxUpdateRatePerSecond))
			{
				...
				UE_LOG(LogTemp, Warning,
					TEXT("FEquipmentCacheManager: Rejected update due to excessive rate (KeyHash=%u, Rate=%d/s, Limit=%d/s)"),
			...
			if (ValidationFunc && !ValidationFunc(Key, Value))
			{
				RejectedEntries.fetch_add(1);
				UE_LOG(LogTemp, Warning,
					TEXT("FEquipmentCacheManager: External validation failed for cache entry (KeyHash=%u)"),
					EquipmentCacheHash::Compute(Key));
				return false;
			}
			...
			if (IsAnomalousNumericValue(Value))
			{
				...
				SuspiciousPatterns.fetch_add(1);
				UE_LOG(LogTemp, Warning,
					TEXT("FEquipmentCacheManager: Anomalous numeric value detected (KeyHash=%u)"),
					EquipmentCacheHash::Compute(Key));
				return false;
			}
		}
		...
		if (FCacheEntry<ValueType>* Entry = CacheMap.Find(Key))
		{
			if (!Entry->VerifyIntegrity())
			{
				...
				UE_LOG(LogTemp, Error,
					TEXT("FEquipmentCacheManager: Integrity check failed, entry removed (KeyHash=%u)"),
					EquipmentCacheHash::Compute(Key));
	...
	void SetValidationDelegate(FValidateEntryFunc<KeyType, ValueType> InFunc)
	{
		...
		UE_LOG(LogTemp, Verbose, TEXT("FEquipmentCacheManager: Validation function set"));
	}
...
// Specialized caches
using FTagQueryCache = FEquipmentCacheManager<FGameplayTag, bool>;
using FItemDataCache = FEquipmentCacheManager<FName, TSharedPtr<struct FInventoryItemInstance>>;