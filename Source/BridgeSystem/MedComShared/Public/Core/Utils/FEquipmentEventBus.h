 ...
 */
struct MEDCOMSHARED_API FEquipmentEventData
{
    ...
    virtual ~FEquipmentEventData()=default;
    ...
    FEquipmentEventData Clone() const
    {
        FEquipmentEventData Copy;
 ...
 */
DECLARE_DELEGATE_OneParam(FEventHandlerDelegate,const FEquipmentEventData&);
 ...
 */
class MEDCOMSHARED_API FEquipmentEventBus:public TSharedFromThis<FEquipmentEventBus>
{
public:
    FEquipmentEventBus();
    ~FEquipmentEventBus();

    static TSharedPtr<FEquipmentEventBus> Get()
    {
        static TSharedPtr<FEquipmentEventBus> Instance;
        if(!Instance.IsValid())
        {
            Instance=MakeShareable(new FEquipmentEventBus());
    ...
    void Broadcast(const FEquipmentEventData& EventData);
    ...
    struct FEventBusStats
    {
        ...
        MEDCOMSHARED_API FString ToString() const;
    };
...
private:
    ...
    void DispatchEvent(const FEquipmentEventData& EventData,const TArray<FEventSubscription>& Subscriptions);
    void ExecuteHandler(const FEventSubscription& Subscription,const FEquipmentEventData& EventData);
 ...
 */
class MEDCOMSHARED_API FEventSubscriptionScope
{
public:
    FEventSubscriptionScope(TSharedPtr<FEquipmentEventBus> InBus=nullptr)
        : EventBus(InBus?InBus:FEquipmentEventBus::Get())
    {}
...
#define BROADCAST_EQUIPMENT_EVENT(EventTag,SourceObj,TargetObj,PayloadStr) \
    { \
        FEquipmentEventData _EventData; \
        ...
        FEquipmentEventBus::Get()->Broadcast(_EventData); \
    }
...
#define QUEUE_EQUIPMENT_EVENT(EventTag,SourceObj,PayloadStr) \
    { \
        ...
        _EventData.Payload=PayloadStr; \
        _EventData.Timestamp=FPlatformTime::Seconds(); \
        FEquipmentEventBus::Get()->QueueEvent(_EventData); \
    }
