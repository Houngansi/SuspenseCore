...
#include "IMedComLoadoutInterface.generated.h"
...
struct MEDCOMSHARED_API FLoadoutApplicationResult
{
    GENERATED_BODY()
...
class UMedComLoadoutInterface:public UInterface
{
    GENERATED_BODY()
};

class MEDCOMSHARED_API IMedComLoadoutInterface
{
    GENERATED_BODY()

public:
    UFUNCTION(BlueprintNativeEvent,BlueprintCallable,Category="Loadout")
    FLoadoutApplicationResult ApplyLoadoutConfiguration(const FName& LoadoutID,UMedComLoadoutManager* LoadoutManager,bool bForceApply=false);

    UFUNCTION(BlueprintNativeEvent,BlueprintCallable,Category="Loadout")
    FName GetCurrentLoadoutID() const;

    UFUNCTION(BlueprintNativeEvent,BlueprintCallable,Category="Loadout")
    bool CanAcceptLoadout(const FName& LoadoutID,const UMedComLoadoutManager* LoadoutManager,FString& OutReason) const;
    ...
    static FLoadoutApplicationResult ApplyLoadoutToComponents(const TArray<TScriptInterface<IMedComLoadoutInterface>>& Components,const FName& LoadoutID,UMedComLoadoutManager* LoadoutManager,bool bStopOnFirstError=false);
    static void FindLoadoutComponents(AActor* Actor,TArray<TScriptInterface<IMedComLoadoutInterface>>& OutComponents,const FGameplayTag& ComponentTypeFilter=FGameplayTag());
    static bool IsLoadoutChangeSafe(const TArray<TScriptInterface<IMedComLoadoutInterface>>& Components,const FName& NewLoadoutID,const UMedComLoadoutManager* LoadoutManager,TArray<FString>& OutReasons);
};
