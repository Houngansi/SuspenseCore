...
#include "Types/Equipment/EquipmentTypes.h" // Единый источник истины для снапшотов и конфигов

#include "IMedComEquipmentDataProvider.generated.h"

// Делегаты
DECLARE_MULTICAST_DELEGATE_TwoParams(FOnSlotDataChanged, int32 /*SlotIndex*/, const FInventoryItemInstance& /*NewData*/);
...
class UMedComEquipmentDataProvider : public UInterface
{
    GENERATED_BODY()
};
 ...
 */
class MEDCOMSHARED_API IMedComEquipmentDataProvider
{
    GENERATED_BODY()
...
public:
    ...
    virtual FInventoryItemInstance GetSlotItem(int32 SlotIndex) const = 0;
    ...
    virtual TMap<int32, FInventoryItemInstance> GetAllEquippedItems() const = 0;
    ...
    virtual bool SetSlotItem(int32 SlotIndex, const FInventoryItemInstance& ItemInstance, bool bNotifyObservers = true) = 0;

    /** Очистить слот и вернуть удалённый предмет */
    virtual FInventoryItemInstance ClearSlot(int32 SlotIndex, bool bNotifyObservers = true) = 0;
    ...
    virtual float GetTotalEquippedWeight() const
    {
        ...
        for (int32 i = 0; i < Num; ++i)
        {
            ...
            const FInventoryItemInstance Item = GetSlotItem(i);
     ...
     */
    virtual bool MeetsItemRequirements(const FInventoryItemInstance& /*ItemInstance*/, int32 /*SlotIndex*/) const
    {
        return true;
    }
