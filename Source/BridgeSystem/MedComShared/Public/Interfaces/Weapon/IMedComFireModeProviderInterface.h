...
#include "IMedComFireModeProviderInterface.generated.h"
...
struct MEDCOMSHARED_API FFireModeRuntimeData
{
    GENERATED_BODY()
...
class UMedComFireModeProviderInterface : public UInterface
{
    GENERATED_BODY()
};
 ...
 * Работает в связке с IMedComWeaponInterface для полной функциональности
 * 
 * Архитектура:
 * - Режимы огня определяются в FMedComUnifiedItemData.FireModes
 * - Runtime состояние отслеживается через FFireModeRuntimeData
 * - Переключение режимов транслируется через EventDelegateManager
 */
class MEDCOMSHARED_API IMedComFireModeProviderInterface
{
    GENERATED_BODY()
...
public:
    ...
    bool InitializeFromWeaponData(const FMedComUnifiedItemData& WeaponData);
