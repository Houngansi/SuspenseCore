...
#include "MedComItemManager.generated.h"
...
public:
     ...
     * 
     * @param ItemDataTable DataTable with FMedComUnifiedItemData rows
     ...
     */
    UFUNCTION(BlueprintCallable, Category = "Item Manager")
    bool GetUnifiedItemData(const FName& ItemID, FMedComUnifiedItemData& OutItemData) const;
     ...
     */
    UFUNCTION(BlueprintCallable, Category = "Item Manager")
    int32 CreateItemInstancesFromSpawnData(const TArray<FPickupSpawnData>& SpawnDataArray, TArray<FInventoryItemInstance>& OutInstances) const;
    ...
    UFUNCTION(BlueprintCallable, Category = "Item Manager|Legacy", 
              meta = (DeprecatedFunction, DeprecationMessage = "Use CreateItemInstance instead. Legacy inventory data structures have been replaced with FInventoryItemInstance."))
    bool CreateInventoryItemData(const FName& ItemID, int32 Quantity, FInventoryItemInstance& OutInstance) const;
...
private:
    ...
    TMap<FName, FMedComUnifiedItemData> UnifiedItemCache;
     ...
     * @return True if item passes validation
     */
    bool ValidateItemInternal(const FName& ItemID, const FMedComUnifiedItemData& ItemData, TArray<FString>& OutErrors) const;
     ...
     */
    const FMedComUnifiedItemData* GetCachedItemData(const FName& ItemID) const;
 ...
 /**
     ...
     */
 void InitializeItemRuntimeProperties(FInventoryItemInstance& Instance, const FMedComUnifiedItemData& ItemData) const;
};