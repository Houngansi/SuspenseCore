...
#include "ISuspenseAbilityConnector.generated.h"
...
class USuspenseAbilityConnector : public UInterface
{
    GENERATED_BODY()
};

class BRIDGESYSTEM_API ISuspenseAbilityConnector
{
    GENERATED_BODY()

public:
    /** Initialize connector with ASC and data provider */
    virtual bool Initialize(class UAbilitySystemComponent* InASC, TScriptInterface<class ISuspenseEquipmentDataProvider> InDataProvider) = 0;

    /** Grant abilities for specific item */
    virtual TArray<FGameplayAbilitySpecHandle> GrantEquipmentAbilities(const struct FSuspenseInventoryItemInstance& ItemInstance) = 0;
    ...
    virtual TArray<FActiveGameplayEffectHandle> ApplyEquipmentEffects(const struct FSuspenseInventoryItemInstance& ItemInstance) = 0;
    ...
    virtual bool UpdateEquipmentAttributes(const struct FSuspenseInventoryItemInstance& ItemInstance) = 0;
