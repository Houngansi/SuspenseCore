...
UMedComEquipmentInventoryBridge::UMedComEquipmentInventoryBridge()
{
    PrimaryComponentTick.bCanEverTick = false;
}

void UMedComEquipmentInventoryBridge::BeginPlay()
{
    Super::BeginPlay();
}

void UMedComEquipmentInventoryBridge::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
bool UMedComEquipmentInventoryBridge::Initialize(
    ...
    TScriptInterface<IMedComTransactionManager> InTransactionMgr)
{
    ...
    if (UEquipmentServiceLocator* Locator = UEquipmentServiceLocator::Get(GetWorld()))
    {
    ...
    EventDelegateManager = GameInstance->GetSubsystem<UEventDelegateManager>();
...
void UMedComEquipmentInventoryBridge::SetInventoryInterface(TScriptInterface<IMedComInventoryInterface> InInventoryInterface)
{
...
void UMedComEquipmentInventoryBridge::HandleEquipmentOperationRequest(
    const FEquipmentOperationRequest& Request)
{
    ...
    FInventoryOperationResult InventoryResult;
    ...
    switch (Request.OperationType)
    {
        ...
        case EEquipmentOperationType::Unequip:
        {
            ...
            FInventoryItemInstance UnequippedItem = EquipmentDataProvider->GetSlotItem(Request.SourceSlotIndex);
        ...
        case EEquipmentOperationType::Drop:
        {
            ...
            const FInventoryItemInstance DroppedItem = EquipmentDataProvider->ClearSlot(
...
FInventoryOperationResult UMedComEquipmentInventoryBridge::ExecuteTransfer_FromEquipToInventory(
    const FInventoryTransferRequest& Request)
{
    ...
    FInventoryItemInstance EquippedItem = EquipmentDataProvider->GetSlotItem(Request.SourceSlot);
    ...
    if (!EquippedItem.IsValid())
    {
        ...
        return FInventoryOperationResult::Success(TEXT("TransferFromEquipToInventory"), nullptr);
    }
    ...
    if (TransactionManager.GetInterface())
    {
        ...
        if (EquipTxnId.IsValid())
        {
            ...
            if (TransactionManager->SupportsExtendedOps())
            {
                ...
                ClearOp.ItemAfter = FInventoryItemInstance();
            ...
            FInventoryItemInstance ClearedItem = EquipmentDataProvider->ClearSlot(Request.SourceSlot, true);
            ...
            FInventoryOperationResult AddResult = InventoryInterface->AddItemInstance(EquippedItem);
    ...
    FInventoryOperationResult Result = FInventoryOperationResult::Success(
...
FInventoryOperationResult UMedComEquipmentInventoryBridge::TransferFromInventory(const FInventoryTransferRequest& Request)
{
    return ExecuteTransfer_FromInventoryToEquip(Request);
}

FInventoryOperationResult UMedComEquipmentInventoryBridge::TransferToInventory(const FInventoryTransferRequest& Request)
{
    return ExecuteTransfer_FromEquipToInventory(Request);
}

FInventoryOperationResult UMedComEquipmentInventoryBridge::SwapBetweenInventoryAndEquipment(
...
void UMedComEquipmentInventoryBridge::SynchronizeWithInventory()
{
    ...
    const TMap<int32, FInventoryItemInstance> Equipped = EquipmentDataProvider->GetAllEquippedItems();
    const TArray<FInventoryItemInstance> InvenItems = InventoryInterface->GetAllItemInstances();

    TSet<FGuid> InventoryInstanceIds;
    InventoryInstanceIds.Reserve(InvenItems.Num());
    for (const FInventoryItemInstance& Ii : InvenItems)
    {
        InventoryInstanceIds.Add(Ii.InstanceID);
    }

    for (const TPair<int32, FInventoryItemInstance>& SlotIt : Equipped)
    {
        const int32 SlotIdx = SlotIt.Key;
        const FInventoryItemInstance& EquippedItem = SlotIt.Value;

        if (EquippedItem.IsValid() && !InventoryInstanceIds.Contains(EquippedItem.InstanceID))
        {
            FInventoryItemInstance Found;
...
bool UMedComEquipmentInventoryBridge::CanEquipFromInventory(const FInventoryItemInstance& Item, int32 TargetSlot) const
{
    ...
    if (UMedComEquipmentOperationExecutor* Executor = 
        Cast<UMedComEquipmentOperationExecutor>(EquipmentOperations.GetObject()))
    {
...
bool UMedComEquipmentInventoryBridge::CanUnequipToInventory(int32 SourceSlot) const
{
    if (!EquipmentDataProvider.GetInterface() || !InventoryInterface.GetInterface())
    {
        return false;
    ...
    FInventoryItemInstance EquippedItem = EquipmentDataProvider->GetSlotItem(SourceSlot);
    return InventoryHasSpace(EquippedItem);
}

// ===== Private: Transfer Implementations =====

FInventoryOperationResult UMedComEquipmentInventoryBridge::ExecuteTransfer_FromInventoryToEquip(
    const FInventoryTransferRequest& Request)
{
    const FInventoryItemInstance& Item = Request.Item;
    ...
    if (!InventoryInterface->HasItem(Item.ItemID, FMath::Max(1, Item.Quantity)))
    {
        ...
        return FInventoryOperationResult::ItemNotFound(TEXT("TransferFromInventory"), Item.ItemID);
    }
    ...
    {
        const FInventoryItemInstance OccupiedItem = EquipmentDataProvider->GetSlotItem(Request.TargetSlot);
    ...
    const FInventoryOperationResult RemoveRes = InventoryInterface->RemoveItemInstance(Item.InstanceID);
    ...
    if (TransactionManager.GetInterface())
    {
        ...
        if (TxnId.IsValid())
        {
            if (TransactionManager->SupportsExtendedOps())
            {
                ...
                EquipOp.ItemBefore = FInventoryItemInstance();
    ...
    FInventoryOperationResult SuccessResult = FInventoryOperationResult::Success(TEXT("TransferFromInventory"), nullptr);
    SuccessResult.AffectedItems.Add(Item);
    return SuccessResult;
}

void UMedComEquipmentInventoryBridge::BroadcastUnequippedEvent(
    ...
    int32 SlotIndex)
{
    ...
    TSharedPtr<FEquipmentEventBus> EventBus = FEquipmentEventBus::Get();
    if (EventBus.IsValid())
    {
...
FInventoryOperationResult UMedComEquipmentInventoryBridge::ExecuteSwap_InventoryToEquipment(
    ...
    int32 EquipmentSlot)
{
    ...
    FInventoryItemInstance InventoryItem;
    bool bFoundInInventory = false;
    
    const TArray<FInventoryItemInstance> AllItems = InventoryInterface->GetAllItemInstances();
    ...
    if (!bFoundInInventory)
    {
        ...
        RollbackBridgeTransaction(TransactionID);
        return FInventoryOperationResult::ItemNotFound(TEXT("SwapInventoryEquipment"), FName("Unknown"));
    }
    ...
    if (TransactionManager.GetInterface())
    {
        ...
        if (TransactionManager->SupportsExtendedOps())
        {
            ...
            if (EquippedItem.IsValid())
            {
                ...
                UnequipOp.ItemAfter = FInventoryItemInstance();
            ...
            EquipOp.ItemBefore = EquippedItem.IsValid() ? EquippedItem : FInventoryItemInstance();
            ...
            if (EquippedItem.IsValid())
            {
                ...
                EquipOp.SecondaryItemAfter = FInventoryItemInstance();
            }
        ...
        FInventoryOperationResult RemoveResult = InventoryInterface->RemoveItemInstance(InventoryInstanceID);
    ...
    FInventoryOperationResult Result = FInventoryOperationResult::Success(TEXT("SwapInventoryEquipment"), nullptr);
...
FGuid UMedComEquipmentInventoryBridge::BeginBridgeTransaction()
{
    FScopeLock Lock(&TransactionLock);
...
bool UMedComEquipmentInventoryBridge::CommitBridgeTransaction(const FGuid& TransactionID)
{
    FScopeLock Lock(&TransactionLock);
...
bool UMedComEquipmentInventoryBridge::RollbackBridgeTransaction(const FGuid& TransactionID)
{
    FScopeLock Lock(&TransactionLock);
...
bool UMedComEquipmentInventoryBridge::ValidateInventorySpace(const FInventoryItemInstance& Item) const
{
    if (!InventoryInterface.GetInterface())
    {
        return false;
...
bool UMedComEquipmentInventoryBridge::ValidateEquipmentSlot(int32 SlotIndex, const FInventoryItemInstance& Item) const
{
    if (!EquipmentDataProvider.GetInterface())
    {
        return false;
...
bool UMedComEquipmentInventoryBridge::InventoryHasSpace(const FInventoryItemInstance& Item) const
{
...
void UMedComEquipmentInventoryBridge::CleanupExpiredReservations()
{
...
bool UMedComEquipmentInventoryBridge::FindItemInInventory(const FName& ItemId, FInventoryItemInstance& OutInstance) const
{
    if (!InventoryInterface.GetInterface())
    {
        return false;
    ...
    const TArray<FInventoryItemInstance> Items = InventoryInterface->GetAllItemInstances();
    for (const FInventoryItemInstance& It : Items)
    {
        if (It.ItemID == ItemId)
        {
            OutInstance = It;
...
AActor* UMedComEquipmentInventoryBridge::ResolveCharacterTarget() const
{
...
void UMedComEquipmentInventoryBridge::BroadcastEquippedEvent(const FInventoryItemInstance& Item, int32 SlotIndex)
{
...
void UMedComEquipmentInventoryBridge::BroadcastSwapEvents(
    ...
    int32 SlotIndex)
{
    ...
    if (OldItem.IsValid())
    {
        ...
        if (UnequippedTag.IsValid())
        {
            ...
            FEquipmentEventBus::Get()->Broadcast(EvOut);
...
FSlotValidationResult UMedComEquipmentOperationExecutor::CanEquipItemToSlot(
