...
UMedComSystemCoordinator::UMedComSystemCoordinator()
{
    PrimaryComponentTick.bCanEverTick = false;
    SetIsReplicatedByDefault(false);
}

void UMedComSystemCoordinator::BeginPlay()
{
    Super::BeginPlay();
...
void UMedComSystemCoordinator::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
    Super::EndPlay(EndPlayReason);
}

void UMedComSystemCoordinator::Shutdown()
{
    UE_LOG(LogMedComCoordinator, Log, TEXT("=== Coordinator Shutdown START ==="));
    
    bBootstrapped = false;
    
    UEquipmentServiceLocator* Locator = GetLocator();
    if (Locator)
    {
...
UEquipmentServiceLocator* UMedComSystemCoordinator::GetLocator() const
{
    if (const UWorld* World = GetWorld())
    {
        if (UEquipmentServiceLocator* L = UEquipmentServiceLocator::Get(World))
        {
            return L;
        }
    }
...
FGameplayTag UMedComSystemCoordinator::GetServiceTagFromClass(UClass* ServiceClass) const
{
    if (!ServiceClass)
    {
...
bool UMedComSystemCoordinator::BootstrapServices()
{
    UEquipmentServiceLocator* Locator = GetLocator();
    if (!Locator)
    {
...
void UMedComSystemCoordinator::RegisterCoreServices()
{
    UEquipmentServiceLocator* Locator = GetLocator();
    if (!Locator) 
    {
    ...
    if (!Locator->IsServiceRegistered(TagData))
    {
        ...
        DataInjection.BindLambda([](UObject* ServiceInstance, UEquipmentServiceLocator* ServiceLocator)
        {
            if (!ServiceInstance)
            {
...
void UMedComSystemCoordinator::RegisterPresentationServices()
{
    UEquipmentServiceLocator* Locator = GetLocator();
    if (!Locator)
    {
...
void UMedComSystemCoordinator::WarmUpServices()
{
    UEquipmentServiceLocator* Locator = GetLocator();
    if (!Locator) return;
...
bool UMedComSystemCoordinator::ValidateServices(TArray<FText>& OutErrors) const
{
    OutErrors.Reset();

    UEquipmentServiceLocator* Locator = const_cast<UMedComSystemCoordinator*>(this)->GetLocator();
    if (!Locator)
    {
