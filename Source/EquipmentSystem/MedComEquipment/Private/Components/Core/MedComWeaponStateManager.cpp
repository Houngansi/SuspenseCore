...
UMedComWeaponStateManager::UMedComWeaponStateManager()
{
...
void UMedComWeaponStateManager::BeginPlay()
{
    Super::BeginPlay();
...
void UMedComWeaponStateManager::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void UMedComWeaponStateManager::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
bool UMedComWeaponStateManager::Initialize(
    TScriptInterface<IMedComEquipmentDataProvider> InDataProvider,
    TScriptInterface<IMedComEventDispatcher> InEventDispatcher)
{
    ...
    // Инициализация стейт-машин только для реальных оружейных слотов из вашего EEquipmentSlotType
...
FGameplayTag UMedComWeaponStateManager::GetWeaponState(int32 SlotIndex) const
{
    FScopeLock Lock(&StateMachineLock);
...
FWeaponStateTransitionResult UMedComWeaponStateManager::RequestStateTransition(const FWeaponStateTransitionRequest& Request)
{
    FScopeLock Lock(&StateMachineLock);
...
bool UMedComWeaponStateManager::CanTransitionTo(const FGameplayTag& FromState, const FGameplayTag& ToState) const
{
...
TArray<FGameplayTag> UMedComWeaponStateManager::GetValidTransitions(const FGameplayTag& CurrentState) const
{
...
bool UMedComWeaponStateManager::ForceState(const FGameplayTag& NewState, int32 SlotIndex)
{
    FScopeLock Lock(&StateMachineLock);
...
float UMedComWeaponStateManager::GetTransitionDuration(const FGameplayTag& FromState, const FGameplayTag& ToState) const
{
    const FStateTransitionDef* TransitionDef = FindTransitionDef(FromState, ToState);
    if (TransitionDef)
    {
...
bool UMedComWeaponStateManager::IsTransitioning(int32 SlotIndex) const
{
    FScopeLock Lock(&StateMachineLock);
...
float UMedComWeaponStateManager::GetTransitionProgress(int32 SlotIndex) const
{
    FScopeLock Lock(&StateMachineLock);
...
bool UMedComWeaponStateManager::AbortTransition(int32 SlotIndex)
{
    FScopeLock Lock(&StateMachineLock);
...
TArray<FGameplayTag> UMedComWeaponStateManager::GetStateHistory(int32 MaxCount) const
{
    FScopeLock Lock(&StateMachineLock);
...
void UMedComWeaponStateManager::RegisterTransition(const FStateTransitionDef& Transition)
{
    FScopeLock Lock(&StateMachineLock);
...
void UMedComWeaponStateManager::UpdateTransitions(float DeltaTime)
{
    FScopeLock Lock(&StateMachineLock);
...
void UMedComWeaponStateManager::CompleteTransition(int32 SlotIndex)
{
    FWeaponStateMachine* StateMachine = StateMachines.FindByPredicate(
        [SlotIndex](FWeaponStateMachine& SM) { return SM.SlotIndex == SlotIndex; });
...
FWeaponStateMachine& UMedComWeaponStateManager::GetOrCreateStateMachine(int32 SlotIndex)
{
...
const FStateTransitionDef* UMedComWeaponStateManager::FindTransitionDef(const FGameplayTag& FromState, const FGameplayTag& ToState) const
{
...
void UMedComWeaponStateManager::RecordStateChange(int32 SlotIndex, const FGameplayTag& NewState)
{
...
void UMedComWeaponStateManager::BroadcastStateChange(int32 SlotIndex, const FGameplayTag& OldState, const FGameplayTag& NewState)
{
