...
const TArray<FName> UEquipmentAttachmentComponent::WeaponSocketPriority = {
    FName("GripPoint"),
    FName("weapon_r"),
...
const TArray<FName> UEquipmentAttachmentComponent::ArmorSocketPriority = {
    FName("spine_03"),
    FName("spine_02"), 
    FName("pelvis"),
    FName("root")
...
const TArray<FName> UEquipmentAttachmentComponent::AccessorySocketPriority = {
    FName("head"),
    FName("neck_01"),
    FName("spine_03"),
    FName("pelvis")
...
UEquipmentAttachmentComponent::UEquipmentAttachmentComponent()
{
...
void UEquipmentAttachmentComponent::BeginPlay()
{
    Super::BeginPlay();
    ...
    if (bAutoLinkStanceComponent)
    {
        if (AActor* Owner = GetOwner())
        {
            ...
            {
                if (UMedComWeaponStanceComponent* StanceComp = OwnerPawn->FindComponentByClass<UMedComWeaponStanceComponent>())
                {
...
void UEquipmentAttachmentComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
    // Clear timers
    if (GetWorld())
    {
...
void UEquipmentAttachmentComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
void UEquipmentAttachmentComponent::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
    Super::GetLifetimeReplicatedProps(OutLifetimeProps);
    
    DOREPLIFETIME(UEquipmentAttachmentComponent, SpawnedEquipmentActor);
    DOREPLIFETIME(UEquipmentAttachmentComponent, bIsAttached);
    DOREPLIFETIME(UEquipmentAttachmentComponent, bIsInActiveState);
    DOREPLIFETIME(UEquipmentAttachmentComponent, CurrentSocketName);
    DOREPLIFETIME(UEquipmentAttachmentComponent, CurrentAttachmentOffset);
    DOREPLIFETIME(UEquipmentAttachmentComponent, CurrentWeaponType);
    DOREPLIFETIME(UEquipmentAttachmentComponent, bDidSpawnActor);
    DOREPLIFETIME(UEquipmentAttachmentComponent, AttachmentVersion);
    DOREPLIFETIME(UEquipmentAttachmentComponent, AnimationState);
}

void UEquipmentAttachmentComponent::InitializeWithItemInstance(AActor* InOwner, UAbilitySystemComponent* InASC, const FInventoryItemInstance& ItemInstance)
{
    ...
    if (OwnerActor)
    {
        ...
        if (OwnerActor->FindComponentByClass<UEquipmentMeshComponent>())
        {
...
void UEquipmentAttachmentComponent::Cleanup()
{
    // Detach first
    Detach(false);
...
void UEquipmentAttachmentComponent::UpdateEquippedItem(const FInventoryItemInstance& NewItemInstance)
{
    ...
    if (NewItemInstance.IsValid())
    {
        ...
        if (GetEquippedItemData(ItemData))
        {
            ...
            bool bIsPartOfEquipmentActor = GetOwner() && GetOwner()->FindComponentByClass<UEquipmentMeshComponent>() != nullptr;
...
bool UEquipmentAttachmentComponent::AttachToCharacter(AActor* Character, bool bUseActiveSocket, USceneComponent* ComponentToAttach)
{
    if (!Character)
    {
    ...
    if (!AttachComponent)
    {
        if (SpawnedEquipmentActor)
        {
            ...
            AttachComponent = SpawnedEquipmentActor->FindComponentByClass<UEquipmentMeshComponent>();
            if (!AttachComponent)
            {
...
void UEquipmentAttachmentComponent::Detach(bool bMaintainWorldTransform)
{
    // Handle client prediction
    if (!GetOwner()->HasAuthority())
    {
...
void UEquipmentAttachmentComponent::UpdateAttachmentState(bool bMakeActive, bool bAnimated)
{
    ...
    {
        USceneComponent* ComponentToMove = SpawnedEquipmentActor->FindComponentByClass<UEquipmentMeshComponent>();
        if (!ComponentToMove)
        {
...
void UEquipmentAttachmentComponent::PlayAttachmentAnimation(bool bToActive, float Duration)
{
    // Get animation interface
    TScriptInterface<IMedComWeaponAnimationInterface> AnimInterface = GetAnimationInterface();
    ...
    if (bToActive)
    {
        Montage = IMedComWeaponAnimationInterface::Execute_GetDrawMontage(
            AnimInterface.GetObject(), CurrentWeaponType, false);
    }
    else
    {
        Montage = IMedComWeaponAnimationInterface::Execute_GetHolsterMontage(
    ...
    if (GetWorld())
    {
        GetWorld()->GetTimerManager().SetTimer(
            ...
            this,
            &UEquipmentAttachmentComponent::OnAttachmentAnimationComplete,
            AnimDuration,
            false
        );
    }
...
TScriptInterface<IMedComWeaponAnimationInterface> UEquipmentAttachmentComponent::GetAnimationInterface() const
{
    // Check cache
    ...
    if (LinkedStanceComponent.IsValid())
    {
        TScriptInterface<IMedComWeaponAnimationInterface> StanceInterface = LinkedStanceComponent->GetAnimationInterface();
...
void UEquipmentAttachmentComponent::OnAttachmentAnimationComplete()
{
...
void UEquipmentAttachmentComponent::LinkStanceComponent(UMedComWeaponStanceComponent* StanceComponent)
{
...
void UEquipmentAttachmentComponent::NotifyStanceOfAttachment(bool bAttached)
{
    if (LinkedStanceComponent.IsValid())
    {
        if (bAttached)
...
FGameplayTag UEquipmentAttachmentComponent::GetWeaponTypeTag() const
{
    return CurrentWeaponType;
}

FName UEquipmentAttachmentComponent::GetAttachmentSocketName(bool bActive) const
{
    if (!HasEquippedItem())
    {
        return NAME_None;
...
FTransform UEquipmentAttachmentComponent::GetAttachmentOffset(bool bActive) const
{
    if (!HasEquippedItem())
    {
...
int32 UEquipmentAttachmentComponent::PredictAttachment(AActor* Character, bool bUseActiveSocket, const FName& SocketName, 
...
int32 UEquipmentAttachmentComponent::PredictDetachment()
{
    if (GetOwner()->HasAuthority())
    {
        return 0; // No prediction on server
...
void UEquipmentAttachmentComponent::ConfirmAttachmentPrediction(int32 PredictionKey, bool bSuccess)
{
    // Find prediction
...
void UEquipmentAttachmentComponent::ApplyPredictedAttachment(const FAttachmentPredictionData& Prediction)
{
...
void UEquipmentAttachmentComponent::RevertPredictedAttachment(const FAttachmentPredictionData& Prediction)
{
...
void UEquipmentAttachmentComponent::CleanupExpiredPredictions()
{
    if (!GetWorld())
    {
        return;
...
void UEquipmentAttachmentComponent::UpdateAnimationState(float DeltaTime)
{
    if (!AnimationState.bIsPlaying)
    {
        return;
...
FGameplayTag UEquipmentAttachmentComponent::GetWeaponArchetypeFromItem() const
{
    FMedComUnifiedItemData ItemData;
...
TArray<FSocketSearchResult> UEquipmentAttachmentComponent::GetValidSocketsForItem(const FMedComUnifiedItemData& ItemData, USkeletalMeshComponent* TargetMesh) const
{
...
bool UEquipmentAttachmentComponent::ValidateSocket(const FName& SocketName, USkeletalMeshComponent* TargetMesh) const
{
...
AActor* UEquipmentAttachmentComponent::SpawnEquipmentActor(const FMedComUnifiedItemData& ItemData)
{
    ...
    if (UEquipmentMeshComponent* MeshComp = NewActor->FindComponentByClass<UEquipmentMeshComponent>())
    {
...
void UEquipmentAttachmentComponent::DestroyEquipmentActor()
{
...
FName UEquipmentAttachmentComponent::FindBestAttachmentSocket(USkeletalMeshComponent* TargetMesh, const FMedComUnifiedItemData& ItemData, bool bForActive) const
{
    if (!TargetMesh)
    {
        return NAME_None;
...
FName UEquipmentAttachmentComponent::GetDefaultSocketForSlot(const FGameplayTag& SlotType, bool bForActive) const
{
    if (bForActive)
    {
...
void UEquipmentAttachmentComponent::ApplyAttachment(USceneComponent* ComponentToAttach, USceneComponent* TargetComponent, 
...
void UEquipmentAttachmentComponent::BroadcastAttachmentEvent(bool bAttached, AActor* Character, const FName& SocketName)
{
...
USkeletalMeshComponent* UEquipmentAttachmentComponent::GetCharacterMesh(AActor* Character) const
{
    if (!Character)
    {
        return nullptr;
    ...
    {
        if (Mesh && !Mesh->IsA<UEquipmentMeshComponent>())
        {
            return Mesh;
        }
    }
...
void UEquipmentAttachmentComponent::UpdateReplicatedAttachmentState()
{
    if (!GetOwner() || !GetOwner()->HasAuthority())
    {
        return;
...
void UEquipmentAttachmentComponent::OnRep_AttachmentState()
{
...
void UEquipmentAttachmentComponent::OnRep_SpawnedEquipmentActor()
{
    ...
    {
        if (UEquipmentMeshComponent* MeshComp = SpawnedEquipmentActor->FindComponentByClass<UEquipmentMeshComponent>())
        {
            MeshComp->InitializeFromItemInstance(EquippedItemInstance);
        }
    }
...
void UEquipmentAttachmentComponent::OnRep_AnimationState()
{
...
void UEquipmentAttachmentComponent::ServerRequestAttachment_Implementation(AActor* Character, bool bUseActiveSocket, FName RequestedSocket, 
...
bool UEquipmentAttachmentComponent::ServerRequestAttachment_Validate(AActor* Character, bool bUseActiveSocket, FName RequestedSocket, 
                                                                    FTransform RequestedOffset, FGameplayTag WeaponType, int32 PredictionKey)
{
    return Character != nullptr && PredictionKey > 0;
}

void UEquipmentAttachmentComponent::ServerRequestDetachment_Implementation(bool bMaintainTransform, int32 PredictionKey)
{
...
bool UEquipmentAttachmentComponent::ServerRequestDetachment_Validate(bool bMaintainTransform, int32 PredictionKey)
{
    return PredictionKey > 0;
}

void UEquipmentAttachmentComponent::ClientConfirmAttachment_Implementation(int32 PredictionKey, bool bSuccess, FName ActualSocket, FTransform ActualOffset)
{
...
void UEquipmentAttachmentComponent::MulticastAttachment_Implementation(AActor* Actor, USceneComponent* Parent, FName Socket, 
                                                                      FTransform Offset, FGameplayTag WeaponType, bool bAnimated)
{
    ...
    USceneComponent* ComponentToAttach = Actor->FindComponentByClass<UEquipmentMeshComponent>();
...
void UEquipmentAttachmentComponent::MulticastDetachment_Implementation(AActor* Actor, bool bMaintainTransform)
{
    if (!Actor)
    {
        return;
...
void UEquipmentAttachmentComponent::OnEquipmentInitialized()
{
...
void UEquipmentAttachmentComponent::OnEquippedItemChanged(const FInventoryItemInstance& OldItem, const FInventoryItemInstance& NewItem)
{
