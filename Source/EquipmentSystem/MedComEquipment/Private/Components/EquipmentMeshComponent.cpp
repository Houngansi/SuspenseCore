...
const FName UEquipmentMeshComponent::DefaultMuzzleSocket = FName("Muzzle");
const FName UEquipmentMeshComponent::DefaultScopeSocket = FName("Scope");
const FName UEquipmentMeshComponent::DefaultMagazineSocket = FName("Magazine");

UEquipmentMeshComponent::UEquipmentMeshComponent()
{
...
void UEquipmentMeshComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
    CleanupVisuals();
...
void UEquipmentMeshComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
bool UEquipmentMeshComponent::InitializeFromItemInstance(const FInventoryItemInstance& ItemInstance)
{
...
void UEquipmentMeshComponent::UpdateVisualState(const FInventoryItemInstance& ItemInstance)
{
...
void UEquipmentMeshComponent::CleanupVisuals()
{
...
void UEquipmentMeshComponent::ApplyVisualState(const FEquipmentVisualState& NewState, bool bForceUpdate)
{
    FScopeLock Lock(&VisualStateCriticalSection);
...
bool UEquipmentMeshComponent::HasVisualStateChanged(const FEquipmentVisualState& OtherState) const
{
    FScopeLock Lock(&VisualStateCriticalSection);
    return CurrentVisualState != OtherState;
}

void UEquipmentMeshComponent::NotifyVisualStateChanged()
{
    ...
    TArray<UEquipmentComponentBase*> EquipmentComponents;
    Owner->GetComponents<UEquipmentComponentBase>(EquipmentComponents);
    ...
    for (UEquipmentComponentBase* EquipComp : EquipmentComponents)
    {
    ...
    UE_LOG(LogTemp, VeryVerbose, TEXT("UEquipmentMeshComponent: Notified %d equipment components about visual state change (version: %d)"),
        EquipmentComponents.Num(), CurrentVisualState.StateVersion);
}

void UEquipmentMeshComponent::RequestStateSync()
{
...
void UEquipmentMeshComponent::InitializeVisualComponents(const FMedComUnifiedItemData& ItemData)
{
...
bool UEquipmentMeshComponent::LoadMeshFromItemData(const FMedComUnifiedItemData& ItemData)
{
...
void UEquipmentMeshComponent::CreateDynamicMaterials()
{
    FScopeLock Lock(&VisualStateCriticalSection);
...
void UEquipmentMeshComponent::UpdateDynamicMaterials()
{
    FScopeLock Lock(&VisualStateCriticalSection);
...
void UEquipmentMeshComponent::SetupWeaponVisuals(const FMedComUnifiedItemData& WeaponData)
{
...
FVector UEquipmentMeshComponent::GetSocketLocationSafe(const FName& SocketName) const
{
...
FRotator UEquipmentMeshComponent::GetSocketRotationSafe(const FName& SocketName) const
{
    if (DoesSocketExist(SocketName))
    {
...
FTransform UEquipmentMeshComponent::GetSocketTransformSafe(const FName& SocketName) const
{
    if (DoesSocketExist(SocketName))
    {
...
void UEquipmentMeshComponent::ApplyOffsetTransform(const FTransform& Offset)
{
...
FVector UEquipmentMeshComponent::GetMuzzleLocation() const
{
...
FVector UEquipmentMeshComponent::GetMuzzleDirection() const
{
    FName MuzzleSocket = CachedItemData.MuzzleSocket.IsNone() ? DefaultMuzzleSocket : CachedItemData.MuzzleSocket;
...
int32 UEquipmentMeshComponent::PlayMuzzleFlash()
{
    // Create prediction
...
void UEquipmentMeshComponent::SetupScopeCamera(float FOV, bool bShouldAutoActivate)
{
    if (!ScopeCamera)
    {
...
void UEquipmentMeshComponent::SetScopeCameraActive(bool bActivate)
{
    if (ScopeCamera)
    {
...
void UEquipmentMeshComponent::SetConditionVisual(float ConditionPercent)
{
    FScopeLock Lock(&VisualStateCriticalSection);
...
void UEquipmentMeshComponent::SetRarityVisual(const FGameplayTag& RarityTag)
{
    FScopeLock Lock(&VisualStateCriticalSection);
...
int32 UEquipmentMeshComponent::PlayEquipmentEffect(const FGameplayTag& EffectType)
{
    // Create prediction
    FVisualEffectPrediction Prediction;
    Prediction.PredictionKey = NextPredictionKey++;
...
void UEquipmentMeshComponent::ConfirmEffectPrediction(int32 PredictionKey, bool bSuccess)
{
...
void UEquipmentMeshComponent::SetMaterialParameter(const FName& ParameterName, float Value)
{
    FScopeLock Lock(&VisualStateCriticalSection);
...
void UEquipmentMeshComponent::SetMaterialColorParameter(const FName& ParameterName, const FLinearColor& Color)
{
    FScopeLock Lock(&VisualStateCriticalSection);
...
FName UEquipmentMeshComponent::GetAttachmentSocket(const FGameplayTag& ModificationType) const
{
    if (!CachedItemData.bIsWeapon)
    {
        return NAME_None;
...
bool UEquipmentMeshComponent::HasAttachmentSocket(const FGameplayTag& ModificationType) const
{
    FName SocketName = GetAttachmentSocket(ModificationType);
    return SocketName != NAME_None && DoesSocketExist(SocketName);
}

FName UEquipmentMeshComponent::GetWeaponSocketName(const FMedComUnifiedItemData& WeaponData, const FGameplayTag& ModificationType) const
{
...
UNiagaraComponent* UEquipmentMeshComponent::PlayVisualEffectAtLocation(const FGameplayTag& EffectType, const FVector& Location, const FRotator& Rotation)
{
...
void UEquipmentMeshComponent::ApplyPredictedEffect(const FVisualEffectPrediction& Prediction)
{
...
void UEquipmentMeshComponent::StopPredictedEffect(const FVisualEffectPrediction& Prediction)
{
...
void UEquipmentMeshComponent::CleanupExpiredPredictions()
{
    if (!GetWorld())
    {
        return;
...
UMedComItemManager* UEquipmentMeshComponent::GetItemManager() const
{
...
UEventDelegateManager* UEquipmentMeshComponent::GetDelegateManager() const
{
    const float CurrentTime = GetWorld() ? GetWorld()->GetTimeSeconds() : 0.0f;
