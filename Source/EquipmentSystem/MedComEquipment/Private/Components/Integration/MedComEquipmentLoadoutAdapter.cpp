...
#include "Components/Integration/MedComEquipmentLoadoutAdapter.h"
...
UMedComEquipmentLoadoutAdapter::UMedComEquipmentLoadoutAdapter()
{
...
void UMedComEquipmentLoadoutAdapter::BeginPlay()
{
	Super::BeginPlay();
	UE_LOG(LogLoadoutAdapter, Log, TEXT("LoadoutAdapter: BeginPlay"));
}

void UMedComEquipmentLoadoutAdapter::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
bool UMedComEquipmentLoadoutAdapter::Initialize(
...
UEquipmentOperationServiceImpl* UMedComEquipmentLoadoutAdapter::GetOperationService()
{
...
FLoadoutApplicationResult UMedComEquipmentLoadoutAdapter::ApplyLoadout(const FName& LoadoutId, bool bForce)
{
	FScopeLock Lock(&AdapterCriticalSection);
	...
	const UMedComLoadoutManager* Manager = GetLoadoutManager();
...
bool UMedComEquipmentLoadoutAdapter::SaveAsLoadout(const FName& LoadoutId)
{
	FScopeLock Lock(&AdapterCriticalSection);
...
bool UMedComEquipmentLoadoutAdapter::ValidateLoadout(const FName& LoadoutId, TArray<FText>& OutErrors) const
{
	FScopeLock Lock(&AdapterCriticalSection);
	OutErrors.Empty();
...
FName UMedComEquipmentLoadoutAdapter::GetCurrentLoadout() const
{
	FScopeLock Lock(&AdapterCriticalSection);
	return CurrentLoadoutId;
}

FLoadoutConfiguration UMedComEquipmentLoadoutAdapter::ConvertToLoadoutFormat(const FEquipmentStateSnapshot& State) const
{
...
TArray<FEquipmentOperationRequest> UMedComEquipmentLoadoutAdapter::ConvertFromLoadoutFormat(const FLoadoutConfiguration& Loadout) const
{
	return CreateOperationsFromLoadout(Loadout);
}

FString UMedComEquipmentLoadoutAdapter::GetLoadoutPreview(const FName& LoadoutId) const
{
	UMedComLoadoutManager* LoadoutManager = GetLoadoutManager();
...
void UMedComEquipmentLoadoutAdapter::SetApplicationStrategy(ELoadoutApplicationStrategy Strategy)
{
	ApplicationStrategy = Strategy;
	UE_LOG(LogLoadoutAdapter, Log, TEXT("SetApplicationStrategy: %d"), (int32)Strategy);
}

void UMedComEquipmentLoadoutAdapter::SetValidationOptions(const FLoadoutValidationOptions& Options)
{
	ValidationOptions = Options;
	UE_LOG(LogLoadoutAdapter, Log, TEXT("SetValidationOptions: updated"));
}

void UMedComEquipmentLoadoutAdapter::SetInventoryBridge(TScriptInterface<IMedComInventoryBridge> Bridge)
{
	InventoryBridge = Bridge;
	UE_LOG(LogLoadoutAdapter, Log, TEXT("SetInventoryBridge: %s"), Bridge.GetInterface() ? TEXT("set") : TEXT("cleared"));
}

void UMedComEquipmentLoadoutAdapter::SetEventDispatcher(TScriptInterface<IMedComEventDispatcher> Dispatcher)
{
	EventDispatcher = Dispatcher;
	UE_LOG(LogLoadoutAdapter, Log, TEXT("SetEventDispatcher: %s"), Dispatcher.GetInterface() ? TEXT("set") : TEXT("cleared"));
}

TArray<FName> UMedComEquipmentLoadoutAdapter::GetCompatibleLoadouts() const
{
...
float UMedComEquipmentLoadoutAdapter::EstimateApplicationTime(const FName& LoadoutId) const
{
	UMedComLoadoutManager* LoadoutManager = GetLoadoutManager();
...
FLoadoutApplicationResult UMedComEquipmentLoadoutAdapter::ApplyLoadoutConfiguration(const FLoadoutConfiguration& Config, bool bForce)
{
	FLoadoutApplicationResult Result;
	Result.AppliedLoadoutID = Config.LoadoutID;
	Result.ApplicationTime  = FDateTime::Now();
...
TArray<FEquipmentOperationRequest> UMedComEquipmentLoadoutAdapter::CreateOperationsFromLoadout(const FLoadoutConfiguration& Config) const
{
...
FEquipmentOperationRequest UMedComEquipmentLoadoutAdapter::CreateEquipOperation(const FEquipmentSlotConfig& SlotConfig, const FName& ItemId, int32 SlotIndex) const
{
...
bool UMedComEquipmentLoadoutAdapter::ValidateLoadoutConfiguration(const FLoadoutConfiguration& Config, const FLoadoutValidationOptions& Options, TArray<FText>& OutErrors) const
{
	bool bValid = true;
...
bool UMedComEquipmentLoadoutAdapter::CheckSlotCompatibility(const FEquipmentSlotConfig& SlotConfig, const FName& ItemId) const
{
...
bool UMedComEquipmentLoadoutAdapter::CheckInventorySpace(const FLoadoutConfiguration& Config) const
{
...
bool UMedComEquipmentLoadoutAdapter::CheckItemAvailability(const FName& ItemId) const
{
	UMedComItemManager* ItemManager = GetItemManager();
	if (!ItemManager) { return false; }
...
FLoadoutConfiguration UMedComEquipmentLoadoutAdapter::BuildLoadoutFromCurrentState(const FName& LoadoutId) const
{
	FLoadoutConfiguration Loadout;
	...
	const TMap<int32, FInventoryItemInstance> Equipped = DataProvider->GetAllEquippedItems();
	for (const auto& Pair : Equipped)
	{
		const int32 SlotIndex = Pair.Key;
		const FInventoryItemInstance& Instance = Pair.Value;
...
FString UMedComEquipmentLoadoutAdapter::GenerateLoadoutPreview(const FLoadoutConfiguration& Config) const
{
	FString S;
...
bool UMedComEquipmentLoadoutAdapter::ClearCurrentEquipment()
{
...
int32 UMedComEquipmentLoadoutAdapter::ApplyStartingEquipment(const TMap<EEquipmentSlotType, FName>& StartingEquipment)
{
...
void UMedComEquipmentLoadoutAdapter::NotifyLoadoutChange(const FName& LoadoutId, bool bSuccess)
{
...
void UMedComEquipmentLoadoutAdapter::LogAdapterState() const
{
...
UMedComLoadoutManager* UMedComEquipmentLoadoutAdapter::GetLoadoutManager() const
{
	if (!CachedLoadoutManager.IsValid() || (GetWorld() && GetWorld()->GetTimeSeconds() - LastCacheTime > CacheLifetime))
	{
		if (UWorld* World = GetWorld())
		{
			...
			{
				CachedLoadoutManager = GI->GetSubsystem<UMedComLoadoutManager>();
...
UMedComItemManager* UMedComEquipmentLoadoutAdapter::GetItemManager() const
{
