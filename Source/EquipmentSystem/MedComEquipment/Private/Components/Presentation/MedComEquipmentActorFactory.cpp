...
static UMedComItemManager* GetItemManagerSafe(const UWorld* World)
{
...
UMedComEquipmentActorFactory::UMedComEquipmentActorFactory()
...
void UMedComEquipmentActorFactory::BeginPlay()
{
    Super::BeginPlay();
    ...
    if (FactoryConfig.PoolCleanupInterval > 0.0f)
    {
        GetWorld()->GetTimerManager().SetTimer(
            ...
            this,
            &UMedComEquipmentActorFactory::CleanupPool,
...
void UMedComEquipmentActorFactory::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
    UEquipmentServiceLocator* Locator = UEquipmentServiceLocator::Get(this);
    if (Locator)
    {
...
void UMedComEquipmentActorFactory::TickComponent(
...
//========================================
/* IMedComActorFactory Implementation */
//========================================

// MedComEquipmentActorFactory.cpp

FEquipmentActorSpawnResult UMedComEquipmentActorFactory::SpawnEquipmentActor(const FEquipmentActorSpawnParams& Params)
{
    ...
    // ============================================================================
    UMedComItemManager* ItemManager = GetItemManagerSafe(GetWorld());
    if (!ItemManager)
    {
    ...
    FInventoryItemInstance EnrichedInstance = Params.ItemInstance;
...
bool UMedComEquipmentActorFactory::DestroyEquipmentActor(AActor* Actor, bool bImmediate)
{
...
bool UMedComEquipmentActorFactory::ConfigureEquipmentActor(AActor* Actor, const FInventoryItemInstance& ItemInstance)
{
    ...
    if (Actor->GetClass()->ImplementsInterface(UMedComEquipmentInterface::StaticClass()))
    {
        IMedComEquipmentInterface::Execute_OnItemInstanceEquipped(Actor, ItemInstance);
    }
    ...
    if (Actor->GetClass()->ImplementsInterface(UMedComWeaponInterface::StaticClass()))
    {
        ...
        if (bHasAnyAmmo)
        {
            IMedComWeaponInterface::Execute_SetAmmoState(Actor, AmmoState);
        }
    }
...
bool UMedComEquipmentActorFactory::RecycleActor(AActor* Actor)
{
    if (!IsActorValid(Actor))
    {
        return false;
...
AActor* UMedComEquipmentActorFactory::GetPooledActor(TSubclassOf<AActor> ActorClass)
{
    if (!ActorClass)
    {
        return nullptr;
...
bool UMedComEquipmentActorFactory::PreloadActorClass(const FName& ItemId)
{
    ...
    if (UMedComItemManager* ItemManager = GetItemManagerSafe(GetWorld()))
    {
        FMedComUnifiedItemData ItemData;
        if (ItemManager->GetUnifiedItemData(ItemId, ItemData) && !ItemData.EquipmentActorClass.IsNull())
        {
            ...
            TSharedPtr<FStreamableHandle> Handle = StreamableManager.RequestAsyncLoad(
                Path,
                FStreamableDelegate::CreateUObject(this, &UMedComEquipmentActorFactory::OnAsyncLoadComplete, ItemId)
            );
...
FTransform UMedComEquipmentActorFactory::GetSpawnTransformForSlot(int32 SlotIndex, AActor* Owner) const
{
...
bool UMedComEquipmentActorFactory::RegisterSpawnedActor(AActor* Actor, int32 SlotIndex)
{
...
bool UMedComEquipmentActorFactory::UnregisterActor(AActor* Actor)
{
    if (!Actor)
    {
        return false;
...
TMap<int32, AActor*> UMedComEquipmentActorFactory::GetAllSpawnedActors() const
{
    EQUIPMENT_SCOPE_LOCK(RegistryLock);
    return SpawnedActorRegistry;
}

void UMedComEquipmentActorFactory::ClearAllActors(bool bDestroy)
{
    // Clear registry
    {
...
void UMedComEquipmentActorFactory::SetFactoryConfiguration(const FActorFactoryConfig& NewConfig)
{
...
void UMedComEquipmentActorFactory::GetPoolStatistics(int32& TotalActors, int32& ActiveActors, int32& PooledActors) const
{
    EQUIPMENT_SCOPE_LOCK(PoolLock);
...
void UMedComEquipmentActorFactory::PreloadItemClasses(const TArray<FName>& ItemIds)
{
...
AActor* UMedComEquipmentActorFactory::SpawnActorInternal(TSubclassOf<AActor> ActorClass, const FTransform& SpawnTransform, AActor* Owner)
{
...
void UMedComEquipmentActorFactory::DestroyActorInternal(AActor* Actor, bool bImmediate)
{
    if (!IsActorValid(Actor))
    {
        return;
...
TSubclassOf<AActor> UMedComEquipmentActorFactory::GetActorClassForItem(const FName& ItemId)
{
    // Сначала кеш
    {
    ...
    if (GetWorld())
    {
        ...
        {
            ItemManager = GI->GetSubsystem<UMedComItemManager>();
        }
    }
...
FActorPoolEntry* UMedComEquipmentActorFactory::FindPoolEntry(AActor* Actor)
{
    for (FActorPoolEntry& Entry : ActorPool)
    {
...
FActorPoolEntry* UMedComEquipmentActorFactory::FindAvailablePoolEntry(TSubclassOf<AActor> ActorClass)
{
    for (FActorPoolEntry& Entry : ActorPool)
    {
...
FActorPoolEntry* UMedComEquipmentActorFactory::CreatePoolEntry(TSubclassOf<AActor> ActorClass)
{
    FActorPoolEntry NewEntry;
...
void UMedComEquipmentActorFactory::CleanupPool()
{
    EQUIPMENT_SCOPE_LOCK(PoolLock);
...
void UMedComEquipmentActorFactory::OnAsyncLoadComplete(FName ItemId)
{
...
bool UMedComEquipmentActorFactory::IsActorValid(AActor* Actor) const
{
    return Actor && IsValid(Actor) && !Actor->IsPendingKillPending();
}

void UMedComEquipmentActorFactory::LogFactoryOperation(const FString& Operation, const FString& Details) const
{
