...
UMedComEquipmentVisualController::UMedComEquipmentVisualController()
...
void UMedComEquipmentVisualController::BeginPlay()
{
	Super::BeginPlay();
	...
	if (ControllerConfig.PoolCleanupInterval > 0.0f)
	{
		GetWorld()->GetTimerManager().SetTimer(
			...
			&UMedComEquipmentVisualController::CleanupEffectPool,
	...
	if (ControllerConfig.bEnableBatching)
	{
		GetWorld()->GetTimerManager().SetTimer(
			...
			&UMedComEquipmentVisualController::ProcessBatchQueue,
			0.1f, true);
	}
...
void UMedComEquipmentVisualController::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void UMedComEquipmentVisualController::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
FGuid UMedComEquipmentVisualController::ApplyVisualEffect(AActor* Equipment, const FEquipmentVisualEffect& Effect)
{
...
bool UMedComEquipmentVisualController::RemoveVisualEffect(const FGuid& EffectId)
{
	EQUIPMENT_WRITE_LOCK(EffectLock);
...
bool UMedComEquipmentVisualController::ApplyMaterialOverride(AActor* Equipment, const FEquipmentMaterialOverride& Override)
{
...
void UMedComEquipmentVisualController::ResetMaterials(AActor* Equipment)
{
...
void UMedComEquipmentVisualController::UpdateWearState(AActor* Equipment, float WearPercent)
{
	if (!IsValid(Equipment)) return;
...
void UMedComEquipmentVisualController::SetHighlighted(AActor* Equipment, bool bHighlighted, const FLinearColor& HighlightColor)
{
	if (!IsValid(Equipment)) return;
...
bool UMedComEquipmentVisualController::PlayEquipmentAnimation(AActor* Equipment, const FGameplayTag& AnimationTag)
{
	if (!IsValid(Equipment)) return false;
...
bool UMedComEquipmentVisualController::ApplyVisualProfile(AActor* Equipment, const FGameplayTag& ProfileTag, bool bSmooth)
{
	if (!IsValid(Equipment)) return false;
...
int32 UMedComEquipmentVisualController::BatchProcessVisualRequests(const TArray<FBatchVisualRequest>& Requests)
{
...
void UMedComEquipmentVisualController::PreWarmEffectPool(const TArray<UNiagaraSystem*>& EffectSystems, int32 Count)
{
...
void UMedComEquipmentVisualController::StartMaterialTransition(AActor* Equipment, const FName& ParameterName, float TargetValue, float Duration, UCurveFloat* Curve)
{
...
void UMedComEquipmentVisualController::StartColorTransition(AActor* Equipment, const FName& ParameterName, const FLinearColor& TargetColor, float Duration)
{
	if (!IsValid(Equipment) || Duration <= 0.0f) return;
...
void UMedComEquipmentVisualController::ClearAllEffectsForEquipment(AActor* Equipment, bool bImmediate)
{
	if (!IsValid(Equipment)) return;
...
void UMedComEquipmentVisualController::SetVisualQualityLevel(int32 QualityLevel)
{
	CurrentQualityLevel = FMath::Clamp(QualityLevel, 0, 3);
	UE_LOG(LogTemp, Log, TEXT("[VisualController] Quality=%d"), CurrentQualityLevel);
}

void UMedComEquipmentVisualController::LoadVisualProfileTable(UDataTable* ProfileTable)
{
...
void UMedComEquipmentVisualController::SetupEventHandlers()
{
	...
	EventSubscriptions.Add(Bus->Subscribe(
		...
		FEventHandlerDelegate::CreateUObject(this, &UMedComEquipmentVisualController::OnEquipmentStateChanged),
		EEventPriority::Normal, EEventExecutionContext::GameThread, this));

	EventSubscriptions.Add(Bus->Subscribe(
		FGameplayTag::RequestGameplayTag(TEXT("Weapon.Event.Fired")),
		FEventHandlerDelegate::CreateUObject(this, &UMedComEquipmentVisualController::OnWeaponFired),
		EEventPriority::High, EEventExecutionContext::GameThread, this));

	EventSubscriptions.Add(Bus->Subscribe(
		FGameplayTag::RequestGameplayTag(TEXT("Weapon.Event.Reload")),
		FEventHandlerDelegate::CreateUObject(this, &UMedComEquipmentVisualController::OnWeaponReload),
		EEventPriority::Normal, EEventExecutionContext::GameThread, this));

	EventSubscriptions.Add(Bus->Subscribe(
		FGameplayTag::RequestGameplayTag(TEXT("Equipment.Event.QuickSwitch")),
		FEventHandlerDelegate::CreateUObject(this, &UMedComEquipmentVisualController::OnQuickSwitch),
		EEventPriority::High, EEventExecutionContext::GameThread, this));
}

void UMedComEquipmentVisualController::OnEquipmentStateChanged(const FEquipmentEventData& EventData)
{
...
void UMedComEquipmentVisualController::OnWeaponFired(const FEquipmentEventData& EventData)
{
...
void UMedComEquipmentVisualController::OnWeaponReload(const FEquipmentEventData& EventData)
{
	if (AActor* Wep = EventData.GetTargetAs<AActor>())
	{
...
void UMedComEquipmentVisualController::OnQuickSwitch(const FEquipmentEventData& EventData)
{
...
void UMedComEquipmentVisualController::ProcessBatchQueue()
{
	EQUIPMENT_WRITE_LOCK(BatchLock);
...
void UMedComEquipmentVisualController::UpdateMaterialTransitions(float DeltaTime)
{
	EQUIPMENT_WRITE_LOCK(MaterialLock);
...
void UMedComEquipmentVisualController::UpdateWearInterpolation(float DeltaTime)
{
...
void UMedComEquipmentVisualController::UpdateActiveEffects(float DeltaTime)
{
	EQUIPMENT_WRITE_LOCK(EffectLock);
...
UNiagaraComponent* UMedComEquipmentVisualController::GetPooledEffectComponent(UNiagaraSystem* System, const FGameplayTag& /*ProfileTag*/)
{
	EQUIPMENT_WRITE_LOCK(EffectLock);
...
bool UMedComEquipmentVisualController::ReturnEffectToPool(UNiagaraComponent* Component)
{
...
UNiagaraComponent* UMedComEquipmentVisualController::CreateEffectComponent(UNiagaraSystem* System)
{
...
void UMedComEquipmentVisualController::CleanupEffectPool()
{
	EQUIPMENT_WRITE_LOCK(EffectLock);
	const float Now = GetWorld() ? GetWorld()->GetTimeSeconds() : 0.0f;
...
const FEquipmentVisualProfile* UMedComEquipmentVisualController::FindBestVisualProfile(const FGameplayTag& ItemType, const FGameplayTag& StateTag) const
{
...
void UMedComEquipmentVisualController::LoadVisualProfiles()
{
...
void UMedComEquipmentVisualController::ApplyProfileToMaterials(AActor* Equipment, const FEquipmentVisualProfile& Profile, bool bSmooth)
{
	if (!IsValid(Equipment)) return;
...
void UMedComEquipmentVisualController::ApplyProfileEffects(AActor* Equipment, const FEquipmentVisualProfile& Profile)
{
	if (!IsValid(Equipment)) return;
...
UMaterialInstanceDynamic* UMedComEquipmentVisualController::GetOrCreateDynamicMaterial(UMaterialInterface* BaseMaterial, const FName& CacheKey)
{
...
void UMedComEquipmentVisualController::ApplyWearToMaterial(UMaterialInstanceDynamic* Material, float WearPercent)
{
	if (!Material) return;
...
void UMedComEquipmentVisualController::ApplyHighlightToMaterial(UMaterialInstanceDynamic* Material, bool bHighlight, const FLinearColor& Color)
{
	if (!Material) return;
...
TArray<UMeshComponent*> UMedComEquipmentVisualController::GetMeshComponents(AActor* Actor) const
{
...
FGuid UMedComEquipmentVisualController::GenerateEffectId() const { return FGuid::NewGuid(); }

FGameplayTag UMedComEquipmentVisualController::GetQualityTag() const
{
...
FString UMedComEquipmentVisualController::GetVisualStatistics() const
{
	EQUIPMENT_READ_LOCK(EffectLock);
...
void UMedComEquipmentVisualController::ToggleDebugOverlay()
{
	bDebugOverlayEnabled = !bDebugOverlayEnabled;
	if (bDebugOverlayEnabled)
	{
		GetWorld()->GetTimerManager().SetTimer(
			DebugOverlayTimerHandle, this, &UMedComEquipmentVisualController::UpdateDebugOverlay,
...
void UMedComEquipmentVisualController::UpdateDebugOverlay()
{
...
void UMedComEquipmentVisualController::DrawDebugInfoForActor(AActor* Actor, const FString& Info) const
{
...
void UMedComEquipmentVisualController::LogVisualMetrics() const
{
...
void UMedComEquipmentVisualController::SetControllerConfiguration(const FVisualControllerConfig& NewConfig)
{
	...
	if (GetWorld())
	{
		...
		{
			GetWorld()->GetTimerManager().SetTimer(
				PoolCleanupTimerHandle, this, &UMedComEquipmentVisualController::CleanupEffectPool,
				ControllerConfig.PoolCleanupInterval, true);
		}
	}
}
...
bool UMedComEquipmentVisualController::ShouldPlayEffect(AActor* Equipment, const FGameplayTag& EffectTag, float& OutCooldownMs) const
{
	OutCooldownMs = 0.0f;
...
void UMedComEquipmentVisualController::MarkEffectPlayed(AActor* Equipment, const FGameplayTag& EffectTag)
{
	const TWeakObjectPtr<AActor> Key = Equipment;
