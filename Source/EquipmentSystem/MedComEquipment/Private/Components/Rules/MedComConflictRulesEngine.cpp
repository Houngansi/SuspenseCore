...
UMedComConflictRulesEngine::UMedComConflictRulesEngine()
{
    // Инициализация произойдет через Initialize()
}

bool UMedComConflictRulesEngine::Initialize(TScriptInterface<IMedComEquipmentDataProvider> InDataProvider)
{
...
void UMedComConflictRulesEngine::InitializeDefaultRules()
{
...
FMedComRuleCheckResult UMedComConflictRulesEngine::CheckItemConflicts(
    ...
    const TArray<FInventoryItemInstance>& ExistingItems) const
{
    ...
    for (const FInventoryItemInstance& ExistingItem : ExistingItems)
    {
        ...
        {
            FMedComUnifiedItemData ExistingData;
        ...
        {
            FMedComUnifiedItemData NewData, ExistingData;
...
FMedComRuleCheckResult UMedComConflictRulesEngine::CheckSlotConflicts(
    const FInventoryItemInstance& NewItem,
    int32 TargetSlot,
    const TArray<FEquipmentSlotSnapshot>& Slots) const
{
    ...
    // а не с самодельной картой TMap<int32, FInventoryItemInstance>, которая давала ложные индексы
...
FMedComAggregatedRuleResult UMedComConflictRulesEngine::EvaluateConflictRules(
    const FMedComRuleContext& Context) const
{
    ...
    // Причина: старая логика создавала TMap<int32, FInventoryItemInstance> из CurrentItems,
...
TArray<FMedComConflictResolution> UMedComConflictRulesEngine::FindAllConflicts(
    ...
    const TArray<FInventoryItemInstance>& CurrentItems) const
{
    ...
    for (const FInventoryItemInstance& CurrentItem : CurrentItems)
    {
...
TArray<FMedComConflictResolution> UMedComConflictRulesEngine::PredictConflicts(
...
EMedComConflictType UMedComConflictRulesEngine::GetConflictType(
...
bool UMedComConflictRulesEngine::AreItemsCompatible(
    const FInventoryItemInstance& Item1,
    const FInventoryItemInstance& Item2) const
{
...
float UMedComConflictRulesEngine::CalculateCompatibilityScore(
    const FInventoryItemInstance& Item,
    const TArray<FInventoryItemInstance>& ExistingItems) const
{
...
FMedComRuleCheckResult UMedComConflictRulesEngine::CheckTypeExclusivity(
...
TArray<FMedComSetBonusInfo> UMedComConflictRulesEngine::DetectSetBonuses(
...
bool UMedComConflictRulesEngine::WouldBreakSetBonus(
...
TArray<FName> UMedComConflictRulesEngine::GetMissingSetItems(
    ...
    const TArray<FInventoryItemInstance>& CurrentItems) const
{
    ...
    for (const FName& SetItemID : *SetItems)
    {
        ...
        for (const FInventoryItemInstance& Item : CurrentItems)
        {
...
bool UMedComConflictRulesEngine::SuggestResolutions(
    ...
    TArray<FMedComResolutionAction>& OutActions) const
{
    ...
    for (const FMedComConflictResolution& C : Conflicts)
    {
        switch (Strategy)
        {
            ...
            {
                for (const FInventoryItemInstance& It : C.ConflictingItems)
                {
                    FMedComResolutionAction A;
...
EMedComConflictResolution UMedComConflictRulesEngine::SuggestResolutionStrategy(
...
FText UMedComConflictRulesEngine::GetConflictDescription(const FMedComConflictResolution& Conflict) const
{
...
void UMedComConflictRulesEngine::RegisterMutualExclusion(
...
void UMedComConflictRulesEngine::RegisterRequiredCompanions(
...
void UMedComConflictRulesEngine::RegisterItemSet(
    const FGameplayTag& SetTag,
...
void UMedComConflictRulesEngine::ClearAllRules()
{
    FScopeLock Lock(&RulesLock);
...
void UMedComConflictRulesEngine::ClearCache()
{
...
void UMedComConflictRulesEngine::ResetStatistics()
{
...
bool UMedComConflictRulesEngine::CheckMutualExclusion(
    const FGameplayTag& Type1,
...
bool UMedComConflictRulesEngine::CheckRequiredCompanions(
    const FInventoryItemInstance& Item,
    const TArray<FInventoryItemInstance>& CurrentItems) const
{
    FGameplayTag ItemType = GetItemType(Item);
...
FGameplayTag UMedComConflictRulesEngine::GetItemType(const FInventoryItemInstance& Item) const
{
...
FGameplayTag UMedComConflictRulesEngine::GetArmorClass(const FMedComUnifiedItemData& ItemData) const
{
...
FString UMedComConflictRulesEngine::GetConflictTypeString(EMedComConflictType ConflictType) const
{
    switch (ConflictType)
    {
...
bool UMedComConflictRulesEngine::GetItemData(FName ItemID, FMedComUnifiedItemData& OutData) const
{
...
FMedComAggregatedRuleResult UMedComConflictRulesEngine::EvaluateConflictRulesWithSlots(
