...
#include "Components/Rules/MedComEquipmentRulesEngine.h"
...
UMedComEquipmentRulesEngine::UMedComEquipmentRulesEngine()
{
...
void UMedComEquipmentRulesEngine::BeginPlay()
{
    Super::BeginPlay();
    ...
    {
        UE_LOG(LogEquipmentRules, Log, TEXT("Equipment Rules Engine (PRODUCTION DISABLED) - use UMedComRulesCoordinator"));
    }
}

void UMedComEquipmentRulesEngine::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
    // Clear all data
...
bool UMedComEquipmentRulesEngine::ShouldUseDevFallback() const
{
    return bDevFallbackEnabled && CVarMedComUseMonolith.GetValueOnGameThread() != 0;
}

FRuleEvaluationResult UMedComEquipmentRulesEngine::CreateDisabledResult(const FString& MethodName) const
{
    ...
    Result.FailureReason = FText::FromString(FString::Printf(
        TEXT("Monolithic rules engine is disabled. Use UMedComRulesCoordinator for production validation. (Method: %s)"), 
        *MethodName));
    Result.ConfidenceScore = 1.0f;
    Result.Details.Add(TEXT("Production path: Use UMedComRulesCoordinator"));
    Result.Details.Add(FString::Printf(TEXT("Enable with: medcom.rules.use_monolith 1 or bDevFallbackEnabled=true")));
    
    return Result;
}

void UMedComEquipmentRulesEngine::SetDevFallbackEnabled(bool bEnabled)
{
    bDevFallbackEnabled = bEnabled;
    UE_LOG(LogEquipmentRules, Log, TEXT("Dev fallback mode %s"), bEnabled ? TEXT("ENABLED") : TEXT("DISABLED"));
}

bool UMedComEquipmentRulesEngine::IsDevFallbackEnabled() const
{
    return ShouldUseDevFallback();
}

//========================================
// IMedComEquipmentRules Implementation (DEV FALLBACK)
//========================================

FRuleEvaluationResult UMedComEquipmentRulesEngine::EvaluateRules(const FEquipmentOperationRequest& Operation) const
{
    if (!ShouldUseDevFallback())
    {
...
FRuleEvaluationResult UMedComEquipmentRulesEngine::EvaluateRulesWithContext(
...
FRuleEvaluationResult UMedComEquipmentRulesEngine::CheckItemCompatibility(
...
FRuleEvaluationResult UMedComEquipmentRulesEngine::CheckCharacterRequirements(
    const AActor* Character,
    const FInventoryItemInstance& ItemInstance) const
{
    if (!ShouldUseDevFallback())
    {
...
FRuleEvaluationResult UMedComEquipmentRulesEngine::CheckWeightLimit(
    float CurrentWeight,
...
FRuleEvaluationResult UMedComEquipmentRulesEngine::CheckConflictingEquipment(
    const TArray<FInventoryItemInstance>& ExistingItems,
    const FInventoryItemInstance& NewItem) const
{
    if (!ShouldUseDevFallback())
    {
    ...
    FMedComUnifiedItemData NewItemData;
    ...
    for (const FInventoryItemInstance& ExistingItem : ExistingItems)
    {
        ...
        FMedComUnifiedItemData ExistingItemData;
...
TArray<FEquipmentRule> UMedComEquipmentRulesEngine::GetActiveRules() const
{
    if (!ShouldUseDevFallback())
    {
...
bool UMedComEquipmentRulesEngine::RegisterRule(const FEquipmentRule& Rule)
{
    if (!ShouldUseDevFallback())
    {
...
bool UMedComEquipmentRulesEngine::UnregisterRule(const FGameplayTag& RuleTag)
{
    if (!ShouldUseDevFallback())
    {
        return false;
...
bool UMedComEquipmentRulesEngine::SetRuleEnabled(const FGameplayTag& RuleTag, bool bEnabled)
{
    if (!ShouldUseDevFallback())
    {
        return false;
...
FString UMedComEquipmentRulesEngine::GenerateComplianceReport(const FEquipmentStateSnapshot& CurrentState) const
{
    if (!ShouldUseDevFallback())
    {
        return TEXT("Monolithic Rules Engine Compliance Report\n") 
               TEXT("==========================================\n\n")
               TEXT("Status: DISABLED (Production path uses UMedComRulesCoordinator)\n\n")
    ...
    TArray<FInventoryItemInstance> AllEquippedItems;
    for (const FEquipmentSlotSnapshot& SlotSnapshot : CurrentState.SlotSnapshots)
    {
        if (SlotSnapshot.ItemInstance.IsValid())
        {
    ...
    Report += TEXT("\n[DEV FALLBACK MODE ACTIVE - Use UMedComRulesCoordinator for production]\n");
...
bool UMedComEquipmentRulesEngine::Initialize(TScriptInterface<IMedComEquipmentDataProvider> InDataProvider)
{
    if (!ShouldUseDevFallback())
    {
...
int32 UMedComEquipmentRulesEngine::LoadRulesFromDataTable(UDataTable* RulesTable)
{
    ...
    // Assuming data table has FEquipmentRule rows
...
FRuleEvaluationResult UMedComEquipmentRulesEngine::EvaluateSpecificRule(
...
TArray<FRuleEvaluationResult> UMedComEquipmentRulesEngine::BatchEvaluateRules(
...
void UMedComEquipmentRulesEngine::ClearAllRules()
{
    if (!ShouldUseDevFallback())
    {
        return;
...
void UMedComEquipmentRulesEngine::ResetStatistics()
{
    if (!ShouldUseDevFallback())
    {
        return;
...
bool UMedComEquipmentRulesEngine::GetItemData(FName ItemID, FMedComUnifiedItemData& OutItemData) const
{
    ...
    if (UFunction* Func = ProviderObj->FindFunction(FuncName))
    {
        ...
        {
            FName ItemID;
            FMedComUnifiedItemData OutData;
            bool ReturnValue;
        } Params{ ItemID, FMedComUnifiedItemData(), false };
...
bool UMedComEquipmentRulesEngine::EvaluateExpression(const FString& Expression, const FRuleExecutionContext& Context) const
{
    if (!ShouldUseDevFallback())
    {
        return true;
    ...
    if (Expression.Contains(TEXT("WEIGHT")))
    {
        ...
        {
            TArray<FInventoryItemInstance> All;
...
FRuleEvaluationResult UMedComEquipmentRulesEngine::ExecuteRule(
...
bool UMedComEquipmentRulesEngine::CheckPreconditions(
    const FEquipmentRule& Rule,
    const FRuleExecutionContext& Context) const
{
    // Check dependencies
...
TArray<FEquipmentRule> UMedComEquipmentRulesEngine::PrioritizeRules(const TArray<FEquipmentRule>& Rules) const
{
...
void UMedComEquipmentRulesEngine::CacheRuleResult(const FGameplayTag& RuleTag, const FRuleEvaluationResult& Result) const
{
    if (!bEnableCaching)
    {
        return;
...
bool UMedComEquipmentRulesEngine::GetCachedResult(const FGameplayTag& RuleTag, FRuleEvaluationResult& OutResult) const
{
    if (!bEnableCaching)
    {
        return false;
...
int32 UMedComEquipmentRulesEngine::GetCharacterLevel(const AActor* Character) const
{
    if (!Character)
    {
        return 1;
...
TMap<FName, float> UMedComEquipmentRulesEngine::GetCharacterAttributes(const AActor* Character) const
{
...
FGameplayTagContainer UMedComEquipmentRulesEngine::GetCharacterTags(const AActor* Character) const
{
...
float UMedComEquipmentRulesEngine::CalculateTotalWeight(const TArray<FInventoryItemInstance>& Items) const
{
...
void UMedComEquipmentRulesEngine::RecordViolation(const FRuleViolation& Violation) const
{
    if (!ShouldUseDevFallback())
    {
        return;
...
void UMedComEquipmentRulesEngine::UpdateStatistics(const FGameplayTag& RuleTag, bool bPassed, float EvaluationTime) const
{
    if (!ShouldUseDevFallback())
    {
        return;
...
void UMedComEquipmentRulesEngine::RegisterDefaultRules()
{
    if (!ShouldUseDevFallback())
    {
        return;
...
FCharacterRequirements UMedComEquipmentRulesEngine::GetItemRequirements(const FInventoryItemInstance& ItemInstance) const
{
...
FRuleEvaluationResult UMedComEquipmentRulesEngine::CheckCharacterMeetsRequirements(
    const AActor* Character,
...
float UMedComEquipmentRulesEngine::CalculateWeightCapacity(const AActor* Character) const
{
...
FRuleEvaluationResult UMedComEquipmentRulesEngine::CheckItemDurability(const FInventoryItemInstance& ItemInstance) const
{
    if (!ShouldUseDevFallback())
    {
...
FRuleEvaluationResult UMedComEquipmentRulesEngine::CheckAmmoCompatibility(
    const FInventoryItemInstance& WeaponInstance,
    const FInventoryItemInstance& AmmoInstance) const
{
    if (!ShouldUseDevFallback())
    {
    ...
    FMedComUnifiedItemData WeaponData, AmmoData;
...
FRuleEvaluationResult UMedComEquipmentRulesEngine::CheckModificationCompatibility(
    const FInventoryItemInstance& BaseItem,
    const FInventoryItemInstance& Modification) const
{
    if (!ShouldUseDevFallback())
    {
...
FRuleEvaluationResult UMedComEquipmentRulesEngine::ValidateLoadout(const TArray<FInventoryItemInstance>& LoadoutItems) const
{
    if (!ShouldUseDevFallback())
    {
...
TArray<FRuleViolation> UMedComEquipmentRulesEngine::FindItemConflicts(
    const FInventoryItemInstance& ItemInstance,
...
bool UMedComEquipmentRulesEngine::ResolveConflicts(const TArray<FRuleViolation>& Conflicts, int32 ResolutionStrategy)
{
    if (!ShouldUseDevFallback())
    {
        return false;
...
FRuleStatistics UMedComEquipmentRulesEngine::GetRuleStatistics(const FGameplayTag& RuleTag) const
{
    if (!ShouldUseDevFallback())
    {
...
TArray<FRuleViolation> UMedComEquipmentRulesEngine::GetViolationHistory(int32 MaxCount) const
{
    if (!ShouldUseDevFallback())
    {
...
FString UMedComEquipmentRulesEngine::ExportRulesToJSON() const
{
    if (!ShouldUseDevFallback())
    {
...
int32 UMedComEquipmentRulesEngine::ImportRulesFromJSON(const FString& JSONString)
{
    if (!ShouldUseDevFallback())
    {
        return 0;
...
FString UMedComEquipmentRulesEngine::GetDebugInfo() const
{
    return FString::Printf(TEXT("Monolithic Rules Engine Debug:\n"
                          ...
                          ResultCache.Num(),
                          EngineVersion,
                          ShouldUseDevFallback() ? TEXT("ACTIVE") : TEXT("DISABLED - Use UMedComRulesCoordinator"));
}