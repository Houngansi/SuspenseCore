...
#include "Components/Rules/MedComConflictRulesEngine.h"
...
UMedComRulesCoordinator::UMedComRulesCoordinator()
{
...
// ============================================================================
bool UMedComRulesCoordinator::Initialize(TScriptInterface<IMedComEquipmentDataProvider> InDataProvider)
{
...
void UMedComRulesCoordinator::CreateSpecializedEngines()
{
    // ============================================================================
    // 1) Compatibility Engine (Critical Priority)
    // ============================================================================
    CompatibilityEngine = NewObject<UMedComCompatibilityRulesEngine>(this, TEXT("CompatibilityRulesEngine"));
    ...
    // ============================================================================
    RequirementEngine = NewObject<UMedComRequirementRulesEngine>(this, TEXT("RequirementRulesEngine"));
    ...
    // ============================================================================
    WeightEngine = NewObject<UMedComWeightRulesEngine>(this, TEXT("WeightRulesEngine"));
    if (WeightEngine)
    {
    ...
    // ============================================================================
    ConflictEngine = NewObject<UMedComConflictRulesEngine>(this, TEXT("ConflictRulesEngine"));
    if (ConflictEngine)
    {
        // ✅ ИЗМЕНЕНИЕ: Передаем DataProvider, только если он доступен
        if (DataProvider.GetInterface())
        ...
        else
        {
            ...
            ConflictEngine->Initialize(TScriptInterface<IMedComEquipmentDataProvider>());
            UE_LOG(LogRulesCoordinator, Verbose,
...
FEquipmentStateSnapshot UMedComRulesCoordinator::BuildShadowSnapshotFromContext(
    const FMedComRuleContext& Context) const
{
    ...
    if (Context.CurrentItems.Num() > 0)
    {
        ...
        {
            const FInventoryItemInstance& Item = Context.CurrentItems[i];
...
static void ApplyOperationToSnapshot(const FEquipmentOperationRequest& Op, FEquipmentStateSnapshot& Snapshot)
{
    ...
    case EEquipmentOperationType::Move:
        ...
        {
            FInventoryItemInstance MovingItem;
            for (FEquipmentSlotSnapshot& S : Snapshot.SlotSnapshots)
            {
                if (S.SlotIndex == Op.SourceSlotIndex)
                {
                    MovingItem = S.ItemInstance;
                    S.ItemInstance = FInventoryItemInstance();
                    break;
                }
            }
            for (FEquipmentSlotSnapshot& S : Snapshot.SlotSnapshots)
...
static void SnapshotToItemsFiltered(const FEquipmentStateSnapshot& Snapshot,
                                    ...
                                    TArray<FInventoryItemInstance>& OutItems)
{
    OutItems.Reset();
...
// ============================================================================
FRuleEvaluationResult UMedComRulesCoordinator::EvaluateRules(const FEquipmentOperationRequest& Operation) const
{
    ...
    if (DataProvider.GetInterface())
    {
        TMap<int32, FInventoryItemInstance> EquippedMap = DataProvider->GetAllEquippedItems();
...
void UMedComRulesCoordinator::RecordEngineMetrics(const FGameplayTag& EngineType, double DurationMs) const
{
...
// ============================================================================
FRuleEvaluationResult UMedComRulesCoordinator::EvaluateRulesWithContext(
    ...
    const FMedComRuleContext& Context) const
{
    ...
    for (const FPipelineEntry& Entry : Pipeline)
    {
        ...
        else if (Entry.Engine == WeightEngine)
        {
            ...
            WeightContext.ItemInstance = FInventoryItemInstance();
...
// ============================================================================
// Остальные методы интерфейса IMedComEquipmentRules
// ============================================================================

FRuleEvaluationResult UMedComRulesCoordinator::CheckItemCompatibility(
...
FRuleEvaluationResult UMedComRulesCoordinator::CheckCharacterRequirements(
...
FRuleEvaluationResult UMedComRulesCoordinator::CheckWeightLimit(
    float CurrentWeight,
...
FRuleEvaluationResult UMedComRulesCoordinator::CheckConflictingEquipment(
...
TArray<FEquipmentRule> UMedComRulesCoordinator::GetActiveRules() const
{
    FScopeLock Lock(&RulesLock);
...
bool UMedComRulesCoordinator::RegisterRule(const FEquipmentRule& Rule)
{
    FScopeLock Lock(&RulesLock);
...
bool UMedComRulesCoordinator::UnregisterRule(const FGameplayTag& RuleTag)
{
    FScopeLock Lock(&RulesLock);
...
bool UMedComRulesCoordinator::SetRuleEnabled(const FGameplayTag& RuleTag, bool bEnabled)
{
    FScopeLock Lock(&RulesLock);
...
void UMedComRulesCoordinator::ClearRuleCache()
{
...
void UMedComRulesCoordinator::ResetStatistics()
{
    FScopeLock Lock(&RulesLock);
...
FString UMedComRulesCoordinator::GenerateComplianceReport(const FEquipmentStateSnapshot& CurrentState) const
{
    ...
    if (WeightEngine)
    {
        TArray<FInventoryItemInstance> FilteredItems;
...
bool UMedComRulesCoordinator::RegisterRuleEngine(
...
bool UMedComRulesCoordinator::UnregisterRuleEngine(const FGameplayTag& EngineType)
{
    FScopeLock Lock(&RulesLock);
...
bool UMedComRulesCoordinator::SetEngineEnabled(const FGameplayTag& EngineType, bool bEnabled)
{
    FScopeLock Lock(&RulesLock);
...
TArray<FRuleEngineRegistration> UMedComRulesCoordinator::GetRegisteredEngines() const
{
    FScopeLock Lock(&RulesLock);
...
TArray<FRuleEngineRegistration> UMedComRulesCoordinator::GetSortedEngines() const
{
    return GetRegisteredEngines();
}

TMap<FString, FString> UMedComRulesCoordinator::GetExecutionStatistics() const
{
...
FString UMedComRulesCoordinator::GetPipelineHealth() const
{
...
FRuleEvaluationResult UMedComRulesCoordinator::ConvertToLegacyResult(
...
FRuleEvaluationResult UMedComRulesCoordinator::ConvertSingleResult(const FMedComRuleCheckResult& NewResult) const
{
    FRuleEvaluationResult LegacyResult;
...
FRuleEvaluationResult UMedComRulesCoordinator::ConvertAggregatedResult(const FMedComAggregatedRuleResult& AggregatedResult) const
{
