...
UMedComEquipmentTransactionProcessor::UMedComEquipmentTransactionProcessor()
{
...
void UMedComEquipmentTransactionProcessor::BeginPlay()
{
    Super::BeginPlay();
...
void UMedComEquipmentTransactionProcessor::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void UMedComEquipmentTransactionProcessor::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
//========================================
// IMedComTransactionManager Implementation (Basic API)
//========================================

FGuid UMedComEquipmentTransactionProcessor::BeginTransaction(const FString& Description)
{
    ...
    if (UMedComEquipmentDataStore* DataStore = Cast<UMedComEquipmentDataStore>(DataProvider.GetObject()))
    {
...
bool UMedComEquipmentTransactionProcessor::CommitTransaction(const FGuid& TransactionId)
{
    // Phase 1: Validate and prepare under lock
...
bool UMedComEquipmentTransactionProcessor::RollbackTransaction(const FGuid& TransactionId)
{
    // Phase 1: Validate and prepare under lock
    FTransactionExecutionContext ContextCopy;
    ...
    if (bCanRollback)
    {
        ...
        if (bSuccess && bGenerateDeltas)
        {
            for (const FTransactionOperation& Op : ContextCopy.Operations)
            {
                ...
                FInventoryItemInstance Temp = Delta.ItemBefore;
...
FGuid UMedComEquipmentTransactionProcessor::BeginNestedTransaction(const FString& Description)
{
...
bool UMedComEquipmentTransactionProcessor::RegisterOperation(const FGuid& OperationId)
{
    FScopeLock Lock(&TransactionLock);
...
bool UMedComEquipmentTransactionProcessor::ValidateTransaction(const FGuid& TransactionId) const
{
    FScopeLock Lock(&TransactionLock);
    return ValidateTransaction_NoLock(TransactionId);
}

TArray<FEquipmentTransaction> UMedComEquipmentTransactionProcessor::GetTransactionHistory(int32 MaxCount) const
{
    FScopeLock Lock(&TransactionLock);
...
//========================================
// IMedComTransactionManager Implementation (Extended API)
//========================================

bool UMedComEquipmentTransactionProcessor::SupportsExtendedOps() const
{
    // Этот процессор реализует расширенное API
    return true;
}

bool UMedComEquipmentTransactionProcessor::RegisterOperation(
...
bool UMedComEquipmentTransactionProcessor::ApplyOperation(
    const FGuid& TransactionId,
    const FTransactionOperation& Operation)
{
...
TArray<FEquipmentDelta> UMedComEquipmentTransactionProcessor::GetTransactionDeltas(
...
FGuid UMedComEquipmentTransactionProcessor::CreateSavepoint(const FString& SavepointName)
{
    // Phase 1: Validate and prepare under lock
...
bool UMedComEquipmentTransactionProcessor::RollbackToSavepoint(const FGuid& SavepointId)
{
...
bool UMedComEquipmentTransactionProcessor::IsTransactionActive() const
{
    FScopeLock Lock(&TransactionLock);
    return TransactionStack.Num() > 0;
}

FEquipmentTransaction UMedComEquipmentTransactionProcessor::GetCurrentTransaction() const
{
    FScopeLock Lock(&TransactionLock);
...
FGuid UMedComEquipmentTransactionProcessor::GetCurrentTransactionId() const
{
    FScopeLock Lock(&TransactionLock);
...
FEquipmentTransaction UMedComEquipmentTransactionProcessor::GetTransaction(const FGuid& TransactionId) const
{
    FScopeLock Lock(&TransactionLock);
...
int32 UMedComEquipmentTransactionProcessor::CommitAllTransactions()
{
...
int32 UMedComEquipmentTransactionProcessor::RollbackAllTransactions()
{
    // Phase 1: Collect transaction list under lock
...
FGuid UMedComEquipmentTransactionProcessor::RecordDetailedOperation(const FTransactionOperation& Operation)
{
    FScopeLock Lock(&TransactionLock);
...
bool UMedComEquipmentTransactionProcessor::ReleaseSavepoint(const FGuid& SavepointId)
{
    FScopeLock Lock(&TransactionLock);
...
void UMedComEquipmentTransactionProcessor::ClearTransactionHistory(bool bKeepActive)
{
...
FTransactionValidationResult UMedComEquipmentTransactionProcessor::ValidateTransactionIntegrity(const FGuid& TransactionId) const
{
    FScopeLock Lock(&TransactionLock);
...
TArray<FTransactionOperation> UMedComEquipmentTransactionProcessor::CheckForConflicts(const FGuid& TransactionId) const
{
    FScopeLock Lock(&TransactionLock);
    return CheckForConflicts_NoLock(TransactionId);
}

bool UMedComEquipmentTransactionProcessor::ResolveConflicts(const FGuid& TransactionId, int32 ResolutionStrategy)
{
    // Phase 1: Collect data under lock
...
bool UMedComEquipmentTransactionProcessor::RecoverTransaction(const FGuid& TransactionId)
{
    // Phase 1: Collect data under lock
...
bool UMedComEquipmentTransactionProcessor::Initialize(TScriptInterface<IMedComEquipmentDataProvider> InDataProvider)
{
    FScopeLock Lock(&TransactionLock);
...
void UMedComEquipmentTransactionProcessor::SetTransactionTimeout(float Seconds)
{
...
void UMedComEquipmentTransactionProcessor::SetMaxNestedDepth(int32 MaxDepth)
{
...
void UMedComEquipmentTransactionProcessor::SetAutoRecovery(bool bEnable)
{
...
FString UMedComEquipmentTransactionProcessor::GetTransactionStatistics() const
{
    FScopeLock Lock(&TransactionLock);
...
int32 UMedComEquipmentTransactionProcessor::GetActiveTransactionCount() const
{
    FScopeLock Lock(&TransactionLock);
    return ActiveTransactions.Num();
}

FString UMedComEquipmentTransactionProcessor::DumpTransactionState(const FGuid& TransactionId) const
{
    FScopeLock Lock(&TransactionLock);
...
FTransactionExecutionContext UMedComEquipmentTransactionProcessor::CreateExecutionContext(
    const FGuid& TransactionId,
...
bool UMedComEquipmentTransactionProcessor::ExecuteCommit(FTransactionExecutionContext& Context)
{
    ...
    for (const FTransactionOperation& Op : Context.Operations)
    {
        ...
        if (bIsSwap)
        {
            ...
            {
                FInventoryItemInstance A = DataProvider->GetSlotItem(Op.SlotIndex);
                FInventoryItemInstance B = DataProvider->GetSlotItem(TargetSlot);
...
bool UMedComEquipmentTransactionProcessor::ExecuteRollback(FTransactionExecutionContext& Context)
{
    ...
    FEquipmentStateSnapshot& State) const
{
    ...
    if (bIsClearLike && Operation.SlotIndex >= 0)
    {
        for (FEquipmentSlotSnapshot& Slot : State.SlotSnapshots)
        {
            if (Slot.SlotIndex == Operation.SlotIndex)
            {
                Slot.ItemInstance = FInventoryItemInstance(); // пусто
                TouchSnapshot();
                return true;
            }
        }
...
bool UMedComEquipmentTransactionProcessor::ReverseOperation(const FTransactionOperation& Operation, FEquipmentStateSnapshot& State) const
{
...
FEquipmentStateSnapshot UMedComEquipmentTransactionProcessor::CaptureStateSnapshot() const
{
...
bool UMedComEquipmentTransactionProcessor::RestoreStateSnapshot(const FEquipmentStateSnapshot& Snapshot)
{
    if (!DataProvider.GetInterface())
    {
        UE_LOG(LogEquipmentTransaction, Error, 
...
bool UMedComEquipmentTransactionProcessor::ValidateStateConsistency(const FEquipmentStateSnapshot& State) const
{
    // Basic validation
    if (!State.IsValid())
    {
...
FTransactionExecutionContext* UMedComEquipmentTransactionProcessor::FindExecutionContext(const FGuid& TransactionId)
{
    return ActiveTransactions.Find(TransactionId);
}

const FTransactionExecutionContext* UMedComEquipmentTransactionProcessor::FindExecutionContext(const FGuid& TransactionId) const
{
    return ActiveTransactions.Find(TransactionId);
}

FTransactionSavepoint* UMedComEquipmentTransactionProcessor::FindSavepoint(
    FTransactionExecutionContext& Context, 
...
void UMedComEquipmentTransactionProcessor::CleanupExpiredTransactions()
{
...
void UMedComEquipmentTransactionProcessor::LogTransactionEvent(
    const FGuid& TransactionId, 
...
void UMedComEquipmentTransactionProcessor::NotifyTransactionStateChange(
    const FGuid& TransactionId,
...
FEquipmentTransaction UMedComEquipmentTransactionProcessor::ConvertToTransaction(
...
bool UMedComEquipmentTransactionProcessor::CommitTransaction(const FGuid& TransactionId, const TArray<FEquipmentDelta>& Deltas)
{
    // Phase 1: Validate and prepare under lock
    FTransactionExecutionContext ContextCopy;
    bool bCanCommit = false;
...
TArray<FEquipmentDelta> UMedComEquipmentTransactionProcessor::GenerateDeltasFromTransaction(
    const FTransactionExecutionContext& Context) const
{
...
FEquipmentDelta UMedComEquipmentTransactionProcessor::CreateDeltaFromOperation(
    const FTransactionOperation& Operation, 
    const FGuid& TransactionId) const
{
...
TArray<FTransactionOperation> UMedComEquipmentTransactionProcessor::CheckForConflicts_NoLock(
    const FGuid& TransactionId) const
{
    TArray<FTransactionOperation> Conflicts;
...
bool UMedComEquipmentTransactionProcessor::ValidateTransaction_NoLock(const FGuid& TransactionId) const
{
    const FTransactionExecutionContext* Context = FindExecutionContext(TransactionId);
    if (!Context)
    {
