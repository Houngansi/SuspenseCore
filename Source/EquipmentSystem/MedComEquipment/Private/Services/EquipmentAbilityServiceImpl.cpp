...
UEquipmentAbilityServiceImpl::UEquipmentAbilityServiceImpl()
{
...
void UEquipmentAbilityServiceImpl::BeginDestroy()
{
    ShutdownService(true);
    Super::BeginDestroy();
}
...
bool UEquipmentAbilityServiceImpl::InitializeService(const FServiceInitParams& Params)
{
    ...
    {
        UE_LOG(LogEquipmentAbility, Warning, TEXT("Service already initialized"));
    ...
    if (bEnablePeriodicCleanup)
    {
        if (UWorld* World = GetWorld())
        {
            World->GetTimerManager().SetTimer(
                CleanupTimerHandle,
                this,
                &UEquipmentAbilityServiceImpl::OnCleanupTimer,
                CleanupInterval,
                true // Loop
            );
            
            UE_LOG(LogEquipmentAbility, Log, TEXT("Periodic cleanup enabled every %.1f seconds"),
                CleanupInterval);
        }
    }
    ...
    UE_LOG(LogEquipmentAbility, Log, TEXT("EquipmentAbilityService initialized with %d mappings"),
        AbilityMappings.Num());
    
    return true;
}

bool UEquipmentAbilityServiceImpl::ShutdownService(bool bForce)
{
    ...
    {
        if (UMedComEquipmentAbilityConnector* Connector = Pair.Value)
        {
    ...
    ServiceMetrics.RecordSuccess();
    UE_LOG(LogEquipmentAbility, Log, TEXT("EquipmentAbilityService shutdown complete"));
    return true;
}

FGameplayTag UEquipmentAbilityServiceImpl::GetServiceTag() const
{
    return FGameplayTag::RequestGameplayTag(TEXT("Service.Equipment.Ability"));
}

FGameplayTagContainer UEquipmentAbilityServiceImpl::GetRequiredDependencies() const
{
...
bool UEquipmentAbilityServiceImpl::ValidateService(TArray<FText>& OutErrors) const
{
...
void UEquipmentAbilityServiceImpl::ResetService()
{
    ...
    {
        if (UMedComEquipmentAbilityConnector* Connector = ConnectorPair.Value)
        {
            if (IsValid(Connector))
            {
                Connector->ClearAll();
    ...
    UE_LOG(LogEquipmentAbility, Log, TEXT("EquipmentAbilityService reset"));
}

FString UEquipmentAbilityServiceImpl::GetServiceStats() const
{
    EQUIPMENT_READ_LOCK(ConnectorLock);
...
int32 UEquipmentAbilityServiceImpl::LoadAbilityMappings(UDataTable* MappingTable)
{
    SCOPED_SERVICE_TIMER("LoadAbilityMappings");
    
    if (!ensure(IsInGameThread()))
    {
        UE_LOG(LogEquipmentAbility, Error, TEXT("LoadAbilityMappings must be called on GameThread"));
        ServiceMetrics.RecordError();
        return 0;
    }
    ...
    for (const FName& RowName : RowNames)
    {
        ...
        if (!Mapping)
        {
            UE_LOG(LogEquipmentAbility, Warning, 
        ...
        {
            UE_LOG(LogEquipmentAbility, Verbose, 
    ...
    {
        UE_LOG(LogEquipmentAbility, Log, 
...
UMedComEquipmentAbilityConnector* UEquipmentAbilityServiceImpl::GetOrCreateConnectorForEquipment(
    ...
    AActor* OwnerActor)
{
    ...
    if (!ensure(IsInGameThread()))
    {
        UE_LOG(LogEquipmentAbility, Error, TEXT("GetOrCreateConnectorForEquipment must be called on GameThread"));
        ServiceMetrics.RecordError();
        return nullptr;
    }
    ...
    {
        UE_LOG(LogEquipmentAbility, Warning, TEXT("Equipment %s or Owner %s is not valid"), 
    ...
    UMedComEquipmentAbilityConnector** ExistingConnector = EquipmentConnectors.Find(EquipmentActor);
    ...
    UMedComEquipmentAbilityConnector* NewConnector = CreateConnectorForEquipment(EquipmentActor, OwnerActor);
    if (NewConnector)
    {
        ...
        EquipmentActor->OnDestroyed.AddDynamic(this, &UEquipmentAbilityServiceImpl::OnEquipmentActorDestroyed);
...
bool UEquipmentAbilityServiceImpl::RemoveConnectorForEquipment(AActor* EquipmentActor)
{
    SCOPED_SERVICE_TIMER("RemoveConnectorForEquipment");
    
    if (!ensure(IsInGameThread()))
    {
        UE_LOG(LogEquipmentAbility, Error, TEXT("RemoveConnectorForEquipment must be called on GameThread"));
        ServiceMetrics.RecordError();
        return false;
    }
    ...
    UMedComEquipmentAbilityConnector* Connector = nullptr;
    if (EquipmentConnectors.RemoveAndCopyValue(EquipmentActor, Connector))
    {
        ...
        EquipmentActor->OnDestroyed.RemoveDynamic(this, &UEquipmentAbilityServiceImpl::OnEquipmentActorDestroyed);
...
bool UEquipmentAbilityServiceImpl::HasAbilityMapping(FName ItemID) const
{
...
bool UEquipmentAbilityServiceImpl::GetAbilityMapping(
    FName ItemID, 
    FEquipmentAbilityMapping& OutMapping) const
{
    ...
    if (Mapping)
    {
        ...
        // Update cache
        const_cast<UEquipmentAbilityServiceImpl*>(this)->MappingCache->Set(
...
bool UEquipmentAbilityServiceImpl::ExportMetricsToCSV(const FString& FilePath) const
{
    ...
    if (bResult)
    {
        UE_LOG(LogEquipmentAbility, Log, TEXT("Exported metrics to %s"), *FilePath);
    }
    else
    {
        UE_LOG(LogEquipmentAbility, Error, TEXT("Failed to export metrics to %s"), *FilePath);
    }
...
void UEquipmentAbilityServiceImpl::ProcessEquipmentSpawn(
    AActor* EquipmentActor,
    AActor* OwnerActor,
    const FInventoryItemInstance& ItemInstance)
{
    ...
    if (!ensure(IsInGameThread()))
    {
        UE_LOG(LogEquipmentAbility, Error, TEXT("ProcessEquipmentSpawn must be called on GameThread"));
        ServiceMetrics.RecordError();
        return;
    }
    ...
    UMedComEquipmentAbilityConnector* Connector = GetOrCreateConnectorForEquipment(EquipmentActor, OwnerActor);
    if (!Connector)
    {
        UE_LOG(LogEquipmentAbility, Error, 
...
void UEquipmentAbilityServiceImpl::ProcessEquipmentDestroy(AActor* EquipmentActor)
{
    SCOPED_SERVICE_TIMER("ProcessEquipmentDestroy");
    
    if (!ensure(IsInGameThread()))
    {
        UE_LOG(LogEquipmentAbility, Error, TEXT("ProcessEquipmentDestroy must be called on GameThread"));
        ServiceMetrics.RecordError();
        return;
    }
...
void UEquipmentAbilityServiceImpl::UpdateEquipmentAbilities(
    AActor* EquipmentActor,
    const FInventoryItemInstance& UpdatedItemInstance)
{
    SCOPED_SERVICE_TIMER("UpdateEquipmentAbilities");
    
    if (!ensure(IsInGameThread()))
    {
        UE_LOG(LogEquipmentAbility, Error, TEXT("UpdateEquipmentAbilities must be called on GameThread"));
        ServiceMetrics.RecordError();
        return;
    }
    ...
    UMedComEquipmentAbilityConnector** ConnectorPtr = EquipmentConnectors.Find(EquipmentActor);
    if (ConnectorPtr && *ConnectorPtr)
    {
        UMedComEquipmentAbilityConnector* Connector = *ConnectorPtr;
...
int32 UEquipmentAbilityServiceImpl::CleanupInvalidConnectors()
{
    ...
    {
        UMedComEquipmentAbilityConnector** ConnectorPtr = EquipmentConnectors.Find(Equipment);
        if (ConnectorPtr)
        {
            UMedComEquipmentAbilityConnector* Connector = *ConnectorPtr;
            if (IsValid(Connector))
...
void UEquipmentAbilityServiceImpl::InitializeDefaultMappings()
{
    // Try to load default mapping table
    if (DefaultMappingTable.IsNull())
    {
        UE_LOG(LogEquipmentAbility, Log, TEXT("No default mapping table configured"));
        return;
    }
    ...
    TWeakObjectPtr<UEquipmentAbilityServiceImpl> WeakThis(this);
    StreamableManager.RequestAsyncLoad(
        DefaultMappingTable.ToSoftObjectPath(),
        [WeakThis]()
        {
            if (UEquipmentAbilityServiceImpl* StrongThis = WeakThis.Get())
            {
...
void UEquipmentAbilityServiceImpl::SetupEventHandlers()
{
    ...
    FEventSubscriptionHandle SpawnedHandle = EventBus->Subscribe(
        ...
        FEventHandlerDelegate::CreateUObject(
            this, &UEquipmentAbilityServiceImpl::OnEquipmentSpawned),
        EEventPriority::High
    );
    ...
    FEventSubscriptionHandle DestroyedHandle = EventBus->Subscribe(
        ...
        FEventHandlerDelegate::CreateUObject(
            this, &UEquipmentAbilityServiceImpl::OnEquipmentDestroyed),
        EEventPriority::High
    );
    ...
    if (Tag_OnEquipped.IsValid())
    {
        EventSubscriptions.Add(EventBus->Subscribe(
            Tag_OnEquipped,
            FEventHandlerDelegate::CreateUObject(this, &UEquipmentAbilityServiceImpl::OnEquipped),
            EEventPriority::High));
    }
    if (Tag_OnUnequipped.IsValid())
    {
        EventSubscriptions.Add(EventBus->Subscribe(
            Tag_OnUnequipped,
            FEventHandlerDelegate::CreateUObject(this, &UEquipmentAbilityServiceImpl::OnUnequipped),
            EEventPriority::High));
    }
    if (Tag_OnAbilitiesRefresh.IsValid())
    {
        EventSubscriptions.Add(EventBus->Subscribe(
            ...
            FEventHandlerDelegate::CreateUObject(this, &UEquipmentAbilityServiceImpl::OnAbilitiesRefresh),
    ...
    if (Tag_OnCommit.IsValid())
    {
        EventSubscriptions.Add(EventBus->Subscribe(
            Tag_OnCommit,
            FEventHandlerDelegate::CreateUObject(this, &UEquipmentAbilityServiceImpl::OnCommit),
            EEventPriority::Normal));
    }
    
    UE_LOG(LogEquipmentAbility, Log, TEXT("Event handlers registered"));
}

void UEquipmentAbilityServiceImpl::EnsureValidConfig()
{
    // Sanitize cache TTL
...
void UEquipmentAbilityServiceImpl::OnEquipmentSpawned(const FEquipmentEventData& EventData)
{
...
void UEquipmentAbilityServiceImpl::OnEquipmentDestroyed(const FEquipmentEventData& EventData)
{
...
void UEquipmentAbilityServiceImpl::OnEquipped(const FEquipmentEventData& EventData)
{
    FInventoryItemInstance ItemInstance;
    AActor* EquipmentActor = nullptr;
    AActor* OwnerActor = nullptr;
    if (!ParseEquipmentEventData(EventData, ItemInstance, EquipmentActor, OwnerActor))
    {
        UE_LOG(LogEquipmentAbility, Warning, TEXT("OnEquipped: parse failed"));
        ServiceMetrics.Inc(FName("Ability.Events.ParseFailed"));
        return;
    }
    ProcessEquipmentSpawn(EquipmentActor, OwnerActor, ItemInstance);
...
void UEquipmentAbilityServiceImpl::OnUnequipped(const FEquipmentEventData& EventData)
{
    AActor* EquipmentActor = Cast<AActor>(EventData.Source.Get());
    if (!EquipmentActor)
    {
        ...
        {
            UE_LOG(LogEquipmentAbility, Warning, TEXT("OnUnequipped: invalid source"));
            ServiceMetrics.Inc(FName("Ability.Events.InvalidSource"));
            return;
        }
    }
...
void UEquipmentAbilityServiceImpl::OnAbilitiesRefresh(const FEquipmentEventData& EventData)
{
    FInventoryItemInstance ItemInstance;
    AActor* EquipmentActor = nullptr;
    AActor* OwnerActor = nullptr;
    if (!ParseEquipmentEventData(EventData, ItemInstance, EquipmentActor, OwnerActor))
    {
        UE_LOG(LogEquipmentAbility, Warning, TEXT("OnAbilitiesRefresh: parse failed"));
        ServiceMetrics.Inc(FName("Ability.Events.ParseFailed"));
        return;
    }
...
void UEquipmentAbilityServiceImpl::OnCommit(const FEquipmentEventData& EventData)
{
    FInventoryItemInstance ItemInstance;
    AActor* EquipmentActor = nullptr;
    AActor* OwnerActor = nullptr;
    if (!ParseEquipmentEventData(EventData, ItemInstance, EquipmentActor, OwnerActor))
    {
        UE_LOG(LogEquipmentAbility, Warning, TEXT("OnCommit: parse failed"));
        ServiceMetrics.Inc(FName("Ability.Events.ParseFailed"));
        return;
    }
    UpdateEquipmentAbilities(EquipmentActor, ItemInstance);
...
void UEquipmentAbilityServiceImpl::OnEquipmentActorDestroyed(AActor* DestroyedActor)
{
    // Idempotent - RemoveConnectorForEquipment handles if connector doesn't exist
    RemoveConnectorForEquipment(DestroyedActor);
}

void UEquipmentAbilityServiceImpl::OnCleanupTimer()
{
...
UMedComEquipmentAbilityConnector* UEquipmentAbilityServiceImpl::CreateConnectorForEquipment(
    AActor* EquipmentActor,
    AActor* OwnerActor)
{
    if (!EquipmentActor || !OwnerActor)
    ...
    UMedComEquipmentAbilityConnector* Connector = NewObject<UMedComEquipmentAbilityConnector>(
        EquipmentActor,
        UMedComEquipmentAbilityConnector::StaticClass(),
...
UAbilitySystemComponent* UEquipmentAbilityServiceImpl::FindOwnerAbilitySystemComponent(AActor* OwnerActor) const
{
    if (!OwnerActor)
    {
        return nullptr;
...
FGameplayTagContainer UEquipmentAbilityServiceImpl::GetEquipmentTags(AActor* EquipmentActor) const
{
    FGameplayTagContainer Tags;
    
    if (!EquipmentActor || !IsValid(EquipmentActor))
    {
        UE_LOG(LogEquipmentAbility, Warning, TEXT("GetEquipmentTags called with invalid equipment"));
        return Tags;
    }
...
bool UEquipmentAbilityServiceImpl::ParseEquipmentEventData(
    const FEquipmentEventData& EventData,
    FInventoryItemInstance& OutItem,
