...
UEquipmentDataServiceImpl::UEquipmentDataServiceImpl()
{
    ...
    ItemCache = MakeShareable(new FEquipmentCacheManager<int32, FInventoryItemInstance>(500));
...
void UEquipmentDataServiceImpl::InjectComponents(UObject* InDataStore, UObject* InItemManager)
{
    ...
    if (!InItemManager)
    {
        UE_LOG(LogEquipmentData, Error, 
            TEXT("InjectComponents: ItemManager is null (REQUIRED)"));
        return;
    }
    
    // Cast ItemManager (REQUIRED)
    UMedComItemManager* CastedItemManager = Cast<UMedComItemManager>(InItemManager);
    ...
    if (InDataStore)
    {
        UMedComEquipmentDataStore* CastedDataStore = Cast<UMedComEquipmentDataStore>(InDataStore);
        
        if (!CastedDataStore)
        {
            UE_LOG(LogEquipmentData, Error,
                TEXT("InjectComponents: Failed to cast DataStore to UMedComEquipmentDataStore. Received type: %s"),
        ...
        {
            UE_LOG(LogEquipmentData, Warning, 
        ...
        DataProviderInterface.SetInterface(Cast<IMedComEquipmentDataProvider>(DataStore));
        
        if (!DataProviderInterface.GetInterface())
        {
            UE_LOG(LogEquipmentData, Error,
                TEXT("InjectComponents: Failed to cast DataStore to IMedComEquipmentDataProvider"));
            DataStore = nullptr;
            return;
        }
...
void UEquipmentDataServiceImpl::SetValidator(UObject* InValidator)
{
    if (!InValidator)
    {
        UE_LOG(LogEquipmentData, Error,
    ...
    UMedComEquipmentSlotValidator* CastedValidator = Cast<UMedComEquipmentSlotValidator>(InValidator);
    
    if (!CastedValidator)
    {
        UE_LOG(LogEquipmentData, Error,
            TEXT("SetValidator: Failed to cast to UMedComEquipmentSlotValidator. Received type: %s"),
...
bool UEquipmentDataServiceImpl::InitializeService(const FServiceInitParams& Params)
{
    ...
    if (!ItemManager)
    {
        UE_LOG(LogEquipmentData, Error, TEXT("InitializeService: ItemManager not injected!"));
        UE_LOG(LogEquipmentData, Error, TEXT("  Call InjectComponents(DataStore, ItemManager) before InitializeService"));
        RECORD_SERVICE_METRIC("Data.Init.NoItemManager", 1);
        ServiceState = EServiceLifecycleState::Failed;
        return false;
    }
    
    UE_LOG(LogEquipmentData, Log, TEXT("InitializeService: ItemManager available (%s)"), 
        *ItemManager->GetName());
    ...
    // Phase 1: Check state and validate under lock
    {
        ...
        if (!bComponentsInjected)
        {
            if (AActor* OwnerActor = Cast<AActor>(Params.Owner))
            {
                ...
                {
                    if (UMedComEquipmentDataStore* AutoDS = OwnerActor->FindComponentByClass<UMedComEquipmentDataStore>())
                    {
                        if (UMedComEquipmentTransactionProcessor* AutoTP = OwnerActor->FindComponentByClass<UMedComEquipmentTransactionProcessor>())
                        {
    ...
    if (TransactionProcessor)
    {
        ...
        TransactionProcessor->SetDeltaCallback(
            FOnTransactionDelta::CreateUObject(this, &UEquipmentDataServiceImpl::OnTransactionDeltas)
        );
    }
    else
    {
...
bool UEquipmentDataServiceImpl::ShutdownService(bool bForce)
{
    ...
    UMedComEquipmentTransactionProcessor* OldTransactionProcessor = TransactionProcessor;
...
FGameplayTag UEquipmentDataServiceImpl::GetServiceTag() const
{
    SCOPED_SERVICE_TIMER("Data.GetServiceTag");
    return FGameplayTag::RequestGameplayTag(TEXT("Service.Equipment.Data"));
}

FGameplayTagContainer UEquipmentDataServiceImpl::GetRequiredDependencies() const
{
...
bool UEquipmentDataServiceImpl::ValidateService(TArray<FText>& OutErrors) const
{
...
void UEquipmentDataServiceImpl::ResetService()
{
    ...
    UE_LOG(LogEquipmentData, Log, TEXT("ResetService: Beginning comprehensive service reset"));
    ...
    UE_LOG(LogEquipmentData, Log, TEXT("ResetService: Service reset completed successfully"));
}

FString UEquipmentDataServiceImpl::GetServiceStats() const
{
...
IMedComEquipmentDataProvider* UEquipmentDataServiceImpl::GetDataProvider()
{
    SCOPED_SERVICE_TIMER("Data.GetDataProvider");
    return DataProviderInterface.GetInterface();
}

IMedComTransactionManager* UEquipmentDataServiceImpl::GetTransactionManager()
{
...
FInventoryItemInstance UEquipmentDataServiceImpl::GetSlotItemCached(int32 SlotIndex) const
{
    ...
    // Step 1: Cache-first approach - check cache under read lock
    {
        ...
        FInventoryItemInstance CachedItem;
...
TMap<int32, FInventoryItemInstance> UEquipmentDataServiceImpl::BatchGetSlotItems(const TArray<int32>& SlotIndices) const
{
    ...
    TMap<int32, FInventoryItemInstance> Result;
    ...
    {
        TMap<int32, FInventoryItemInstance> DataStoreItems;
        
        {
            ...
            {
                FInventoryItemInstance Item = DataStore->GetSlotItem(SlotIndex);
                if (Item.IsValid())
                {
...
FGuid UEquipmentDataServiceImpl::SwapSlotItems(int32 SlotA, int32 SlotB)
{
    ...
    FInventoryItemInstance ItemA;
    FInventoryItemInstance ItemB;
    ...
    if (bSuccess)
    {
        ...
        TWeakObjectPtr<UMedComEquipmentDataStore> WeakDS = DataStore;
...
FGuid UEquipmentDataServiceImpl::BatchUpdateSlots(const TMap<int32, FInventoryItemInstance>& Updates)
{
...

    {
        ...
        for (const auto& UpdatePair : Updates)
        {
            ...
            const FInventoryItemInstance& NewItem = UpdatePair.Value;
...
FGuid UEquipmentDataServiceImpl::CreateDataCheckpoint(const FString& CheckpointName)
{
...
bool UEquipmentDataServiceImpl::RollbackToCheckpoint(const FGuid& CheckpointId)
{
...
bool UEquipmentDataServiceImpl::ValidateDataIntegrity(bool bDeep) const
{
...
void UEquipmentDataServiceImpl::InvalidateCache(int32 SlotIndex)
{
    ...
    if (SlotIndex == -1)
    {
        ...
        UE_LOG(LogEquipmentData, Log, TEXT("InvalidateCache: All caches cleared"));
    }
    else
    {
        ...
        if (bEnableDetailedLogging)
        {
            UE_LOG(LogEquipmentData, Verbose,
                TEXT("InvalidateCache: Cleared cache for slot %d"), SlotIndex);
        }
    }
}

FString UEquipmentDataServiceImpl::GetCacheStatistics() const
{
...
bool UEquipmentDataServiceImpl::ExportMetricsToCSV(const FString& FilePath) const
{
    ...
    if (FilePath.IsEmpty())
    {
        UE_LOG(LogEquipmentData, Warning, TEXT("Empty FilePath for ExportMetricsToCSV"));
    ...
    if (bSuccess)
    {
        ...
        UE_LOG(LogEquipmentData, Log, TEXT("ExportMetricsToCSV: Metrics exported to %s"), *FilePath);
        bOk = true;
    }
    else
    {
        RECORD_SERVICE_METRIC("Data.Metrics.Export.Fail", 1);
        UE_LOG(LogEquipmentData, Error, TEXT("ExportMetricsToCSV: Failed to export metrics to %s"), *FilePath);
    }
...
TArray<FEquipmentDelta> UEquipmentDataServiceImpl::GetRecentDeltas(int32 MaxCount) const
{
...
FString UEquipmentDataServiceImpl::GetDeltaStatistics() const
{
    FScopeLock Lock(&DeltaLock);
...
void UEquipmentDataServiceImpl::InitializeDataStorage()
{
...
void UEquipmentDataServiceImpl::SetupEventSubscriptions()
{
    ...
    TWeakObjectPtr<UEquipmentDataServiceImpl> WeakThis(this);
    
    DataStore->OnSlotDataChanged().AddLambda(
        [WeakThis](int32 SlotIndex, const FInventoryItemInstance& NewItem)
        {
            if (!WeakThis.IsValid())
            {
                return;
            ...
            UEquipmentDataServiceImpl* Service = WeakThis.Get();
    ...
    DataStore->OnEquipmentDelta().AddUObject(this, &UEquipmentDataServiceImpl::OnDataStoreDelta);
    ...
    if (EventBus.IsValid())
    {
        ...
        EventScope.Subscribe(
            ...
            FEventHandlerDelegate::CreateUObject(this, &UEquipmentDataServiceImpl::OnCacheInvalidation)
        );

        // S8: Subscribe to resend requests (state refresh)
        EventScope.Subscribe(
            FGameplayTag::RequestGameplayTag(TEXT("Equipment.Event.RequestResend")),
            FEventHandlerDelegate::CreateUObject(this, &UEquipmentDataServiceImpl::OnResendRequested)
        );
...
void UEquipmentDataServiceImpl::OnCacheInvalidation(const FEquipmentEventData& EventData)
{
...
void UEquipmentDataServiceImpl::OnTransactionCompleted(const FGuid& TransactionId, bool bSuccess)
{
    if (bSuccess)
    {
        SuccessfulTransactions.fetch_add(1);
...
void UEquipmentDataServiceImpl::OnDataStoreDelta(const FEquipmentDelta& Delta)
{
...
void UEquipmentDataServiceImpl::OnTransactionDeltas(const TArray<FEquipmentDelta>& Deltas)
{
...
void UEquipmentDataServiceImpl::BroadcastDelta(const FEquipmentDelta& Delta)
{
...
void UEquipmentDataServiceImpl::BroadcastBatchDeltas(const TArray<FEquipmentDelta>& Deltas)
{
...
bool UEquipmentDataServiceImpl::PerformDeepValidation() const
{
    FRWScopeLock DataScopeLock(DataLock, SLT_ReadOnly);
    return PerformDeepValidation_Internal();
}

bool UEquipmentDataServiceImpl::PerformDeepValidation_Internal() const
{
    ...
    {
        FInventoryItemInstance Item = DataStore->GetSlotItem(i);
...
bool UEquipmentDataServiceImpl::ValidateDataIntegrity_Internal(bool bDeep) const
{
    // Предполагаем, что DataLock уже удерживается вызывающим кодом
    if (!DataStore)
    {
...
TArray<FEquipmentSlotConfig> UEquipmentDataServiceImpl::CreateDefaultSlotConfiguration() const
{
...
void UEquipmentDataServiceImpl::UpdateCacheEntry(int32 SlotIndex, const FInventoryItemInstance& Item) const
{
    FRWScopeLock CacheWriteLock(CacheLock, SLT_Write);
...
void UEquipmentDataServiceImpl::BatchUpdateCache(const TMap<int32, FInventoryItemInstance>& Items) const
{
    // Нечего обновлять
    ...
    for (const auto& Pair : Items)
    {
        ...
        const FInventoryItemInstance& Item = Pair.Value;

        if (Item.IsValid())
        {
            // Double-checked: если кто-то уже положил значение параллельно — не перетираем
            FInventoryItemInstance Existing;
...
void UEquipmentDataServiceImpl::LogDataOperation(const FString& Operation, int32 SlotIndex) const
{
    if (!bEnableDetailedLogging)
    {
        return;
    ...
    if (SlotIndex >= 0)
    {
        UE_LOG(LogEquipmentData, VeryVerbose,
...
void UEquipmentDataServiceImpl::ResetDataStore()
{
    if (!DataStore)
    {
        return;
...
bool UEquipmentDataServiceImpl::IsValidSlotIndex(int32 SlotIndex) const
{
...
void UEquipmentDataServiceImpl::WarmupCachesSafe()
{
    ...
    TMap<int32, FInventoryItemInstance> WarmupItems;
    ...
    // 1) Собираем данные из DataStore под DataLock (read-only)
    {
        ...
        {
            const FInventoryItemInstance Item = DataStore->GetSlotItem(i);
            if (Item.IsValid())
            {
...
void UEquipmentDataServiceImpl::OnResendRequested(const FEquipmentEventData& Event)
{
