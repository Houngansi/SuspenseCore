...
UEquipmentNetworkServiceImpl::UEquipmentNetworkServiceImpl()
...
bool UEquipmentNetworkServiceImpl::ShutdownService(bool bForce)
{
    // Вызываем невиртуальный метод cleanup
...
void UEquipmentNetworkServiceImpl::InternalShutdown(bool bForce, bool bFromDestructor)
{
    ...
    if (NetworkDispatcher.GetInterface())
    {
        UMedComEquipmentNetworkDispatcher* Dispatcher =
            Cast<UMedComEquipmentNetworkDispatcher>(NetworkDispatcher.GetObject());
    ...
    UE_LOG(LogEquipmentNetwork, Log, TEXT("Equipment Network Service shutdown complete"));
}

bool UEquipmentNetworkServiceImpl::ResolveDependencies(
    UWorld* World,
    TScriptInterface<IMedComEquipmentDataProvider>& OutDataProvider,
    TScriptInterface<IMedComEquipmentOperations>& OutOperationExecutor)
{
    if (!World)
    {
        UE_LOG(LogEquipmentNetwork, Error, TEXT("ResolveDependencies: invalid World"));
        return false;
    }
    ...
    if (!ServiceLocator)
    {
        UE_LOG(LogEquipmentNetwork, Error, TEXT("ResolveDependencies: ServiceLocator not available"));
        return false;
    }
    ...
    // Resolve Data Provider
    {
        ...
        if (ServiceObj && ServiceObj->GetClass()->ImplementsInterface(UEquipmentDataService::StaticClass()))
        {
            ...
            if (DataService)
            {
                IMedComEquipmentDataProvider* Provider = DataService->GetDataProvider();
                if (Provider)
                {
        ...
        {
            UE_LOG(LogEquipmentNetwork, Error, TEXT("ResolveDependencies: Failed to resolve DataProvider"));
            return false;
        }
    }
    ...
    // Resolve Operation Executor
    {
        ...
        if (ServiceObj && ServiceObj->GetClass()->ImplementsInterface(UEquipmentOperationService::StaticClass()))
        {
            ...
            if (OpService)
            {
                IMedComEquipmentOperations* Executor = OpService->GetOperationsExecutor();
                if (Executor)
                {
        ...
        {
            UE_LOG(LogEquipmentNetwork, Error, TEXT("ResolveDependencies: Failed to resolve OperationExecutor"));
            return false;
        }
    }
...
UMedComEquipmentNetworkDispatcher* UEquipmentNetworkServiceImpl::CreateAndInitNetworkDispatcher(
    AActor* OwnerActor,
    const TScriptInterface<IMedComEquipmentOperations>& OperationExecutor)
{
    if (!OwnerActor)
    {
        UE_LOG(LogEquipmentNetwork, Error, TEXT("CreateAndInitNetworkDispatcher: OwnerActor is null"));
        return nullptr;
    }

    UMedComEquipmentNetworkDispatcher* Dispatcher = NewObject<UMedComEquipmentNetworkDispatcher>(
        OwnerActor,
        UMedComEquipmentNetworkDispatcher::StaticClass(),
        TEXT("EquipmentNetworkDispatcher"));

    if (!Dispatcher)
    {
        UE_LOG(LogEquipmentNetwork, Error, TEXT("Failed to create EquipmentNetworkDispatcher"));
        return nullptr;
    }
    ...
    UE_LOG(LogEquipmentNetwork, Log, TEXT("NetworkDispatcher created & configured"));

    // ВАЖНО: храним как TScriptInterface, без TSharedPtr
    NetworkDispatcher.SetObject(Dispatcher);
    NetworkDispatcher.SetInterface(Cast<IMedComNetworkDispatcher>(Dispatcher));

    return Dispatcher;
}

UMedComEquipmentPredictionSystem* UEquipmentNetworkServiceImpl::CreateAndInitPredictionSystem(
    AActor* OwnerActor,
    const TScriptInterface<IMedComEquipmentDataProvider>& DataProvider,
    const TScriptInterface<IMedComEquipmentOperations>& OperationExecutor)
{
    if (!OwnerActor)
    {
        UE_LOG(LogEquipmentNetwork, Error, TEXT("CreateAndInitPredictionSystem: OwnerActor is null"));
        return nullptr;
    }

    UMedComEquipmentPredictionSystem* Prediction = NewObject<UMedComEquipmentPredictionSystem>(
        OwnerActor,
        UMedComEquipmentPredictionSystem::StaticClass(),
        TEXT("EquipmentPredictionSystem"));

    if (!Prediction)
    {
        UE_LOG(LogEquipmentNetwork, Error, TEXT("Failed to create EquipmentPredictionSystem"));
        return nullptr;
    }
    ...
    {
        UE_LOG(LogEquipmentNetwork, Error, TEXT("PredictionSystem Initialize failed"));
    ...
    UE_LOG(LogEquipmentNetwork, Log, TEXT("PredictionSystem created & initialized"));

    // НЕ TSharedPtr — используем TScriptInterface (GC-safe)
    PredictionManager.SetObject(Prediction);
    PredictionManager.SetInterface(Cast<IMedComPredictionManager>(Prediction));

    return Prediction;
}

UMedComEquipmentReplicationManager* UEquipmentNetworkServiceImpl::CreateAndInitReplicationManager(
    AActor* OwnerActor,
    const TScriptInterface<IMedComEquipmentDataProvider>& DataProvider)
{
    if (!OwnerActor)
    {
        UE_LOG(LogEquipmentNetwork, Error, TEXT("CreateAndInitReplicationManager: OwnerActor is null"));
        return nullptr;
    }

    UMedComEquipmentReplicationManager* Replication = NewObject<UMedComEquipmentReplicationManager>(
        OwnerActor,
        UMedComEquipmentReplicationManager::StaticClass(),
        TEXT("EquipmentReplicationManager"));

    if (!Replication)
    {
        UE_LOG(LogEquipmentNetwork, Error, TEXT("Failed to create EquipmentReplicationManager"));
        return nullptr;
    }
    ...
    {
        UE_LOG(LogEquipmentNetwork, Error, TEXT("ReplicationManager Initialize failed"));
        return nullptr;
    }
    ...
    UE_LOG(LogEquipmentNetwork, Log, TEXT("ReplicationManager created & initialized"));
...
void UEquipmentNetworkServiceImpl::BindDispatcherToPrediction(
    UMedComEquipmentNetworkDispatcher* Dispatcher,
    IMedComPredictionManager* Prediction)
{
...
void UEquipmentNetworkServiceImpl::StartMonitoringTimers(UWorld* World)
{
    if (!World) return;
    ...
    World->GetTimerManager().SetTimer(
        NonceCleanupTimer,
        this,
        &UEquipmentNetworkServiceImpl::CleanExpiredNonces,
        60.0f, true);

    World->GetTimerManager().SetTimer(
        MetricsUpdateTimer,
        this,
        &UEquipmentNetworkServiceImpl::UpdateNetworkMetrics,
        1.0f, true);

    World->GetTimerManager().SetTimer(
        MetricsExportTimer,
        this,
        &UEquipmentNetworkServiceImpl::ExportMetricsPeriodically,
        300.0f, true);
}

bool UEquipmentNetworkServiceImpl::InitializeService(const FServiceInitParams& Params)
{
    SCOPED_SERVICE_TIMER("InitializeService");

    if (ServiceState != EServiceLifecycleState::Uninitialized)
    {
        UE_LOG(LogEquipmentNetwork, Warning, TEXT("Service already initialized"));
    ...
    {
        UE_LOG(LogEquipmentNetwork, Error, TEXT("Failed to load HMAC secret key from secure storage"));
    ...
    if (!World)
    {
        UE_LOG(LogEquipmentNetwork, Error, TEXT("No valid world context for network service initialization"));
        ServiceState = EServiceLifecycleState::Failed;
        ServiceMetrics.RecordError();
        return false;
    }
    ...
    if (!OwnerActor)
    {
        UE_LOG(LogEquipmentNetwork, Error, TEXT("Service owner is not an Actor, cannot create network components"));
        ServiceState = EServiceLifecycleState::Failed;
        ServiceMetrics.RecordError();
        return false;
    }
    ...
    TScriptInterface<IMedComEquipmentOperations>  OperationExecutor;
    ...
    UMedComEquipmentPredictionSystem* Prediction =
    ...
    UMedComEquipmentReplicationManager* Replication =
    ...
    UE_LOG(LogEquipmentNetwork, Log, TEXT("========================================"));
    UE_LOG(LogEquipmentNetwork, Log, TEXT("Equipment Network Service Initialized"));
    UE_LOG(LogEquipmentNetwork, Log, TEXT("Security: Strict=%s, HMACCritical=%s, IPRateLimit=%s"),
...
FGameplayTag UEquipmentNetworkServiceImpl::GetServiceTag() const
{
...
FGameplayTagContainer UEquipmentNetworkServiceImpl::GetRequiredDependencies() const
{
    FScopedServiceTimer __svc_timer(const_cast<FServiceMetrics&>(ServiceMetrics),
...
bool UEquipmentNetworkServiceImpl::ValidateService(TArray<FText>& OutErrors) const
{
    FScopedServiceTimer __svc_timer(const_cast<FServiceMetrics&>(ServiceMetrics),
...
void UEquipmentNetworkServiceImpl::ResetService()
{
    ...
    UE_LOG(LogEquipmentNetwork, Log,
        TEXT("EquipmentNetworkService reset complete - all security and metric data cleared"));
}

FString UEquipmentNetworkServiceImpl::GetServiceStats() const
{
    FScopedServiceTimer __svc_timer(const_cast<FServiceMetrics&>(ServiceMetrics),
    ...
    if (NetworkDispatcher.GetInterface())
    {
        ...
        if (UMedComEquipmentNetworkDispatcher* Dispatcher =
                Cast<UMedComEquipmentNetworkDispatcher>(NetworkDispatcher.GetObject()))
        {
...
bool UEquipmentNetworkServiceImpl::ExportMetricsToCSV(const FString& FilePath) const
{
...
void UEquipmentNetworkServiceImpl::ReloadSecurityConfig()
{
    ...
    UE_LOG(LogEquipmentNetwork, Log, TEXT("Security configuration reloaded from INI"));
}

FGuid UEquipmentNetworkServiceImpl::SendEquipmentOperation(const FEquipmentOperationRequest& Request, APlayerController* PlayerController)
{
    ...
    if (!IsServiceReady())
    {
        UE_LOG(LogEquipmentNetwork, Error, TEXT("Network service not ready"));
        ServiceMetrics.RecordError();
    ...
    if (!PlayerController)
    {
        UE_LOG(LogEquipmentNetwork, Error, TEXT("Invalid player controller"));
        ServiceMetrics.RecordError();
    ...
    {
        UE_LOG(LogEquipmentNetwork, Error, TEXT("Failed to generate valid player GUID"));
    ...
    UE_LOG(LogEquipmentNetwork, Verbose, TEXT("Sent %s operation %s with nonce %llu from %s"),
...
bool UEquipmentNetworkServiceImpl::ReceiveEquipmentOperation(const FNetworkOperationRequest& NetworkRequest, APlayerController* SendingPlayer)
{
    ...
    if (!NetworkRequest.ValidateIntegrity())
    {
        ...
        UE_LOG(LogEquipmentNetwork, Error, TEXT("Integrity check failed for %s from %s"),
    ...
    if (!MarkNonceAsPending(NetworkRequest.Nonce))
    {
        ...
        UE_LOG(LogEquipmentNetwork, Error, TEXT("Replay blocked, nonce %llu from %s"),
    ...
    if (Age > SecurityConfig.PacketAgeLimit)
    {
        ...
        UE_LOG(LogEquipmentNetwork, Warning, TEXT("Stale packet rejected age=%.2fs from %s"),
        ...
        SecurityConfig.bRequireHMACForCritical)
    {
        if (!NetworkRequest.VerifyHMAC(HMACSecretKey))
        {
            ...
            UE_LOG(LogEquipmentNetwork, Error, TEXT("HMAC failed for %s from %s"),
                *NetworkRequest.RequestId.ToString(), *GetPlayerIdentifier(SendingPlayer));
            return false;
        }
...
void UEquipmentNetworkServiceImpl::SetNetworkQuality(float Quality)
{
    ...
    UE_LOG(LogEquipmentNetwork, Log, TEXT("Network quality set to %.2f"), NetworkQualityLevel);
}

FLatencyCompensationData UEquipmentNetworkServiceImpl::GetNetworkMetrics() const
{
    FScopedServiceTimer __svc_timer(const_cast<FServiceMetrics&>(ServiceMetrics),
...
void UEquipmentNetworkServiceImpl::ForceSynchronization(APlayerController* PlayerController)
{
    SCOPED_SERVICE_TIMER("ForceSynchronization");

    if (!IsServiceReady() || !PlayerController)
    {
        UE_LOG(LogEquipmentNetwork, Warning, TEXT("Cannot force sync - service not ready or invalid controller"));
        ServiceMetrics.RecordError();
    ...
    if (ReplicationProvider)
    {
        ...
        UE_LOG(LogEquipmentNetwork, Log, TEXT("Forced synchronization for %s"),
            *GetPlayerIdentifier(PlayerController));
    }
...
bool UEquipmentNetworkServiceImpl::ExportSecurityMetrics(const FString& FilePath) const
{
...
FGuid UEquipmentNetworkServiceImpl::CreatePlayerGuid(const FUniqueNetIdRepl& UniqueId) const
{
    FScopedServiceTimer __svc_timer(const_cast<FServiceMetrics&>(ServiceMetrics),
...
bool UEquipmentNetworkServiceImpl::CheckRateLimit(const FGuid& PlayerGuid, APlayerController* PlayerController)
{
        ...
        SecurityConfig.TemporaryBanDuration))
    {
        ...
        UE_LOG(LogEquipmentNetwork, Warning,
        ...
        (CurrentTime - RateLimitData.LastOperationTime) < SecurityConfig.MinOperationInterval)
    {
        ...
        UE_LOG(LogEquipmentNetwork, Verbose, TEXT("Operation too fast from %s (%.3f seconds since last)"),
            *RateLimitData.PlayerIdentifier,
...
bool UEquipmentNetworkServiceImpl::CheckIPRateLimit(const FString& IPAddress)
{
...
bool UEquipmentNetworkServiceImpl::MarkNonceAsPending(uint64 Nonce)
{
...
bool UEquipmentNetworkServiceImpl::ConfirmNonce(uint64 Nonce)
{
    if (!PendingNonces.Contains(Nonce))
    {
...
void UEquipmentNetworkServiceImpl::RejectNonce(uint64 Nonce)
{
    PendingNonces.Remove(Nonce);
    RECORD_SERVICE_METRIC("nonce_rejected", 1);
}

uint64 UEquipmentNetworkServiceImpl::GenerateSecureNonce()
{
    uint64 Nonce = 0;
    ...
    {
        UE_LOG(LogEquipmentNetwork, Warning, TEXT("Nonce generation collision detected after %d attempts"), MaxAttempts);
...
FString UEquipmentNetworkServiceImpl::GenerateRequestHMAC(const FNetworkOperationRequest& Request) const
{
    return Request.GenerateHMAC(HMACSecretKey);
}

bool UEquipmentNetworkServiceImpl::VerifyRequestHMAC(const FNetworkOperationRequest& Request) const
{
    return Request.VerifyHMAC(HMACSecretKey);
}

void UEquipmentNetworkServiceImpl::CleanExpiredNonces()
{
    ...
    if (CleanedCount > 0)
    {
        UE_LOG(LogEquipmentNetwork, Verbose, TEXT("Cleaned %d expired nonces, %d inactive players, %d inactive IPs"),
            CleanedCount, PlayersToRemove.Num(), IPsToRemove.Num());
    }
}

void UEquipmentNetworkServiceImpl::LogSuspiciousActivity(APlayerController* PlayerController, const FString& Reason)
{
    ...
    UE_LOG(LogEquipmentNetwork, Warning, TEXT("%s"), *LogEntry);
    ...
    if (Count >= SecurityConfig.MaxSuspiciousActivities)
    {
        ...
        UE_LOG(LogEquipmentNetwork, Error,
            TEXT("SECURITY ALERT: Player %s exceeded suspicious activity threshold! Immediate action required."),
            *PlayerID);
    }
}

FString UEquipmentNetworkServiceImpl::GetPlayerIdentifier(APlayerController* PlayerController) const
{
    if (!PlayerController)
    {
        return TEXT("Unknown");
...
FString UEquipmentNetworkServiceImpl::GetIPAddress(APlayerController* PlayerController) const
{
    if (!PlayerController)
    {
        return TEXT("Unknown");
...
void UEquipmentNetworkServiceImpl::UpdateNetworkMetrics()
{
    ...
    if (NetworkDispatcher.GetInterface())
    {
        ...
        if (UMedComEquipmentReplicationManager* ReplicationMgr = ReplicationProvider)
        {
            float NewHz = 10.0f;
...
void UEquipmentNetworkServiceImpl::UpdateSecurityMetrics(double StartTime)
{
...
void UEquipmentNetworkServiceImpl::ExportMetricsPeriodically()
{
...
void UEquipmentNetworkServiceImpl::AdaptNetworkStrategies()
{
    if (NetworkQualityLevel < 0.3f)
    {
        ...
        UE_LOG(LogEquipmentNetwork, Log, TEXT("Adapted to POOR network - optimized for playability"));
    }
    else if (NetworkQualityLevel < 0.7f)
    {
        ...
        UE_LOG(LogEquipmentNetwork, Log, TEXT("Adapted to MEDIUM network - balanced security"));
    }
    else
    {
        if (PredictionManager.GetInterface())
        ...
        UE_LOG(LogEquipmentNetwork, Log, TEXT("Adapted to GOOD network - maximum security enabled"));
    }
}

void UEquipmentNetworkServiceImpl::InitializeSecurity()
{
    ...
    UE_LOG(LogEquipmentNetwork, Log, TEXT("Security subsystems initialized with enhanced protection"));
}

void UEquipmentNetworkServiceImpl::ShutdownSecurity()
{
    ...
    UE_LOG(LogEquipmentNetwork, Log, TEXT("Final Security Report:\n%s"), *FinalReport);
    ...
    UE_LOG(LogEquipmentNetwork, Log, TEXT("Security subsystems shutdown complete"));
}

FString UEquipmentNetworkServiceImpl::LoadHMACKeyFromSecureStorage()
{
    FString Key;
    ...
    if (Key.IsEmpty())
    {
        UE_LOG(LogEquipmentNetwork, Warning, TEXT("No HMAC key found, generating new secure key"));
        ...
        {
            UE_LOG(LogEquipmentNetwork, Error, TEXT("Failed to save HMAC key to secure storage"));
        }
        else
        {
            UE_LOG(LogEquipmentNetwork, Log, TEXT("Generated and saved new HMAC key to secure storage"));
    ...
    if (Key.Len() < 32)
    {
        UE_LOG(LogEquipmentNetwork, Error, TEXT("HMAC key is too short (%d chars), security compromised!"), Key.Len());
