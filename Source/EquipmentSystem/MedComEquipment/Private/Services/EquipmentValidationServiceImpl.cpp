...
bool FShadowEquipmentSnapshot::ApplyOperation(const FEquipmentOperationRequest& Operation)
{
    switch (Operation.OperationType)
    {
        ...
        case EEquipmentOperationType::Unequip:
        {
            ...
            const FInventoryItemInstance RemovedItem = SlotItems[Operation.SourceSlotIndex];
        ...
        case EEquipmentOperationType::Move:
        {
            ...
            const FInventoryItemInstance MovedItem = SlotItems[Operation.SourceSlotIndex];
            SlotItems.Remove(Operation.SourceSlotIndex);
        ...
        case EEquipmentOperationType::Swap:
        {
            ...
            const FInventoryItemInstance Temp = SlotItems[Operation.SourceSlotIndex];
...
FInventoryItemInstance FShadowEquipmentSnapshot::GetItemAtSlot(int32 SlotIndex) const
{
    if (const FInventoryItemInstance* Found = SlotItems.Find(SlotIndex))
    {
        return *Found;
    }
...
UEquipmentValidationServiceImpl::UEquipmentValidationServiceImpl()
{
...
void UEquipmentValidationServiceImpl::EnsureValidConfig()
{
    // Sanitize CacheTTL
    ...
    UE_LOG(LogEquipmentValidation, Verbose,
...
bool UEquipmentValidationServiceImpl::InitializeService(const FServiceInitParams& Params)
{
    ...
    if (!InitializeDependencies())
    {
        ...
        UE_LOG(LogEquipmentValidation, Error, TEXT("InitializeService: Failed to initialize dependencies"));
        ServiceMetrics.RecordError();
        return false;
    }
    ...
    {
        ServiceState = EServiceLifecycleState::Failed;
        UE_LOG(LogEquipmentValidation, Error, TEXT("InitializeService: Failed to create rules coordinator"));
        ServiceMetrics.RecordError();
        return false;
    }
    ...
    {
        ServiceState = EServiceLifecycleState::Failed;
        UE_LOG(LogEquipmentValidation, Error, TEXT("InitializeService: Failed to initialize rules coordinator"));
        ServiceMetrics.RecordError();
        return false;
    }
...
bool UEquipmentValidationServiceImpl::ShutdownService(bool bForce)
{
...
FGameplayTag UEquipmentValidationServiceImpl::GetServiceTag() const
{
...
FGameplayTagContainer UEquipmentValidationServiceImpl::GetRequiredDependencies() const
{
...
bool UEquipmentValidationServiceImpl::ValidateService(TArray<FText>& OutErrors) const
{
...
void UEquipmentValidationServiceImpl::ResetService()
{
    SCOPED_SERVICE_TIMER("Validation.ResetService");
    EQUIPMENT_WRITE_LOCK(CacheLock);
    FEquipmentRWGuard StatsWriteGuard(StatsLock, ELockType::Write);
    
    UE_LOG(LogEquipmentValidation, Log, TEXT("ResetService: Beginning service reset"));
    ...
    UE_LOG(LogEquipmentValidation, Log, TEXT("ResetService: Service reset complete"));
    
    ServiceMetrics.RecordSuccess();
    RECORD_SERVICE_METRIC("Validation.Service.Reset", 1);
}

FString UEquipmentValidationServiceImpl::GetServiceStats() const
{
...
IMedComEquipmentRules* UEquipmentValidationServiceImpl::GetRulesEngine()
{
    SCOPED_SERVICE_TIMER("Validation.GetRulesEngine");
    return Rules.GetInterface();
}

bool UEquipmentValidationServiceImpl::RegisterValidator(const FGameplayTag& ValidatorTag, TFunction<bool(const void*)> Validator)
{
...
void UEquipmentValidationServiceImpl::ClearValidationCache()
{
    ...
    UE_LOG(LogEquipmentValidation, Verbose, TEXT("Validation cache cleared"));
}
...
FSlotValidationResult UEquipmentValidationServiceImpl::ValidateEquipmentOperation(const FEquipmentOperationRequest& Request)
{
    SCOPED_SERVICE_TIMER("Validation.ValidateEquipmentOperation");
    return ValidateSingleRequest(Request, true);
}

FSlotValidationResult UEquipmentValidationServiceImpl::ValidateSingleRequest(
...
FSlotValidationResult UEquipmentValidationServiceImpl::ValidateSingleRequestParallel(
    const FEquipmentOperationRequest& Request)
{
...
FSlotValidationResult UEquipmentValidationServiceImpl::ValidateInTransaction(
    const FEquipmentOperationRequest& Request,
...
FSlotValidationBatchResult UEquipmentValidationServiceImpl::BatchValidateEx(
...
TArray<FSlotValidationResult> UEquipmentValidationServiceImpl::BatchValidate(
...
FBatchValidationReport UEquipmentValidationServiceImpl::BatchValidateWithReport(
    const TArray<FEquipmentOperationRequest>& Requests,
    bool bAtomic)
{
...
TArray<FSlotValidationResult> UEquipmentValidationServiceImpl::ProcessParallelBatch(
    const TArray<FEquipmentOperationRequest>& Requests)
{
...
TArray<FSlotValidationResult> UEquipmentValidationServiceImpl::ProcessSequentialBatch(
    const TArray<FEquipmentOperationRequest>& Requests,
    bool bStopOnFailure)
{
...
FBatchValidationReport UEquipmentValidationServiceImpl::ProcessBatchWithShadowSnapshot(
    const TArray<FEquipmentOperationRequest>& Requests)
{
    ...
    if (DataProvider.GetInterface())
    {
        const TMap<int32, FInventoryItemInstance> CurrentEquipment = DataProvider->GetAllEquippedItems();
...
FSlotValidationResult UEquipmentValidationServiceImpl::ValidateAgainstShadowSnapshot(
    const FEquipmentOperationRequest& Request,
...
void UEquipmentValidationServiceImpl::DispatchBufferedEvents(const TArray<FBufferedValidationEvent>& Events)
{
    if (!IsInGameThread())
    {
        UE_LOG(LogEquipmentValidation, Error, TEXT("DispatchBufferedEvents called from non-game thread"));
        return;
    }
...
void UEquipmentValidationServiceImpl::ValidateAsync(
    const FEquipmentOperationRequest& Request,
...
bool UEquipmentValidationServiceImpl::ExportMetricsToCSV(const FString& AbsoluteFilePath) const
{
...
bool UEquipmentValidationServiceImpl::InitializeDependencies()
{
    ...
    IMedComEquipmentDataProvider* RawDataProvider = DataServiceInterface->GetDataProvider();
    ...
    IMedComTransactionManager* RawTransactionManager = DataServiceInterface->GetTransactionManager();
...
void UEquipmentValidationServiceImpl::SetupEventSubscriptions()
{
    auto EventBus = FEquipmentEventBus::Get();
    if (!EventBus.IsValid())
    {
        UE_LOG(LogEquipmentValidation, Warning, TEXT("SetupEventSubscriptions: EventBus not available"));
        return;
    }
    ...
    UE_LOG(LogEquipmentValidation, Log, TEXT("SetupEventSubscriptions: Event handlers registered"));
}

void UEquipmentValidationServiceImpl::OnRulesOrConfigChanged()
{
...
uint32 UEquipmentValidationServiceImpl::GenerateCacheKey(const FEquipmentOperationRequest& Request) const
{
...
uint32 UEquipmentValidationServiceImpl::GetStableActorIdentifier(AActor* Actor) const
{
    if (!Actor)
    {
        return 0u;
...
void UEquipmentValidationServiceImpl::PublishValidationEvent(
    ...
    const FSlotValidationResult& Result)
{
    if (!IsInGameThread())
    {
        UE_LOG(LogEquipmentValidation, Error, TEXT("PublishValidationEvent called from non-game thread"));
        return;
    }
...
EEquipmentValidationFailure UEquipmentValidationServiceImpl::DetermineFailureType(const FRuleEvaluationResult& RuleResult) const
{
