...
#include "EquipmentActorBase.generated.h"

class UAbilitySystemComponent;
class UAttributeSet;
class UEquipmentMeshComponent;
class UEquipmentAttributeComponent;
class UEquipmentAttachmentComponent;
...
class MEDCOMEQUIPMENT_API AEquipmentActorBase : public AActor, public IMedComEquipmentInterface
{
    GENERATED_BODY()
...
public:
    ...
    UFUNCTION(BlueprintCallable, Category="Equipment|GAS")
    void SetCachedASC(UAbilitySystemComponent* InASC);

    // ===== IMedComEquipmentInterface (BlueprintNativeEvent) overrides =====
    virtual void OnEquipped_Implementation(AActor* NewOwner) override;
    virtual void OnUnequipped_Implementation() override;

    virtual void OnItemInstanceEquipped_Implementation(const FInventoryItemInstance& ItemInstance) override;
    virtual void OnItemInstanceUnequipped_Implementation(const FInventoryItemInstance& ItemInstance) override;

    virtual FInventoryItemInstance GetEquippedItemInstance_Implementation() const override;
    ...
    virtual bool CanEquipItemInstance_Implementation(const FInventoryItemInstance& ItemInstance) const override;
    ...
    virtual FInventoryOperationResult EquipItemInstance_Implementation(const FInventoryItemInstance& ItemInstance, bool bForceEquip) override;
    virtual FInventoryOperationResult UnequipItem_Implementation(FInventoryItemInstance& OutUnequippedInstance) override;
    virtual FInventoryOperationResult SwapEquipmentWith_Implementation(const TScriptInterface<IMedComEquipmentInterface>& OtherEquipment) override;
    ...
    bool InitializeFromItemInstance(const FInventoryItemInstance& ItemInstance);

protected:
    /** Initialize all internal components from item instance (SSOT-driven) */
    void InitializeEquipmentComponents(const FInventoryItemInstance& ItemInstance);

    /** Setup visual mesh defaults (does not attach) */
    void SetupEquipmentMesh(const FMedComUnifiedItemData& ItemData);
    ...
    void NotifyEquipmentEvent(const FGameplayTag& EventTag, const FInventoryItemInstance* Payload = nullptr) const;
    ...
    /** SSOT helper */
    bool GetUnifiedItemData(FMedComUnifiedItemData& OutData) const;
...
protected:
    ...
    UEquipmentMeshComponent*        MeshComponent;

    UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category="Equipment|Components")
    UEquipmentAttributeComponent*   AttributeComponent;

    UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category="Equipment|Components")
    UEquipmentAttachmentComponent*  AttachmentComponent;
    ...
    FInventoryItemInstance EquippedItemInstance;

    UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category="Equipment|Slot")
    ...
    struct FPendingInit
    {
        ...
        FInventoryItemInstance                  PendingItemInstance;
        ...
        void Reset()
        {
            PendingOwner.Reset();
            PendingASC.Reset();
            PendingItemInstance = FInventoryItemInstance();
            bHasOwnerData = false;
            bHasItemData  = false;
        }
