...
class MEDCOMEQUIPMENT_API UEquipmentMeshComponent : public USkeletalMeshComponent
{
    GENERATED_BODY()

public:
    UEquipmentMeshComponent();
     ...
     */
    UFUNCTION(BlueprintCallable, Category = "Equipment|Mesh")
    void UpdateVisualState(const FInventoryItemInstance& ItemInstance);
 ...
 bool HasVisualStateChanged(const FEquipmentVisualState& OtherState) const;
    ...
    void SetupWeaponVisuals(const FMedComUnifiedItemData& WeaponData);
...
protected:
     ...
     */
    void InitializeVisualComponents(const FMedComUnifiedItemData& ItemData);
     ...
     */
    bool LoadMeshFromItemData(const FMedComUnifiedItemData& ItemData);
     ...
     */
    FName GetWeaponSocketName(const FMedComUnifiedItemData& WeaponData, const FGameplayTag& ModificationType) const;
...
private:
    ...
    UPROPERTY()
    FInventoryItemInstance CurrentItemInstance;
