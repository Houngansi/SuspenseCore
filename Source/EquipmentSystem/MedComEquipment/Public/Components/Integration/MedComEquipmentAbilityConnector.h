...
#include "Interfaces/Equipment/IMedComAbilityConnector.h"
#include "GameplayAbilitySpec.h"
#include "GameplayEffectTypes.h"
#include "GameplayTagContainer.h"
#include "Types/Inventory/InventoryTypes.h"
#include "MedComEquipmentAbilityConnector.generated.h"
...
class MEDCOMEQUIPMENT_API UMedComEquipmentAbilityConnector : public UActorComponent, public IMedComAbilityConnector
{
    GENERATED_BODY()

public:
    UMedComEquipmentAbilityConnector();

    // UActorComponent
    virtual void BeginPlay() override;
    virtual void EndPlay(const EEndPlayReason::Type EndPlayReason) override;

    // IMedComAbilityConnector
    virtual bool Initialize(UAbilitySystemComponent* InASC, TScriptInterface<IMedComEquipmentDataProvider> InDataProvider) override;
    virtual TArray<FGameplayAbilitySpecHandle> GrantEquipmentAbilities(const FInventoryItemInstance& ItemInstance) override;
    virtual int32 RemoveGrantedAbilities(const TArray<FGameplayAbilitySpecHandle>& Handles) override;
    virtual TArray<FActiveGameplayEffectHandle> ApplyEquipmentEffects(const FInventoryItemInstance& ItemInstance) override;
    virtual int32 RemoveAppliedEffects(const TArray<FActiveGameplayEffectHandle>& Handles) override;
    virtual bool UpdateEquipmentAttributes(const FInventoryItemInstance& ItemInstance) override;
    ...
    /** Slot-scoped helpers */
    TArray<FGameplayAbilitySpecHandle> GrantAbilitiesForSlot(int32 SlotIndex, const FInventoryItemInstance& ItemInstance);
    int32 RemoveAbilitiesForSlot(int32 SlotIndex);
    TArray<FActiveGameplayEffectHandle> ApplyEffectsForSlot(int32 SlotIndex, const FInventoryItemInstance& ItemInstance);
    int32 RemoveEffectsForSlot(int32 SlotIndex);

protected:
    // Internal operations used by public API
    TArray<FGameplayAbilitySpecHandle> GrantAbilitiesFromItemData(const struct FMedComUnifiedItemData& ItemData, const FInventoryItemInstance& ItemInstance, int32 SlotIndex);
    TArray<FActiveGameplayEffectHandle> ApplyEffectsFromItemData(const struct FMedComUnifiedItemData& ItemData, const FInventoryItemInstance& ItemInstance, int32 SlotIndex);
    UAttributeSet* CreateAttributeSetFromItemData(const struct FMedComUnifiedItemData& ItemData, const FInventoryItemInstance& ItemInstance, int32 SlotIndex);
    ...
    bool InitializeAttributeSet(UAttributeSet* AttributeSet, TSubclassOf<class UGameplayEffect> InitEffect, const FInventoryItemInstance& ItemInstance);
    class UMedComItemManager* GetItemManager() const;
