...
#include "MedComEquipmentRulesEngine.generated.h"
 ...
 * Production path uses UMedComRulesCoordinator with specialized engines for better performance
 * and maintainability.
 * 
...
class MEDCOMEQUIPMENT_API UMedComEquipmentRulesEngine : public UActorComponent, public IMedComEquipmentRules
{
    GENERATED_BODY()

public:
    UMedComEquipmentRulesEngine();
    virtual ~UMedComEquipmentRulesEngine();
    ...
    bool Initialize(TScriptInterface<IMedComEquipmentDataProvider> InDataProvider) override;
    ...
    FRuleEvaluationResult CheckItemDurability(const FInventoryItemInstance& ItemInstance) const;
     ...
     */
    UFUNCTION(BlueprintCallable, Category = "Equipment|Validation")
    FRuleEvaluationResult ValidateLoadout(const TArray<FInventoryItemInstance>& LoadoutItems) const;
    ...
    FCharacterRequirements GetItemRequirements(const FInventoryItemInstance& ItemInstance) const;
...
protected:
     ...
     */
    bool GetItemData(FName ItemID, struct FMedComUnifiedItemData& OutItemData) const;
     ...
     * @return Total weight
     */
    float CalculateTotalWeight(const TArray<FInventoryItemInstance>& Items) const;
