...
#include "Interfaces/Equipment/IMedComSlotValidator.h"
...
#include "MedComEquipmentSlotValidator.generated.h"
...
{
public:
	virtual ~IMedComItemDataProvider() = default;
	virtual bool GetUnifiedItemData(const FName& ItemID, struct FMedComUnifiedItemData& OutData) const = 0;
};
...
struct FEquipmentValidationRule
{
	...
	TFunction<bool(const FInventoryItemInstance&, const FEquipmentSlotConfig&, const FSlotRestrictionData*)> RuleFunction;
};
...
class MEDCOMEQUIPMENT_API UMedComEquipmentSlotValidator : public UObject, public IMedComSlotValidator
{
	GENERATED_BODY()

public:
	UMedComEquipmentSlotValidator();
	...
	virtual FSlotValidationResult CanSwapItems(
		...
		const FInventoryItemInstance& ItemB) const override;
	...
	void SetItemDataProvider(TSharedPtr<IMedComItemDataProvider> Provider);
...
protected:
	...
	FSlotValidationResult ValidateItemType(const FInventoryItemInstance& ItemInstance, const FEquipmentSlotConfig& SlotConfig) const;
	FSlotValidationResult ValidateItemLevel(const FInventoryItemInstance& ItemInstance, const FEquipmentSlotConfig& SlotConfig) const;
	FSlotValidationResult ValidateItemWeight(const FInventoryItemInstance& ItemInstance, const FEquipmentSlotConfig& SlotConfig, const FSlotRestrictionData& Restrictions) const;
	FSlotValidationResult ValidateUniqueItem(const FInventoryItemInstance& ItemInstance, const FEquipmentSlotConfig& SlotConfig, const FSlotRestrictionData* Restrictions, const TScriptInterface<IMedComEquipmentDataProvider>& DataProvider = TScriptInterface<IMedComEquipmentDataProvider>()) const;

	// Helpers
	bool GetItemData(const FName& ItemID, struct FMedComUnifiedItemData& OutData) const;
	bool ItemHasTag(const FInventoryItemInstance& ItemInstance, const FGameplayTag& RequiredTag) const;
	...
	bool CheckSlotCompatibilityConflicts(int32 SlotIndexA, int32 SlotIndexB, const TScriptInterface<IMedComEquipmentDataProvider>& DataProvider) const;
	...
	FString GenerateCacheKey(const FInventoryItemInstance& Item, const FEquipmentSlotConfig& Slot) const;
...
private:
	...
	TSharedPtr<IMedComItemDataProvider> ItemDataProvider;
