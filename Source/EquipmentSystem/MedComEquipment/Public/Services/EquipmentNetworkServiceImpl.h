...
#include "Interfaces/Equipment/IMedComPredictionManager.h"
#include "Interfaces/Equipment/IMedComReplicationProvider.h"
...
#include "EquipmentNetworkServiceImpl.generated.h"
 ...
 */
UCLASS()
class MEDCOMEQUIPMENT_API UEquipmentNetworkServiceImpl : public UObject,
    public IEquipmentNetworkService
{
    GENERATED_BODY()

public:
    UEquipmentNetworkServiceImpl();
    virtual ~UEquipmentNetworkServiceImpl();
    ...
    virtual IMedComNetworkDispatcher* GetNetworkDispatcher() override
    {
        return NetworkDispatcher.GetInterface();
    }

    virtual IMedComPredictionManager* GetPredictionManager() override
    {
        return PredictionManager.GetInterface();
    }

    virtual IMedComReplicationProvider* GetReplicationProvider() override
    {
        if (!ReplicationProvider) return nullptr;
        if (ReplicationProvider->GetClass()->ImplementsInterface(UMedComReplicationProvider::StaticClass()))
        {
            return Cast<IMedComReplicationProvider>(ReplicationProvider);
        }
        return nullptr;
    }
...
private:
    ...
    TScriptInterface<IMedComNetworkDispatcher> NetworkDispatcher;
    TScriptInterface<IMedComPredictionManager>  PredictionManager;

    UPROPERTY(Transient)
    TObjectPtr<UMedComEquipmentReplicationManager> ReplicationProvider = nullptr;
    ...
    UMedComEquipmentNetworkDispatcher* CreateAndInitNetworkDispatcher(
        AActor* OwnerActor,
        const TScriptInterface<IMedComEquipmentOperations>& OperationExecutor);

    UMedComEquipmentPredictionSystem* CreateAndInitPredictionSystem(
        AActor* OwnerActor,
        const TScriptInterface<IMedComEquipmentDataProvider>& DataProvider,
        const TScriptInterface<IMedComEquipmentOperations>& OperationExecutor);

    UMedComEquipmentReplicationManager* CreateAndInitReplicationManager(
        AActor* OwnerActor,
        const TScriptInterface<IMedComEquipmentDataProvider>& DataProvider);
