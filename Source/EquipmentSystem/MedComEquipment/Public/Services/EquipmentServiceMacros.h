...
// У вас уже есть класс FEquipmentRWLock в FEquipmentThreadGuard.h — конфликт решаем удалением alias.

// Переиспользуем ваши макросы, но если где-то этот header подключается раньше
// FEquipmentThreadGuard.h — дадим безопасный fallback. В обычном пути
// (мы выше инклюдим FEquipmentThreadGuard.h) эти #ifndef просто не сработают.
...
#define LOG_DELTA_OPERATION(DeltaType, SlotIndex) \
    UE_LOG(LogEquipmentDelta, Verbose, TEXT("Delta[%s]: Slot %d"), TEXT(DeltaType), SlotIndex)

#define LOG_BATCH_DELTA(DeltaType, Count) \
    UE_LOG(LogEquipmentDelta, Verbose, TEXT("BatchDelta[%s]: %d operations"), TEXT(DeltaType), Count)
