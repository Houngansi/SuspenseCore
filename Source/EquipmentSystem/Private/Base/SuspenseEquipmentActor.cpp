...
ASuspenseEquipmentActor::ASuspenseEquipmentActor()
{
    PrimaryActorTick.bCanEverTick = false;
    bReplicates = true;
    bNetUseOwnerRelevancy = true;

    MeshComponent       = CreateDefaultSubobject<USuspenseEquipmentMeshComponent>(TEXT("MeshComponent"));
    RootComponent       = MeshComponent;

    AttributeComponent  = CreateDefaultSubobject<USuspenseEquipmentAttributeComponent>(TEXT("AttributeComponent"));
    AttachmentComponent = CreateDefaultSubobject<USuspenseEquipmentAttachmentComponent>(TEXT("AttachmentComponent"));

    CurrentState     = EqTags().State_Inactive;
    EquipmentSlotTag = EqTags().Slot_None;
}

void ASuspenseEquipmentActor::BeginPlay()
{
    Super::BeginPlay();
...
void ASuspenseEquipmentActor::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
    Super::GetLifetimeReplicatedProps(OutLifetimeProps);

    DOREPLIFETIME(ASuspenseEquipmentActor, OwnerActor);
    DOREPLIFETIME(ASuspenseEquipmentActor, CurrentState);

    DOREPLIFETIME_CONDITION(ASuspenseEquipmentActor, ReplicatedItemID,        COND_SkipOwner);
    DOREPLIFETIME_CONDITION(ASuspenseEquipmentActor, ReplicatedItemQuantity,  COND_SkipOwner);
    DOREPLIFETIME_CONDITION(ASuspenseEquipmentActor, ReplicatedItemCondition, COND_SkipOwner);
}
...
void ASuspenseEquipmentActor::SetCachedASC(UAbilitySystemComponent* InASC)
{
    if (!InASC)
    {
...
void ASuspenseEquipmentActor::OnEquipped_Implementation(AActor* NewOwner)
{
...
void ASuspenseEquipmentActor::OnUnequipped_Implementation()
{
...
void ASuspenseEquipmentActor::OnItemInstanceEquipped_Implementation(const FSuspenseInventoryItemInstance& ItemInstance)
{
...
void ASuspenseEquipmentActor::OnItemInstanceUnequipped_Implementation(const FSuspenseInventoryItemInstance& ItemInstance)
{
...
void ASuspenseEquipmentActor::InitializeEquipmentComponents(const FSuspenseInventoryItemInstance& ItemInstance)
{
    if (!ItemInstance.IsValid())
    {
...
void ASuspenseEquipmentActor::SetupEquipmentMesh(const FSuspenseUnifiedItemData& ItemData)
{
...
FSuspenseInventoryItemInstance ASuspenseEquipmentActor::GetEquippedItemInstance_Implementation() const
{
    return EquippedItemInstance;
}

FEquipmentSlotConfig ASuspenseEquipmentActor::GetSlotConfiguration_Implementation() const
{
    const FEquipmentSlotConfig* Cfg = GetSlotConfigurationPtr();
    return Cfg ? *Cfg : FEquipmentSlotConfig();
}

const FEquipmentSlotConfig* ASuspenseEquipmentActor::GetSlotConfigurationPtr() const
{
    return nullptr; // base has no predefined slot profile
}

EEquipmentSlotType ASuspenseEquipmentActor::GetEquipmentSlotType_Implementation() const
{
...
FGameplayTag ASuspenseEquipmentActor::GetEquipmentSlotTag_Implementation() const
{
    return EquipmentSlotTag;
}

bool ASuspenseEquipmentActor::IsEquipped_Implementation() const
{
    return (OwnerActor != nullptr) && EquippedItemInstance.IsValid();
}

bool ASuspenseEquipmentActor::IsRequiredSlot_Implementation() const
{
    const FEquipmentSlotConfig* Cfg = GetSlotConfigurationPtr();
    return Cfg ? Cfg->bIsRequired : false;
}

FText ASuspenseEquipmentActor::GetSlotDisplayName_Implementation() const
{
    const FEquipmentSlotConfig* Cfg = GetSlotConfigurationPtr();
    return Cfg ? Cfg->DisplayName : FText::GetEmpty();
}

FName ASuspenseEquipmentActor::GetAttachmentSocket_Implementation() const
{
...
FTransform ASuspenseEquipmentActor::GetAttachmentOffset_Implementation() const
{
...
bool ASuspenseEquipmentActor::CanEquipItemInstance_Implementation(const FSuspenseInventoryItemInstance& ItemInstance) const
{
...
FGameplayTagContainer ASuspenseEquipmentActor::GetAllowedItemTypes_Implementation() const
{
    const FEquipmentSlotConfig* Cfg = GetSlotConfigurationPtr();
    return Cfg ? Cfg->AllowedItemTypes : FGameplayTagContainer();
}

bool ASuspenseEquipmentActor::ValidateEquipmentRequirements_Implementation(TArray<FString>& OutErrors) const
{
    OutErrors.Empty();
...
FSuspenseInventoryOperationResult ASuspenseEquipmentActor::EquipItemInstance_Implementation(const FSuspenseInventoryItemInstance& ItemInstance, bool bForceEquip)
{
...
FSuspenseInventoryOperationResult ASuspenseEquipmentActor::UnequipItem_Implementation(FSuspenseInventoryItemInstance& OutUnequippedInstance)
{
    FSuspenseInventoryOperationResult R;
...
FSuspenseInventoryOperationResult ASuspenseEquipmentActor::SwapEquipmentWith_Implementation(const TScriptInterface<ISuspenseEquipment>& OtherEquipment)
{
    FSuspenseInventoryOperationResult R;
...
UAbilitySystemComponent* ASuspenseEquipmentActor::GetAbilitySystemComponent_Implementation() const
{
    return CachedASC;
}

UAttributeSet* ASuspenseEquipmentActor::GetEquipmentAttributeSet_Implementation() const
{
    return AttributeComponent ? AttributeComponent->GetAttributeSet() : nullptr;
}

TArray<TSubclassOf<UGameplayAbility>> ASuspenseEquipmentActor::GetGrantedAbilities_Implementation() const
{
...
TArray<TSubclassOf<UGameplayEffect>> ASuspenseEquipmentActor::GetPassiveEffects_Implementation() const
{
...
void ASuspenseEquipmentActor::ApplyEquipmentEffects_Implementation()      { ApplyInitializationEffects(); /* intentionally empty */ }
void ASuspenseEquipmentActor::RemoveEquipmentEffects_Implementation()     { RemoveGrantedAbilities(); RemoveAppliedEffects(); }
void ASuspenseEquipmentActor::GrantAbilitiesFromItemData()                { GrantedAbilityHandles.Reset(); }
void ASuspenseEquipmentActor::ApplyPassiveEffectsFromItemData()           { AppliedEffectHandles.Reset(); }
void ASuspenseEquipmentActor::ApplyInitializationEffects()                {}
void ASuspenseEquipmentActor::RemoveGrantedAbilities()                    { GrantedAbilityHandles.Reset(); }
void ASuspenseEquipmentActor::RemoveAppliedEffects()                      { AppliedEffectHandles.Reset(); }

// ==============================
// State
// ==============================

FGameplayTag ASuspenseEquipmentActor::GetCurrentEquipmentState_Implementation() const
{
    return CurrentState;
}

bool ASuspenseEquipmentActor::SetEquipmentState_Implementation(const FGameplayTag& NewState, bool bForceTransition)
{
...
bool ASuspenseEquipmentActor::IsInEquipmentState_Implementation(const FGameplayTag& StateTag) const
{
    return CurrentState.MatchesTag(StateTag);
}

TArray<FGameplayTag> ASuspenseEquipmentActor::GetAvailableStateTransitions_Implementation() const
{
...
void ASuspenseEquipmentActor::SetEquipmentStateInternal(const FGameplayTag& NewState)
{
...
float ASuspenseEquipmentActor::GetEquipmentRuntimeProperty_Implementation(const FName& PropertyName, float DefaultValue) const
{
...
void ASuspenseEquipmentActor::SetEquipmentRuntimeProperty_Implementation(const FName& PropertyName, float Value)
{
...
float ASuspenseEquipmentActor::GetEquipmentConditionPercent_Implementation() const
{
...
bool ASuspenseEquipmentActor::IsWeaponEquipment_Implementation() const
{
...
FGameplayTag ASuspenseEquipmentActor::GetWeaponArchetype_Implementation() const
{
...
bool ASuspenseEquipmentActor::CanFireWeapon_Implementation() const
{
...
bool ASuspenseEquipmentActor::InitializeFromItemInstance(const FSuspenseInventoryItemInstance& ItemInstance)
{
    if (!ItemInstance.IsValid()) { return false; }
...
void ASuspenseEquipmentActor::OnRep_ItemData()
{
...
void ASuspenseEquipmentActor::NotifyEquipmentEvent(const FGameplayTag& EventTag, const FSuspenseInventoryItemInstance* Payload) const
{
    ISuspenseEquipment::BroadcastEquipmentOperationEvent(this, EventTag, Payload);
}

void ASuspenseEquipmentActor::NotifyEquipmentStateChanged(const FGameplayTag& NewState, bool bIsRefresh) const
{
...
bool ASuspenseEquipmentActor::GetUnifiedItemData(FSuspenseUnifiedItemData& OutData) const
{
    if (!EquippedItemInstance.IsValid()) { return false; }
    if (USuspenseItemManager* IM = GetItemManager())
    {
...
USuspenseItemManager* ASuspenseEquipmentActor::GetItemManager() const
{
