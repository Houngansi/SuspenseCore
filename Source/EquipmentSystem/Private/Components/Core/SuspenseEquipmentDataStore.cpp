...
// Constructor
USuspenseEquipmentDataStore::USuspenseEquipmentDataStore()
{
...
void USuspenseEquipmentDataStore::BeginPlay()
{
    Super::BeginPlay();
...
void USuspenseEquipmentDataStore::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
    // Clear all delegates
...
FSuspenseInventoryItemInstance USuspenseEquipmentDataStore::GetSlotItem(int32 SlotIndex) const
{
...
FEquipmentSlotConfig USuspenseEquipmentDataStore::GetSlotConfiguration(int32 SlotIndex) const
{
...
FEquipmentSlotConfig USuspenseEquipmentDataStore::GetFreshSlotConfiguration(int32 SlotIndex) const
{
...
void USuspenseEquipmentDataStore::RefreshSlotConfigurations()
{
    UWorld* World = GetWorld();
    if (!World)
    {
...
void USuspenseEquipmentDataStore::SetCurrentLoadoutID(const FName& LoadoutID)
{
...
TArray<FEquipmentSlotConfig> USuspenseEquipmentDataStore::GetAllSlotConfigurations() const
{
...
TMap<int32, FSuspenseInventoryItemInstance> USuspenseEquipmentDataStore::GetAllEquippedItems() const
{
    FScopeLock Lock(&DataCriticalSection);
...
int32 USuspenseEquipmentDataStore::GetSlotCount() const
{
    FScopeLock Lock(&DataCriticalSection);
    return DataStorage.SlotConfigurations.Num();
}

bool USuspenseEquipmentDataStore::IsValidSlotIndex(int32 SlotIndex) const
{
    FScopeLock Lock(&DataCriticalSection);
    return SlotIndex >= 0 && SlotIndex < DataStorage.SlotConfigurations.Num();
}

bool USuspenseEquipmentDataStore::IsSlotOccupied(int32 SlotIndex) const
{
    FScopeLock Lock(&DataCriticalSection);
...
bool USuspenseEquipmentDataStore::SetSlotItem(int32 SlotIndex, const FSuspenseInventoryItemInstance& ItemInstance, bool bNotifyObservers)
{
...
FSuspenseInventoryItemInstance USuspenseEquipmentDataStore::ClearSlot(int32 SlotIndex, bool bNotifyObservers)
{
...
bool USuspenseEquipmentDataStore::InitializeSlots(const TArray<FEquipmentSlotConfig>& Configurations)
{
    return ModifyDataWithEvents(
...
int32 USuspenseEquipmentDataStore::GetActiveWeaponSlot() const
{
    FScopeLock Lock(&DataCriticalSection);
    return DataStorage.ActiveWeaponSlot;
}

bool USuspenseEquipmentDataStore::SetActiveWeaponSlot(int32 SlotIndex)
{
    return ModifyDataWithEvents(
...
FGameplayTag USuspenseEquipmentDataStore::GetCurrentEquipmentState() const
{
    FScopeLock Lock(&DataCriticalSection);
    return DataStorage.CurrentState;
}

bool USuspenseEquipmentDataStore::SetEquipmentState(const FGameplayTag& NewState)
{
    return ModifyDataWithEvents(
...
FEquipmentStateSnapshot USuspenseEquipmentDataStore::CreateSnapshot() const
{
    FScopeLock Lock(&DataCriticalSection);
...
bool USuspenseEquipmentDataStore::RestoreSnapshot(const FEquipmentStateSnapshot& Snapshot)
{
    if (!Snapshot.IsValid())
    {
        UE_LOG(LogEquipmentDataStore, Warning, 
...
EEquipmentState USuspenseEquipmentDataStore::ConvertTagToEquipmentState(const FGameplayTag& StateTag) const
{
...
FGameplayTag USuspenseEquipmentDataStore::ConvertEquipmentStateToTag(EEquipmentState State) const
{
...
FEquipmentSlotSnapshot USuspenseEquipmentDataStore::CreateSlotSnapshot(int32 SlotIndex) const
{
    FScopeLock Lock(&DataCriticalSection);
...
void USuspenseEquipmentDataStore::SetActiveTransaction(const FGuid& TransactionId)
{
    FScopeLock Lock(&DataCriticalSection);
    DataStorage.ActiveTransactionId = TransactionId;
}

void USuspenseEquipmentDataStore::ClearActiveTransaction()
{
    FScopeLock Lock(&DataCriticalSection);
    DataStorage.ActiveTransactionId.Invalidate();
}

FGuid USuspenseEquipmentDataStore::GetActiveTransaction() const
{
    FScopeLock Lock(&DataCriticalSection);
    return DataStorage.ActiveTransactionId;
}

void USuspenseEquipmentDataStore::ClearActiveTransactionIfMatches(const FGuid& TxnId)
{
    FScopeLock Lock(&DataCriticalSection);
...
void USuspenseEquipmentDataStore::OnTransactionDelta(const TArray<FEquipmentDelta>& Deltas)
{
    if (Deltas.Num() == 0)
    {
...
uint32 USuspenseEquipmentDataStore::GetDataVersion() const
{
    FScopeLock Lock(&DataCriticalSection);
    return DataStorage.DataVersion;
}

FDateTime USuspenseEquipmentDataStore::GetLastModificationTime() const
{
    FScopeLock Lock(&DataCriticalSection);
    return DataStorage.LastModified;
}

void USuspenseEquipmentDataStore::ResetToDefault()
{
    ModifyDataWithEvents(
...
int32 USuspenseEquipmentDataStore::GetMemoryUsage() const
{
    FScopeLock Lock(&DataCriticalSection);
...
bool USuspenseEquipmentDataStore::ModifyDataWithEvents(
...
FEquipmentDelta USuspenseEquipmentDataStore::CreateDelta(
...
void USuspenseEquipmentDataStore::BroadcastPendingEvents(const TArray<FSuspensePendingEventData>& PendingEvents)
{
...
bool USuspenseEquipmentDataStore::ValidateSlotIndexInternal(int32 SlotIndex, const FString& FunctionName) const
{
...
FEquipmentDataStorage USuspenseEquipmentDataStore::CreateDataSnapshot() const
{
    FScopeLock Lock(&DataCriticalSection);
    return DataStorage;
}

bool USuspenseEquipmentDataStore::ApplyDataSnapshot(const FEquipmentDataStorage& Snapshot, bool bNotifyObservers)
{
    return ModifyDataWithEvents(
...
void USuspenseEquipmentDataStore::IncrementVersion()
{
    // Assumes caller already holds DataCriticalSection
    DataStorage.DataVersion++;
...
void USuspenseEquipmentDataStore::LogDataModification(const FString& ModificationType, const FString& Details) const
{
    UE_LOG(LogEquipmentDataStore, Verbose, 
...
void USuspenseEquipmentDataStore::UpdateStatistics()
{
    // Assumes caller already holds DataCriticalSection
    TotalModifications++;
...
TArray<int32> USuspenseEquipmentDataStore::FindCompatibleSlots(const FGameplayTag& ItemSlotTag) const
{
    TArray<int32> Result;
...
float USuspenseEquipmentDataStore::GetTotalEquippedWeight() const
{
...
bool USuspenseEquipmentDataStore::MeetsItemRequirements(const FSuspenseInventoryItemInstance& /*Item*/, int32 /*TargetSlotIndex*/) const
{
    // Validation service performs heavy checks; datastore returns permissive default.
    return true;
}

FString USuspenseEquipmentDataStore::GetDebugInfo() const
{
    const int32 Num = GetSlotCount();
    int32 Occupied = 0;
    for (int32 i = 0; i < Num; ++i)
