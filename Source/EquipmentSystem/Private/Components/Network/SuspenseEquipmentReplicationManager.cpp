...
USuspenseEquipmentReplicationManager::USuspenseEquipmentReplicationManager()
{
...
void USuspenseEquipmentReplicationManager::BeginPlay()
{
    Super::BeginPlay();
    if(DataProvider.GetInterface())
    {
        DataProvider->OnSlotDataChanged().AddUObject(this,&USuspenseEquipmentReplicationManager::OnDataChanged);
    }
...
void USuspenseEquipmentReplicationManager::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
    if(DataProvider.GetInterface())
    {
...
void USuspenseEquipmentReplicationManager::TickComponent(float DeltaTime,ELevelTick TickType,FActorComponentTickFunction* ThisTickFunction)
{
...
void USuspenseEquipmentReplicationManager::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps)const
{
    Super::GetLifetimeReplicatedProps(OutLifetimeProps);
    DOREPLIFETIME(USuspenseEquipmentReplicationManager,ReplicatedSlotArray);
    DOREPLIFETIME(USuspenseEquipmentReplicationManager,CurrentVersion);
    DOREPLIFETIME(USuspenseEquipmentReplicationManager,ReplicatedActiveWeaponSlot);
    DOREPLIFETIME(USuspenseEquipmentReplicationManager,ReplicatedEquipmentState);
    DOREPLIFETIME_CONDITION(USuspenseEquipmentReplicationManager,CompressedData,COND_Custom);
}

void USuspenseEquipmentReplicationManager::PreReplication(IRepChangedPropertyTracker& ChangedPropertyTracker)
{
    ...
    DOREPLIFETIME_ACTIVE_OVERRIDE(USuspenseEquipmentReplicationManager,CompressedData,bActiveCompressed);
}

// ISuspenseReplicationProvider
void USuspenseEquipmentReplicationManager::MarkForReplication(int32 SlotIndex,bool bForceUpdate)
{
    ...
    MARK_PROPERTY_DIRTY_FROM_NAME(USuspenseEquipmentReplicationManager,CurrentVersion,this);
    UE_LOG(LogEquipmentReplication,Verbose,TEXT("MarkForReplication: Slot %d dirty, version %d"),SlotIndex,CurrentVersion);
}

FReplicatedEquipmentData USuspenseEquipmentReplicationManager::GetReplicatedData()const
{
    FScopeLock Lock(&SlotStateLock);
...
void USuspenseEquipmentReplicationManager::ApplyReplicatedData(const FReplicatedEquipmentData& Data,bool bIsInitialReplication)
{
    if(GetOwnerRole()==ROLE_Authority){return;}
    if(DataProvider.GetInterface())
    {
...
void USuspenseEquipmentReplicationManager::SetReplicationPolicy(EEquipmentReplicationPolicy Policy)
{
    CurrentPolicy=Policy;
    UE_LOG(LogEquipmentReplication,Log,TEXT("SetReplicationPolicy: %s"),*UEnum::GetValueAsString(Policy));
}

void USuspenseEquipmentReplicationManager::ForceFullReplication()
{
...
bool USuspenseEquipmentReplicationManager::ShouldReplicateTo(APlayerController* ViewTarget)const
{
...
bool USuspenseEquipmentReplicationManager::GetReplicationPriority(APlayerController* ViewTarget,float& OutPriority)const
{
...
FReplicatedEquipmentData USuspenseEquipmentReplicationManager::OptimizeReplicationData(const FReplicatedEquipmentData& Data)const
{
    ...
    for(FSuspenseInventoryItemInstance& I:O.SlotInstances)
    {
        if(I.IsValid())
        {
...
FReplicatedEquipmentData USuspenseEquipmentReplicationManager::GetReplicationDelta(uint32 LastVersion)const
{
    FScopeLock Lock(&SlotStateLock);
...
void USuspenseEquipmentReplicationManager::OnReplicationCallback(const FName& PropertyName)
{
    if(PropertyName==GET_MEMBER_NAME_CHECKED(USuspenseEquipmentReplicationManager,ReplicatedSlotArray)){OnRep_SlotArray();}
    else if(PropertyName==GET_MEMBER_NAME_CHECKED(USuspenseEquipmentReplicationManager,CurrentVersion)){OnRep_Version();}
    else if(PropertyName==GET_MEMBER_NAME_CHECKED(USuspenseEquipmentReplicationManager,ReplicatedActiveWeaponSlot)){OnRep_ActiveWeaponSlot();}
    else if(PropertyName==GET_MEMBER_NAME_CHECKED(USuspenseEquipmentReplicationManager,ReplicatedEquipmentState)){OnRep_EquipmentState();}
}

// Configuration
bool USuspenseEquipmentReplicationManager::Initialize(TScriptInterface<ISuspenseEquipmentDataProvider> InDataProvider)
{
    ...
    if(GetOwnerRole()==ROLE_Authority)
    {
        ...
        {
            FSuspenseInventoryItemInstance SlotItem=DataProvider->GetSlotItem(i);
            if(SlotItem.IsValid())
            {
...
void USuspenseEquipmentReplicationManager::SetUpdateRate(float UpdatesPerSecond)
{
...
void USuspenseEquipmentReplicationManager::OnNetworkQualityUpdated(float Quality)
{
...
// OnRep callbacks
void USuspenseEquipmentReplicationManager::OnRep_SlotArray()
{
    if(GetOwnerRole()!=ROLE_Authority)
    {
...
void USuspenseEquipmentReplicationManager::ProcessReplication()
{
    ...
    {FScopeLock Lock(&SlotStateLock);
        ...
        for(int32 SlotIndex:DirtySlots)
        {
            FSuspenseInventoryItemInstance NewItem=DataProvider->GetSlotItem(SlotIndex);
            ...
            if(Existing)
            {
                ...
                const bool bEqual = FSuspenseInventoryItemInstance::StaticStruct()
...
void USuspenseEquipmentReplicationManager::UpdateClientReplication(FClientReplicationState& ClientState)
{
...
FReplicatedEquipmentData USuspenseEquipmentReplicationManager::BuildReplicationData(APlayerController* Client,bool bForceFull)const
{
...
FReplicationDeltaMask USuspenseEquipmentReplicationManager::BuildDeltaMask(uint32 FromVersion,uint32 ToVersion)const
{
...
FString USuspenseEquipmentReplicationManager::GenerateSlotHMAC(const FSuspenseInventoryItemInstance& SlotData)const
{
...
bool USuspenseEquipmentReplicationManager::VerifySlotHMAC(const FSuspenseInventoryItemInstance& SlotData,const FString& HMACSignature)const
{
...
FCompressedReplicationData USuspenseEquipmentReplicationManager::CompressData(const FReplicatedEquipmentData& Data)const
{
    ...
    auto WriteStructArray = [&W](UScriptStruct* Struct, TArray<FSuspenseInventoryItemInstance>& Array)
    {
    ...
    // SlotInstances
    {
        ...
        TArray<FSuspenseInventoryItemInstance> Copy = Data.SlotInstances;
        WriteStructArray(FSuspenseInventoryItemInstance::StaticStruct(), Copy);
    }
...
bool USuspenseEquipmentReplicationManager::DecompressData(const FCompressedReplicationData& Compressed,FReplicatedEquipmentData& OutData)const
{
    ...
    auto ReadStructArray = [&R](UScriptStruct* Struct, TArray<FSuspenseInventoryItemInstance>& OutArray)
    {
        int32 Count = 0;
        R << Count;
    ...
    // SlotInstances
    ReadStructArray(FSuspenseInventoryItemInstance::StaticStruct(), OutData.SlotInstances);
...
float USuspenseEquipmentReplicationManager::CalculateEnhancedRelevancy(APlayerController* ViewTarget) const
{
...
void USuspenseEquipmentReplicationManager::UpdateSlotPriority(int32 SlotIndex)
{
...
bool USuspenseEquipmentReplicationManager::SlotNeedsReplication(int32 SlotIndex,uint32 ClientVersion)const
{
...
uint32 USuspenseEquipmentReplicationManager::CalculateChecksum(const FReplicatedEquipmentData& Data)const
{
    TArray<uint8> Bytes;
    Bytes.Reset(256);
...
void USuspenseEquipmentReplicationManager::OnDataChanged(int32 SlotIndex,const FSuspenseInventoryItemInstance& NewData)
{
    MarkForReplication(SlotIndex,false);
    UE_LOG(LogEquipmentReplication,Verbose,TEXT("OnDataChanged: Slot %d changed"),SlotIndex);
}

void USuspenseEquipmentReplicationManager::CleanupClientStates()
{
...
void USuspenseEquipmentReplicationManager::UpdateStatistics(int32 BytesSent,bool bWasDelta)
{
    Statistics.TotalUpdates++;
...
void USuspenseEquipmentReplicationManager::AdaptReplicationStrategy(float NetworkQuality)
{
