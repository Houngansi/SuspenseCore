...
void USuspenseEquipmentRulesEngine::BeginPlay()
{
    ...
    {
        UE_LOG(LogEquipmentRules, Log, TEXT("Equipment Rules Engine (PRODUCTION DISABLED) - use USuspenseRulesCoordinator"));
    }
}

void USuspenseEquipmentRulesEngine::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
    // Clear all data
...
bool USuspenseEquipmentRulesEngine::ShouldUseDevFallback() const
{
...
void USuspenseEquipmentRulesEngine::SetDevFallbackEnabled(bool bEnabled)
{
    bDevFallbackEnabled = bEnabled;
    UE_LOG(LogEquipmentRules, Log, TEXT("Dev fallback mode %s"), bEnabled ? TEXT("ENABLED") : TEXT("DISABLED"));
}

bool USuspenseEquipmentRulesEngine::IsDevFallbackEnabled() const
{
...
bool USuspenseEquipmentRulesEngine::UnregisterRule(const FGameplayTag& RuleTag)
{
    if (!ShouldUseDevFallback())
    {
        return false;
...
bool USuspenseEquipmentRulesEngine::SetRuleEnabled(const FGameplayTag& RuleTag, bool bEnabled)
{
    if (!ShouldUseDevFallback())
    {
        return false;
...
FString USuspenseEquipmentRulesEngine::GenerateComplianceReport(const FEquipmentStateSnapshot& CurrentState) const
{
    if (!ShouldUseDevFallback())
    {
...
bool USuspenseEquipmentRulesEngine::Initialize(TScriptInterface<ISuspenseEquipmentDataProvider> InDataProvider)
{
    if (!ShouldUseDevFallback())
    {
...
int32 USuspenseEquipmentRulesEngine::LoadRulesFromDataTable(UDataTable* RulesTable)
{
...
void USuspenseEquipmentRulesEngine::ClearAllRules()
{
    if (!ShouldUseDevFallback())
    {
        return;
...
void USuspenseEquipmentRulesEngine::ResetStatistics()
{
    if (!ShouldUseDevFallback())
    {
        return;
...
bool USuspenseEquipmentRulesEngine::GetItemData(FName ItemID, FSuspenseUnifiedItemData& OutItemData) const
{
...
bool USuspenseEquipmentRulesEngine::EvaluateExpression(const FString& Expression, const FRuleExecutionContext& Context) const
{
    if (!ShouldUseDevFallback())
    {
        return true;
    ...
    if (Expression.Contains(TEXT("WEIGHT")))
    {
        ...
        {
            TArray<FSuspenseInventoryItemInstance> All;
...
int32 USuspenseEquipmentRulesEngine::GetCharacterLevel(const AActor* Character) const
{
    if (!Character)
    {
        return 1;
...
void USuspenseEquipmentRulesEngine::RecordViolation(const FRuleViolation& Violation) const
{
    if (!ShouldUseDevFallback())
    {
        return;
...
void USuspenseEquipmentRulesEngine::UpdateStatistics(const FGameplayTag& RuleTag, bool bPassed, float EvaluationTime) const
{
    if (!ShouldUseDevFallback())
    {
        return;
...
void USuspenseEquipmentRulesEngine::RegisterDefaultRules()
{
    if (!ShouldUseDevFallback())
    {
        return;
...
FCharacterRequirements USuspenseEquipmentRulesEngine::GetItemRequirements(const FSuspenseInventoryItemInstance& ItemInstance) const
{
...
TArray<FRuleViolation> USuspenseEquipmentRulesEngine::FindItemConflicts(
    const FSuspenseInventoryItemInstance& ItemInstance,
...
bool USuspenseEquipmentRulesEngine::ResolveConflicts(const TArray<FRuleViolation>& Conflicts, int32 ResolutionStrategy)
{
    if (!ShouldUseDevFallback())
    {
        return false;
...
FRuleStatistics USuspenseEquipmentRulesEngine::GetRuleStatistics(const FGameplayTag& RuleTag) const
{
    if (!ShouldUseDevFallback())
    {
...
TArray<FRuleViolation> USuspenseEquipmentRulesEngine::GetViolationHistory(int32 MaxCount) const
{
    if (!ShouldUseDevFallback())
    {
...
FString USuspenseEquipmentRulesEngine::ExportRulesToJSON() const
{
    if (!ShouldUseDevFallback())
    {
...
int32 USuspenseEquipmentRulesEngine::ImportRulesFromJSON(const FString& JSONString)
{
    if (!ShouldUseDevFallback())
    {
        return 0;
...
FString USuspenseEquipmentRulesEngine::GetDebugInfo() const
{
    return FString::Printf(TEXT("Monolithic Rules Engine Debug:\n"
                          ...
                          ResultCache.Num(),
                          EngineVersion,
                          ShouldUseDevFallback() ? TEXT("ACTIVE") : TEXT("DISABLED - Use USuspenseRulesCoordinator"));
}
