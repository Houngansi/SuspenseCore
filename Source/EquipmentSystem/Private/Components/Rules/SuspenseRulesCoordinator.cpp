...
USuspenseRulesCoordinator::USuspenseRulesCoordinator()
{
...
// ============================================================================
bool USuspenseRulesCoordinator::Initialize(TScriptInterface<ISuspenseEquipmentDataProvider> InDataProvider)
{
...
void USuspenseRulesCoordinator::CreateSpecializedEngines()
{
    // ============================================================================
    // 1) Compatibility Engine (Critical Priority)
    // ============================================================================
    CompatibilityEngine = NewObject<USuspenseCompatibilityRulesEngine>(this, TEXT("CompatibilityRulesEngine"));
    ...
    // ============================================================================
    RequirementEngine = NewObject<USuspenseRequirementRulesEngine>(this, TEXT("RequirementRulesEngine"));
    ...
    // ============================================================================
    WeightEngine = NewObject<USuspenseWeightRulesEngine>(this, TEXT("WeightRulesEngine"));
    if (WeightEngine)
    {
    ...
    // ============================================================================
    ConflictEngine = NewObject<USuspenseConflictRulesEngine>(this, TEXT("ConflictRulesEngine"));
    if (ConflictEngine)
    {
        // ✅ ИЗМЕНЕНИЕ: Передаем DataProvider, только если он доступен
        if (DataProvider.GetInterface())
        ...
        else
        {
            ...
            ConflictEngine->Initialize(TScriptInterface<ISuspenseEquipmentDataProvider>());
            UE_LOG(LogRulesCoordinator, Verbose,
...
static void ApplyOperationToSnapshot(const FEquipmentOperationRequest& Op, FEquipmentStateSnapshot& Snapshot)
{
    ...
    case EEquipmentOperationType::Move:
        ...
        {
            FSuspenseInventoryItemInstance MovingItem;
            for (FEquipmentSlotSnapshot& S : Snapshot.SlotSnapshots)
            {
                if (S.SlotIndex == Op.SourceSlotIndex)
                {
                    MovingItem = S.ItemInstance;
                    S.ItemInstance = FSuspenseInventoryItemInstance();
                    break;
                }
            }
            for (FEquipmentSlotSnapshot& S : Snapshot.SlotSnapshots)
...
static void SnapshotToItemsFiltered(const FEquipmentStateSnapshot& Snapshot,
                                    ...
                                    TArray<FSuspenseInventoryItemInstance>& OutItems)
{
    OutItems.Reset();
...
FSuspenseRuleEvaluationResult USuspenseRulesCoordinator::EvaluateRules(const FEquipmentOperationRequest& Operation) const
{
    ...
    if (DataProvider.GetInterface())
    {
        TMap<int32, FSuspenseInventoryItemInstance> EquippedMap = DataProvider->GetAllEquippedItems();
...
void USuspenseRulesCoordinator::ClearRuleCache()
{
...
FString USuspenseRulesCoordinator::GenerateComplianceReport(const FEquipmentStateSnapshot& CurrentState) const
{
...
bool USuspenseRulesCoordinator::RegisterRuleEngine(
...
TArray<FRuleEngineRegistration> USuspenseRulesCoordinator::GetSortedEngines() const
{
