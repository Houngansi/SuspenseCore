...
const FName USuspenseEquipmentMeshComponent::DefaultMuzzleSocket = FName("Muzzle");
const FName USuspenseEquipmentMeshComponent::DefaultScopeSocket = FName("Scope");
const FName USuspenseEquipmentMeshComponent::DefaultMagazineSocket = FName("Magazine");

USuspenseEquipmentMeshComponent::USuspenseEquipmentMeshComponent()
{
...
bool USuspenseEquipmentMeshComponent::InitializeFromItemInstance(const FSuspenseInventoryItemInstance& ItemInstance)
{
...
void USuspenseEquipmentMeshComponent::UpdateVisualState(const FSuspenseInventoryItemInstance& ItemInstance)
{
...
void USuspenseEquipmentMeshComponent::CleanupVisuals()
{
...
bool USuspenseEquipmentMeshComponent::HasVisualStateChanged(const FEquipmentVisualState& OtherState) const
{
    FScopeLock Lock(&VisualStateCriticalSection);
    return CurrentVisualState != OtherState;
}

void USuspenseEquipmentMeshComponent::NotifyVisualStateChanged()
{
...
void USuspenseEquipmentMeshComponent::RequestStateSync()
{
...
void USuspenseEquipmentMeshComponent::InitializeVisualComponents(const FSuspenseUnifiedItemData& ItemData)
{
...
bool USuspenseEquipmentMeshComponent::LoadMeshFromItemData(const FSuspenseUnifiedItemData& ItemData)
{
...
void USuspenseEquipmentMeshComponent::SetupWeaponVisuals(const FSuspenseUnifiedItemData& WeaponData)
{
...
FVector USuspenseEquipmentMeshComponent::GetSocketLocationSafe(const FName& SocketName) const
{
...
FRotator USuspenseEquipmentMeshComponent::GetSocketRotationSafe(const FName& SocketName) const
{
    if (DoesSocketExist(SocketName))
    {
...
FTransform USuspenseEquipmentMeshComponent::GetSocketTransformSafe(const FName& SocketName) const
{
    if (DoesSocketExist(SocketName))
    {
...
void USuspenseEquipmentMeshComponent::ApplyOffsetTransform(const FTransform& Offset)
{
...
FVector USuspenseEquipmentMeshComponent::GetMuzzleLocation() const
{
...
FVector USuspenseEquipmentMeshComponent::GetMuzzleDirection() const
{
    FName MuzzleSocket = CachedItemData.MuzzleSocket.IsNone() ? DefaultMuzzleSocket : CachedItemData.MuzzleSocket;
...
int32 USuspenseEquipmentMeshComponent::PlayMuzzleFlash()
{
    // Create prediction
...
void USuspenseEquipmentMeshComponent::SetupScopeCamera(float FOV, bool bShouldAutoActivate)
{
    if (!ScopeCamera)
    {
...
void USuspenseEquipmentMeshComponent::SetScopeCameraActive(bool bActivate)
{
    if (ScopeCamera)
    {
...
int32 USuspenseEquipmentMeshComponent::PlayEquipmentEffect(const FGameplayTag& EffectType)
{
    // Create prediction
    FVisualEffectPrediction Prediction;
    Prediction.PredictionKey = NextPredictionKey++;
...
void USuspenseEquipmentMeshComponent::ConfirmEffectPrediction(int32 PredictionKey, bool bSuccess)
{
...
FName USuspenseEquipmentMeshComponent::GetAttachmentSocket(const FGameplayTag& ModificationType) const
{
    if (!CachedItemData.bIsWeapon)
    {
        return NAME_None;
...
bool USuspenseEquipmentMeshComponent::HasAttachmentSocket(const FGameplayTag& ModificationType) const
{
    FName SocketName = GetAttachmentSocket(ModificationType);
    return SocketName != NAME_None && DoesSocketExist(SocketName);
}

FName USuspenseEquipmentMeshComponent::GetWeaponSocketName(const FSuspenseUnifiedItemData& WeaponData, const FGameplayTag& ModificationType) const
{
...
UNiagaraComponent* USuspenseEquipmentMeshComponent::PlayVisualEffectAtLocation(const FGameplayTag& EffectType, const FVector& Location, const FRotator& Rotation)
{
...
void USuspenseEquipmentMeshComponent::ApplyPredictedEffect(const FVisualEffectPrediction& Prediction)
{
...
void USuspenseEquipmentMeshComponent::StopPredictedEffect(const FVisualEffectPrediction& Prediction)
{
...
void USuspenseEquipmentMeshComponent::CleanupExpiredPredictions()
{
    if (!GetWorld())
    {
        return;
