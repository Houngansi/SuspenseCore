...
USuspenseEquipmentTransactionProcessor::USuspenseEquipmentTransactionProcessor()
{
...
void USuspenseEquipmentTransactionProcessor::BeginPlay()
{
    Super::BeginPlay();
...
void USuspenseEquipmentTransactionProcessor::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void USuspenseEquipmentTransactionProcessor::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
//========================================
// ISuspenseTransactionManager Implementation (Basic API)
//========================================

FGuid USuspenseEquipmentTransactionProcessor::BeginTransaction(const FString& Description)
{
    ...
    if (USuspenseEquipmentDataStore* DataStore = Cast<USuspenseEquipmentDataStore>(DataProvider.GetObject()))
    {
...
bool USuspenseEquipmentTransactionProcessor::CommitTransaction(const FGuid& TransactionId)
{
    // Phase 1: Validate and prepare under lock
...
bool USuspenseEquipmentTransactionProcessor::RollbackTransaction(const FGuid& TransactionId)
{
    // Phase 1: Validate and prepare under lock
    FTransactionExecutionContext ContextCopy;
    ...
    if (bCanRollback)
    {
        ...
        if (bSuccess && bGenerateDeltas)
        {
            for (const FTransactionOperation& Op : ContextCopy.Operations)
            {
                ...
                FSuspenseInventoryItemInstance Temp = Delta.ItemBefore;
...
FGuid USuspenseEquipmentTransactionProcessor::BeginNestedTransaction(const FString& Description)
{
...
bool USuspenseEquipmentTransactionProcessor::RegisterOperation(const FGuid& OperationId)
{
    FScopeLock Lock(&TransactionLock);
...
bool USuspenseEquipmentTransactionProcessor::ValidateTransaction(const FGuid& TransactionId) const
{
    FScopeLock Lock(&TransactionLock);
    return ValidateTransaction_NoLock(TransactionId);
}

TArray<FEquipmentTransaction> USuspenseEquipmentTransactionProcessor::GetTransactionHistory(int32 MaxCount) const
{
    FScopeLock Lock(&TransactionLock);
...
//========================================
// ISuspenseTransactionManager Implementation (Extended API)
//========================================

bool USuspenseEquipmentTransactionProcessor::SupportsExtendedOps() const
{
    // Этот процессор реализует расширенное API
    return true;
}

bool USuspenseEquipmentTransactionProcessor::RegisterOperation(
...
bool USuspenseEquipmentTransactionProcessor::ApplyOperation(
    const FGuid& TransactionId,
    const FTransactionOperation& Operation)
{
...
TArray<FEquipmentDelta> USuspenseEquipmentTransactionProcessor::GetTransactionDeltas(
...
FGuid USuspenseEquipmentTransactionProcessor::CreateSavepoint(const FString& SavepointName)
{
    // Phase 1: Validate and prepare under lock
...
bool USuspenseEquipmentTransactionProcessor::RollbackToSavepoint(const FGuid& SavepointId)
{
...
bool USuspenseEquipmentTransactionProcessor::IsTransactionActive() const
{
    FScopeLock Lock(&TransactionLock);
    return TransactionStack.Num() > 0;
}

FEquipmentTransaction USuspenseEquipmentTransactionProcessor::GetCurrentTransaction() const
{
    FScopeLock Lock(&TransactionLock);
...
FGuid USuspenseEquipmentTransactionProcessor::GetCurrentTransactionId() const
{
    FScopeLock Lock(&TransactionLock);
...
FEquipmentTransaction USuspenseEquipmentTransactionProcessor::GetTransaction(const FGuid& TransactionId) const
{
    FScopeLock Lock(&TransactionLock);
...
int32 USuspenseEquipmentTransactionProcessor::CommitAllTransactions()
{
...
int32 USuspenseEquipmentTransactionProcessor::RollbackAllTransactions()
{
    // Phase 1: Collect transaction list under lock
...
FGuid USuspenseEquipmentTransactionProcessor::RecordDetailedOperation(const FTransactionOperation& Operation)
{
    FScopeLock Lock(&TransactionLock);
...
bool USuspenseEquipmentTransactionProcessor::ReleaseSavepoint(const FGuid& SavepointId)
{
    FScopeLock Lock(&TransactionLock);
...
void USuspenseEquipmentTransactionProcessor::ClearTransactionHistory(bool bKeepActive)
{
...
FTransactionValidationResult USuspenseEquipmentTransactionProcessor::ValidateTransactionIntegrity(const FGuid& TransactionId) const
{
    FScopeLock Lock(&TransactionLock);
...
TArray<FTransactionOperation> USuspenseEquipmentTransactionProcessor::CheckForConflicts(const FGuid& TransactionId) const
{
    FScopeLock Lock(&TransactionLock);
    return CheckForConflicts_NoLock(TransactionId);
}

bool USuspenseEquipmentTransactionProcessor::ResolveConflicts(const FGuid& TransactionId, int32 ResolutionStrategy)
{
    // Phase 1: Collect data under lock
...
bool USuspenseEquipmentTransactionProcessor::RecoverTransaction(const FGuid& TransactionId)
{
    // Phase 1: Collect data under lock
...
bool USuspenseEquipmentTransactionProcessor::Initialize(TScriptInterface<ISuspenseEquipmentDataProvider> InDataProvider)
{
    FScopeLock Lock(&TransactionLock);
...
void USuspenseEquipmentTransactionProcessor::SetTransactionTimeout(float Seconds)
{
...
void USuspenseEquipmentTransactionProcessor::SetMaxNestedDepth(int32 MaxDepth)
{
...
void USuspenseEquipmentTransactionProcessor::SetAutoRecovery(bool bEnable)
{
...
FString USuspenseEquipmentTransactionProcessor::GetTransactionStatistics() const
{
    FScopeLock Lock(&TransactionLock);
...
int32 USuspenseEquipmentTransactionProcessor::GetActiveTransactionCount() const
{
    FScopeLock Lock(&TransactionLock);
    return ActiveTransactions.Num();
}

FString USuspenseEquipmentTransactionProcessor::DumpTransactionState(const FGuid& TransactionId) const
{
    FScopeLock Lock(&TransactionLock);
...
FTransactionExecutionContext USuspenseEquipmentTransactionProcessor::CreateExecutionContext(
    const FGuid& TransactionId,
...
bool USuspenseEquipmentTransactionProcessor::ExecuteCommit(FTransactionExecutionContext& Context)
{
    ...
    for (const FTransactionOperation& Op : Context.Operations)
    {
        ...
        if (bIsSwap)
        {
            ...
            {
                FSuspenseInventoryItemInstance A = DataProvider->GetSlotItem(Op.SlotIndex);
                FSuspenseInventoryItemInstance B = DataProvider->GetSlotItem(TargetSlot);
...
bool USuspenseEquipmentTransactionProcessor::ExecuteRollback(FTransactionExecutionContext& Context)
{
    ...
    FEquipmentStateSnapshot& State) const
{
    ...
    if (bIsClearLike && Operation.SlotIndex >= 0)
    {
        for (FEquipmentSlotSnapshot& Slot : State.SlotSnapshots)
        {
            if (Slot.SlotIndex == Operation.SlotIndex)
            {
                Slot.ItemInstance = FSuspenseInventoryItemInstance(); // пусто
                TouchSnapshot();
                return true;
            }
        }
...
bool USuspenseEquipmentTransactionProcessor::ReverseOperation(const FTransactionOperation& Operation, FEquipmentStateSnapshot& State) const
{
...
FEquipmentStateSnapshot USuspenseEquipmentTransactionProcessor::CaptureStateSnapshot() const
{
...
bool USuspenseEquipmentTransactionProcessor::RestoreStateSnapshot(const FEquipmentStateSnapshot& Snapshot)
{
    if (!DataProvider.GetInterface())
    {
        UE_LOG(LogEquipmentTransaction, Error, 
...
bool USuspenseEquipmentTransactionProcessor::ValidateStateConsistency(const FEquipmentStateSnapshot& State) const
{
    // Basic validation
    if (!State.IsValid())
    {
...
FTransactionExecutionContext* USuspenseEquipmentTransactionProcessor::FindExecutionContext(const FGuid& TransactionId)
{
    return ActiveTransactions.Find(TransactionId);
}

const FTransactionExecutionContext* USuspenseEquipmentTransactionProcessor::FindExecutionContext(const FGuid& TransactionId) const
{
    return ActiveTransactions.Find(TransactionId);
}

FTransactionSavepoint* USuspenseEquipmentTransactionProcessor::FindSavepoint(
    FTransactionExecutionContext& Context, 
...
void USuspenseEquipmentTransactionProcessor::CleanupExpiredTransactions()
{
...
void USuspenseEquipmentTransactionProcessor::LogTransactionEvent(
    const FGuid& TransactionId, 
...
void USuspenseEquipmentTransactionProcessor::NotifyTransactionStateChange(
    const FGuid& TransactionId,
...
FEquipmentTransaction USuspenseEquipmentTransactionProcessor::ConvertToTransaction(
...
bool USuspenseEquipmentTransactionProcessor::CommitTransaction(const FGuid& TransactionId, const TArray<FEquipmentDelta>& Deltas)
{
    // Phase 1: Validate and prepare under lock
    FTransactionExecutionContext ContextCopy;
    bool bCanCommit = false;
...
TArray<FEquipmentDelta> USuspenseEquipmentTransactionProcessor::GenerateDeltasFromTransaction(
    const FTransactionExecutionContext& Context) const
{
...
FEquipmentDelta USuspenseEquipmentTransactionProcessor::CreateDeltaFromOperation(
    const FTransactionOperation& Operation, 
    const FGuid& TransactionId) const
{
...
TArray<FTransactionOperation> USuspenseEquipmentTransactionProcessor::CheckForConflicts_NoLock(
    const FGuid& TransactionId) const
{
    TArray<FTransactionOperation> Conflicts;
...
bool USuspenseEquipmentTransactionProcessor::ValidateTransaction_NoLock(const FGuid& TransactionId) const
{
    const FTransactionExecutionContext* Context = FindExecutionContext(TransactionId);
    if (!Context)
    {
