...
void USuspenseEquipmentDataService::InjectComponents(UObject* InDataStore, UObject* InItemManager)
{
    ...
    if (InDataStore)
    {
        ...
        {
            UE_LOG(LogSuspenseEquipmentData, Error,
                TEXT("InjectComponents: Failed to cast DataStore to USuspenseEquipmentDataStore. Received type: %s"),
...
void USuspenseEquipmentDataService::SetValidator(UObject* InValidator)
{
    if (!InValidator)
    {
        UE_LOG(LogSuspenseEquipmentData, Error,
    ...
    {
        UE_LOG(LogSuspenseEquipmentData, Error,
            TEXT("SetValidator: Failed to cast to USuspenseEquipmentSlotValidator. Received type: %s"),
...
bool USuspenseEquipmentDataService::InitializeService(const FServiceInitParams& Params)
{
    ...
    if (!ItemManager)
    {
        UE_LOG(LogSuspenseEquipmentData, Error, TEXT("InitializeService: ItemManager not injected!"));
        UE_LOG(LogSuspenseEquipmentData, Error, TEXT("  Call InjectComponents(DataStore, ItemManager) before InitializeService"));
    ...
    // Phase 1: Check state and validate under lock
    {
        ...
        if (!bComponentsInjected)
        {
            if (AActor* OwnerActor = Cast<AActor>(Params.Owner))
            {
                ...
                {
                    if (USuspenseEquipmentDataStore* AutoDS = OwnerActor->FindComponentByClass<USuspenseEquipmentDataStore>())
                    {
                        if (USuspenseEquipmentTransactionProcessor* AutoTP = OwnerActor->FindComponentByClass<USuspenseEquipmentTransactionProcessor>())
                        {
...
bool USuspenseEquipmentDataService::ShutdownService(bool bForce)
{
...
FGameplayTag USuspenseEquipmentDataService::GetServiceTag() const
{
    SCOPED_SERVICE_TIMER("Data.GetServiceTag");
    return FGameplayTag::RequestGameplayTag(TEXT("Service.Equipment.Data"));
}

FGameplayTagContainer USuspenseEquipmentDataService::GetRequiredDependencies() const
{
...
bool USuspenseEquipmentDataService::ValidateService(TArray<FText>& OutErrors) const
{
...
FString USuspenseEquipmentDataService::GetServiceStats() const
{
...
ISuspenseEquipmentDataProvider* USuspenseEquipmentDataService::GetDataProvider()
{
    SCOPED_SERVICE_TIMER("Data.GetDataProvider");
    return DataProviderInterface.GetInterface();
}

ISuspenseTransactionManager* USuspenseEquipmentDataService::GetTransactionManager()
{
...
FSuspenseInventoryItemInstance USuspenseEquipmentDataService::GetSlotItemCached(int32 SlotIndex) const
{
...
TMap<int32, FSuspenseInventoryItemInstance> USuspenseEquipmentDataService::BatchGetSlotItems(const TArray<int32>& SlotIndices) const
{
...
FGuid USuspenseEquipmentDataService::SwapSlotItems(int32 SlotA, int32 SlotB)
{
...
FGuid USuspenseEquipmentDataService::BatchUpdateSlots(const TMap<int32, FSuspenseInventoryItemInstance>& Updates)
{
...
FGuid USuspenseEquipmentDataService::CreateDataCheckpoint(const FString& CheckpointName)
{
...
bool USuspenseEquipmentDataService::RollbackToCheckpoint(const FGuid& CheckpointId)
{
...
bool USuspenseEquipmentDataService::ValidateDataIntegrity(bool bDeep) const
{
...
bool USuspenseEquipmentDataService::ExportMetricsToCSV(const FString& FilePath) const
{
    ...
    if (FilePath.IsEmpty())
    {
        UE_LOG(LogSuspenseEquipmentData, Warning, TEXT("Empty FilePath for ExportMetricsToCSV"));
    ...
    if (bSuccess)
    {
        ...
        UE_LOG(LogSuspenseEquipmentData, Log, TEXT("ExportMetricsToCSV: Metrics exported to %s"), *FilePath);
        bOk = true;
    }
    else
    {
...
void USuspenseEquipmentDataService::InitializeDataStorage()
{
...
void USuspenseEquipmentDataService::SetupEventSubscriptions()
{
    ...
    if (EventBus.IsValid())
    {
        ...
        EventScope.Subscribe(
            ...
            FEventHandlerDelegate::CreateUObject(this, &USuspenseEquipmentDataService::OnCacheInvalidation)
        );
...
void USuspenseEquipmentDataService::OnTransactionCompleted(const FGuid& TransactionId, bool bSuccess)
{
    if (bSuccess)
    {
        SuccessfulTransactions.fetch_add(1);
...
void USuspenseEquipmentDataService::OnDataStoreDelta(const FEquipmentDelta& Delta)
{
...
void USuspenseEquipmentDataService::OnTransactionDeltas(const TArray<FEquipmentDelta>& Deltas)
{
...
void USuspenseEquipmentDataService::BroadcastDelta(const FEquipmentDelta& Delta)
{
...
void USuspenseEquipmentDataService::BroadcastBatchDeltas(const TArray<FEquipmentDelta>& Deltas)
{
...
bool USuspenseEquipmentDataService::PerformDeepValidation() const
{
    FRWScopeLock DataScopeLock(DataLock, SLT_ReadOnly);
    return PerformDeepValidation_Internal();
}

bool USuspenseEquipmentDataService::PerformDeepValidation_Internal() const
{
...
bool USuspenseEquipmentDataService::ValidateDataIntegrity_Internal(bool bDeep) const
{
    // Предполагаем, что DataLock уже удерживается вызывающим кодом
    if (!DataStore)
    {
...
TArray<FEquipmentSlotConfig> USuspenseEquipmentDataService::CreateDefaultSlotConfiguration() const
{
...
void USuspenseEquipmentDataService::BatchUpdateCache(const TMap<int32, FSuspenseInventoryItemInstance>& Items) const
{
    // Нечего обновлять
...
void USuspenseEquipmentDataService::LogDataOperation(const FString& Operation, int32 SlotIndex) const
{
    if (!bEnableDetailedLogging)
    {
        return;
    ...
    if (SlotIndex >= 0)
    {
        UE_LOG(LogSuspenseEquipmentData, VeryVerbose,
...
void USuspenseEquipmentDataService::ResetDataStore()
{
    if (!DataStore)
    {
        return;
...
bool USuspenseEquipmentDataService::IsValidSlotIndex(int32 SlotIndex) const
{
...
void USuspenseEquipmentDataService::WarmupCachesSafe()
{
