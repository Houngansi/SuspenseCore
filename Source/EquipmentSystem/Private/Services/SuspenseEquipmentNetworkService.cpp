...
USuspenseEquipmentNetworkService::USuspenseEquipmentNetworkService()
...
bool USuspenseEquipmentNetworkService::ShutdownService(bool bForce)
{
    // Вызываем невиртуальный метод cleanup
...
void USuspenseEquipmentNetworkService::InternalShutdown(bool bForce, bool bFromDestructor)
{
    ...
    UE_LOG(LogSuspenseEquipmentNetwork, Log, TEXT("Equipment Network Service shutdown complete"));
}

bool USuspenseEquipmentNetworkService::ResolveDependencies(
    UWorld* World,
    ...
    TScriptInterface<ISuspenseEquipmentOperations>& OutOperationExecutor)
{
    if (!World)
    {
        UE_LOG(LogSuspenseEquipmentNetwork, Error, TEXT("ResolveDependencies: invalid World"));
        return false;
    }
    ...
    // Resolve Data Provider
    {
        ...
        {
            UE_LOG(LogSuspenseEquipmentNetwork, Error, TEXT("ResolveDependencies: Failed to resolve DataProvider"));
            return false;
        }
    }
    ...
    // Resolve Operation Executor
    {
        ...
        {
            UE_LOG(LogSuspenseEquipmentNetwork, Error, TEXT("ResolveDependencies: Failed to resolve OperationExecutor"));
            return false;
        }
    }
...
USuspenseEquipmentNetworkDispatcher* USuspenseEquipmentNetworkService::CreateAndInitNetworkDispatcher(
    AActor* OwnerActor,
    const TScriptInterface<ISuspenseEquipmentOperations>& OperationExecutor)
{
    if (!OwnerActor)
    {
        UE_LOG(LogSuspenseEquipmentNetwork, Error, TEXT("CreateAndInitNetworkDispatcher: OwnerActor is null"));
        return nullptr;
    }

    USuspenseEquipmentNetworkDispatcher* Dispatcher = NewObject<USuspenseEquipmentNetworkDispatcher>(
        OwnerActor,
        USuspenseEquipmentNetworkDispatcher::StaticClass(),
        TEXT("EquipmentNetworkDispatcher"));

    if (!Dispatcher)
    {
        UE_LOG(LogSuspenseEquipmentNetwork, Error, TEXT("Failed to create EquipmentNetworkDispatcher"));
        return nullptr;
    }
    ...
    UE_LOG(LogSuspenseEquipmentNetwork, Log, TEXT("NetworkDispatcher created & configured"));
...
USuspenseEquipmentPredictionSystem* USuspenseEquipmentNetworkService::CreateAndInitPredictionSystem(
    AActor* OwnerActor,
    ...
    const TScriptInterface<ISuspenseEquipmentOperations>& OperationExecutor)
{
    if (!OwnerActor)
    {
        UE_LOG(LogSuspenseEquipmentNetwork, Error, TEXT("CreateAndInitPredictionSystem: OwnerActor is null"));
        return nullptr;
    }

    USuspenseEquipmentPredictionSystem* Prediction = NewObject<USuspenseEquipmentPredictionSystem>(
        OwnerActor,
        USuspenseEquipmentPredictionSystem::StaticClass(),
        TEXT("EquipmentPredictionSystem"));

    if (!Prediction)
    {
        UE_LOG(LogSuspenseEquipmentNetwork, Error, TEXT("Failed to create EquipmentPredictionSystem"));
        return nullptr;
    }
    ...
    {
        UE_LOG(LogSuspenseEquipmentNetwork, Error, TEXT("PredictionSystem Initialize failed"));
    ...
    UE_LOG(LogSuspenseEquipmentNetwork, Log, TEXT("PredictionSystem created & initialized"));
...
USuspenseEquipmentReplicationManager* USuspenseEquipmentNetworkService::CreateAndInitReplicationManager(
    AActor* OwnerActor,
    const TScriptInterface<ISuspenseEquipmentDataProvider>& DataProvider)
{
    if (!OwnerActor)
    {
        UE_LOG(LogSuspenseEquipmentNetwork, Error, TEXT("CreateAndInitReplicationManager: OwnerActor is null"));
        return nullptr;
    }

    USuspenseEquipmentReplicationManager* Replication = NewObject<USuspenseEquipmentReplicationManager>(
        OwnerActor,
        USuspenseEquipmentReplicationManager::StaticClass(),
        TEXT("EquipmentReplicationManager"));

    if (!Replication)
    {
        UE_LOG(LogSuspenseEquipmentNetwork, Error, TEXT("Failed to create EquipmentReplicationManager"));
        return nullptr;
    }
    ...
    {
        UE_LOG(LogSuspenseEquipmentNetwork, Error, TEXT("ReplicationManager Initialize failed"));
        return nullptr;
    }
    ...
    UE_LOG(LogSuspenseEquipmentNetwork, Log, TEXT("ReplicationManager created & initialized"));
...
void USuspenseEquipmentNetworkService::BindDispatcherToPrediction(
    USuspenseEquipmentNetworkDispatcher* Dispatcher,
...
void USuspenseEquipmentNetworkService::StartMonitoringTimers(UWorld* World)
{
    if (!World) return;
    ...
    World->GetTimerManager().SetTimer(
        NonceCleanupTimer,
        this,
        &USuspenseEquipmentNetworkService::CleanExpiredNonces,
        60.0f, true);

    World->GetTimerManager().SetTimer(
        MetricsUpdateTimer,
        this,
        &USuspenseEquipmentNetworkService::UpdateNetworkMetrics,
        1.0f, true);

    World->GetTimerManager().SetTimer(
        MetricsExportTimer,
        this,
        &USuspenseEquipmentNetworkService::ExportMetricsPeriodically,
        300.0f, true);
}

bool USuspenseEquipmentNetworkService::InitializeService(const FServiceInitParams& Params)
{
    SCOPED_SERVICE_TIMER("InitializeService");

    if (ServiceState != EServiceLifecycleState::Uninitialized)
    {
        UE_LOG(LogSuspenseEquipmentNetwork, Warning, TEXT("Service already initialized"));
    ...
    {
        UE_LOG(LogSuspenseEquipmentNetwork, Error, TEXT("Failed to load HMAC secret key from secure storage"));
    ...
    if (!World)
    {
        UE_LOG(LogSuspenseEquipmentNetwork, Error, TEXT("No valid world context for network service initialization"));
        ServiceState = EServiceLifecycleState::Failed;
        ServiceMetrics.RecordError();
        return false;
    }
    ...
    if (!OwnerActor)
    {
        UE_LOG(LogSuspenseEquipmentNetwork, Error, TEXT("Service owner is not an Actor, cannot create network components"));
        ServiceState = EServiceLifecycleState::Failed;
        ServiceMetrics.RecordError();
        return false;
    }
    ...
    USuspenseEquipmentPredictionSystem* Prediction =
    ...
    USuspenseEquipmentReplicationManager* Replication =
    ...
    UE_LOG(LogSuspenseEquipmentNetwork, Log, TEXT("========================================"));
    UE_LOG(LogSuspenseEquipmentNetwork, Log, TEXT("Equipment Network Service Initialized"));
    UE_LOG(LogSuspenseEquipmentNetwork, Log, TEXT("Security: Strict=%s, HMACCritical=%s, IPRateLimit=%s"),
...
FGameplayTag USuspenseEquipmentNetworkService::GetServiceTag() const
{
...
FGameplayTagContainer USuspenseEquipmentNetworkService::GetRequiredDependencies() const
{
    FScopedServiceTimer __svc_timer(const_cast<FServiceMetrics&>(ServiceMetrics),
...
bool USuspenseEquipmentNetworkService::ValidateService(TArray<FText>& OutErrors) const
{
    FScopedServiceTimer __svc_timer(const_cast<FServiceMetrics&>(ServiceMetrics),
...
void USuspenseEquipmentNetworkService::ResetService()
{
    ...
    UE_LOG(LogSuspenseEquipmentNetwork, Log,
        TEXT("EquipmentNetworkService reset complete - all security and metric data cleared"));
}

FString USuspenseEquipmentNetworkService::GetServiceStats() const
{
    FScopedServiceTimer __svc_timer(const_cast<FServiceMetrics&>(ServiceMetrics),
    ...
    if (NetworkDispatcher.GetInterface())
    {
        ...
        if (USuspenseEquipmentNetworkDispatcher* Dispatcher =
                Cast<USuspenseEquipmentNetworkDispatcher>(NetworkDispatcher.GetObject()))
        {
...
bool USuspenseEquipmentNetworkService::ExportMetricsToCSV(const FString& FilePath) const
{
...
void USuspenseEquipmentNetworkService::ReloadSecurityConfig()
{
    ...
    UE_LOG(LogSuspenseEquipmentNetwork, Log, TEXT("Security configuration reloaded from INI"));
}

FGuid USuspenseEquipmentNetworkService::SendEquipmentOperation(const FEquipmentOperationRequest& Request, APlayerController* PlayerController)
{
    ...
    if (!IsServiceReady())
    {
        UE_LOG(LogSuspenseEquipmentNetwork, Error, TEXT("Network service not ready"));
        ServiceMetrics.RecordError();
    ...
    if (!PlayerController)
    {
        UE_LOG(LogSuspenseEquipmentNetwork, Error, TEXT("Invalid player controller"));
        ServiceMetrics.RecordError();
    ...
    {
        UE_LOG(LogSuspenseEquipmentNetwork, Error, TEXT("Failed to generate valid player GUID"));
    ...
    UE_LOG(LogSuspenseEquipmentNetwork, Verbose, TEXT("Sent %s operation %s with nonce %llu from %s"),
...
bool USuspenseEquipmentNetworkService::ReceiveEquipmentOperation(const FNetworkOperationRequest& NetworkRequest, APlayerController* SendingPlayer)
{
    ...
    if (!NetworkRequest.ValidateIntegrity())
    {
        ...
        UE_LOG(LogSuspenseEquipmentNetwork, Error, TEXT("Integrity check failed for %s from %s"),
    ...
    if (!MarkNonceAsPending(NetworkRequest.Nonce))
    {
        ...
        UE_LOG(LogSuspenseEquipmentNetwork, Error, TEXT("Replay blocked, nonce %llu from %s"),
    ...
    if (Age > SecurityConfig.PacketAgeLimit)
    {
        ...
        UE_LOG(LogSuspenseEquipmentNetwork, Warning, TEXT("Stale packet rejected age=%.2fs from %s"),
        ...
        SecurityConfig.bRequireHMACForCritical)
    {
        if (!NetworkRequest.VerifyHMAC(HMACSecretKey))
        {
            ...
            UE_LOG(LogSuspenseEquipmentNetwork, Error, TEXT("HMAC failed for %s from %s"),
                *NetworkRequest.RequestId.ToString(), *GetPlayerIdentifier(SendingPlayer));
            return false;
        }
...
void USuspenseEquipmentNetworkService::SetNetworkQuality(float Quality)
{
    ...
    UE_LOG(LogSuspenseEquipmentNetwork, Log, TEXT("Network quality set to %.2f"), NetworkQualityLevel);
}

FLatencyCompensationData USuspenseEquipmentNetworkService::GetNetworkMetrics() const
{
    FScopedServiceTimer __svc_timer(const_cast<FServiceMetrics&>(ServiceMetrics),
...
void USuspenseEquipmentNetworkService::ForceSynchronization(APlayerController* PlayerController)
{
    SCOPED_SERVICE_TIMER("ForceSynchronization");

    if (!IsServiceReady() || !PlayerController)
    {
        UE_LOG(LogSuspenseEquipmentNetwork, Warning, TEXT("Cannot force sync - service not ready or invalid controller"));
        ServiceMetrics.RecordError();
    ...
    if (ReplicationProvider)
    {
        ...
        UE_LOG(LogSuspenseEquipmentNetwork, Log, TEXT("Forced synchronization for %s"),
            *GetPlayerIdentifier(PlayerController));
    }
...
bool USuspenseEquipmentNetworkService::ExportSecurityMetrics(const FString& FilePath) const
{
...
FGuid USuspenseEquipmentNetworkService::CreatePlayerGuid(const FUniqueNetIdRepl& UniqueId) const
{
    FScopedServiceTimer __svc_timer(const_cast<FServiceMetrics&>(ServiceMetrics),
...
bool USuspenseEquipmentNetworkService::CheckRateLimit(const FGuid& PlayerGuid, APlayerController* PlayerController)
{
        ...
        SecurityConfig.TemporaryBanDuration))
    {
        ...
        UE_LOG(LogSuspenseEquipmentNetwork, Warning,
        ...
        (CurrentTime - RateLimitData.LastOperationTime) < SecurityConfig.MinOperationInterval)
    {
        ...
        UE_LOG(LogSuspenseEquipmentNetwork, Verbose, TEXT("Operation too fast from %s (%.3f seconds since last)"),
            *RateLimitData.PlayerIdentifier,
...
bool USuspenseEquipmentNetworkService::CheckIPRateLimit(const FString& IPAddress)
{
...
bool USuspenseEquipmentNetworkService::MarkNonceAsPending(uint64 Nonce)
{
...
bool USuspenseEquipmentNetworkService::ConfirmNonce(uint64 Nonce)
{
    if (!PendingNonces.Contains(Nonce))
    {
...
void USuspenseEquipmentNetworkService::RejectNonce(uint64 Nonce)
{
    PendingNonces.Remove(Nonce);
    RECORD_SERVICE_METRIC("nonce_rejected", 1);
}

uint64 USuspenseEquipmentNetworkService::GenerateSecureNonce()
{
    uint64 Nonce = 0;
    ...
    {
        UE_LOG(LogSuspenseEquipmentNetwork, Warning, TEXT("Nonce generation collision detected after %d attempts"), MaxAttempts);
...
FString USuspenseEquipmentNetworkService::GenerateRequestHMAC(const FNetworkOperationRequest& Request) const
{
    return Request.GenerateHMAC(HMACSecretKey);
}

bool USuspenseEquipmentNetworkService::VerifyRequestHMAC(const FNetworkOperationRequest& Request) const
{
    return Request.VerifyHMAC(HMACSecretKey);
}

void USuspenseEquipmentNetworkService::CleanExpiredNonces()
{
    ...
    if (CleanedCount > 0)
    {
        UE_LOG(LogSuspenseEquipmentNetwork, Verbose, TEXT("Cleaned %d expired nonces, %d inactive players, %d inactive IPs"),
            CleanedCount, PlayersToRemove.Num(), IPsToRemove.Num());
    }
}

void USuspenseEquipmentNetworkService::LogSuspiciousActivity(APlayerController* PlayerController, const FString& Reason)
{
    ...
    UE_LOG(LogSuspenseEquipmentNetwork, Warning, TEXT("%s"), *LogEntry);
    ...
    if (Count >= SecurityConfig.MaxSuspiciousActivities)
    {
        ...
        UE_LOG(LogSuspenseEquipmentNetwork, Error,
            TEXT("SECURITY ALERT: Player %s exceeded suspicious activity threshold! Immediate action required."),
            *PlayerID);
    }
}

FString USuspenseEquipmentNetworkService::GetPlayerIdentifier(APlayerController* PlayerController) const
{
    if (!PlayerController)
    {
        return TEXT("Unknown");
...
FString USuspenseEquipmentNetworkService::GetIPAddress(APlayerController* PlayerController) const
{
    if (!PlayerController)
    {
        return TEXT("Unknown");
...
void USuspenseEquipmentNetworkService::UpdateNetworkMetrics()
{
    ...
    if (NetworkDispatcher.GetInterface())
    {
        ...
        if (USuspenseEquipmentReplicationManager* ReplicationMgr = ReplicationProvider)
        {
            float NewHz = 10.0f;
            ...
            if (FMath::Abs(NewHz - LastHz) > 2.0f)
            {
                ...
                UE_LOG(LogSuspenseEquipmentNetwork, Verbose,
...
void USuspenseEquipmentNetworkService::UpdateSecurityMetrics(double StartTime)
{
...
void USuspenseEquipmentNetworkService::ExportMetricsPeriodically()
{
...
void USuspenseEquipmentNetworkService::AdaptNetworkStrategies()
{
    if (NetworkQualityLevel < 0.3f)
    {
        ...
        UE_LOG(LogSuspenseEquipmentNetwork, Log, TEXT("Adapted to POOR network - optimized for playability"));
    }
    else if (NetworkQualityLevel < 0.7f)
    {
        ...
        UE_LOG(LogSuspenseEquipmentNetwork, Log, TEXT("Adapted to MEDIUM network - balanced security"));
    }
    else
    {
        if (PredictionManager.GetInterface())
        ...
        UE_LOG(LogSuspenseEquipmentNetwork, Log, TEXT("Adapted to GOOD network - maximum security enabled"));
    }
}

void USuspenseEquipmentNetworkService::InitializeSecurity()
{
    ...
    UE_LOG(LogSuspenseEquipmentNetwork, Log, TEXT("Security subsystems initialized with enhanced protection"));
}

void USuspenseEquipmentNetworkService::ShutdownSecurity()
{
    ...
    UE_LOG(LogSuspenseEquipmentNetwork, Log, TEXT("Final Security Report:\n%s"), *FinalReport);
    ...
    UE_LOG(LogSuspenseEquipmentNetwork, Log, TEXT("Security subsystems shutdown complete"));
}

FString USuspenseEquipmentNetworkService::LoadHMACKeyFromSecureStorage()
{
    FString Key;
    ...
    if (Key.IsEmpty())
    {
        UE_LOG(LogSuspenseEquipmentNetwork, Warning, TEXT("No HMAC key found, generating new secure key"));
        ...
        {
            UE_LOG(LogSuspenseEquipmentNetwork, Error, TEXT("Failed to save HMAC key to secure storage"));
        }
        else
        {
            UE_LOG(LogSuspenseEquipmentNetwork, Log, TEXT("Generated and saved new HMAC key to secure storage"));
    ...
    if (Key.Len() < 32)
    {
        UE_LOG(LogSuspenseEquipmentNetwork, Error, TEXT("HMAC key is too short (%d chars), security compromised!"), Key.Len());
