...
bool USuspenseEquipmentOperationService::InitializeService(const FServiceInitParams& Params)
{
...
EServiceLifecycleState USuspenseEquipmentOperationService::GetServiceState() const
{
    SCOPED_SERVICE_TIMER("GetServiceState");
    return ServiceState;
}

bool USuspenseEquipmentOperationService::IsServiceReady() const
{
    SCOPED_SERVICE_TIMER("IsServiceReady");
    return ServiceState == EServiceLifecycleState::Ready;
}

FGameplayTag USuspenseEquipmentOperationService::GetServiceTag() const
{
    SCOPED_SERVICE_TIMER("GetServiceTag");
    return FGameplayTag::RequestGameplayTag(TEXT("Service.Equipment.Operations"));
}

FGameplayTagContainer USuspenseEquipmentOperationService::GetRequiredDependencies() const
{
...
void USuspenseEquipmentOperationService::InitializeWithOwner(APlayerState* InOwnerPS, bool bInServerAuthority)
{
...
ISuspenseEquipmentOperations* USuspenseEquipmentOperationService::GetOperationsExecutor()
{
...
bool USuspenseEquipmentOperationService::QueueOperation(const FEquipmentOperationRequest& Request)
{
...
int32 USuspenseEquipmentOperationService::GetQueueSize() const
{
...
FTransactionOperation USuspenseEquipmentOperationService::MakeTxnOpFromStep(const FTransactionPlanStep& Step) const
{
...
FGameplayTag USuspenseEquipmentOperationService::MapOperationTypeToTag(EEquipmentOperationType OpType) const
{
...
bool USuspenseEquipmentOperationService::BatchValidatePlan(const FTransactionPlan& Plan, FText& OutError) const
{
...
bool USuspenseEquipmentOperationService::ExecutePlanTransactional(
    ...
    TArray<FEquipmentDelta>& OutDeltas)
{
    ...
    if (!TransactionManager.GetInterface())
    {
        UE_LOG(LogSuspenseEquipmentOperations, Error, TEXT("No transaction manager available for plan execution"));
        return false;
    }
    ...
    if (TransactionManager->SupportsExtendedOps())
    {
        ...
        if (bOwnTxn)
        {
            ...
            if (USuspenseEquipmentTransactionProcessor* Processor = Cast<USuspenseEquipmentTransactionProcessor>(TransactionManager.GetObject()))
            {
                ...
                {
                    UE_LOG(LogSuspenseEquipmentOperations, Error, TEXT("CommitWithDeltas failed for plan %s"), *Plan.PlanId.ToString());
                    return false;
                }
            }
            else
            {
                ...
                {
                    UE_LOG(LogSuspenseEquipmentOperations, Error, TEXT("Legacy commit failed for plan %s"), *Plan.PlanId.ToString());
                    return false;
                }
            }
    ...
    if (bOwnTxn)
    {
        if (USuspenseEquipmentTransactionProcessor* Processor = Cast<USuspenseEquipmentTransactionProcessor>(TransactionManager.GetObject()))
        {
            if (!Processor->CommitTransaction(TxnId, OutDeltas))
            {
                UE_LOG(LogSuspenseEquipmentOperations, Error, TEXT("Fallback commit with deltas failed for plan %s"), *Plan.PlanId.ToString());
                return false;
            }
        }
        else
        {
            if (!TransactionManager->CommitTransaction(TxnId))
            {
                UE_LOG(LogSuspenseEquipmentOperations, Error, TEXT("Fallback legacy commit failed for plan %s"), *Plan.PlanId.ToString());
                return false;
            }
        }
        ServiceMetrics.Inc(TEXT("TransactionsCommitted"));
...
FTransactionPlanStep USuspenseEquipmentOperationService::MakePlanStepFromRequest(const FEquipmentOperationRequest& Request) const
{
...
bool USuspenseEquipmentOperationService::BuildSingleStepPlanFromRequest(const FEquipmentOperationRequest& Request, FTransactionPlan& OutPlan) const
{
...
bool USuspenseEquipmentOperationService::ProcessBatchUsingPlans(
    ...
    TArray<FEquipmentOperationResult>* OutResults)
{
    ...
    if (bExecOk)
    {
        ...
        if (!bCommitOk)
        {
            UE_LOG(LogSuspenseEquipmentOperations, Error, TEXT("Batch commit failed (%s)"), *BatchTxnId.ToString());
            return false;
        }
    ...
    // 2) DataProvider — в STATELESS-режиме может отсутствовать на старте (НЕ падать)
{
    ...
    // Вариант B: USuspenseEquipmentDataService::GetDataProvider()
    if (!bResolvedProvider)
    {
        if (USuspenseEquipmentDataService* DataSvc = Cast<USuspenseEquipmentDataService>(DataSvcObj))
        {
...
// 4) Rules (опционально): пробуем получить ISuspenseEquipmentRules через сервис валидации
{
    ...
    else
    {
        ...
        if (!bBoundRules)
        {
            if (USuspenseEquipmentValidationService* VS = Cast<USuspenseEquipmentValidationService>(ValidationObj))
            {
...
void USuspenseEquipmentOperationService::SetOperationsExecutor(TScriptInterface<ISuspenseEquipmentOperations> InExecutor)
{
    if (!InExecutor.GetObject() || !InExecutor.GetInterface())
    {
        ...
        UE_LOG(LogSuspenseEquipmentOperations, Warning, TEXT("SetOperationsExecutor: cleared executor (null injected)"));
        return;
    }
...
void USuspenseEquipmentOperationService::StartQueueProcessing()
{
    if (UWorld* World = GetWorld())
    {
        World->GetTimerManager().SetTimer(
            QueueProcessTimer,
            this,
            &USuspenseEquipmentOperationService::ProcessQueueAsync,
            QueueProcessInterval,
            true
        );
    }
    else
    {
        ...
        TickerHandle = FTSTicker::GetCoreTicker().AddTicker(
            FTickerDelegate::CreateUObject(this, &USuspenseEquipmentOperationService::TickQueueFallback),
            QueueProcessInterval
        );
    }
}

void USuspenseEquipmentOperationService::StopQueueProcessing()
{
    if (UWorld* World = GetWorld())
    {
...
bool USuspenseEquipmentOperationService::ShouldDelegateToServer(const FEquipmentOperationRequest& Request) const
{
...
FEquipmentOperationResult USuspenseEquipmentOperationService::DelegateOperationToServer(const FEquipmentOperationRequest& Request)
{
...
void USuspenseEquipmentOperationService::StartPrediction(const FEquipmentOperationRequest& Request)
{
    if (!PredictionManager.GetInterface())
    {
        return;
...
void USuspenseEquipmentOperationService::ConfirmPrediction(const FGuid& OperationId, const FEquipmentOperationResult& ServerResult)
{
    if (!PredictionManager.GetInterface())
    {
        return;
...
int32 USuspenseEquipmentOperationService::TryCoalesceOperation(FQueuedOperation* NewOp)
{
    if (!NewOp)
    {
        return INDEX_NONE;
...
void USuspenseEquipmentOperationService::OptimizeQueue()
{
...
FEquipmentOperationResult USuspenseEquipmentOperationService::ProcessSingleOperation(
...
bool USuspenseEquipmentOperationService::PreflightRequests(
    const TArray<FQueuedOperation*>& BatchOps,
    TArray<FEquipmentOperationResult>* OutResults)
{
...
bool USuspenseEquipmentOperationService::ProcessBatch(
    const TArray<FQueuedOperation*>& BatchOps,
    bool bAtomic,
    TArray<FEquipmentOperationResult>* OutResults)
{
    ...
    if (bAtomic && BatchTransactionId.IsValid() && TransactionManager.GetInterface())
    {
        if (bAllSuccess)
        {
            ...
            if (USuspenseEquipmentTransactionProcessor* Processor = Cast<USuspenseEquipmentTransactionProcessor>(TransactionManager.GetObject()))
            {
                ...
                {
                    UE_LOG(LogSuspenseEquipmentOperations, Error, TEXT("Batch commit with deltas failed (%s)"), *BatchTransactionId.ToString());
            ...
            else
            {
                ...
                {
                    UE_LOG(LogSuspenseEquipmentOperations, Error, TEXT("Batch legacy commit failed (%s)"), *BatchTransactionId.ToString());
                    TransactionManager->RollbackTransaction(BatchTransactionId);
                    return false;
                }
            }
            ...
            ServiceMetrics.Inc(TEXT("BatchTransactionsCommitted"));
            UE_LOG(LogSuspenseEquipmentOperations, Verbose, TEXT("Committed batch transaction %s - %d operations succeeded"),
                *BatchTransactionId.ToString(), ProcessedCount);
        }
        else
        {
            TransactionManager->RollbackTransaction(BatchTransactionId);
            UE_LOG(LogSuspenseEquipmentOperations, Verbose, TEXT("Rolled back batch transaction %s"), *BatchTransactionId.ToString());
        }
    }
...
void USuspenseEquipmentOperationService::ProcessQueueAsync()
{
...
bool USuspenseEquipmentOperationService::TickQueueFallback(float DeltaTime)
{
    ProcessQueueAsync();
    return true; // Continue ticking
}

uint32 USuspenseEquipmentOperationService::GenerateValidationCacheKey(const FEquipmentOperationRequest& Request) const
{
...
FSlotValidationResult USuspenseEquipmentOperationService::ValidateOperationCached(const FEquipmentOperationRequest& Request) const
{
...
void USuspenseEquipmentOperationService::InvalidateValidationCache()
{
    ValidationCache->Clear();
    ServiceMetrics.Inc(TEXT("ValidationCacheInvalidations"));
}

FGuid USuspenseEquipmentOperationService::BeginOperationTransaction(
    ...
    bool bIsOuter)
{
    ...
    if (bSuccess)
    {
        ...
        if (USuspenseEquipmentTransactionProcessor* Processor = Cast<USuspenseEquipmentTransactionProcessor>(TransactionManager.GetObject()))
        {
            ...
            {
                UE_LOG(LogSuspenseEquipmentOperations, Error, TEXT("Commit with deltas failed (%s)"), *TransactionId.ToString());
        ...
        else
        {
            ...
            {
                UE_LOG(LogSuspenseEquipmentOperations, Error, TEXT("Legacy commit failed (%s)"), *TransactionId.ToString());
                TransactionManager->RollbackTransaction(TransactionId);
                ServiceMetrics.Inc(TEXT("TransactionsRolledBack"));
                return;
            }
...
void USuspenseEquipmentOperationService::PruneHistory()
{
...
void USuspenseEquipmentOperationService::LogOperation(const FEquipmentOperationRequest& Request,
...
void USuspenseEquipmentOperationService::InitializeObjectPools()
{
    {
...
void USuspenseEquipmentOperationService::CleanupObjectPools()
{
    {
        FScopeLock Lock(&OperationPoolLock);
...
FQueuedOperation* USuspenseEquipmentOperationService::AcquireOperation()
{
    if (!bEnableObjectPooling)
    {
...
void USuspenseEquipmentOperationService::ReleaseOperation(FQueuedOperation* Operation)
{
    if (!Operation)
    {
        return;
...
FEquipmentOperationResult* USuspenseEquipmentOperationService::AcquireResult()
{
    if (!bEnableObjectPooling)
    {
...
void USuspenseEquipmentOperationService::ReleaseResult(FEquipmentOperationResult* Result)
{
    if (!Result)
    {
        return;
...
float USuspenseEquipmentOperationService::GetPoolEfficiency() const
{
...
void USuspenseEquipmentOperationService::EnsureValidConfig()
{
...
void USuspenseEquipmentOperationService::TrimPools(int32 KeepPerPool)
{
