...
FSuspenseInventoryOperationResult ASuspenseCoreEquipmentActor::SwapEquipmentWith_Implementation(const TScriptInterface<ISuspenseCoreEquipment>& OtherEquipment)
{
    FSuspenseInventoryOperationResult R;
    ...
    const FSuspenseCoreInventoryItemInstance OtherItem = ISuspenseCoreEquipment::Execute_GetEquippedItemInstance(OtherEquipment.GetObject());

    const bool bThisCanEquipOther  = !OtherItem.IsValid() || CanEquipItemInstance_Implementation(OtherItem);
    const bool bOtherCanEquipThis  = !ThisItem.IsValid() || ISuspenseCoreEquipment::Execute_CanEquipItemInstance(OtherEquipment.GetObject(), ThisItem);
    ...
    if (OtherItem.IsValid())
    {
        ...
        ISuspenseCoreEquipment::Execute_UnequipItem(OtherEquipment.GetObject(), OtherUnequipped);
        EquipItemInstance_Implementation(OtherUnequipped, false);
        R.AffectedItems.Add(OtherUnequipped);
    }
    if (ThisItem.IsValid())
    {
        ISuspenseCoreEquipment::Execute_EquipItemInstance(OtherEquipment.GetObject(), Tmp, false);
    }
...
void ASuspenseCoreEquipmentActor::SetEquipmentRuntimeProperty_Implementation(const FName& PropertyName, float Value)
{
    ...
    BroadcastPropertyChanged(PropertyName, Old, Value);
}
...
{
    const_cast<ASuspenseCoreEquipmentActor*>(this)->BroadcastOperationEvent(EventTag, Payload);
}
...
void ASuspenseCoreEquipmentActor::NotifyEquipmentStateChanged(const FGameplayTag& NewState, bool bIsRefresh) const
{
    ...
    const_cast<ASuspenseCoreEquipmentActor*>(this)->BroadcastOperationEvent(EqTags().Event_PropertyChanged, Payload);
}
