...
USuspenseCoreEquipmentVisualController::USuspenseCoreEquipmentVisualController()
...
void USuspenseCoreEquipmentVisualController::BeginPlay()
{
	Super::BeginPlay();

	// Регистрируемся в ServiceLocator как Service.VisualController
	if (auto* Locator = USuspenseCoreEquipmentServiceLocator::Get(this))
	{
	...
	if (ControllerConfig.PoolCleanupInterval > 0.0f)
	{
		GetWorld()->GetTimerManager().SetTimer(
			...
			&USuspenseCoreEquipmentVisualController::CleanupEffectPool,
	...
	if (ControllerConfig.bEnableBatching)
	{
		GetWorld()->GetTimerManager().SetTimer(
			...
			&USuspenseCoreEquipmentVisualController::ProcessBatchQueue,
			0.1f, true);
	}
...
void USuspenseCoreEquipmentVisualController::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void USuspenseCoreEquipmentVisualController::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
bool USuspenseCoreEquipmentVisualController::RemoveVisualEffect(const FGuid& EffectId)
{
	EQUIPMENT_WRITE_LOCK(EffectLock);
...
void USuspenseCoreEquipmentVisualController::ResetMaterials(AActor* Equipment)
{
...
void USuspenseCoreEquipmentVisualController::UpdateWearState(AActor* Equipment, float WearPercent)
{
	if (!IsValid(Equipment)) return;
...
void USuspenseCoreEquipmentVisualController::SetHighlighted(AActor* Equipment, bool bHighlighted, const FLinearColor& HighlightColor)
{
	if (!IsValid(Equipment)) return;
...
bool USuspenseCoreEquipmentVisualController::PlayEquipmentAnimation(AActor* Equipment, const FGameplayTag& AnimationTag)
{
	if (!IsValid(Equipment)) return false;
...
bool USuspenseCoreEquipmentVisualController::ApplyVisualProfile(AActor* Equipment, const FGameplayTag& ProfileTag, bool bSmooth)
{
	if (!IsValid(Equipment)) return false;
...
void USuspenseCoreEquipmentVisualController::PreWarmEffectPool(const TArray<UNiagaraSystem*>& EffectSystems, int32 Count)
{
...
void USuspenseCoreEquipmentVisualController::StartMaterialTransition(AActor* Equipment, const FName& ParameterName, float TargetValue, float Duration, UCurveFloat* Curve)
{
...
void USuspenseCoreEquipmentVisualController::StartColorTransition(AActor* Equipment, const FName& ParameterName, const FLinearColor& TargetColor, float Duration)
{
	if (!IsValid(Equipment) || Duration <= 0.0f) return;
...
void USuspenseCoreEquipmentVisualController::ClearAllEffectsForEquipment(AActor* Equipment, bool bImmediate)
{
	if (!IsValid(Equipment)) return;
...
void USuspenseCoreEquipmentVisualController::SetVisualQualityLevel(int32 QualityLevel)
{
	CurrentQualityLevel = FMath::Clamp(QualityLevel, 0, 3);
	UE_LOG(LogTemp, Log, TEXT("[VisualController] Quality=%d"), CurrentQualityLevel);
}

void USuspenseCoreEquipmentVisualController::LoadVisualProfileTable(UDataTable* ProfileTable)
{
...
void USuspenseCoreEquipmentVisualController::ProcessBatchQueue()
{
	EQUIPMENT_WRITE_LOCK(BatchLock);
...
void USuspenseCoreEquipmentVisualController::UpdateMaterialTransitions(float DeltaTime)
{
	EQUIPMENT_WRITE_LOCK(MaterialLock);
...
void USuspenseCoreEquipmentVisualController::UpdateWearInterpolation(float DeltaTime)
{
...
void USuspenseCoreEquipmentVisualController::UpdateActiveEffects(float DeltaTime)
{
	EQUIPMENT_WRITE_LOCK(EffectLock);
...
bool USuspenseCoreEquipmentVisualController::ReturnEffectToPool(UNiagaraComponent* Component)
{
...
UNiagaraComponent* USuspenseCoreEquipmentVisualController::CreateEffectComponent(UNiagaraSystem* System)
{
...
void USuspenseCoreEquipmentVisualController::CleanupEffectPool()
{
	EQUIPMENT_WRITE_LOCK(EffectLock);
	const float Now = GetWorld() ? GetWorld()->GetTimeSeconds() : 0.0f;
...
void USuspenseCoreEquipmentVisualController::LoadVisualProfiles()
{
...
UMaterialInstanceDynamic* USuspenseCoreEquipmentVisualController::GetOrCreateDynamicMaterial(UMaterialInterface* BaseMaterial, const FName& CacheKey)
{
...
void USuspenseCoreEquipmentVisualController::ApplyWearToMaterial(UMaterialInstanceDynamic* Material, float WearPercent)
{
	if (!Material) return;
...
void USuspenseCoreEquipmentVisualController::ApplyHighlightToMaterial(UMaterialInstanceDynamic* Material, bool bHighlight, const FLinearColor& Color)
{
	if (!Material) return;
...
TArray<UMeshComponent*> USuspenseCoreEquipmentVisualController::GetMeshComponents(AActor* Actor) const
{
...
FGuid USuspenseCoreEquipmentVisualController::GenerateEffectId() const { return FGuid::NewGuid(); }

FGameplayTag USuspenseCoreEquipmentVisualController::GetQualityTag() const
{
...
FString USuspenseCoreEquipmentVisualController::GetVisualStatistics() const
{
	EQUIPMENT_READ_LOCK(EffectLock);
...
void USuspenseCoreEquipmentVisualController::ToggleDebugOverlay()
{
	bDebugOverlayEnabled = !bDebugOverlayEnabled;
	if (bDebugOverlayEnabled)
	{
		GetWorld()->GetTimerManager().SetTimer(
			DebugOverlayTimerHandle, this, &USuspenseCoreEquipmentVisualController::UpdateDebugOverlay,
...
void USuspenseCoreEquipmentVisualController::UpdateDebugOverlay()
{
...
void USuspenseCoreEquipmentVisualController::DrawDebugInfoForActor(AActor* Actor, const FString& Info) const
{
...
void USuspenseCoreEquipmentVisualController::LogVisualMetrics() const
{
...
void USuspenseCoreEquipmentVisualController::SetControllerConfiguration(const FVisualControllerConfig& NewConfig)
{
	...
	if (GetWorld())
	{
		...
		{
			GetWorld()->GetTimerManager().SetTimer(
				PoolCleanupTimerHandle, this, &USuspenseCoreEquipmentVisualController::CleanupEffectPool,
				ControllerConfig.PoolCleanupInterval, true);
		}
	}
}
...
bool USuspenseCoreEquipmentVisualController::ShouldPlayEffect(AActor* Equipment, const FGameplayTag& EffectTag, float& OutCooldownMs) const
{
	OutCooldownMs = 0.0f;
...
void USuspenseCoreEquipmentVisualController::MarkEffectPlayed(AActor* Equipment, const FGameplayTag& EffectTag)
{
	const TWeakObjectPtr<AActor> Key = Equipment;
