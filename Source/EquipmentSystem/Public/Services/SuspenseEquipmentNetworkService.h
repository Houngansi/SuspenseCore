...
#include "SuspenseEquipmentNetworkService.generated.h"
 ...
 */
UCLASS()
class EQUIPMENTSYSTEM_API USuspenseEquipmentNetworkService : public UObject,
    public IEquipmentNetworkService
{
    GENERATED_BODY()

public:
    USuspenseEquipmentNetworkService();
    virtual ~USuspenseEquipmentNetworkService();
...
private:
    ...
    UPROPERTY(Transient)
    TObjectPtr<USuspenseEquipmentReplicationManager> ReplicationProvider = nullptr;
    ...
    USuspenseEquipmentNetworkDispatcher* CreateAndInitNetworkDispatcher(
        AActor* OwnerActor,
        const TScriptInterface<ISuspenseEquipmentOperations>& OperationExecutor);

    USuspenseEquipmentPredictionSystem* CreateAndInitPredictionSystem(
        AActor* OwnerActor,
        const TScriptInterface<ISuspenseEquipmentDataProvider>& DataProvider,
        const TScriptInterface<ISuspenseEquipmentOperations>& OperationExecutor);

    USuspenseEquipmentReplicationManager* CreateAndInitReplicationManager(
        AActor* OwnerActor,
        const TScriptInterface<ISuspenseEquipmentDataProvider>& DataProvider);
