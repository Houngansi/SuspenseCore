...
DECLARE_LOG_CATEGORY_EXTERN(LogSuspenseCoreEquipmentData, Log, All);
DECLARE_LOG_CATEGORY_EXTERN(LogSuspenseCoreEquipmentNetwork, Log, All);
DECLARE_LOG_CATEGORY_EXTERN(LogSuspenseCoreEquipmentOperation, Log, All);
DECLARE_LOG_CATEGORY_EXTERN(LogSuspenseCoreEquipmentVisualization, Log, All);
DECLARE_LOG_CATEGORY_EXTERN(LogSuspenseCoreEquipmentValidation, Log, All);
...
// У вас уже есть класс FEquipmentRWLock в FSuspenseCoreEquipmentThreadGuard.h — конфликт решаем удалением alias.

// Переиспользуем ваши макросы, но если где-то этот header подключается раньше
// FSuspenseCoreEquipmentThreadGuard.h — дадим безопасный fallback. В обычном пути
// (мы выше инклюдим FSuspenseCoreEquipmentThreadGuard.h) эти #ifndef просто не сработают.
...
#define LOG_BATCH_DELTA(DeltaType, Count) \
    UE_LOG(LogSuspenseCoreEquipmentDelta, Verbose, TEXT("BatchDelta[%s]: %d operations"), TEXT(DeltaType), Count)
