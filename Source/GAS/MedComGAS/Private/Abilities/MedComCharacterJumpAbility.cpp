...
#include "Abilities/MedComCharacterJumpAbility.h"
...
UMedComCharacterJumpAbility::UMedComCharacterJumpAbility()
{
...
bool UMedComCharacterJumpAbility::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, 
    ...
    const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
    ...
    if (!IMedComMovementInterface::Execute_CanJump(Avatar))
    {
...
void UMedComCharacterJumpAbility::ActivateAbility(const FGameplayAbilitySpecHandle Handle,
    ...
    const FGameplayEventData* TriggerEventData)
{
    ...
    IMedComMovementInterface::NotifyMovementStateChanged(Avatar, JumpMovementState, true);
    IMedComMovementInterface::NotifyJumpStateChanged(Avatar, true);
    ...
    if (UWorld* World = GetWorld())
    {
        ...
        World->GetTimerManager().SetTimer(
            LandingCheckTimer,
            this,
            &UMedComCharacterJumpAbility::CheckForLanding,
            GroundCheckInterval,
            true,  // Повторяющийся
        ...
        World->GetTimerManager().SetTimer(
            SafetyTimer,
            this,
            &UMedComCharacterJumpAbility::ForceEndAbility,
            MaxJumpDuration,
            false  // Одноразовый
        );
...
void UMedComCharacterJumpAbility::EndAbility(const FGameplayAbilitySpecHandle Handle,
    const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo,
    bool bReplicateEndAbility, bool bWasCancelled)
{
    ...
    {
        IMedComMovementInterface::Execute_StopJumping(Avatar);
        IMedComMovementInterface::NotifyJumpStateChanged(Avatar, false);
    }
...
void UMedComCharacterJumpAbility::InputReleased(const FGameplayAbilitySpecHandle Handle,
    const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo)
{
...
bool UMedComCharacterJumpAbility::IsCharacterGrounded(const FGameplayAbilityActorInfo* ActorInfo) const
{
    ...
    if (Avatar->GetClass()->ImplementsInterface(UMedComMovementInterface::StaticClass()))
    {
        return IMedComMovementInterface::Execute_IsGrounded(Avatar);
    }
    
    return false;
}

bool UMedComCharacterJumpAbility::ApplyStaminaCost(const FGameplayAbilityActorInfo* ActorInfo)
{
    UAbilitySystemComponent* ASC = GetAbilitySystemComponentFromActorInfo();
...
void UMedComCharacterJumpAbility::PerformJump(const FGameplayAbilityActorInfo* ActorInfo)
{
    if (!ActorInfo || !ActorInfo->AvatarActor.IsValid())
    {
        return;
    ...
    if (Avatar->GetClass()->ImplementsInterface(UMedComMovementInterface::StaticClass()))
    {
        ...
        float OriginalJumpPower = IMedComMovementInterface::Execute_GetJumpZVelocity(Avatar);
        float ModifiedJumpPower = OriginalJumpPower * JumpPowerMultiplier;
        
        // Применяем модифицированную силу
        IMedComMovementInterface::Execute_SetJumpZVelocity(Avatar, ModifiedJumpPower);
        
        // Выполняем прыжок
        IMedComMovementInterface::Execute_Jump(Avatar);
        
        // Восстанавливаем оригинальную силу
        IMedComMovementInterface::Execute_SetJumpZVelocity(Avatar, OriginalJumpPower);
        
        ABILITY_LOG(Display, TEXT("[Jump] Прыжок выполнен с силой: %.1f"), ModifiedJumpPower);
    }
}

void UMedComCharacterJumpAbility::CheckForLanding()
{
...
void UMedComCharacterJumpAbility::ForceEndAbility()
{
