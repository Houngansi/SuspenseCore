...
#include "Abilities/MedComInteractAbility.h"
...
UMedComInteractAbility::UMedComInteractAbility()
{
...
bool UMedComInteractAbility::CanActivateAbility(
...
void UMedComInteractAbility::ActivateAbility(
    const FGameplayAbilitySpecHandle Handle,
    const FGameplayAbilityActorInfo* ActorInfo,
...
void UMedComInteractAbility::EndAbility(
    const FGameplayAbilitySpecHandle Handle,
    const FGameplayAbilityActorInfo* ActorInfo,
    const FGameplayAbilityActivationInfo ActivationInfo,
...
void UMedComInteractAbility::ServerPerformInteraction_Implementation(AActor* TargetActor)
{
...
void UMedComInteractAbility::ClientInteractionResult_Implementation(bool bSuccess, AActor* TargetActor)
{
    if (!CurrentActorInfo)
    {
        return;
...
AActor* UMedComInteractAbility::PerformInteractionTrace(const FGameplayAbilityActorInfo* ActorInfo) const
{
    if (!ActorInfo || !ActorInfo->AvatarActor.IsValid())
    {
...
void UMedComInteractAbility::ApplyCooldown(
    const FGameplayAbilitySpecHandle Handle,
    const FGameplayAbilityActorInfo* ActorInfo,
...
void UMedComInteractAbility::SendInteractionEvent(
    const FGameplayAbilityActorInfo* ActorInfo, 
    bool bSuccess, 
...
void UMedComInteractAbility::NotifyInteraction(bool bSuccess, AActor* TargetActor) const
{
    if (UEventDelegateManager* Manager = GetDelegateManager())
    {
        ...
        Manager->NotifyEquipmentEvent(
            const_cast<UMedComInteractAbility*>(this), 
            EventTag, 
            EventData);
    }
}

UEventDelegateManager* UMedComInteractAbility::GetDelegateManager() const
{
...
void UMedComInteractAbility::LogInteractionDebugInfo(const FString& Message, bool bError) const
{
...
void UMedComInteractAbility::DrawDebugInteraction(
    const FVector& Start, 
    const FVector& End, 
    bool bHit, 
