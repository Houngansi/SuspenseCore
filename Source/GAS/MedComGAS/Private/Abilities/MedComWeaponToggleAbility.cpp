...
#include "Abilities/MedComWeaponToggleAbility.h"
...
UMedComWeaponToggleAbility::UMedComWeaponToggleAbility()
{
...
bool UMedComWeaponToggleAbility::CanActivateAbility(
...
void UMedComWeaponToggleAbility::ActivateAbility(
    const FGameplayAbilitySpecHandle Handle,
    const FGameplayAbilityActorInfo* ActorInfo,
...
void UMedComWeaponToggleAbility::EndAbility(
    const FGameplayAbilitySpecHandle Handle,
    const FGameplayAbilityActorInfo* ActorInfo,
    const FGameplayAbilityActivationInfo ActivationInfo,
    ...
    if (CurrentMontage)
    {
        if (CurrentActorInfo && CurrentActorInfo->AvatarActor.IsValid())
        {
            if (ACharacter* Character = Cast<ACharacter>(CurrentActorInfo->AvatarActor.Get()))
            {
                ...
                {
                    AnimInstance->OnMontageBlendingOut.RemoveDynamic(this, &UMedComWeaponToggleAbility::OnMontageBlendingOut);
                    AnimInstance->OnMontageEnded.RemoveDynamic(this, &UMedComWeaponToggleAbility::OnMontageEnded);
                }
            }
        }
    }
...
int32 UMedComWeaponToggleAbility::DetermineToggleSlot(const FGameplayEventData* TriggerEventData) const
{
...
bool UMedComWeaponToggleAbility::IsWeaponDrawn(const TScriptInterface<IMedComEquipmentInterface>& EquipmentInterface, int32 SlotIndex) const
{
...
FGameplayTag UMedComWeaponToggleAbility::GetCurrentEquipmentState(const TScriptInterface<IMedComEquipmentInterface>& EquipmentInterface) const
{
    if (!EquipmentInterface.GetInterface())
    {
        return FGameplayTag();
...
void UMedComWeaponToggleAbility::SetEquipmentState(const TScriptInterface<IMedComEquipmentInterface>& EquipmentInterface, const FGameplayTag& NewState)
{
    if (!EquipmentInterface.GetInterface())
    {
        return;
...
void UMedComWeaponToggleAbility::PerformDraw(int32 SlotIndex)
{
...
void UMedComWeaponToggleAbility::PerformHolster(int32 SlotIndex)
{
    // Update equipment state through interface
...
void UMedComWeaponToggleAbility::PlayDrawAnimation(const FGameplayTag& WeaponType, bool bFirstDraw)
{
    ...
    if (CurrentActorInfo && CurrentActorInfo->AvatarActor.IsValid())
    {
        if (ACharacter* Character = Cast<ACharacter>(CurrentActorInfo->AvatarActor.Get()))
        {
            ...
            {
                // Bind delegates
                AnimInstance->OnMontageBlendingOut.AddDynamic(this, &UMedComWeaponToggleAbility::OnMontageBlendingOut);
                AnimInstance->OnMontageEnded.AddDynamic(this, &UMedComWeaponToggleAbility::OnMontageEnded);
                ...
                if (Duration > 0.0f)
                {
                    GetWorld()->GetTimerManager().SetTimer(
                        ...
                        this,
                        &UMedComWeaponToggleAbility::OnDrawAnimationComplete,
...
void UMedComWeaponToggleAbility::PlayHolsterAnimation(const FGameplayTag& WeaponType)
{
    if (!CachedAnimationInterface.GetInterface())
    {
        OnHolsterAnimationComplete();
            ...
            if (AnimInstance)
            {
                ...
                if (Duration > 0.0f)
                {
                    GetWorld()->GetTimerManager().SetTimer(
                        AnimationTimeoutHandle,
                        this,
                        &UMedComWeaponToggleAbility::OnHolsterAnimationComplete,
                        Duration / AnimationPlayRate,
                        false
                    );
                }
...
void UMedComWeaponToggleAbility::OnDrawAnimationComplete()
{
...
void UMedComWeaponToggleAbility::OnHolsterAnimationComplete()
{
...
void UMedComWeaponToggleAbility::OnMontageBlendingOut(UAnimMontage* Montage, bool bInterrupted)
{
...
void UMedComWeaponToggleAbility::OnMontageEnded(UAnimMontage* Montage, bool bInterrupted)
{
    if (Montage == CurrentMontage)
    {
...
TScriptInterface<IMedComEquipmentInterface> UMedComWeaponToggleAbility::FindEquipmentInterface() const
{
...
TScriptInterface<IMedComWeaponAnimationInterface> UMedComWeaponToggleAbility::GetAnimationInterface() const
{
...
FGameplayTag UMedComWeaponToggleAbility::GetWeaponTypeForSlot(const TScriptInterface<IMedComEquipmentInterface>& EquipmentInterface, int32 SlotIndex) const
{
    if (!EquipmentInterface.GetInterface())
    {
        return FGameplayTag();
...
void UMedComWeaponToggleAbility::ApplyToggleTags(bool bApply, bool bIsDrawingWeapon)
{
    if (!CurrentActorInfo || !CurrentActorInfo->AbilitySystemComponent.IsValid())
    {
        return;
...
void UMedComWeaponToggleAbility::SendToggleEvent(bool bStarted, int32 SlotIndex, bool bIsDrawingWeapon) const
{
...
void UMedComWeaponToggleAbility::LogToggleDebug(const FString& Message, bool bError) const
{
...
void UMedComWeaponToggleAbility::ServerRequestToggle_Implementation(int32 SlotIndex, bool bDraw, int32 PredictionKey)
{
...
void UMedComWeaponToggleAbility::ClientConfirmToggle_Implementation(int32 SlotIndex, bool bSuccess, int32 PredictionKey)
{
