...
#include "Components/MedComInteractionComponent.h"
...
UMedComInteractionComponent::UMedComInteractionComponent()
{
...
void UMedComInteractionComponent::BeginPlay()
{
    Super::BeginPlay();
    ...
    if (CachedASC.IsValid())
    {
        ...
        CachedASC->GenericGameplayEventCallbacks.FindOrAdd(InteractSuccessTag).AddUObject(
            this, &UMedComInteractionComponent::HandleInteractionSuccessDelegate);
        
        CachedASC->GenericGameplayEventCallbacks.FindOrAdd(InteractFailedTag).AddUObject(
            this, &UMedComInteractionComponent::HandleInteractionFailureDelegate);
...
void UMedComInteractionComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void UMedComInteractionComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
    Super::TickComponent(DeltaTime, TickType, ThisTickFunction);

    // Update UI information in tick if needed
    // Optimization: trace only for UI, all interaction logic is in MedComInteractAbility
...
bool UMedComInteractionComponent::CanInteractNow() const
{
    ...
    if (!CachedASC.IsValid())
    {
        ...
        const_cast<UMedComInteractionComponent*>(this)->CachedASC = CurrentASC;
    }
...
AActor* UMedComInteractionComponent::PerformUIInteractionTrace() const
{
...
void UMedComInteractionComponent::HandleInteractionSuccessDelegate(const FGameplayEventData* Payload)
{
    if (Payload) 
    {
...
void UMedComInteractionComponent::HandleInteractionFailureDelegate(const FGameplayEventData* Payload)
{
    if (Payload) 
    {
...
void UMedComInteractionComponent::HandleInteractionSuccess(const FGameplayEventData& Payload)
{
...
void UMedComInteractionComponent::HandleInteractionFailure(const FGameplayEventData& Payload)
{
    const AActor* TargetActor = nullptr;
...
bool UMedComInteractionComponent::HasBlockingTags() const
{
...
void UMedComInteractionComponent::LogInteraction(const FString& Message, bool bError) const
{
    const FString Prefix = FString::Printf(TEXT("[%s] "), *GetNameSafe(GetOwner()));
    
    if (bError)
    {
        UE_LOG(LogMedComInteraction, Warning, TEXT("%s%s"), *Prefix, *Message);
    }
    else
    {
        UE_LOG(LogMedComInteraction, Log, TEXT("%s%s"), *Prefix, *Message);
    }
}

UAbilitySystemComponent* UMedComInteractionComponent::GetOwnerASC() const
{
    AActor* OwnerActor = GetOwner();
    if (!OwnerActor)
    {
...
UEventDelegateManager* UMedComInteractionComponent::GetDelegateManager() const
{
    ...
    if (Manager)
    {
        const_cast<UMedComInteractionComponent*>(this)->CachedDelegateManager = Manager;
    }
    
    return Manager;
}

void UMedComInteractionComponent::SetInteractionCooldown()
{
...
void UMedComInteractionComponent::ResetInteractionCooldown()
{
    bInteractionOnCooldown = false;
}

void UMedComInteractionComponent::UpdateInteractionFocus(AActor* NewFocusActor)
{
...
void UMedComInteractionComponent::BroadcastInteractionAttempt(AActor* TargetActor)
{
...
void UMedComInteractionComponent::BroadcastInteractionResult(AActor* TargetActor, bool bSuccess)
{
    UEventDelegateManager* Manager = GetDelegateManager();
    if (Manager)
    {
