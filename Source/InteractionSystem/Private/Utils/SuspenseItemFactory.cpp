...
void USuspenseItemFactory::Initialize(FSubsystemCollectionBase& Collection)
{
    ...
    {
        DefaultPickupClass = ASuspensePickupItem::StaticClass();
    }
    
    UE_LOG(LogTemp, Log, TEXT("USuspenseItemFactory: Initialized with default class %s"), 
        *DefaultPickupClass->GetName());
}

void USuspenseItemFactory::Deinitialize()
{
...
AActor* USuspenseItemFactory::CreatePickupFromItemID_Implementation(
    FName ItemID,
    UWorld* World,
...
AActor* USuspenseItemFactory::CreatePickupWithAmmo(
    FName ItemID,
    UWorld* World,
    const FTransform& Transform,
    int32 Quantity,
...
TSubclassOf<AActor> USuspenseItemFactory::GetDefaultPickupClass_Implementation() const
{
    return DefaultPickupClass;
}

void USuspenseItemFactory::SetDefaultPickupClass(TSubclassOf<AActor> NewDefaultClass)
{
