...
#include "Debug/InventoryDebugger.h"
...
void UInventoryDebugger::Initialize(UMedComInventoryComponent* InInventoryComponent)
{
...
void UInventoryDebugger::StartMonitoring()
{
...
void UInventoryDebugger::StopMonitoring()
{
    if (!bMonitoringActive)
    {
...
FInventoryPerformanceMetrics UInventoryDebugger::GetPerformanceMetrics() const
{
...
void UInventoryDebugger::ResetMetrics()
{
    // Очищаем все метрики
...
FString UInventoryDebugger::GetMemoryUsageReport() const
{
    if (!InventoryComponent)
    {
...
FString UInventoryDebugger::GetCachePerformanceReport() const
{
...
FString UInventoryDebugger::GetInventoryDump(bool bIncludeRuntimeProperties) const
{
    if (!InventoryComponent)
    {
        return TEXT("Inventory component not available");
...
FString UInventoryDebugger::GetInstanceDump(const FGuid& InstanceID) const
{
    if (!InventoryComponent)
    {
        return TEXT("Inventory component not available");
    ...
    if (IMedComInventoryInterface* InventoryInterface = Cast<IMedComInventoryInterface>(InventoryComponent))
    {
        ...
        for (const FInventoryItemInstance& Instance : AllInstances)
        {
            ...
            {
                return UMedComInventoryFunctionLibrary::GetItemInstanceDebugInfo(Instance, InventoryComponent);
            }
        }
    }
...
FString UInventoryDebugger::GetGridOccupancyMap() const
{
    if (!InventoryComponent)
    {
        return TEXT("Inventory component not available");
    ...
    if (IMedComInventoryInterface* InventoryInterface = Cast<IMedComInventoryInterface>(InventoryComponent))
    {
        ...
        for (int32 i = 0; i < AllInstances.Num(); i++)
        {
            ...
            TArray<int32> OccupiedSlots = UMedComInventoryFunctionLibrary::GetOccupiedSlots(
...
//==================================================================
bool UInventoryDebugger::QuickValidateInventory(bool bVerbose) const
{
...
bool UInventoryDebugger::ValidateInventoryConsistency(bool bVerbose, TArray<FString>& OutErrorMessages) const
{
    ...
    {
        TArray<FInventoryItemInstance> AllInstances = InventoryInterface->GetAllItemInstances();
        CalculatedWeight = UMedComInventoryFunctionLibrary::CalculateTotalWeightFromInstances(
...
bool UInventoryDebugger::ValidateItemInstance(const FGuid& InstanceID, TArray<FString>& OutErrorMessages) const
{
    OutErrorMessages.Empty();
    ...
    if (IMedComInventoryInterface* InventoryInterface = Cast<IMedComInventoryInterface>(InventoryComponent))
    {
        ...
        for (const FInventoryItemInstance& Instance : AllInstances)
        {
            if (Instance.InstanceID == InstanceID)
            {
                return UMedComInventoryFunctionLibrary::ValidateItemInstance(
...
bool UInventoryDebugger::ValidateGridInstanceIntegrity(TArray<FString>& OutErrorMessages) const
{
    OutErrorMessages.Empty();
...
bool UInventoryDebugger::ValidateDataTableReferences(TArray<FName>& OutMissingItems) const
{
...
FString UInventoryDebugger::RunComprehensivePerformanceTest(int32 OperationCount, 
...
FString UInventoryDebugger::RunGridPerformanceTest(int32 GridTestCount) const
{
    if (!InventoryComponent)
    {
    ...
    for (int32 i = 0; i < GridTestCount; i++)
    {
        ...
        int32 PlacementSlot = UMedComInventoryFunctionLibrary::FindOptimalPlacementForItem(
    ...
    for (int32 i = 0; i < GridTestCount * 10; i++) // More iterations for coordinate conversion
    {
        ...
        int32 X, Y;
        UMedComInventoryFunctionLibrary::IndexToGridCoords(TestIndex, GridWidth, X, Y);
        int32 BackToIndex = UMedComInventoryFunctionLibrary::GridCoordsToIndex(X, Y, GridWidth, GridHeight);
...
FString UInventoryDebugger::RunInstanceCreationTest(int32 InstanceCount) const
{
...
void UInventoryDebugger::OnInstanceAdded(const FInventoryItemInstance& ItemInstance, int32 SlotIndex)
{
...
void UInventoryDebugger::OnInstanceRemoved(const FName& ItemID, int32 Quantity, int32 SlotIndex)
{
    // Увеличиваем счетчик операций
...
void UInventoryDebugger::OnItemMoved(UObject* Item, int32 OldSlotIndex, int32 NewSlotIndex, bool bWasRotated)
{
    // Увеличиваем счетчик операций
...
void UInventoryDebugger::OnItemsSwapped(UObject* FirstItem, UObject* SecondItem, int32 FirstNewIndex, int32 SecondNewIndex)
{
    // Увеличиваем счетчик операций
...
void UInventoryDebugger::OnInventoryError(EInventoryErrorCode ErrorCode, const FString& Context)
{
...
void UInventoryDebugger::OnStackOperation(const FInventoryItemInstance& SourceInstance, 
...
void UInventoryDebugger::RecordOperationTime(const FName& OperationType, double StartTime)
{
...
void UInventoryDebugger::SubscribeToEvents()
{
    if (!InventoryComponent)
    {
    ...
    // Это зависит от реализации системы событий в UMedComInventoryComponent
    // Пример:
    // if (InventoryComponent->OnItemInstanceAdded.IsBound())
    // {
    //     InventoryComponent->OnItemInstanceAdded.AddDynamic(this, &UInventoryDebugger::OnInstanceAdded);
    // }
    
    UE_LOG(LogInventory, Log, TEXT("InventoryDebugger: Subscribed to inventory events"));
}

void UInventoryDebugger::UnsubscribeFromEvents()
{
    if (!InventoryComponent)
    {
        return;
    ...
    // if (InventoryComponent->OnItemInstanceAdded.IsBound())
    // {
    //     InventoryComponent->OnItemInstanceAdded.RemoveDynamic(this, &UInventoryDebugger::OnInstanceAdded);
    // }
...
void UInventoryDebugger::UpdateMemoryMetrics() const
{
    if (!InventoryComponent)
    {
        return;
...
bool UInventoryDebugger::ValidateInstanceInternal(const FInventoryItemInstance& Instance, TArray<FString>& OutErrors) const
{
    // Используем библиотечную функцию для validation
    return UMedComInventoryFunctionLibrary::ValidateItemInstance(Instance, InventoryComponent, OutErrors);
}

UMedComItemManager* UInventoryDebugger::GetItemManager() const
{
    if (!InventoryComponent)
    {
        return nullptr;
...
void UInventoryDebugger::ClearValidationCacheIfNeeded() const
{
    double CurrentTime = FPlatformTime::Seconds();
...
FString UInventoryDebugger::FormatTime(double TimeInSeconds)
{
...
FString UInventoryDebugger::FormatMemorySize(int32 SizeInBytes)
{
    if (SizeInBytes < 1024)
    {
...
FString UInventoryDebugger::RunDataTableAccessTest(int32 AccessCount) const
{
...
void UInventoryDebugger::EnableEventLogging(bool bEnable)
{
    if (!InventoryComponent)
    {
...
UMedComInventoryComponent* UInventoryDebugger::CreateInventoryCopy() const
{
    if (!InventoryComponent)
    {
    ...
    UMedComInventoryComponent* CopyComponent = NewObject<UMedComInventoryComponent>(
        Owner, 
        UMedComInventoryComponent::StaticClass(),
        NAME_None,
        RF_Transient
    );
...
FString UInventoryDebugger::ExportInventoryToJSON(bool bIncludeRuntimeData) const
{
    if (!InventoryComponent)
    {
        return TEXT("{}");
    ...
    return UMedComInventorySerializer::SerializeInventoryToJson(InventoryComponent, true);
}

// Run stress test
FString UInventoryDebugger::RunStressTest(float Duration, int32 OperationsPerSecond) const
{
    ...
    for (int32 i = 0; i < TotalOperations; i++)
    {
        ...
        switch (OpType)
        {
            case 0: // Add
            {
                ...
                bSuccess = const_cast<UMedComInventoryComponent*>(InventoryComponent)->AddItemByID(ItemID, 1);
                break;
            }
            case 1: // Remove
            {
                FName ItemID = TestItemIDs[FMath::RandRange(0, TestItemIDs.Num() - 1)];
                const_cast<UMedComInventoryComponent*>(InventoryComponent)->RemoveItemByID(ItemID, 1);
