...
FMoveOperation::FMoveOperation(
    ...
    : FInventoryOperation(EInventoryOperationType::Move, InComponent)
    , Item(InItem)
...
FMoveOperation FMoveOperation::Create(
    UMedComInventoryComponent* InComponent, 
    AMedComInventoryItem* InItem, 
    int32 InTargetIndex, 
    bool InTargetRotated, 
...
{
    FMoveOperation Operation(InComponent, InItem, InTargetIndex, InTargetRotated, InTargetInventory);
    
    // Базовая валидация
    if (!InItem)
    {
        Operation.ErrorCode = EInventoryErrorCode::InvalidItem;
        UE_LOG(LogInventory, Error, TEXT("FMoveOperation::Create: Invalid item"));
        return Operation;
    }
    ...
    if (!InComponent)
    {
        ...
        UE_LOG(LogInventory, Error, TEXT("FMoveOperation::Create: Invalid component"));
        return Operation;
    }
    ...
    {
        Operation.ErrorCode = EInventoryErrorCode::NotInitialized;
        UE_LOG(LogInventory, Error, TEXT("FMoveOperation::Create: ItemManager not available"));
        return Operation;
    }
    ...
    {
        Operation.ErrorCode = EInventoryErrorCode::InvalidItem;
        UE_LOG(LogInventory, Error, TEXT("FMoveOperation::Create: Failed to cache item data for %s"), 
            *InItem->GetName());
        return Operation;
    }
    ...
    {
        UE_LOG(LogInventory, Warning, TEXT("FMoveOperation::Create: Validation failed - %s"), *ErrorMessage);
    }
...
FMoveOperation FMoveOperation::CreateWithOptimalRotation(
    UMedComInventoryComponent* InComponent,
    AMedComInventoryItem* InItem,
    int32 InTargetIndex,
    UMedComItemManager* InItemManager)
...
bool FMoveOperation::CacheItemDataFromTable(UMedComItemManager* InItemManager)
{
    ...
    if (!ItemInterface)
    {
        UE_LOG(LogInventory, Error, TEXT("FMoveOperation::CacheItemDataFromTable: Item doesn't implement required interface"));
        return false;
    }
    ...
    {
        UE_LOG(LogInventory, Error, TEXT("FMoveOperation::CacheItemDataFromTable: Failed to get data for %s"), 
            *ItemID.ToString());
        return false;
    }
    ...
    UE_LOG(LogInventory, VeryVerbose, TEXT("FMoveOperation::CacheItemDataFromTable: Cached data for %s - Size: %dx%d, Weight: %.2f"), 
        *ItemID.ToString(), 
...
void FMoveOperation::CalculateEffectiveSizes()
{
    if (!bHasCachedData)
    {
        UE_LOG(LogInventory, Warning, TEXT("FMoveOperation::CalculateEffectiveSizes: No cached data available"));
        return;
    }
    if (bSourceRotated)
    {
    ...
    UE_LOG(LogInventory, VeryVerbose, TEXT("FMoveOperation::CalculateEffectiveSizes: Source: %.0fx%.0f, Target: %.0fx%.0f"), 
        SourceEffectiveSize.X, SourceEffectiveSize.Y,
        TargetEffectiveSize.X, TargetEffectiveSize.Y);
}

// Получение веса из кэша
float FMoveOperation::GetCachedItemWeight() const
{
    return ItemTotalWeight;
}

// Полная валидация операции
bool FMoveOperation::ValidateOperation(
...
// Проверка весовых ограничений
bool FMoveOperation::ValidateWeightConstraints() const
{
...
// Проверка типовых ограничений
bool FMoveOperation::ValidateItemTypeConstraints() const
{
...
FString FMoveOperation::GetOperationTypeDescription() const
{
    if (IsCrossInventoryMove())
    {
...
// Выполнение операции
bool FMoveOperation::ExecuteOperation(
    EInventoryErrorCode& OutErrorCode,
    UMedComItemManager* InItemManager)
{
...
bool FMoveOperation::HandleSwapOperation(
...
void FMoveOperation::ApplyNewState()
{
    if (!Item)
    {
        return;
...
void FMoveOperation::RestoreOriginalState()
{
    if (!Item)
    {
        return;
...
void FMoveOperation::UpdateRuntimeProperties()
{
    if (!Item)
    {
        return;
...
void FMoveOperation::LogOperationDetails(const FString& Message, bool bIsError) const
{
...
FString FMoveOperation::ToString() const
{
    FString ItemName = TEXT("None");
