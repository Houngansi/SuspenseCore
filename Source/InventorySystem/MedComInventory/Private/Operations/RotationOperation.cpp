...
FRotationOperation FRotationOperation::Create(
    AMedComInventoryItem* InItem, 
    bool InTargetRotation,
    UMedComItemManager* InItemManager)
{
    FRotationOperation Operation;
    
    // Базовая валидация
    if (!InItem)
    {
        Operation.ErrorCode = EInventoryErrorCode::InvalidItem;
        UE_LOG(LogInventory, Error, TEXT("FRotationOperation::Create: Invalid item"));
        return Operation;
    }
    ...
    if (!InItemManager)
    {
        ...
        UE_LOG(LogInventory, Error, TEXT("FRotationOperation::Create: ItemManager not available"));
        return Operation;
    }
    ...
    {
        Operation.ErrorCode = EInventoryErrorCode::InvalidItem;
        UE_LOG(LogInventory, Error, TEXT("FRotationOperation::Create: Item doesn't implement required interface"));
        return Operation;
    }
    ...
    {
        Operation.ErrorCode = EInventoryErrorCode::InvalidItem;
        UE_LOG(LogInventory, Error, TEXT("FRotationOperation::Create: Failed to cache item data"));
        return Operation;
    }
    ...
    if (!Operation.ValidateRotation(ValidationError, ErrorMessage))
    {
        ...
        UE_LOG(LogInventory, Warning, TEXT("FRotationOperation::Create: Validation failed - %s"), *ErrorMessage);
    }
...
FRotationOperation FRotationOperation::Create(
    ...
    UMedComItemManager* InItemManager)
{
    FRotationOperation Operation = Create(InItem, InTargetRotation, InItemManager);
...
FRotationOperation FRotationOperation::CreateToggle(
    UMedComInventoryComponent* InComponent,
    AMedComInventoryItem* InItem,
    UMedComItemManager* InItemManager)
{
...
// Кэширование данных из DataTable
bool FRotationOperation::CacheItemDataFromTable(UMedComItemManager* InItemManager)
{
    ...
    if (!ItemInterface)
    {
        UE_LOG(LogInventory, Error, TEXT("FRotationOperation::CacheItemDataFromTable: Item doesn't implement required interface"));
        return false;
    }
    ...
    {
        UE_LOG(LogInventory, Error, TEXT("FRotationOperation::CacheItemDataFromTable: Failed to get data for %s"), 
            *ItemID.ToString());
        return false;
    }
    ...
    UE_LOG(LogInventory, VeryVerbose, TEXT("FRotationOperation::CacheItemDataFromTable: Cached data for %s - Size: %dx%d"), 
...
void FRotationOperation::CalculateEffectiveSizes()
{
    if (!bHasCachedData)
    {
        UE_LOG(LogInventory, Warning, TEXT("FRotationOperation::CalculateEffectiveSizes: No cached data"));
        return;
    }
    ...
    UE_LOG(LogInventory, VeryVerbose, TEXT("FRotationOperation::CalculateEffectiveSizes: Initial: %.0fx%.0f, Target: %.0fx%.0f"), 
        InitialEffectiveSize.X, InitialEffectiveSize.Y,
        TargetEffectiveSize.X, TargetEffectiveSize.Y);
}

// Полная валидация операции
bool FRotationOperation::ValidateRotation(
...
bool FRotationOperation::CheckCollisions() const
{
...
void FRotationOperation::CalculateTargetCells()
{
    ...
    UE_LOG(LogInventory, VeryVerbose, TEXT("FRotationOperation::CalculateTargetCells: %d cells for item at anchor %d"), 
        TargetOccupiedCells.Num(), AnchorIndex);
}

// Получение описания операции
FString FRotationOperation::GetOperationDescription() const
{
    if (!HasRotationChanged())
    {
...
bool FRotationOperation::ExecuteRotation(EInventoryErrorCode& OutErrorCode)
{
...
void FRotationOperation::ApplyRotation(bool bRotated)
{
    if (!Item)
    {
        return;
...
void FRotationOperation::UpdateGridPlacement()
{
    ...
    if (InventoryComponent->CanRotateItemAtSlot(AnchorIndex))
    {
        ...
        else
        {
            UE_LOG(LogInventory, Warning, 
                TEXT("FRotationOperation::UpdateGridPlacement: RotateItemAtSlot failed for slot %d"),
                AnchorIndex);
        }
    }
    else
    {
        ...
        UE_LOG(LogInventory, Warning, 
            TEXT("FRotationOperation::UpdateGridPlacement: CanRotateItemAtSlot returned false for slot %d"),
            AnchorIndex);
...
void FRotationOperation::LogOperationDetails(const FString& Message, bool bIsError) const
{
...
// Реализация Undo
bool FRotationOperation::CanUndo() const
{
    return bSuccess && Item != nullptr && HasRotationChanged();
}

bool FRotationOperation::Undo()
{
    if (!CanUndo())
    {
        return false;
...
// Реализация Redo  
bool FRotationOperation::CanRedo() const
{
    return Item != nullptr && HasRotationChanged();
}

bool FRotationOperation::Redo()
{
    if (!CanRedo())
    {
        return false;
...
FString FRotationOperation::ToString() const
{
    const FString ItemName = Item ? Item->GetItemID().ToString() : TEXT("None");
