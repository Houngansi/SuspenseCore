...
#include "Operations/StackOperation.h"
...
FStackOperation::FStackOperation(
    ...
    : FInventoryOperation(EInventoryOperationType::Stack, InComponent)
...
FStackOperation FStackOperation::Create(
    UMedComInventoryComponent* InComponent, 
    AMedComInventoryItem* InSourceItem, 
    AMedComInventoryItem* InTargetItem, 
    int32 InAmountToTransfer,
...
{
    FStackOperation Operation(InComponent, InSourceItem, InTargetItem, InAmountToTransfer, InTargetInventory);
    
    // Базовая валидация
    if (!InSourceItem)
    {
        Operation.ErrorCode = EInventoryErrorCode::InvalidItem;
        UE_LOG(LogInventory, Error, TEXT("FStackOperation::Create: Invalid source item"));
        return Operation;
    }
    ...
    {
        Operation.ErrorCode = EInventoryErrorCode::InvalidItem;
        UE_LOG(LogInventory, Error, TEXT("FStackOperation::Create: Invalid target item and no target index"));
        return Operation;
    }
    ...
    if (!InComponent)
    {
        ...
        UE_LOG(LogInventory, Error, TEXT("FStackOperation::Create: Invalid component"));
        return Operation;
    }
    ...
    {
        Operation.ErrorCode = EInventoryErrorCode::NotInitialized;
        UE_LOG(LogInventory, Error, TEXT("FStackOperation::Create: ItemManager not available"));
        return Operation;
    }
    ...
    {
        Operation.ErrorCode = EInventoryErrorCode::InvalidItem;
        UE_LOG(LogInventory, Error, TEXT("FStackOperation::Create: Failed to cache item data"));
        return Operation;
    }
    ...
    {
        UE_LOG(LogInventory, Warning, TEXT("FStackOperation::Create: Validation failed - %s"), *ErrorMessage);
    }
...
FStackOperation FStackOperation::CreateFullStack(
    UMedComInventoryComponent* InComponent,
    AMedComInventoryItem* InSourceItem,
    AMedComInventoryItem* InTargetItem,
    UMedComItemManager* InItemManager)
{
    ...
    FStackOperation Operation = Create(
        InComponent, 
        InSourceItem, 
        InTargetItem, 
...
FStackOperation FStackOperation::CreateSplit(
    UMedComInventoryComponent* InComponent,
    AMedComInventoryItem* InSourceItem,
    int32 InSplitAmount,
    int32 InTargetIndex,
    UMedComItemManager* InItemManager)
{
    ...
    if (!InSourceItem || InSplitAmount <= 0 || InSplitAmount >= Operation.SourceInitialAmount)
    {
        ...
        UE_LOG(LogInventory, Error, TEXT("FStackOperation::CreateSplit: Invalid split amount"));
        return Operation;
    }
...
bool FStackOperation::CacheItemDataFromTable(UMedComItemManager* InItemManager)
{
    ...
    if (!SourceInterface)
    {
        UE_LOG(LogInventory, Error, TEXT("FStackOperation::CacheItemDataFromTable: Source item doesn't implement required interface"));
        return false;
    }
    ...
    {
        UE_LOG(LogInventory, Error, TEXT("FStackOperation::CacheItemDataFromTable: Failed to get data for %s"), 
            *ItemID.ToString());
        return false;
    }
    ...
    UE_LOG(LogInventory, VeryVerbose, TEXT("FStackOperation::CacheItemDataFromTable: Cached data for %s - MaxStack: %d, Weight: %.2f"), 
...
int32 FStackOperation::CalculateMaxTransferAmount() const
{
...
bool FStackOperation::ValidateStacking(
...
// Проверка совместимости предметов
bool FStackOperation::AreItemsStackable() const
{
...
bool FStackOperation::AreRuntimePropertiesCompatible() const
{
    if (!SourceItem || !TargetItem)
    {
        return true;
    ...
    if (CachedItemData.bIsEquippable && !bAllowDifferentDurability)
    {
        ...
        {
            UE_LOG(LogInventory, VeryVerbose, TEXT("FStackOperation: Durability mismatch - Source: %.2f, Target: %.2f"), 
    ...
    if (CachedItemData.bIsWeapon && !bAllowDifferentAmmo)
    {
        ...
        {
            UE_LOG(LogInventory, VeryVerbose, TEXT("FStackOperation: Ammo mismatch - Source: %d, Target: %d"), 
...
bool FStackOperation::ValidateWeightConstraints() const
{
...
FString FStackOperation::GetOperationDescription() const
{
    if (IsSplitOperation())
    {
...
bool FStackOperation::ExecuteStacking(
    EInventoryErrorCode& OutErrorCode,
    UMedComItemManager* InItemManager)
{
...
void FStackOperation::MergeRuntimeProperties()
{
    ...
    if (CachedItemData.bIsEquippable)
    {
        ...
        UE_LOG(LogInventory, VeryVerbose, TEXT("FStackOperation::MergeRuntimeProperties: Average durability: %.2f"), 
            AverageDurability);
    }
    ...
    if (CachedItemData.bIsWeapon)
    {
        ...
        UE_LOG(LogInventory, VeryVerbose, TEXT("FStackOperation::MergeRuntimeProperties: Using minimum ammo: %.0f"), 
            AverageAmmo);
    }
}

// Применение объединенных свойств
void FStackOperation::ApplyMergedProperties()
{
...
// Перенос количества
bool FStackOperation::TransferAmount()
{
    if (!SourceItem || !TargetItem || ActualTransferred <= 0)
    {
        return false;
...
void FStackOperation::HandleSourceDepletion()
{
...
bool FStackOperation::CreateNewStackForSplit(UMedComItemManager* InItemManager)
{
...
void FStackOperation::UpdateInventoryWeights()
{
...
void FStackOperation::LogOperationDetails(const FString& Message, bool bIsError) const
{
...
// Реализация Undo
bool FStackOperation::CanUndo() const
{
    return bSuccess && SourceItem != nullptr && !bSourceDepleted;
}

bool FStackOperation::Undo()
{
    if (!CanUndo())
    {
        return false;
...
FString FStackOperation::ToString() const
{
    FString ItemName = TEXT("None");
