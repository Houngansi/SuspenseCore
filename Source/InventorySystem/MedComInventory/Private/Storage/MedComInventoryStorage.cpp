...
UMedComInventoryStorage::UMedComInventoryStorage()
{
    ...
    UE_LOG(LogInventory, VeryVerbose, TEXT("UMedComInventoryStorage: Constructor called"));
}

void UMedComInventoryStorage::BeginPlay()
{
    Super::BeginPlay();
    ...
    if (bInitialized)
    {
        ...
        {
            UE_LOG(LogInventory, Warning, TEXT("UMedComInventoryStorage: Storage integrity validation failed"));
...
void UMedComInventoryStorage::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
    // Откатываем активную транзакцию если есть
    if (IsTransactionActive())
    {
        UE_LOG(LogInventory, Warning, TEXT("UMedComInventoryStorage: Rolling back active transaction during EndPlay"));
        RollbackTransaction();
    }
    
    UE_LOG(LogInventory, VeryVerbose, TEXT("UMedComInventoryStorage: EndPlay cleanup completed"));
...
bool UMedComInventoryStorage::InitializeGrid(int32 Width, int32 Height, float InMaxWeight)
{
...
int32 UMedComInventoryStorage::GetFreeCellCount() const
{
    if (!bInitialized)
    {
        return 0;
...
bool UMedComInventoryStorage::AddItemInstance(const FInventoryItemInstance& ItemInstance, bool bAllowRotation)
{
    // Базовая валидация
    if (!bInitialized)
    {
...
bool UMedComInventoryStorage::RemoveItemInstance(const FGuid& InstanceID)
{
    if (!bInitialized)
    {
...
bool UMedComInventoryStorage::GetItemInstance(const FGuid& InstanceID, FInventoryItemInstance& OutInstance) const
{
...
TArray<FInventoryItemInstance> UMedComInventoryStorage::GetAllItemInstances() const
{
    return StoredInstances;
}

bool UMedComInventoryStorage::UpdateItemInstance(const FInventoryItemInstance& UpdatedInstance)
{
...
int32 UMedComInventoryStorage::FindFreeSpace(const FName& ItemID, bool bAllowRotation, bool bOptimizeFragmentation) const
{
...
bool UMedComInventoryStorage::AreCellsFreeForItem(int32 StartIndex, const FName& ItemID, bool bIsRotated) const
{
...
bool UMedComInventoryStorage::AreCellsFree(int32 StartIndex, const FVector2D& Size) const
{
...
bool UMedComInventoryStorage::PlaceItemInstance(const FInventoryItemInstance& ItemInstance, int32 AnchorIndex)
{
...
bool UMedComInventoryStorage::MoveItem(const FGuid& InstanceID, int32 NewAnchorIndex, bool bAllowRotation)
{
...
bool UMedComInventoryStorage::GetItemInstanceAt(int32 Index, FInventoryItemInstance& OutInstance) const
{
...
int32 UMedComInventoryStorage::GetItemCountByID(const FName& ItemID) const
{
    if (!bInitialized || ItemID.IsNone())
    {
        return 0;
...
TArray<FInventoryItemInstance> UMedComInventoryStorage::FindItemsByType(const FGameplayTag& ItemType) const
{
...
TArray<int32> UMedComInventoryStorage::GetOccupiedCells(const FGuid& InstanceID) const
{
...
float UMedComInventoryStorage::GetCurrentWeight() const
{
    if (!bInitialized)
    {
        return 0.0f;
...
void UMedComInventoryStorage::SetMaxWeight(float NewMaxWeight)
{
...
bool UMedComInventoryStorage::HasWeightCapacity(const FName& ItemID, int32 Quantity) const
{
...
void UMedComInventoryStorage::BeginTransaction()
{
    if (!bInitialized)
    {
...
void UMedComInventoryStorage::CommitTransaction()
{
    if (!ActiveTransaction.bIsActive)
    {
...
void UMedComInventoryStorage::RollbackTransaction()
{
    if (!ActiveTransaction.bIsActive)
    {
...
bool UMedComInventoryStorage::IsTransactionActive() const
{
    if (!ActiveTransaction.bIsActive)
    {
        return false;
...
void UMedComInventoryStorage::ClearAllItems()
{
    if (!bInitialized)
    {
        return;
...
bool UMedComInventoryStorage::ValidateStorageIntegrity(TArray<FString>& OutErrors, bool bAutoFix)
{
    OutErrors.Empty();
...
FString UMedComInventoryStorage::GetStorageDebugInfo() const
{
    if (!bInitialized)
    {
...
int32 UMedComInventoryStorage::DefragmentStorage()
{
...
bool UMedComInventoryStorage::IsValidIndex(int32 Index) const
{
    return bInitialized && Index >= 0 && Index < (GridWidth * GridHeight);
}

void UMedComInventoryStorage::UpdateFreeCellsBitmap()
{
    if (!bInitialized)
    {
        return;
...
UMedComItemManager* UMedComInventoryStorage::GetItemManager() const
{
...
bool UMedComInventoryStorage::GetItemData(const FName& ItemID, FMedComUnifiedItemData& OutData) const
{
    UMedComItemManager* ItemManager = GetItemManager();
    if (!ItemManager)
    {
...
bool UMedComInventoryStorage::PlaceInstanceInCells(const FInventoryItemInstance& ItemInstance, int32 AnchorIndex)
{
    // Базовая валидация
    if (!bInitialized || !ItemInstance.IsValid() || !IsValidIndex(AnchorIndex))
    {
...
bool UMedComInventoryStorage::RemoveInstanceFromCells(const FGuid& InstanceID)
{
...
FInventoryItemInstance* UMedComInventoryStorage::FindStoredInstance(const FGuid& InstanceID)
{
...
const FInventoryItemInstance* UMedComInventoryStorage::FindStoredInstance(const FGuid& InstanceID) const
{
    for (const FInventoryItemInstance& Instance : StoredInstances)
    {
        if (Instance.InstanceID == InstanceID)
...
void UMedComInventoryStorage::CreateTransactionSnapshot()
{
    ActiveTransaction.CellsSnapshot = Cells;
    ActiveTransaction.InstancesSnapshot = StoredInstances;
}

void UMedComInventoryStorage::RestoreFromTransactionSnapshot()
{
...
int32 UMedComInventoryStorage::FindOptimalPlacement(const FVector2D& ItemSize, bool bOptimizeFragmentation) const
{
...
bool UMedComInventoryStorage::GetGridCoordinates(int32 Index, int32& OutX, int32& OutY) const
{
...
bool UMedComInventoryStorage::GetLinearIndex(int32 X, int32 Y, int32& OutIndex) const
{
