// MedComInventory/Components/MedComInventoryComponent.h
...
#include "MedComInventoryComponent.generated.h"

// Forward declarations
class UMedComInventoryStorage;
class UMedComInventoryConstraints;
class UMedComInventoryTransaction;
class UInventoryReplicator;
class UMedComInventoryEvents;
class UMedComInventorySerializer;
class UMedComInventoryUIConnector;
class UMedComInventoryGASIntegration;
...
/**
 * Primary component for inventory management with new DataTable architecture
 * 
 * ARCHITECTURAL IMPROVEMENTS:
 * - Полностью работает с FInventoryItemInstance и FGuid
 * - Не создает объекты предметов (UObject/Actor)
 * - Использует DataTable как единственный источник истины
 * - Интегрирован с LoadoutSettings для конфигурации
 * - Thread-safe операции с поддержкой транзакций
 */
UCLASS(ClassGroup=(MedCom), meta=(BlueprintSpawnableComponent))
class MEDCOMINVENTORY_API UMedComInventoryComponent : public UActorComponent, public IMedComInventoryInterface
{
    GENERATED_BODY()
...
public:
    ...
    // Основные операции с предметами через DataTable
    virtual bool AddItem(const FMedComUnifiedItemData& ItemData, int32 Amount) override;
    virtual bool AddItemWithErrorCode(const FMedComUnifiedItemData& ItemData, int32 Amount, EInventoryErrorCode& OutErrorCode) override;
    virtual bool TryAddItem_Implementation(const FMedComUnifiedItemData& ItemData, int32 Quantity) override;
    virtual bool RemoveItem(const FName& ItemID, int32 Amount) override;

    // Валидация и получение предметов
    virtual bool ReceiveItem_Implementation(const FMedComUnifiedItemData& ItemData, int32 Quantity) override;
    virtual bool CanReceiveItem_Implementation(const FMedComUnifiedItemData& ItemData, int32 Quantity) const override;
    virtual FGameplayTagContainer GetAllowedItemTypes_Implementation() const override;
    
    // Grid placement operations (ОБНОВЛЕНО для работы с экземплярами)
    ...
    // Properties and queries (ОБНОВЛЕНО)
    ...
    // UI support (ОБНОВЛЕНО для работы со слотами)
    ...
    virtual void InitializeInventory(const struct FInventoryConfig& Config) override;
    
    // Events & delegates
    virtual void BroadcastInventoryUpdated() override;
    virtual UEventDelegateManager* GetDelegateManager() const override;
    virtual void BindToInventoryUpdates(const FOnInventoryUpdated::FDelegate& Delegate) override;
    virtual void UnbindFromInventoryUpdates(const FOnInventoryUpdated::FDelegate& Delegate) override;
    ...
    //==================================================================
    // Extended API (Non-Interface) - Public Methods
    //==================================================================
    ...
    UMedComInventoryEvents* GetEventsComponent() const { return EventsComponent; }
    
    UFUNCTION(BlueprintCallable, Category = "Inventory|Integration")
    UMedComInventoryUIConnector* GetUIAdapter() const { return UIAdapter; }
    
    UFUNCTION(BlueprintCallable, Category = "Inventory|Integration")
    UMedComInventoryGASIntegration* GetGASIntegration() const { return GASIntegration; }
    ...
    UInventoryReplicator* GetReplicatorComponent() const { return ReplicatorComponent; }
...
protected:
    ...
    UPROPERTY()
    UMedComInventoryStorage* StorageComponent;
    
    UPROPERTY()
    UMedComInventoryConstraints* ConstraintsComponent;
    
    UPROPERTY()
    UMedComInventoryTransaction* TransactionComponent;
    
    UPROPERTY()
    UInventoryReplicator* ReplicatorComponent;
    
    UPROPERTY()
    UMedComInventoryEvents* EventsComponent;
    
    UPROPERTY()
    UMedComInventorySerializer* SerializerComponent;
    
    UPROPERTY()
    UMedComInventoryUIConnector* UIAdapter;
    
    UPROPERTY()
    UMedComInventoryGASIntegration* GASIntegration;
    ...
    mutable TWeakObjectPtr<UInventoryManager> CachedInventoryManager;
