...
USuspenseItemBase::USuspenseItemBase()
{
...
void USuspenseItemBase::BeginDestroy()
{
...
void USuspenseItemBase::RefreshItemData()
{
    CachedItemData = nullptr;
    LastCacheUpdateTime = 0.0f;
    UpdateCacheIfNeeded();
...
bool USuspenseItemBase::HasValidItemData() const
{
...
void USuspenseItemBase::UpdateCacheIfNeeded() const
{
...
float USuspenseItemBase::GetRuntimeProperty(const FName& PropertyName, float DefaultValue) const
{
...
void USuspenseItemBase::SetRuntimeProperty(const FName& PropertyName, float Value)
{
...
TArray<FItemRuntimeProperty> USuspenseItemBase::GetAllRuntimeProperties() const
{
    // Убеждаемся, что массив актуален
    const_cast<USuspenseItemBase*>(this)->SyncPropertiesArrayWithCache();
    return RuntimePropertiesArray;
}
void USuspenseItemBase::RemoveRuntimeProperty(const FName& PropertyName)
{
    // Удаляем из кэша
...
bool USuspenseItemBase::HasRuntimeProperty(const FName& PropertyName) const
{
    // Проверяем кэш
...
float USuspenseItemBase::GetMaxDurability() const
{
...
bool USuspenseItemBase::HasDurability() const
{
    return GetMaxDurability() > 0.0f;
}

float USuspenseItemBase::GetDurabilityPercent() const
{
...
void USuspenseItemBase::SetCurrentDurability(float NewDurability)
{
    float MaxDur = GetMaxDurability();
    CurrentDurability = FMath::Clamp(NewDurability, 0.0f, MaxDur);
}

float USuspenseItemBase::DamageDurability(float Damage)
{
    SetCurrentDurability(CurrentDurability - FMath::Abs(Damage));
    return CurrentDurability;
}

float USuspenseItemBase::RepairDurability(float RepairAmount)
{
...
int32 USuspenseItemBase::GetCurrentAmmo() const
{
    return FMath::RoundToInt(GetRuntimeProperty(TEXT("Ammo"), 0.0f));
}

void USuspenseItemBase::SetCurrentAmmo(int32 AmmoCount)
{
...
int32 USuspenseItemBase::GetMaxAmmo() const
{
...
bool USuspenseItemBase::HasAmmo() const
{
...
bool USuspenseItemBase::IsOnCooldown(float CurrentTime) const
{
    float CooldownEndTime = GetRuntimeProperty(TEXT("CooldownEnd"), 0.0f);
    return CurrentTime < CooldownEndTime;
}

void USuspenseItemBase::StartCooldown(float CurrentTime, float CooldownDuration)
{
    SetRuntimeProperty(TEXT("CooldownEnd"), CurrentTime + CooldownDuration);
}

float USuspenseItemBase::GetRemainingCooldown(float CurrentTime) const
{
    float CooldownEndTime = GetRuntimeProperty(TEXT("CooldownEnd"), 0.0f);
...
void USuspenseItemBase::UseItem_Implementation(ACharacter* Character)
{
    if (!Character)
    {
...
bool USuspenseItemBase::IsValid() const
{
...
void USuspenseItemBase::ResetToDefaults()
{
    CurrentDurability = 0.0f;
    LastUsedTime = 0.0f;
...
void USuspenseItemBase::SyncPropertiesCacheWithArray() const
{
...
void USuspenseItemBase::SyncPropertiesArrayWithCache()
{
...
void USuspenseItemBase::OnRep_RuntimeProperties()
{
...
void USuspenseItemBase::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
    ...
    DOREPLIFETIME(USuspenseItemBase, RuntimePropertiesArray); // Теперь массив вместо TMap
    DOREPLIFETIME(USuspenseItemBase, LastUsedTime);
}
