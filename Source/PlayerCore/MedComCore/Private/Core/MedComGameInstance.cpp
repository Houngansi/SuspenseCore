...
DEFINE_LOG_CATEGORY_STATIC(LogMedComGameInstance, Log, All);

//========================================
// Construction
//========================================

UMedComGameInstance::UMedComGameInstance()
...
void UMedComGameInstance::Init()
{
    Super::Init();
    
    UE_LOG(LogMedComGameInstance, Log, TEXT("=== GAME INSTANCE INITIALIZATION START ==="));
    ...
    {
        UE_LOG(LogMedComGameInstance, Error, TEXT("Failed to initialize item system!"));
    ...
    {
        UE_LOG(LogMedComGameInstance, Error, TEXT("Failed to initialize loadout system!"));
    }
    
    // Initialize Animation system
    if (!InitializeAnimationSystem())
    {
        UE_LOG(LogMedComGameInstance, Error, TEXT("Failed to initialize animation system!"));
    }
    ...
    if (CoordinatorSubsystem)
    {
        ...
        {
            UE_LOG(LogMedComGameInstance, Log, TEXT("Equipment services already initialized"));
        }
        else
        {
            UE_LOG(LogMedComGameInstance, Warning, TEXT("Equipment services not ready yet - will initialize on world load"));
        }
    }
    else
    {
        UE_LOG(LogMedComGameInstance, Error, TEXT("SystemCoordinatorSubsystem not available!"));
    }
    
    // Register global event handlers
    RegisterGlobalEventHandlers();
    
    UE_LOG(LogMedComGameInstance, Log, TEXT("=== GAME INSTANCE INITIALIZATION COMPLETE ==="));
    UE_LOG(LogMedComGameInstance, Log, TEXT("Game Version: %s"), *CachedGameVersion);
    UE_LOG(LogMedComGameInstance, Log, TEXT("Network Mode: %s"), *GetNetworkMode());
}

void UMedComGameInstance::Shutdown()
{
    UE_LOG(LogMedComGameInstance, Log, TEXT("=== GAME INSTANCE SHUTDOWN START ==="));
    ...
    UE_LOG(LogMedComGameInstance, Log, TEXT("=== GAME INSTANCE SHUTDOWN COMPLETE ==="));
}

void UMedComGameInstance::OnStart()
{
    Super::OnStart();
    
    UE_LOG(LogMedComGameInstance, Log, TEXT("MedComGameInstance OnStart called"));
    
    // Double-check system readiness (fallback if Init() failed)
    if (!bItemSystemInitialized)
    {
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Item system not initialized on game start, attempting initialization..."));
        InitializeItemSystem();
    }
    
    if (!bLoadoutSystemInitialized)
    {
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Loadout system not initialized on game start, attempting initialization..."));
        InitializeLoadoutSystem();
    }
    
    if (!bAnimationSystemInitialized)
    {
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Animation system not initialized on game start, attempting initialization..."));
...
UMedComGameInstance* UMedComGameInstance::GetMedComGameInstance(const UObject* WorldContextObject)
{
    ...
    return Cast<UMedComGameInstance>(World->GetGameInstance());
}
...
bool UMedComGameInstance::IsOfflineMode() const
{
    UWorld* World = GetWorld();
    return !World || World->GetNetMode() == NM_Standalone;
}

FString UMedComGameInstance::GetNetworkMode() const
{
    UWorld* World = GetWorld();
    if (!World)
    {
...
FString UMedComGameInstance::GetGameVersion() const
{
...
UMedComLoadoutManager* UMedComGameInstance::GetLoadoutManager() const
{
    return GetSubsystem<UMedComLoadoutManager>();
}

UMedComWeaponAnimationSubsystem* UMedComGameInstance::GetWeaponAnimationSubsystem() const
{
    return GetSubsystem<UMedComWeaponAnimationSubsystem>();
}

UMedComItemManager* UMedComGameInstance::GetItemManager() const
{
...
bool UMedComGameInstance::InitializeItemSystem()
{
    UE_LOG(LogMedComGameInstance, Warning, TEXT("=== ITEM SYSTEM INITIALIZATION START ==="));
    
    if (bItemSystemInitialized)
    {
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Item system already initialized"));
        return true;
    }
    ...
    //========================================
    {
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Step 1: Getting ItemManager subsystem..."));
        UMedComItemManager* ItemManager = GetItemManager();
        if (!ItemManager)
        {
            UE_LOG(LogMedComGameInstance, Error, TEXT("ItemManager subsystem not found!"));
            return false;
        }
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Step 1: SUCCESS - ItemManager found at %p"), ItemManager);
    }
    ...
    //========================================
    {
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Step 2: Checking ItemDataTable..."));
        if (!ItemDataTable)
        {
            UE_LOG(LogMedComGameInstance, Error, TEXT("ItemDataTable not set in GameInstance!"));
            UE_LOG(LogMedComGameInstance, Error, TEXT("CRITICAL: Make sure BP_MedComGameInstance has ItemDataTable field set!"));
            UE_LOG(LogMedComGameInstance, Error, TEXT("ItemManager will attempt fallback to default path, but this is not recommended."));
        ...
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Step 2: SUCCESS - DataTable found: %s"), *ItemDataTable->GetName());
        UE_LOG(LogMedComGameInstance, Warning, TEXT("  - DataTable Path: %s"), *ItemDataTable->GetPathName());
    }
    ...
    //========================================
    {
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Step 3: Verifying DataTable structure..."));
        const UScriptStruct* RowStruct = ItemDataTable->GetRowStruct();
        if (!RowStruct)
        {
            UE_LOG(LogMedComGameInstance, Error, TEXT("ItemDataTable has no row structure!"));
            return false;
        }
        UE_LOG(LogMedComGameInstance, Warning, TEXT("  - Row Structure Name: %s"), *RowStruct->GetName());
        UE_LOG(LogMedComGameInstance, Warning, TEXT("  - Expected Structure: FMedComUnifiedItemData"));
        
        // Verify correct structure
        if (RowStruct != FMedComUnifiedItemData::StaticStruct())
        {
            UE_LOG(LogMedComGameInstance, Error, 
                TEXT("ItemDataTable has incorrect row structure! Expected: FMedComUnifiedItemData, Got: %s"), 
                *RowStruct->GetName());
            return false;
        }
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Step 3: SUCCESS - Row structure is correct"));
    }
    ...
    //========================================
    {
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Step 4: Examining DataTable contents..."));
        TArray<FName> RowNames = ItemDataTable->GetRowNames();
        UE_LOG(LogMedComGameInstance, Warning, TEXT("  - Total rows in DataTable: %d"), RowNames.Num());
        
        if (bLogItemOperations && RowNames.Num() > 0)
        {
            for (int32 i = 0; i < FMath::Min(RowNames.Num(), 10); i++)
            {
                UE_LOG(LogMedComGameInstance, Warning, TEXT("  - Row[%d]: %s"), i, *RowNames[i].ToString());
            }
            if (RowNames.Num() > 10)
            {
                UE_LOG(LogMedComGameInstance, Warning, TEXT("  - ... and %d more rows"), RowNames.Num() - 10);
            }
        }
    }
    ...
    //========================================
    {
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Step 5: Loading item data into ItemManager..."));
        UMedComItemManager* ItemManager = GetItemManager();
        ...
        if (!bLoadSuccess)
        {
            UE_LOG(LogMedComGameInstance, Error, TEXT("Failed to load ItemDataTable into ItemManager!"));
            UE_LOG(LogMedComGameInstance, Error, TEXT("Check ItemManager logs for specific errors"));
            return false;
        }
        
        int32 LoadedCount = ItemManager->GetCachedItemCount();
        UE_LOG(LogMedComGameInstance, Log, TEXT("Step 5: SUCCESS - Loaded %d items into cache"), LoadedCount);
        
        if (LoadedCount == 0)
        {
            UE_LOG(LogMedComGameInstance, Error, TEXT("No items were loaded from DataTable!"));
            UE_LOG(LogMedComGameInstance, Error, TEXT("This usually means the DataTable is empty or has structural issues"));
            return false;
        }
    }
    ...
    {
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Step 6: Validating item configurations..."));
        ...
        if (!bValidationPassed)
        {
            ...
            {
                UE_LOG(LogMedComGameInstance, Error, TEXT("CRITICAL: Item validation failed with strict mode enabled!"));
                UE_LOG(LogMedComGameInstance, Error, TEXT("Game cannot start with invalid critical items."));
                UE_LOG(LogMedComGameInstance, Error, TEXT("Please fix the validation errors in the DataTable."));
            ...
            {
                UE_LOG(LogMedComGameInstance, Warning, TEXT("Item validation found issues, but strict mode is disabled"));
                UE_LOG(LogMedComGameInstance, Warning, TEXT("Game will continue, but some items may not work correctly"));
            }
        }
        else
        {
            UE_LOG(LogMedComGameInstance, Log, TEXT("Step 6: SUCCESS - All items validated successfully"));
        }
    }
    ...
    {
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Step 7: Final item system summary:"));
        UMedComItemManager* ItemManager = GetItemManager();
        
        TArray<FName> AllItems = ItemManager->GetAllItemIDs();
        UE_LOG(LogMedComGameInstance, Log, TEXT("Total items available: %d"), AllItems.Num());
        ...
        UE_LOG(LogMedComGameInstance, Log, TEXT("  - Weapons: %d"), WeaponCount);
        UE_LOG(LogMedComGameInstance, Log, TEXT("  - Armor: %d"), ArmorCount);
        UE_LOG(LogMedComGameInstance, Log, TEXT("  - Ammunition: %d"), AmmoCount);
        UE_LOG(LogMedComGameInstance, Log, TEXT("  - Consumables: %d"), ConsumableCount);
    }
    
    bItemSystemInitialized = true;
    UE_LOG(LogMedComGameInstance, Warning, TEXT("=== ITEM SYSTEM INITIALIZATION COMPLETE ==="));
    return true;
}

bool UMedComGameInstance::ValidateItemConfigurations()
{
    UE_LOG(LogMedComGameInstance, Warning, TEXT("=== ITEM VALIDATION START ==="));
    
    UMedComItemManager* ItemManager = GetItemManager();
    if (!ItemManager)
    {
        UE_LOG(LogMedComGameInstance, Error, TEXT("ItemManager not available for validation"));
        return false;
    }
    ...
    if (!LoadoutManager)
    {
        UE_LOG(LogMedComGameInstance, Warning, TEXT("LoadoutManager not available for critical item validation"));
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Will perform basic item validation only"));
        ...
        if (ErrorCount > 0)
        {
            UE_LOG(LogMedComGameInstance, Warning, TEXT("Basic validation found %d items with errors"), ErrorCount);
            for (const FString& Error : ValidationErrors)
            {
                UE_LOG(LogMedComGameInstance, Warning, TEXT("  %s"), *Error);
            }
        }
    ...
    //========================================
    UE_LOG(LogMedComGameInstance, Log, TEXT("Phase 1: Validating all items in DataTable..."));
    ...
    if (ErrorCount > 0)
    {
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Phase 1: Found %d items with validation errors:"), ErrorCount);
    ...
    {
        UE_LOG(LogMedComGameInstance, Log, TEXT("Phase 1: All items passed basic validation"));
    }
    
    //========================================
    // Phase 2: Critical Items Validation
    //========================================
    UE_LOG(LogMedComGameInstance, Warning, TEXT("Phase 2: Validating critical items referenced in loadouts..."));
    ...
    {
        UE_LOG(LogMedComGameInstance, Error, TEXT("=== CRITICAL ITEM VALIDATION FAILURES ==="));
        UE_LOG(LogMedComGameInstance, Error, TEXT("The following items are used in loadouts but have critical errors:"));
        UE_LOG(LogMedComGameInstance, Error, TEXT(""));
        ...
        for (const FString& CriticalError : CriticalErrors)
        {
            ...
            {
                UE_LOG(LogMedComGameInstance, Error, TEXT("%s"), *Line);
            }
            UE_LOG(LogMedComGameInstance, Error, TEXT(""));
        }
        
        UE_LOG(LogMedComGameInstance, Error, TEXT("=== END CRITICAL VALIDATION FAILURES ==="));
        UE_LOG(LogMedComGameInstance, Error, TEXT(""));
        UE_LOG(LogMedComGameInstance, Error, TEXT("GAME CANNOT START WITH INVALID CRITICAL ITEMS"));
        UE_LOG(LogMedComGameInstance, Error, TEXT("Please fix the above errors in your item DataTable"));
        UE_LOG(LogMedComGameInstance, Error, TEXT(""));
        
        if (bStrictItemValidation)
        {
            UE_LOG(LogMedComGameInstance, Error, TEXT("Strict validation mode is ENABLED - blocking startup"));
            return false;
        }
        else
        {
            UE_LOG(LogMedComGameInstance, Warning, TEXT("Strict validation mode is DISABLED - continuing with warnings"));
            UE_LOG(LogMedComGameInstance, Warning, TEXT("Loadouts using invalid items will NOT function correctly!"));
        }
    }
    else
    {
        UE_LOG(LogMedComGameInstance, Log, TEXT("Phase 2: All critical items validated successfully"));
    }
    
    //========================================
    // Phase 3: Summary
    //========================================
    UE_LOG(LogMedComGameInstance, Warning, TEXT("=== ITEM VALIDATION SUMMARY ==="));
    UE_LOG(LogMedComGameInstance, Warning, TEXT("Total Items in DataTable: %d"), ItemManager->GetCachedItemCount());
    UE_LOG(LogMedComGameInstance, Warning, TEXT("Valid Items: %d"), ItemManager->GetValidItemCount());
    UE_LOG(LogMedComGameInstance, Warning, TEXT("Items with Errors: %d"), ErrorCount);
    UE_LOG(LogMedComGameInstance, Warning, TEXT("Critical Items Validated: %d"), CriticalErrors.Num() == 0 ? TEXT("All passed") : *FString::Printf(TEXT("%d failed"), CriticalErrors.Num()));
    UE_LOG(LogMedComGameInstance, Warning, TEXT("================================"));
    ...
    UE_LOG(LogMedComGameInstance, Warning, TEXT("=== ITEM VALIDATION %s ==="), 
        bOverallSuccess ? TEXT("COMPLETE") : TEXT("FAILED"));
    
    return bOverallSuccess;
}

bool UMedComGameInstance::ValidateCriticalItems(
    ...
    TArray<FString>& OutCriticalErrors)
{
    ...
    //========================================
    UE_LOG(LogMedComGameInstance, Log, TEXT("ValidateCriticalItems: Collecting items from all loadouts..."));
    ...
    UE_LOG(LogMedComGameInstance, Log, TEXT("ValidateCriticalItems: Found %d unique critical items"), CriticalItems.Num());
    
    if (CriticalItems.Num() == 0)
    {
        UE_LOG(LogMedComGameInstance, Warning, TEXT("ValidateCriticalItems: No items found in any loadout"));
    ...
    {
        UE_LOG(LogMedComGameInstance, Log, 
...
void UMedComGameInstance::BuildCriticalItemErrorReport(
    const FName& ItemID,
...
bool UMedComGameInstance::InitializeLoadoutSystem()
{
    UE_LOG(LogMedComGameInstance, Warning, TEXT("=== LOADOUT SYSTEM INITIALIZATION START ==="));
    
    if (bLoadoutSystemInitialized)
    {
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Loadout system already initialized"));
        return true;
    }
    ...
    //========================================
    {
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Step 1: Getting LoadoutManager subsystem..."));
        UMedComLoadoutManager* LoadoutManager = GetLoadoutManager();
        if (!LoadoutManager)
        {
            UE_LOG(LogMedComGameInstance, Error, TEXT("LoadoutManager subsystem not found!"));
            return false;
        }
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Step 1: SUCCESS - LoadoutManager found at %p"), LoadoutManager);
    }
    ...
    //========================================
    {
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Step 2: Checking LoadoutConfigurationsTable..."));
        if (!LoadoutConfigurationsTable)
        {
            UE_LOG(LogMedComGameInstance, Error, TEXT("LoadoutConfigurationsTable not set in GameInstance!"));
            UE_LOG(LogMedComGameInstance, Error, TEXT("CRITICAL: Make sure BP_MedComGameInstance has LoadoutConfigurationsTable field set!"));
            return false;
        }
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Step 2: SUCCESS - DataTable found: %s"), *LoadoutConfigurationsTable->GetName());
        UE_LOG(LogMedComGameInstance, Warning, TEXT("  - DataTable Path: %s"), *LoadoutConfigurationsTable->GetPathName());
    }
    ...
    //========================================
    {
        ...
        if (!RowStruct)
        {
            UE_LOG(LogMedComGameInstance, Error, TEXT("LoadoutConfigurationsTable has no row structure!"));
            return false;
        }
        UE_LOG(LogMedComGameInstance, Warning, TEXT("  - Row Structure Name: %s"), *RowStruct->GetName());
        UE_LOG(LogMedComGameInstance, Warning, TEXT("  - Expected Structure: FLoadoutConfiguration"));
    ...
    //========================================
    {
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Step 5: Loading configurations into LoadoutManager..."));
        UMedComLoadoutManager* LoadoutManager = GetLoadoutManager();
        
        int32 LoadedCount = LoadoutManager->LoadLoadoutTable(LoadoutConfigurationsTable);
        UE_LOG(LogMedComGameInstance, Warning, TEXT("  - LoadLoadoutTable returned: %d configurations"), LoadedCount);
        
        if (LoadedCount == 0)
        {
            UE_LOG(LogMedComGameInstance, Error, TEXT("No loadout configurations were loaded from DataTable!"));
            UE_LOG(LogMedComGameInstance, Error, TEXT("Check LoadoutManager->LoadLoadoutTable implementation"));
            return false;
        }
        
        UE_LOG(LogMedComGameInstance, Log, TEXT("Step 5: SUCCESS - Loaded %d loadout configurations"), LoadedCount);
    }
    ...
    {
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Step 6: Validating loadout configurations..."));
    ...
    //========================================
    {
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Step 7: Checking default loadout..."));
        UE_LOG(LogMedComGameInstance, Warning, TEXT("  - DefaultLoadoutID: %s"), *DefaultLoadoutID.ToString());
        
        UMedComLoadoutManager* LoadoutManager = GetLoadoutManager();
        
        if (!LoadoutManager->IsLoadoutValid(DefaultLoadoutID))
        {
            UE_LOG(LogMedComGameInstance, Warning, TEXT("  - Default loadout '%s' not found in LoadoutManager"), *DefaultLoadoutID.ToString());
            
            TArray<FName> AllLoadouts = LoadoutManager->GetAllLoadoutIDs();
            UE_LOG(LogMedComGameInstance, Warning, TEXT("  - Available loadouts in manager: %d"), AllLoadouts.Num());
            
            if (AllLoadouts.Num() > 0)
            {
                for (int32 i = 0; i < AllLoadouts.Num(); i++)
                {
                    UE_LOG(LogMedComGameInstance, Warning, TEXT("    [%d]: %s"), i, *AllLoadouts[i].ToString());
                }
                
                UE_LOG(LogMedComGameInstance, Warning, TEXT("  - Using first available loadout: '%s'"), 
                    *AllLoadouts[0].ToString());
                DefaultLoadoutID = AllLoadouts[0];
            }
            else
            {
                UE_LOG(LogMedComGameInstance, Error, TEXT("No valid loadouts available in LoadoutManager!"));
                return false;
            }
        }
        else
        {
            UE_LOG(LogMedComGameInstance, Warning, TEXT("Step 7: SUCCESS - Default loadout '%s' is valid"), *DefaultLoadoutID.ToString());
        }
    }
    ...
    {
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Step 8: Final loadout summary:"));
        UMedComLoadoutManager* LoadoutManager = GetLoadoutManager();
        TArray<FName> AllLoadouts = LoadoutManager->GetAllLoadoutIDs();
        UE_LOG(LogMedComGameInstance, Log, TEXT("Available loadouts (%d total):"), AllLoadouts.Num());
        for (const FName& LoadoutID : AllLoadouts)
        {
            UE_LOG(LogMedComGameInstance, Log, TEXT("  - %s"), *LoadoutID.ToString());
        }
    }
    
    bLoadoutSystemInitialized = true;
    UE_LOG(LogMedComGameInstance, Warning, TEXT("=== LOADOUT SYSTEM INITIALIZATION COMPLETE ==="));
    return true;
}

void UMedComGameInstance::ValidateLoadoutConfigurations()
{
    UMedComLoadoutManager* LoadoutManager = GetLoadoutManager();
    if (!LoadoutManager)
    {
    ...
    if (!bAllValid)
    {
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Loadout validation found %d issues:"), ValidationErrors.Num());
        for (const FString& Error : ValidationErrors)
        {
            UE_LOG(LogMedComGameInstance, Warning, TEXT("  - %s"), *Error);
        }
    ...
    {
        UE_LOG(LogMedComGameInstance, Log, TEXT("All loadout configurations validated successfully"));
    }
}
...
bool UMedComGameInstance::InitializeAnimationSystem()
{
    UE_LOG(LogMedComGameInstance, Log, TEXT("=== ANIMATION SYSTEM INITIALIZATION START ==="));
    
    if (bAnimationSystemInitialized)
    {
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Animation system already initialized"));
        return true;
    }
    ...
    //========================================
    {
        ...
        {
            UE_LOG(LogMedComGameInstance, Error, TEXT("WeaponAnimationSubsystem not found!"));
            return false;
        }
    }
    ...
    //========================================
    {
        ...
        {
            UE_LOG(LogMedComGameInstance, Error, TEXT("WeaponAnimationsTable not set in GameInstance!"));
            UE_LOG(LogMedComGameInstance, Error, TEXT("Please configure it in BP_MedComGameInstance"));
            
            // Attempt to load default
            WeaponAnimationsTable = LoadObject<UDataTable>(nullptr, TEXT("/Game/Data/DT_Anims"));
            if (WeaponAnimationsTable)
            {
                UE_LOG(LogMedComGameInstance, Warning, TEXT("Loaded DT_Anims from default path"));
            }
            else
            {
                return false;
    ...
    //========================================
    {
        ...
        if (!RowStruct)
        {
            UE_LOG(LogMedComGameInstance, Error, TEXT("WeaponAnimationsTable has no row structure!"));
            return false;
        }
    ...
    //========================================
    {
        ...
        {
            UE_LOG(LogMedComGameInstance, Error, TEXT("Failed to load animation DataTable into subsystem"));
            return false;
        }
        
        // Get loaded weapon types
        TArray<FGameplayTag> LoadedWeaponTypes = AnimationSubsystem->GetAvailableWeaponTypes_Implementation();
        UE_LOG(LogMedComGameInstance, Log, TEXT("Loaded %d weapon animation configurations:"), LoadedWeaponTypes.Num());
        
        if (bLogAnimationOperations)
        {
            for (const FGameplayTag& WeaponType : LoadedWeaponTypes)
            {
                UE_LOG(LogMedComGameInstance, Log, TEXT("  - %s"), *WeaponType.ToString());
            }
        }
    }
    ...
    UE_LOG(LogMedComGameInstance, Log, TEXT("=== ANIMATION SYSTEM INITIALIZATION COMPLETE ==="));
    return true;
}

void UMedComGameInstance::ValidateAnimationConfigurations()
{
    UMedComWeaponAnimationSubsystem* AnimationSubsystem = GetWeaponAnimationSubsystem();
    if (!AnimationSubsystem)
    {
    ...
    for (const FGameplayTag& WeaponType : WeaponTypes)
    {
        ...
        {
            ErrorCount++;
            UE_LOG(LogMedComGameInstance, Warning, TEXT("Validation errors for %s:"), *WeaponType.ToString());
            for (const FString& Error : ValidationErrors)
            {
                UE_LOG(LogMedComGameInstance, Warning, TEXT("  - %s"), *Error);
            }
    ...
    if (ErrorCount == 0)
    {
        UE_LOG(LogMedComGameInstance, Log, TEXT("All animation configurations validated successfully"));
    }
    else
    {
        UE_LOG(LogMedComGameInstance, Warning, TEXT("Found validation issues in %d weapon configurations"), ErrorCount);
    }
}
...
void UMedComGameInstance::RegisterGlobalEventHandlers()
{
    if (GEngine)
    {
        // Register network error handlers
        GEngine->OnNetworkFailure().AddUObject(this, &UMedComGameInstance::HandleNetworkError);
        
        // Register travel error handlers
        GEngine->OnTravelFailure().AddUObject(this, &UMedComGameInstance::HandleTravelFailure);
        
        UE_LOG(LogMedComGameInstance, Verbose, TEXT("Global event handlers registered"));
    }
}

void UMedComGameInstance::UnregisterGlobalEventHandlers()
{
    if (GEngine)
    {
        ...
        UE_LOG(LogMedComGameInstance, Verbose, TEXT("Global event handlers unregistered"));
    }
}

void UMedComGameInstance::HandleSystemMessage(const FString& Message, float Duration)
{
    if (bIsShuttingDown)
    {
        return;
    ...
    UE_LOG(LogMedComGameInstance, Warning, TEXT("System Message: %s"), *Message);
...
void UMedComGameInstance::HandleNetworkError(UWorld* World, UNetDriver* NetDriver, ENetworkFailure::Type FailureType, const FString& ErrorString)
{
    ...
    UE_LOG(LogMedComGameInstance, Error, TEXT("%s"), *ErrorMessage);
    
    HandleSystemMessage(ErrorMessage, 10.0f);
}

void UMedComGameInstance::HandleTravelFailure(UWorld* World, ETravelFailure::Type FailureType, const FString& ErrorString)
{
    if (bIsShuttingDown || !World)
    {
        return;
...
void UMedComGameInstance::CacheGameVersion()
{
