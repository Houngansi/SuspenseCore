...
/* ===== Constructor & Basic ===== */
AMedComPlayerController::AMedComPlayerController()
{ 
...
void AMedComPlayerController::BeginPlay()
{
    Super::BeginPlay();
    ...
    if (UEventDelegateManager* Manager = GetDelegateManager())
    {
        ...
        InventoryDelegate.BindUObject(this, &AMedComPlayerController::OnInventoryInitialized);
        ...
        EquipmentDelegate.BindUObject(this, &AMedComPlayerController::HandleEquipmentInitializationRequest);
        ...
        LoadoutReadyDelegate.BindUObject(this, &AMedComPlayerController::OnLoadoutReady);
        ...
        LoadoutFailedDelegate.BindUObject(this, &AMedComPlayerController::OnLoadoutFailed);
...
void AMedComPlayerController::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void AMedComPlayerController::OnInventoryInitialized(const UObject* Source, const FGameplayTag& EventTag, const FString& EventData)
{
    ...
    AMedComPlayerState* PS = GetPlayerState<AMedComPlayerState>();
    if (!PS || PS != Source)
    {
...
void AMedComPlayerController::OnLoadoutReady(const UObject* Source, const FGameplayTag& EventTag, const FString& EventData)
{
...
void AMedComPlayerController::OnLoadoutFailed(const UObject* Source, const FGameplayTag& EventTag, const FString& EventData)
{
...
void AMedComPlayerController::SetupInputComponent()
{
    Super::SetupInputComponent();
    
    if(UEnhancedInputComponent* EIC = Cast<UEnhancedInputComponent>(InputComponent))
    {
        if(IA_Move)   EIC->BindAction(IA_Move, ETriggerEvent::Triggered, this, &AMedComPlayerController::HandleMove);
        if(IA_Look)   EIC->BindAction(IA_Look, ETriggerEvent::Triggered, this, &AMedComPlayerController::HandleLook);
        
        if(IA_Jump)   {
            EIC->BindAction(IA_Jump, ETriggerEvent::Started, this, &AMedComPlayerController::OnJumpPressed);
            EIC->BindAction(IA_Jump, ETriggerEvent::Completed, this, &AMedComPlayerController::OnJumpReleased);
        }
        
        if(IA_Sprint) {
            EIC->BindAction(IA_Sprint, ETriggerEvent::Started, this, &AMedComPlayerController::OnSprintPressed);
            EIC->BindAction(IA_Sprint, ETriggerEvent::Completed, this, &AMedComPlayerController::OnSprintReleased);
        }
        
        if(IA_Crouch) {
            EIC->BindAction(IA_Crouch, ETriggerEvent::Started, this, &AMedComPlayerController::OnCrouchPressed);
            EIC->BindAction(IA_Crouch, ETriggerEvent::Completed, this, &AMedComPlayerController::OnCrouchReleased);
        }
        
        if(IA_Interact) {
            EIC->BindAction(IA_Interact, ETriggerEvent::Started, this, &AMedComPlayerController::OnInteractPressed);
        }
        
        if(IA_OpenInventory) {
            EIC->BindAction(IA_OpenInventory, ETriggerEvent::Started, this, &AMedComPlayerController::OnInventoryToggle);
        }

        if(IA_NextWeapon) {
            EIC->BindAction(IA_NextWeapon, ETriggerEvent::Started, this, &AMedComPlayerController::OnNextWeapon);
        }
        
        if(IA_PrevWeapon) {
            EIC->BindAction(IA_PrevWeapon, ETriggerEvent::Started, this, &AMedComPlayerController::OnPrevWeapon);
        }
        
        if(IA_QuickSwitch) {
            EIC->BindAction(IA_QuickSwitch, ETriggerEvent::Started, this, &AMedComPlayerController::OnQuickSwitch);
        }
        
        if(IA_WeaponSlot1) {
            EIC->BindAction(IA_WeaponSlot1, ETriggerEvent::Started, this, &AMedComPlayerController::OnWeaponSlot1);
        }
        if(IA_WeaponSlot2) {
            EIC->BindAction(IA_WeaponSlot2, ETriggerEvent::Started, this, &AMedComPlayerController::OnWeaponSlot2);
        }
        if(IA_WeaponSlot3) {
            EIC->BindAction(IA_WeaponSlot3, ETriggerEvent::Started, this, &AMedComPlayerController::OnWeaponSlot3);
        }
        if(IA_WeaponSlot4) {
            EIC->BindAction(IA_WeaponSlot4, ETriggerEvent::Started, this, &AMedComPlayerController::OnWeaponSlot4);
        }
        if(IA_WeaponSlot5) {
            EIC->BindAction(IA_WeaponSlot5, ETriggerEvent::Started, this, &AMedComPlayerController::OnWeaponSlot5);
        }
    }
}

void AMedComPlayerController::OnPossess(APawn* InPawn)
{
    Super::OnPossess(InPawn);
    
    // Initialize ASC
    if(AMedComPlayerState* PS = GetPlayerState<AMedComPlayerState>())
    {
    ...
    if (bAutoCreateHUD && IsLocalController())
    {
        if (GetWorld())
        {
            GetWorld()->GetTimerManager().SetTimer(
                ...
                this,
                &AMedComPlayerController::TryCreateHUD,
                HUDCreationDelay,
                false
            );
        }
...
void AMedComPlayerController::OnUnPossess()
{
    DestroyHUD();
    Super::OnUnPossess();
}

void AMedComPlayerController::OnRep_PlayerState()
{
...
void AMedComPlayerController::CreateHUD()
{
...
void AMedComPlayerController::DestroyHUD()
{
...
UUserWidget* AMedComPlayerController::GetHUDWidget() const
{
    if (UMedComUIManager* UIManager = GetUIManager())
    {
        FGameplayTag HUDTag = FGameplayTag::RequestGameplayTag("UI.HUD.Main");
...
void AMedComPlayerController::SetHUDVisibility(bool bShow)
{
    if (UMedComUIManager* UIManager = GetUIManager())
    {
        FGameplayTag HUDTag = FGameplayTag::RequestGameplayTag("UI.HUD.Main");
...
bool AMedComPlayerController::IsHUDCreated() const
{
    if (UMedComUIManager* UIManager = GetUIManager())
    {
        FGameplayTag HUDTag = FGameplayTag::RequestGameplayTag("UI.HUD.Main");
...
void AMedComPlayerController::ShowInGameMenu()
{
    if (UMedComUIManager* UIManager = GetUIManager())
    {
...
void AMedComPlayerController::HideInGameMenu()
{
    if (UMedComUIManager* UIManager = GetUIManager())
    {
        FGameplayTag MenuTag = FGameplayTag::RequestGameplayTag("UI.Menu.Pause");
...
void AMedComPlayerController::ToggleInventory()
{
...
void AMedComPlayerController::TryCreateHUD()
{
...
void AMedComPlayerController::UpdateHUDData()
{
    if (UMedComUIManager* UIManager = GetUIManager())
    {
...
void AMedComPlayerController::HandleAttributeChanged(const FGameplayTag& AttributeTag, float NewValue, float OldValue)
{
    UE_LOG(LogTemp, Verbose, TEXT("[PlayerController] Attribute changed: %s (%.2f -> %.2f)"), 
        *AttributeTag.ToString(), OldValue, NewValue);
}

UMedComUIManager* AMedComPlayerController::GetUIManager() const
{
    if (CachedUIManager)
    {
...
/* ===== Enhanced Input ===== */
void AMedComPlayerController::SetupEnhancedInput()
{
...
/* ===== Movement ===== */
void AMedComPlayerController::HandleMove(const FInputActionValue& V)
{
    if(AMedComBaseCharacter* C = GetMedComCharacter())
    {
        C->Move(V.Get<FVector2D>());
    }
}

void AMedComPlayerController::HandleLook(const FInputActionValue& V)
{
    if(AMedComBaseCharacter* C = GetMedComCharacter())
    {
...
void AMedComPlayerController::OnJumpPressed(const FInputActionValue&) 
{ 
    ActivateAbility(FIRE_TAG("Jump"), true); 
}

void AMedComPlayerController::OnJumpReleased(const FInputActionValue&) 
{ 
    ActivateAbility(FIRE_TAG("Jump"), false); 
}

void AMedComPlayerController::OnSprintPressed(const FInputActionValue&) 
{ 
    ActivateAbility(FIRE_TAG("Sprint"), true); 
}

void AMedComPlayerController::OnSprintReleased(const FInputActionValue&) 
{ 
    ActivateAbility(FIRE_TAG("Sprint"), false); 
}

void AMedComPlayerController::OnCrouchPressed(const FInputActionValue&) 
{ 
    ActivateAbility(FIRE_TAG("Crouch"), true); 
}

void AMedComPlayerController::OnCrouchReleased(const FInputActionValue&) 
{ 
    ActivateAbility(FIRE_TAG("Crouch"), false); 
}

void AMedComPlayerController::OnInteractPressed(const FInputActionValue&) 
{ 
    ActivateAbility(FIRE_TAG("Interact"), true); 
}

void AMedComPlayerController::OnInventoryToggle(const FInputActionValue& Value)
{
    ToggleInventory();
}

void AMedComPlayerController::OnNextWeapon(const FInputActionValue&)
{
    ActivateAbility(FIRE_TAG("NextWeapon"), true);
}

void AMedComPlayerController::OnPrevWeapon(const FInputActionValue&)
{
    ActivateAbility(FIRE_TAG("PrevWeapon"), true);
}

void AMedComPlayerController::OnQuickSwitch(const FInputActionValue&)
{
    ActivateAbility(FIRE_TAG("QuickSwitch"), true);
}

void AMedComPlayerController::OnWeaponSlot1(const FInputActionValue&)
{
    ActivateAbility(FIRE_TAG("WeaponSlot1"), true);
}

void AMedComPlayerController::OnWeaponSlot2(const FInputActionValue&)
{
    ActivateAbility(FIRE_TAG("WeaponSlot2"), true);
}

void AMedComPlayerController::OnWeaponSlot3(const FInputActionValue&)
{
    ActivateAbility(FIRE_TAG("WeaponSlot3"), true);
}

void AMedComPlayerController::OnWeaponSlot4(const FInputActionValue&)
{
    ActivateAbility(FIRE_TAG("WeaponSlot4"), true);
}

void AMedComPlayerController::OnWeaponSlot5(const FInputActionValue&)
{
...
/* ===== GAS ===== */
void AMedComPlayerController::ActivateAbility(const FGameplayTag& Tag, bool bPressed)
{
    if(!Tag.IsValid()) 
    {
...
AMedComBaseCharacter* AMedComPlayerController::GetMedComCharacter() const
{
    return Cast<AMedComBaseCharacter>(GetPawn());
}

UAbilitySystemComponent* AMedComPlayerController::GetCharacterASC() const
{
    if(AMedComBaseCharacter* MedComCharacter = GetMedComCharacter())
    {
...
void AMedComPlayerController::HandleEquipmentStateChange(FGameplayTag OldState, FGameplayTag NewState, bool bInterrupted)
{
...
void AMedComPlayerController::NotifyWeaponChanged_Implementation(AActor* NewWeapon)
{
    CurrentWeapon = NewWeapon;
    
    if (AMedComBaseCharacter* CharacterPawn = GetMedComCharacter())
    {
...
AActor* AMedComPlayerController::GetCurrentWeapon_Implementation() const
{
    return CurrentWeapon;
}

void AMedComPlayerController::NotifyWeaponStateChanged_Implementation(FGameplayTag WeaponState)
{
...
APawn* AMedComPlayerController::GetControlledPawn_Implementation() const
{
    return GetPawn();
}

bool AMedComPlayerController::CanUseWeapon_Implementation() const
{
    return CurrentWeapon != nullptr;
}

bool AMedComPlayerController::HasValidPawn_Implementation() const
{
    return GetPawn() != nullptr && IsValid(GetPawn());
}

void AMedComPlayerController::UpdateInputBindings_Implementation()
{
    SetupEnhancedInput();
}

int32 AMedComPlayerController::GetInputPriority_Implementation() const
{
    return 0;
}

UEventDelegateManager* AMedComPlayerController::GetDelegateManager() const
{
...
void AMedComPlayerController::EnsureInventoryBridgeInitialized()
{
...
void AMedComPlayerController::ConnectInventoryToBridge(UMedComInventoryUIBridge* Bridge)
{
    if (!Bridge)
    {
...
void AMedComPlayerController::HandleInventoryInitializationRequest(const UObject* Source, const FGameplayTag& EventTag, const FString& EventData)
{
...
void AMedComPlayerController::EnsureEquipmentBridgeInitialized()
{
    if (!IsLocalController())
    {
        return;
...
void AMedComPlayerController::ConnectEquipmentToBridge(UMedComEquipmentUIBridge* Bridge)
{
    if (!Bridge)
    {
...
void AMedComPlayerController::HandleEquipmentInitializationRequest(const UObject* Source, const FGameplayTag& EventTag, const FString& EventData)
{
    UE_LOG(LogTemp, Log, TEXT("[PlayerController] Received equipment initialization request from: %s"), 
        Source ? *Source->GetName() : TEXT("Unknown"));
}

void AMedComPlayerController::ShowCharacterScreen(const FGameplayTag& DefaultTab)
{
...
void AMedComPlayerController::RegisterDebugCommands()
{
