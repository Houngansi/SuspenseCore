// MedComPlayerState.cpp
// Copyright MedCom Team. All Rights Reserved.

#include "Core/MedComPlayerState.h"
#include "Core/MedComGameInstance.h"
...
DEFINE_LOG_CATEGORY_STATIC(LogMedComPlayerState, Log, All);

AMedComPlayerState::AMedComPlayerState()
{
    // ASC
...
void AMedComPlayerState::BeginPlay()
{
    Super::BeginPlay();
    ...
    if (HasAuthority())
    {
        UMedComGameInstance* GameInstance = Cast<UMedComGameInstance>(GetGameInstance());
        if (!GameInstance)
        {
            UE_LOG(LogMedComPlayerState, Error, TEXT("Failed to get MedComGameInstance"));
            return;
        }
        ...
        if (!LoadoutManager)
        {
            UE_LOG(LogMedComPlayerState, Error, TEXT("LoadoutManager not available from GameInstance"));
            return;
        }
        ...
        {
            UE_LOG(LogMedComPlayerState, Error, TEXT("No loadouts available in LoadoutManager!"));
            return;
        }
        ...
        {
            UE_LOG(LogMedComPlayerState, Warning, TEXT("Loadout '%s' not valid, using first available"), 
                *LoadoutToApply.ToString());
            LoadoutToApply = AvailableLoadouts[0];
        }

        UE_LOG(LogMedComPlayerState, Log, TEXT("Applying loadout '%s' to player %s"), 
        ...
        if (Result.bSuccess)
        {
            ...
            UE_LOG(LogMedComPlayerState, Warning, TEXT("=== LOADOUT APPLIED SUCCESSFULLY ==="));
            UE_LOG(LogMedComPlayerState, Warning, TEXT("Loadout: %s"), *LoadoutToApply.ToString());
            UE_LOG(LogMedComPlayerState, Warning, TEXT("Now verifying equipment components before wiring..."));
            
            UE_LOG(LogMedComPlayerState, Warning, TEXT("Component Verification:"));
            UE_LOG(LogMedComPlayerState, Warning, TEXT("  - EquipmentDataStore: %s"), 
                EquipmentDataStore ? TEXT("✓ OK") : TEXT("✗ NULL"));
            UE_LOG(LogMedComPlayerState, Warning, TEXT("  - EquipmentTxnProcessor: %s"), 
                EquipmentTxnProcessor ? TEXT("✓ OK") : TEXT("✗ NULL"));
            UE_LOG(LogMedComPlayerState, Warning, TEXT("  - EquipmentOps: %s"), 
                EquipmentOps ? TEXT("✓ OK") : TEXT("✗ NULL"));
            UE_LOG(LogMedComPlayerState, Warning, TEXT("  - EquipmentInventoryBridge: %s"), 
                EquipmentInventoryBridge ? TEXT("✓ OK") : TEXT("✗ NULL"));
            UE_LOG(LogMedComPlayerState, Warning, TEXT("  - InventoryComponent: %s"), 
            ...
            if (EquipmentDataStore)
            {
                ...
                UE_LOG(LogMedComPlayerState, Warning, TEXT("  - DataStore Slots: %d"), SlotCount);
                
                if (SlotCount == 0)
                {
                    UE_LOG(LogMedComPlayerState, Warning, 
                        TEXT("  ⚠ WARNING: DataStore has 0 slots! Loadout may not have applied correctly."));
                }
            }
            
            UE_LOG(LogMedComPlayerState, Warning, TEXT("Starting Equipment Module wiring with retry mechanism..."));
            ...
            auto RetryLambda = [this, LoadoutToApply]()
            {
                ...
                if (EquipmentWireRetryCount >= MaxEquipmentWireRetries)
                {
                    ...
                    UE_LOG(LogMedComPlayerState, Error,
                ...
                {
                    UE_LOG(LogMedComPlayerState, Verbose, 
            ...
            if (GetWorld()->GetTimerManager().IsTimerActive(EquipmentWireRetryHandle) == false)
            {
                ...
                UE_LOG(LogMedComPlayerState, Log, 
        ...
        else
        {
            ...
            {
                UE_LOG(LogMedComPlayerState, Error, TEXT("  - %s"), *Error);
            }
...
void AMedComPlayerState::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void AMedComPlayerState::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
    Super::GetLifetimeReplicatedProps(OutLifetimeProps);
    
    DOREPLIFETIME(AMedComPlayerState, ASC);
    DOREPLIFETIME(AMedComPlayerState, InventoryComponent);
    DOREPLIFETIME(AMedComPlayerState, CurrentLoadoutID);
    DOREPLIFETIME(AMedComPlayerState, EquipmentDataStore);
    DOREPLIFETIME(AMedComPlayerState, EquipmentTxnProcessor);
    DOREPLIFETIME(AMedComPlayerState, EquipmentOps);
    DOREPLIFETIME(AMedComPlayerState, EquipmentReplication);
    DOREPLIFETIME(AMedComPlayerState, EquipmentPrediction);
    DOREPLIFETIME(AMedComPlayerState, EquipmentNetworkDispatcher);
    DOREPLIFETIME(AMedComPlayerState, EquipmentEventDispatcher);
    DOREPLIFETIME(AMedComPlayerState, WeaponStateManager);
    DOREPLIFETIME(AMedComPlayerState, EquipmentInventoryBridge);
    DOREPLIFETIME(AMedComPlayerState, EquipmentSystemCoordinator);
}

bool AMedComPlayerState::ReplicateSubobjects(UActorChannel* Channel, FOutBunch* Bunch, FReplicationFlags* RepFlags)
{
...
UAbilitySystemComponent* AMedComPlayerState::GetAbilitySystemComponent() const
{
    return ASC;
}

UAbilitySystemComponent* AMedComPlayerState::GetASC_Implementation() const
{
    return ASC;
}

void AMedComPlayerState::SetHasWeapon_Implementation(bool bNewHasWeapon)
{
...
void AMedComPlayerState::SetCurrentWeaponActor_Implementation(AActor* WeaponActor)
{
...
AActor* AMedComPlayerState::GetCurrentWeaponActor_Implementation() const
{
    return CurrentWeaponActor;
}

bool AMedComPlayerState::HasWeapon_Implementation() const
{
    return bHasWeapon && CurrentWeaponActor != nullptr;
}

float AMedComPlayerState::GetCharacterLevel_Implementation() const
{
    if (Attributes)
    {
...
bool AMedComPlayerState::IsAlive_Implementation() const
{
    if (Attributes)
    {
...
int32 AMedComPlayerState::GetTeamId_Implementation() const
{
    return 0; // TODO: implement proper team id
}

UEventDelegateManager* AMedComPlayerState::GetDelegateManager() const
{
...
FLoadoutApplicationResult AMedComPlayerState::ApplyLoadoutConfiguration_Implementation(
...
FName AMedComPlayerState::GetCurrentLoadoutID_Implementation() const
{
    return CurrentLoadoutID;
}

bool AMedComPlayerState::CanAcceptLoadout_Implementation(
    const FName& LoadoutID,
...
FGameplayTag AMedComPlayerState::GetLoadoutComponentType_Implementation() const
{
    return FGameplayTag::RequestGameplayTag(TEXT("Loadout.Component.PlayerState"));
}

void AMedComPlayerState::ResetForLoadout_Implementation(bool bPreserveRuntimeData)
{
    ResetComponentsForLoadout(bPreserveRuntimeData);
}

FString AMedComPlayerState::SerializeLoadoutState_Implementation() const
{
    return FString::Printf(TEXT("LoadoutID=%s"), *CurrentLoadoutID.ToString());
}

bool AMedComPlayerState::RestoreLoadoutState_Implementation(const FString& SerializedState)
{
...
void AMedComPlayerState::OnLoadoutPreChange_Implementation(const FName& OldLoadoutID, const FName& NewLoadoutID)
{
    if (bLogLoadoutOperations)
    {
        UE_LOG(LogMedComPlayerState, Log, TEXT("Preparing to change loadout from '%s' to '%s' for player %s"),
            *OldLoadoutID.ToString(), *NewLoadoutID.ToString(), *GetPlayerName());
    }
}

void AMedComPlayerState::OnLoadoutPostChange_Implementation(const FName& PreviousLoadoutID, const FName& NewLoadoutID)
{
    if (bLogLoadoutOperations)
    {
        UE_LOG(LogMedComPlayerState, Log, TEXT("Successfully changed loadout from '%s' to '%s' for player %s"),
...
FGameplayTagContainer AMedComPlayerState::GetRequiredLoadoutFeatures_Implementation() const
{
...
bool AMedComPlayerState::ValidateAgainstLoadout_Implementation(TArray<FString>& OutViolations) const
{
    OutViolations.Empty();
...
bool AMedComPlayerState::ApplyLoadout(const FName& LoadoutID, bool bForceReapply)
{
    ...
    if (LoadoutID.IsNone())
    {
        UE_LOG(LogMedComPlayerState, Error, TEXT("Cannot apply loadout: LoadoutID is None"));
        return false;
    }
    UMedComLoadoutManager* LoadoutManager = GetLoadoutManager();
    if (!LoadoutManager)
    {
        UE_LOG(LogMedComPlayerState, Error, TEXT("Cannot apply loadout: LoadoutManager not found"));
        return false;
    }
    ...
    if (bLogLoadoutOperations)
    {
        if (Result.bSuccess)
        {
            UE_LOG(LogMedComPlayerState, Log, TEXT("Successfully applied loadout '%s' to player %s"),
                *LoadoutID.ToString(), *GetPlayerName());
            for (const FGameplayTag& ComponentTag : Result.AppliedComponents)
            {
                UE_LOG(LogMedComPlayerState, Log, TEXT("  - Applied to: %s"), *ComponentTag.ToString());
            }
        }
        else
        {
            UE_LOG(LogMedComPlayerState, Error, TEXT("Failed to apply loadout '%s' to player %s: %s"),
                *LoadoutID.ToString(), *GetPlayerName(), *Result.GetSummary());
            for (const FString& Error : Result.ErrorMessages)
            {
                UE_LOG(LogMedComPlayerState, Error, TEXT("  - Error: %s"), *Error);
            }
            for (const FString& Warning : Result.Warnings)
            {
                UE_LOG(LogMedComPlayerState, Warning, TEXT("  - Warning: %s"), *Warning);
            }
        }
    }
...
bool AMedComPlayerState::SwitchLoadout(const FName& NewLoadoutID, bool bPreserveRuntimeData)
{
    ...
    {
        UE_LOG(LogMedComPlayerState, Warning, TEXT("Already using loadout '%s'"), *NewLoadoutID.ToString());
        return true;
    }
...
void AMedComPlayerState::LogLoadoutStatus()
{
    UE_LOG(LogMedComPlayerState, Log, TEXT("=== Loadout Status for %s ==="), *GetPlayerName());
    UE_LOG(LogMedComPlayerState, Log, TEXT("Current Loadout: %s"), 
    ...
    if (UMedComLoadoutManager* LoadoutManager = GetLoadoutManager())
    {
        if (!CurrentLoadoutID.IsNone())
        {
            ...
            UE_LOG(LogMedComPlayerState, Log, TEXT("Configured Inventories: %d"), InventoryNames.Num());
            for (const FName& InvName : InventoryNames)
            {
                ...
                {
                    UE_LOG(LogMedComPlayerState, Log, TEXT("  - %s: %dx%d grid, %.1f max weight"),
            ...
            UE_LOG(LogMedComPlayerState, Log, TEXT("Equipment Slots: %d"), EquipmentSlots.Num());
            for (const FEquipmentSlotConfig& Slot : EquipmentSlots)
            {
                UE_LOG(LogMedComPlayerState, Log, TEXT("  - %s (%s)"),
                    *Slot.DisplayName.ToString(),
                    Slot.bIsRequired ? TEXT("Required") : TEXT("Optional"));
            }
        }
    }
    UE_LOG(LogMedComPlayerState, Log, TEXT("================================="));
}
...
UMedComLoadoutManager* AMedComPlayerState::GetLoadoutManager() const
{
...
FLoadoutApplicationResult AMedComPlayerState::ApplyLoadoutToComponents(const FName& LoadoutID, UMedComLoadoutManager* LoadoutManager)
{
    UE_LOG(LogMedComPlayerState , Warning, TEXT("=== ApplyLoadoutToComponents START ==="));
    UE_LOG(LogMedComPlayerState , Warning, TEXT("LoadoutID: %s"), *LoadoutID.ToString());
    
    if (!LoadoutManager)
    {
        UE_LOG(LogMedComPlayerState , Error, TEXT("LoadoutManager is null"));
        return FLoadoutApplicationResult::CreateFailure(LoadoutID, TEXT("LoadoutManager is null"));
    }
    
    if (!LoadoutManager->IsLoadoutValid(LoadoutID))
    {
        UE_LOG(LogMedComPlayerState , Error, TEXT("Loadout '%s' not found or invalid"), *LoadoutID.ToString());
    ...
    {
        UE_LOG(LogMedComPlayerState , Error, TEXT("Failed to get loadout configuration for '%s'"), *LoadoutID.ToString());
    ...
    {
        UE_LOG(LogMedComPlayerState , Warning, TEXT("Step 1: Applying loadout to inventory component..."));
        ...
        {
            UE_LOG(LogMedComPlayerState , Log, TEXT("Inventory configuration applied successfully"));
        }
        else
        {
            UE_LOG(LogMedComPlayerState , Error, TEXT("Failed to apply inventory configuration"));
        }
    }
    else
    {
        UE_LOG(LogMedComPlayerState , Warning, TEXT("No inventory component found"));
    ...
    if (!EquipmentDataStore)
    {
        UE_LOG(LogMedComPlayerState , Error, TEXT("EquipmentDataStore is NULL"));
        Result.Warnings.Add(TEXT("No equipment DataStore found"));
    }
    else
    {
        UE_LOG(LogMedComPlayerState , Warning, TEXT("Step 2: Applying loadout to equipment DataStore..."));
        ...
        {
            UE_LOG(LogMedComPlayerState , Warning, TEXT("No equipment slot configurations in loadout"));
            Result.Warnings.Add(TEXT("No equipment slot config in loadout"));
        }
        else
        {
            UE_LOG(LogMedComPlayerState , Log, TEXT("Initializing %d equipment slots..."), SlotConfigs.Num());
            
            if (!EquipmentDataStore->InitializeSlots(SlotConfigs))
            {
                UE_LOG(LogMedComPlayerState , Error, TEXT("Failed to initialize equipment slots in DataStore"));
            ...
            {
                UE_LOG(LogMedComPlayerState , Log, TEXT("Equipment slots initialized successfully"));
                ...
                {
                    UE_LOG(LogMedComPlayerState , Warning, TEXT("Step 2b: Applying %d starting equipment items..."), 
                    ...
                    for (const auto& EquipPair : LoadoutConfig.StartingEquipment)
                    {
                        ...
                        UE_LOG(LogMedComPlayerState , Log, TEXT("  Equipping %s to slot %s..."), 
                            *ItemID.ToString(), 
                        ...
                        {
                            UE_LOG(LogMedComPlayerState , Warning, TEXT("    Slot type %s not found in slot configs"), 
                                *UEnum::GetValueAsString(SlotType));
                            continue;
                        }
                        ...
                        {
                            SuccessfulEquips++;
                            UE_LOG(LogMedComPlayerState , Log, TEXT("    Successfully equipped %s to slot %d"), 
                                *ItemID.ToString(), 
                                TargetSlotIndex);
                        }
                        else
                        {
                            UE_LOG(LogMedComPlayerState , Warning, TEXT("    Failed to equip %s to slot %d"), 
                                *ItemID.ToString(), 
                                TargetSlotIndex);
                        }
                    }
                    
                    UE_LOG(LogMedComPlayerState , Log, TEXT("Equipment application: %d items equipped successfully"), 
                        SuccessfulEquips);
                    ...
                    if (SuccessfulEquips > 0)
                    {
                        ...
                        if (UGameInstance* GI = GetGameInstance())
                        {
                            if (UEventDelegateManager* EventManager = GI->GetSubsystem<UEventDelegateManager>())
                            {
                                ...
                                UE_LOG(LogMedComPlayerState , Log, TEXT("Broadcasted Equipment.Initialized event"));
                            }
                        }
                    }
                }
    ...
    if (Result.bSuccess)
    {
        ...
        UE_LOG(LogMedComPlayerState , Log, TEXT("Loadout '%s' applied successfully"), *LoadoutID.ToString());
    }
    
    UE_LOG(LogMedComPlayerState , Warning, TEXT("=== ApplyLoadoutToComponents END ==="));
    
    return Result;
}

void AMedComPlayerState::ResetComponentsForLoadout(bool bPreserveRuntimeData)
{
    if (bLogLoadoutOperations)
    {
        UE_LOG(LogMedComPlayerState, Log, TEXT("Resetting components for loadout change (preserve runtime: %s)"),
...
void AMedComPlayerState::HandleComponentInitialized()
{
    UE_LOG(LogMedComPlayerState, Log, TEXT("Component initialized for player %s"), *GetPlayerName());
}

void AMedComPlayerState::HandleComponentUpdated()
{
    UE_LOG(LogMedComPlayerState, Verbose, TEXT("Component updated for player %s"), *GetPlayerName());
}

bool AMedComPlayerState::SetupComponentListeners()
{
    ...
    UE_LOG(LogMedComPlayerState, Log, TEXT("Set up component listeners for player %s"), *GetPlayerName());
    return true;
}

void AMedComPlayerState::CleanupComponentListeners()
{
    if (!bComponentListenersSetup)
    {
        return;
    ...
    bComponentListenersSetup = false;
    UE_LOG(LogMedComPlayerState, Verbose, TEXT("Cleaned up component listeners for player %s"), *GetPlayerName());
}
...
UAttributeSet* AMedComPlayerState::GetAttributeSet_Implementation() const
{
    return Attributes;
}

TSubclassOf<UAttributeSet> AMedComPlayerState::GetAttributeSetClass_Implementation() const
{
    return InitialAttributeSetClass;
}

TSubclassOf<UGameplayEffect> AMedComPlayerState::GetBaseStatsEffect_Implementation() const
{
    return InitialAttributesEffect;
}

void AMedComPlayerState::InitializeAttributes_Implementation(UAttributeSet* AttributeSet) const
{
    // Empty - we initialize via GE
}

void AMedComPlayerState::ApplyEffects_Implementation(UAbilitySystemComponent* InASC) const
{
    ...
    EffectContext.AddSourceObject(const_cast<AMedComPlayerState*>(this));
...
bool AMedComPlayerState::HasAttributes_Implementation() const
{
    return Attributes != nullptr;
}

void AMedComPlayerState::SetAttributeSetClass_Implementation(TSubclassOf<UAttributeSet> NewClass)
{
    InitialAttributeSetClass = NewClass;
}

FMedComAttributeData AMedComPlayerState::GetAttributeData_Implementation(const FGameplayTag& AttributeTag) const
{
...
FMedComAttributeData AMedComPlayerState::GetHealthData_Implementation() const
{
    if (!Attributes)
    {
        return FMedComAttributeData();
...
FMedComAttributeData AMedComPlayerState::GetStaminaData_Implementation() const
{
    if (!Attributes)
    {
        return FMedComAttributeData();
...
FMedComAttributeData AMedComPlayerState::GetArmorData_Implementation() const
{
    if (!Attributes)
    {
        return FMedComAttributeData();
...
TArray<FMedComAttributeData> AMedComPlayerState::GetAllAttributeData_Implementation() const
{
...
bool AMedComPlayerState::GetAttributeValue_Implementation(const FGameplayTag& AttributeTag, float& OutCurrentValue, float& OutMaxValue) const
{
...
void AMedComPlayerState::NotifyAttributeChanged_Implementation(const FGameplayTag& AttributeTag, float NewValue, float OldValue)
{
    if (AttributeTag.MatchesTag(FGameplayTag::RequestGameplayTag(TEXT("Attribute.Health"))))
    {
        if (Attributes)
...
void AMedComPlayerState::SetupAttributeChangeCallbacks()
{
    if (!ASC || !Attributes)
    {
        return;
    ...
    HealthChangedDelegateHandle = ASC->GetGameplayAttributeValueChangeDelegate(
        BaseAttributes->GetHealthAttribute()).AddUObject(this, &AMedComPlayerState::OnHealthChanged);
    MaxHealthChangedDelegateHandle = ASC->GetGameplayAttributeValueChangeDelegate(
        BaseAttributes->GetMaxHealthAttribute()).AddUObject(this, &AMedComPlayerState::OnMaxHealthChanged);
    StaminaChangedDelegateHandle = ASC->GetGameplayAttributeValueChangeDelegate(
        BaseAttributes->GetStaminaAttribute()).AddUObject(this, &AMedComPlayerState::OnStaminaChanged);
    MaxStaminaChangedDelegateHandle = ASC->GetGameplayAttributeValueChangeDelegate(
        BaseAttributes->GetMaxStaminaAttribute()).AddUObject(this, &AMedComPlayerState::OnMaxStaminaChanged);
    MovementSpeedChangedDelegateHandle = ASC->GetGameplayAttributeValueChangeDelegate(
        BaseAttributes->GetMovementSpeedAttribute()).AddUObject(this, &AMedComPlayerState::OnMovementSpeedChanged);
    SprintTagChangedDelegateHandle = ASC->RegisterGameplayTagEvent(
        SprintingTag, EGameplayTagEventType::NewOrRemoved).AddUObject(this, &AMedComPlayerState::OnSprintTagChanged);
    UE_LOG(LogMedComPlayerState, Log, TEXT("[PlayerState] Attribute change callbacks setup completed"));
}

void AMedComPlayerState::CleanupAttributeChangeCallbacks()
{
    if (!ASC || !Attributes)
    {
        return;
...
void AMedComPlayerState::OnHealthChanged(const FOnAttributeChangeData& Data)
{
    if (Attributes)
    {
        UMedComBaseAttributeSet* BaseAttributes = Cast<UMedComBaseAttributeSet>(Attributes);
        if (BaseAttributes)
        {
            ...
            UE_LOG(LogMedComPlayerState, Verbose, TEXT("[PlayerState] Health changed: %.1f/%.1f (was %.1f)"), 
                Data.NewValue, MaxHealth, Data.OldValue);
        }
    }
}

void AMedComPlayerState::OnMaxHealthChanged(const FOnAttributeChangeData& Data)
{
    if (Attributes)
    {
        UMedComBaseAttributeSet* BaseAttributes = Cast<UMedComBaseAttributeSet>(Attributes);
        if (BaseAttributes)
        {
            ...
            UE_LOG(LogMedComPlayerState, Verbose, TEXT("[PlayerState] Max health changed: %.1f (was %.1f)"), 
                Data.NewValue, Data.OldValue);
        }
    }
}

void AMedComPlayerState::OnStaminaChanged(const FOnAttributeChangeData& Data)
{
    if (Attributes)
    {
        UMedComBaseAttributeSet* BaseAttributes = Cast<UMedComBaseAttributeSet>(Attributes);
        if (BaseAttributes)
        {
            ...
            UE_LOG(LogMedComPlayerState, Verbose, TEXT("[PlayerState] Stamina changed: %.1f/%.1f (was %.1f)"), 
                Data.NewValue, MaxStamina, Data.OldValue);
        }
    }
}

void AMedComPlayerState::OnMaxStaminaChanged(const FOnAttributeChangeData& Data)
{
    if (Attributes)
    {
        UMedComBaseAttributeSet* BaseAttributes = Cast<UMedComBaseAttributeSet>(Attributes);
        if (BaseAttributes)
        {
            ...
            UE_LOG(LogMedComPlayerState, Verbose, TEXT("[PlayerState] Max stamina changed: %.1f (was %.1f)"), 
                Data.NewValue, Data.OldValue);
        }
    }
}

void AMedComPlayerState::OnMovementSpeedChanged(const FOnAttributeChangeData& Data)
{
    UE_LOG(LogMedComPlayerState, Warning, TEXT("[PlayerState] OnMovementSpeedChanged: OldValue=%.1f, NewValue=%.1f"), 
...
void AMedComPlayerState::OnSprintTagChanged(const FGameplayTag CallbackTag, int32 NewCount)
{
    ...
    if (bIsSprinting)
    {
        UE_LOG(LogMedComPlayerState, Log, TEXT("[PlayerState] Sprint tag added, speed will be updated through attribute change"));
    }
    else
    {
        UE_LOG(LogMedComPlayerState, Log, TEXT("[PlayerState] Sprint tag removed, speed will be updated through attribute change"));
    }
...
bool AMedComPlayerState::TryWireEquipmentModuleOnce()
{
    check(IsInGameThread());

    UE_LOG(LogMedComPlayerState, Warning, TEXT("=== TryWireEquipmentModuleOnce: ATTEMPT START ==="));
    ...
    if (!GI)
    {
        UE_LOG(LogMedComPlayerState, Verbose, TEXT("TryWireEquipmentModuleOnce: GI not ready"));
        return false;
    }
    UE_LOG(LogMedComPlayerState, Warning, TEXT("✓ GameInstance available: %s"), *GI->GetClass()->GetName());
    ...
    if (!SysSub)
    {
        UE_LOG(LogMedComPlayerState, Error, TEXT("✗ CoordinatorSubsystem not found in GameInstance"));
        return false;
    }
    UE_LOG(LogMedComPlayerState, Warning, TEXT("✓ CoordinatorSubsystem found"));
    ...
    {
        UE_LOG(LogMedComPlayerState, Verbose, TEXT("✗ Global services NOT ready yet (retry needed)"));
        return false;
    }
    UE_LOG(LogMedComPlayerState, Warning, TEXT("✓ Global services are READY"));
    ...
    if (!Locator)
    {
        UE_LOG(LogMedComPlayerState, Error, TEXT("✗ ServiceLocator is null"));
        return false;
    }
    UE_LOG(LogMedComPlayerState, Warning, TEXT("✓ ServiceLocator available"));
    ...
    if (!EquipmentDataStore)
    {
        UE_LOG(LogMedComPlayerState, Error, TEXT("✗ CRITICAL: EquipmentDataStore is NULL!"));
        return false;
    }
    if (!EquipmentTxnProcessor)
    {
        UE_LOG(LogMedComPlayerState, Error, TEXT("✗ CRITICAL: EquipmentTxnProcessor is NULL!"));
        return false;
    }
    if (!EquipmentOps)
    {
        UE_LOG(LogMedComPlayerState, Error, TEXT("✗ CRITICAL: EquipmentOps is NULL!"));
        return false;
    }
    if (!EquipmentInventoryBridge)
    {
        UE_LOG(LogMedComPlayerState, Error, TEXT("✗ CRITICAL: EquipmentInventoryBridge is NULL!"));
        return false;
    }
    if (!InventoryComponent)
    {
        UE_LOG(LogMedComPlayerState, Error, TEXT("✗ CRITICAL: InventoryComponent is NULL!"));
        return false;
    }
    
    UE_LOG(LogMedComPlayerState, Warning, TEXT("✓ All local components validated"));

    // =====================================================================
    // Step 6: Wire DataStore <-> TransactionProcessor delta callback
    // =====================================================================
    UE_LOG(LogMedComPlayerState, Log, TEXT("Wiring DataStore ↔ TransactionProcessor..."));
    ...
    UE_LOG(LogMedComPlayerState, Warning, TEXT("✓ DataStore delta callback configured"));

    // =====================================================================
    // Step 7: Inject per-player executor into global OperationsService
    // =====================================================================
    UE_LOG(LogMedComPlayerState, Warning, TEXT("Injecting OperationsExecutor into global service..."));
    {
        ...
        if (!OpsObj)
        {
            UE_LOG(LogMedComPlayerState, Error, TEXT("✗ OperationsService not found in locator"));
            return false;
        }
        ...
        if (!OpsService)
        {
            UE_LOG(LogMedComPlayerState, Error, TEXT("✗ Service object is not UEquipmentOperationServiceImpl"));
            return false;
        }
        ...
        UE_LOG(LogMedComPlayerState, Warning, TEXT("✓ Executor injected into OperationsService"));
    }
    ...
    // =====================================================================
    UE_LOG(LogMedComPlayerState, Warning, TEXT("=== Preparing shared DataProvider interface ==="));
    ...
    {
        UE_LOG(LogMedComPlayerState, Error, TEXT("✗ DataStore does not implement IMedComEquipmentDataProvider!"));
        return false;
    }
    UE_LOG(LogMedComPlayerState, Warning, TEXT("✓ DataProvider interface prepared for reuse"));
    ...
    // =====================================================================
    UE_LOG(LogMedComPlayerState, Warning, TEXT("=== Initializing TransactionProcessor START ==="));
    ...
    UE_LOG(LogMedComPlayerState, Warning, TEXT("  ✓ TransactionProcessor initialized with DataProvider"));
    UE_LOG(LogMedComPlayerState, Warning, TEXT("  ✓ Transaction processor ready to accept transactions"));
    UE_LOG(LogMedComPlayerState, Warning, TEXT("=== Initializing TransactionProcessor END ==="));
    ...
    // =====================================================================
    UE_LOG(LogMedComPlayerState, Warning, TEXT("=== Initializing EquipmentOps START ==="));
    ...
    if (EquipmentSlotValidator)
    {
        ...
        UE_LOG(LogMedComPlayerState, Log, TEXT("  ✓ SlotValidator interface prepared"));
    }
    else
    {
        UE_LOG(LogMedComPlayerState, Warning, TEXT("  ⚠ SlotValidator not available - validation will be limited"));
    }
    ...
    UE_LOG(LogMedComPlayerState, Warning, TEXT("  ✓ EquipmentOps initialized with DataProvider"));
    UE_LOG(LogMedComPlayerState, Warning, TEXT("  ✓ Item validation system ready"));
    UE_LOG(LogMedComPlayerState, Warning, TEXT("=== Initializing EquipmentOps END ==="));
    ...
    // =====================================================================
    UE_LOG(LogMedComPlayerState, Warning, TEXT("=== Initializing EquipmentInventoryBridge START ==="));
    ...
    {
        UE_LOG(LogMedComPlayerState, Error, TEXT("✗ EquipmentOps does not implement IMedComEquipmentOperations!"));
        return false;
    }
    UE_LOG(LogMedComPlayerState, Log, TEXT("  ✓ Operations interface prepared"));
    ...
    {
        UE_LOG(LogMedComPlayerState, Error, TEXT("✗ TxnProcessor does not implement IMedComTransactionManager!"));
        return false;
    }
    UE_LOG(LogMedComPlayerState, Log, TEXT("  ✓ TransactionManager interface prepared"));
    ...
    UE_LOG(LogMedComPlayerState, Warning, TEXT("  Calling EquipmentInventoryBridge->Initialize()..."));
    ...
    UE_LOG(LogMedComPlayerState, Warning, TEXT("  ✓ Bridge initialized successfully"));
    UE_LOG(LogMedComPlayerState, Warning, TEXT("  ✓ Bridge has access to all required systems"));

    // Connect Inventory interface to Bridge
    UE_LOG(LogMedComPlayerState, Log, TEXT("  Connecting Inventory to Bridge..."));
    ...
    UE_LOG(LogMedComPlayerState, Warning, TEXT("  ✓ Inventory connected to Bridge"));
    UE_LOG(LogMedComPlayerState, Warning, TEXT("  ✓ Bridge can now transfer items between systems"));

    UE_LOG(LogMedComPlayerState, Warning, TEXT("=== Initializing EquipmentInventoryBridge END ==="));

    // =====================================================================
    // Step 12: Success - All systems operational
    // =====================================================================
    UE_LOG(LogMedComPlayerState, Warning, TEXT("=== Equipment Module Wiring COMPLETE ✓✓✓ ==="));
    UE_LOG(LogMedComPlayerState, Warning, TEXT("All systems operational for player: %s"), *GetPlayerName());
    UE_LOG(LogMedComPlayerState, Warning, TEXT(""));
    UE_LOG(LogMedComPlayerState, Warning, TEXT("Component Status Summary:"));
    UE_LOG(LogMedComPlayerState, Warning, TEXT("  • DataStore:            Initialized with %d slots"), 
        EquipmentDataStore->GetSlotCount());
    UE_LOG(LogMedComPlayerState, Warning, TEXT("  • TransactionProcessor: Initialized and ready"));
    UE_LOG(LogMedComPlayerState, Warning, TEXT("  • EquipmentOps:         Initialized with validation"));
    UE_LOG(LogMedComPlayerState, Warning, TEXT("  • Bridge:               Connected to Inventory"));
    UE_LOG(LogMedComPlayerState, Warning, TEXT(""));
    UE_LOG(LogMedComPlayerState, Warning, TEXT("Players can now:"));
    UE_LOG(LogMedComPlayerState, Warning, TEXT("  • Equip items from inventory to equipment slots"));
    UE_LOG(LogMedComPlayerState, Warning, TEXT("  • Unequip items back to inventory"));
    UE_LOG(LogMedComPlayerState, Warning, TEXT("  • Swap items between equipment slots"));
    UE_LOG(LogMedComPlayerState, Warning, TEXT("  • All operations are atomic and transactional"));
    
    return true;
}

void AMedComPlayerState::InitAttributes()
{
    ...
    if (!ASC)
    {
        UE_LOG(LogMedComPlayerState, Error, TEXT("Failed to initialize attributes: ASC is null"));
        return;
    }
    if (Attributes)
    {
        UE_LOG(LogMedComPlayerState, Warning, TEXT("Attributes already initialized, skipping"));
        return;
    }
    ...
    if (!AttributeSetClassToUse)
    {
        ...
        UE_LOG(LogMedComPlayerState, Warning, TEXT("Using default UMedComDefaultAttributeSet"));
    }
    Attributes = NewObject<UMedComBaseAttributeSet>(this, AttributeSetClassToUse);
    if (!Attributes)
    {
        UE_LOG(LogMedComPlayerState, Error, TEXT("Failed to create AttributeSet"));
        return;
    }
    ASC->AddSpawnedAttribute(Attributes);
    UE_LOG(LogMedComPlayerState, Log, TEXT("AttributeSet created and added to ASC"));

    if (!InitialAttributesEffect)
    {
        UE_LOG(LogMedComPlayerState, Error, TEXT("InitialAttributesEffect not configured! Attributes will remain at 0!"));
        return;
    }
    UE_LOG(LogMedComPlayerState, Log, TEXT("Applying InitialAttributesEffect: %s"), *InitialAttributesEffect->GetName());
    FGameplayEffectContextHandle EffectContext = ASC->MakeEffectContext();
    EffectContext.AddSourceObject(this);
    FGameplayEffectSpecHandle SpecHandle = ASC->MakeOutgoingSpec(InitialAttributesEffect, 1.0f, EffectContext);
    if (!SpecHandle.IsValid())
    {
        UE_LOG(LogMedComPlayerState, Error, TEXT("Failed to create effect spec for InitialAttributesEffect"));
        return;
    }
    FActiveGameplayEffectHandle EffectHandle = ASC->ApplyGameplayEffectSpecToSelf(*SpecHandle.Data.Get());
    if (!EffectHandle.IsValid())
    {
        UE_LOG(LogMedComPlayerState, Error, TEXT("Failed to apply InitialAttributesEffect"));
        return;
    }
    if (UMedComBaseAttributeSet* BaseAttributes = Cast<UMedComBaseAttributeSet>(Attributes))
    {
        UE_LOG(LogMedComPlayerState, Warning, TEXT("=== Final Attribute Values After Initialization ==="));
        UE_LOG(LogMedComPlayerState, Warning, TEXT("MovementSpeed: %.1f"), 
            ASC->GetNumericAttribute(BaseAttributes->GetMovementSpeedAttribute()));
        UE_LOG(LogMedComPlayerState, Warning, TEXT("Health: %.1f / %.1f"), 
            ASC->GetNumericAttribute(BaseAttributes->GetHealthAttribute()),
            ASC->GetNumericAttribute(BaseAttributes->GetMaxHealthAttribute()));
        UE_LOG(LogMedComPlayerState, Warning, TEXT("Stamina: %.1f / %.1f"), 
            ASC->GetNumericAttribute(BaseAttributes->GetStaminaAttribute()),
            ASC->GetNumericAttribute(BaseAttributes->GetMaxStaminaAttribute()));
        UE_LOG(LogMedComPlayerState, Warning, TEXT("Armor: %.1f"), 
            ASC->GetNumericAttribute(BaseAttributes->GetArmorAttribute()));
        UE_LOG(LogMedComPlayerState, Warning, TEXT("AttackPower: %.1f"), 
        ...
        if (APawn* Pawn = GetPawn())
        {
            if (ACharacter* Character = Cast<ACharacter>(Pawn))
            {
                if (UCharacterMovementComponent* MovementComp = Character->GetCharacterMovement())
                {
                    ...
                    UE_LOG(LogMedComPlayerState, Log, TEXT("Applied initial movement speed %.1f to character"), InitialSpeed);
                }
            }
        }
    }
...
void AMedComPlayerState::GrantStartupAbilities()
{
    ...
    if (!ASC)
    {
        UE_LOG(LogMedComPlayerState, Error, TEXT("Failed to grant abilities: ASC is null"));
        return;
    }
    UE_LOG(LogMedComPlayerState, Warning, TEXT("=== GrantStartupAbilities: Starting ability registration ==="));
    ...
    for (const FAbilityInfo& AbilityInfo : AbilityPool)
    {
        if (AbilityInfo.Ability)
        {
            ...
            UE_LOG(LogMedComPlayerState, Warning, TEXT("  - Added ability from pool: %s, InputID=%d, Handle Valid=%s"),
    ...
    if (InteractAbility) 
    {
        ...
        UE_LOG(LogMedComPlayerState, Warning, TEXT("  - Added Interact: %s, InputID=%d, Handle Valid=%s"),
               *InteractAbility->GetName(), InteractInputID, Handle.IsValid() ? TEXT("YES") : TEXT("NO"));
    }
    else
    {
        UE_LOG(LogMedComPlayerState, Error, TEXT("  - InteractAbility NOT SET in Blueprint!"));
    }
    ...
    if (SprintAbility) 
    {
        ...
        UE_LOG(LogMedComPlayerState, Warning, TEXT("  - Added Sprint: %s, InputID=%d, Handle Valid=%s"),
               *SprintAbility->GetName(), SprintInputID, Handle.IsValid() ? TEXT("YES") : TEXT("NO"));
    }
    else
    {
        UE_LOG(LogMedComPlayerState, Error, TEXT("  - SprintAbility NOT SET in Blueprint!"));
    }
    ...
    if (CrouchAbility) 
    {
        ...
        UE_LOG(LogMedComPlayerState, Warning, TEXT("  - Added Crouch: %s, InputID=%d, Handle Valid=%s"),
        ...
        if (Handle.IsValid())
        {
            ...
            {
                UE_LOG(LogMedComPlayerState, Warning, TEXT("    Crouch Spec confirmed: InputID=%d, Level=%d"),
    ...
    {
        UE_LOG(LogMedComPlayerState, Error, TEXT("  - CrouchAbility NOT SET in Blueprint!"));
        UE_LOG(LogMedComPlayerState, Error, TEXT("    CRITICAL: Check BP_MedComPlayerState and set CrouchAbility!"));
    }
    ...
    if (JumpAbility) 
    {
        ...
        UE_LOG(LogMedComPlayerState, Warning, TEXT("  - Added Jump: %s, InputID=%d, Handle Valid=%s"),
               *JumpAbility->GetName(), JumpInputID, Handle.IsValid() ? TEXT("YES") : TEXT("NO"));
    }
    else
    {
        UE_LOG(LogMedComPlayerState, Error, TEXT("  - JumpAbility NOT SET in Blueprint!"));
    }
    ...
    if (WeaponSwitchAbility) 
    {
        ...
        UE_LOG(LogMedComPlayerState, Warning, TEXT("Added WeaponSwitchAbility with 8 different InputIDs for all weapon switch methods"));
    }
    else
    {
        UE_LOG(LogMedComPlayerState, Error, TEXT("  - WeaponSwitchAbility NOT SET in Blueprint!"));
    }

    UE_LOG(LogMedComPlayerState, Warning, TEXT("=== Total abilities granted: %d ==="), ASC->GetActivatableAbilities().Num());
    for (const FGameplayAbilitySpec& Spec : ASC->GetActivatableAbilities())
    {
        if (Spec.Ability)
        {
            UE_LOG(LogMedComPlayerState, Warning, TEXT("  Ability: %s, InputID: %d, Level: %d"),
                *Spec.Ability->GetName(), Spec.InputID, Spec.Level);
        }
    }
}

void AMedComPlayerState::ApplyPassiveStartupEffects()
{
    ...
    if (!ASC)
    {
        UE_LOG(LogMedComPlayerState, Error, TEXT("Failed to apply passive effects: ASC is null"));
        return;
    }
    FGameplayEffectContextHandle EffectContext = ASC->MakeEffectContext();
    EffectContext.AddSourceObject(this);
...
bool AMedComPlayerState::CheckAuthority(const TCHAR* Fn) const
{
    if (!HasAuthority())
    {
        UE_LOG(LogMedComPlayerState, Warning, TEXT("Attempted to call %s on client"), Fn);
        return false;
    }
    return true;
}

void AMedComPlayerState::DebugActiveEffects()
{
    if (!ASC)
    {
        return;
    }
    UE_LOG(LogMedComPlayerState, Warning, TEXT("=== Active GameplayEffects Debug ==="));
    ...
    for (const FActiveGameplayEffectHandle& Handle : ActiveHandles)
    {
        ...
        {
            UE_LOG(LogMedComPlayerState, Warning, TEXT("Active Effect: %s"), *ActiveEffect->Spec.Def->GetName());
            FGameplayTagContainer GrantedTags;
            ActiveEffect->Spec.GetAllGrantedTags(GrantedTags);
            UE_LOG(LogMedComPlayerState, Warning, TEXT("  Granted Tags: %s"), *GrantedTags.ToString());
            for (int32 i = 0; i < ActiveEffect->Spec.Modifiers.Num(); i++)
            {
                float Magnitude = ActiveEffect->Spec.GetModifierMagnitude(i, true);
                UE_LOG(LogMedComPlayerState, Warning, TEXT("  Modifier[%d] Magnitude: %.2f"), i, Magnitude);
            }
        }
    }
    FGameplayTagContainer OwnedTags;
    ASC->GetOwnedGameplayTags(OwnedTags);
    UE_LOG(LogMedComPlayerState, Warning, TEXT("Current Owned Tags: %s"), *OwnedTags.ToString());
    if (Attributes)
    {
        UMedComBaseAttributeSet* BaseAttributes = Cast<UMedComBaseAttributeSet>(Attributes);
        if (BaseAttributes)
        {
            UE_LOG(LogMedComPlayerState, Warning, TEXT("MovementSpeed Base: %.1f, Current: %.1f"), 
...
bool AMedComPlayerState::WireEquipmentModule(UMedComLoadoutManager* LoadoutManager, const FName& AppliedLoadoutID)
{
    UE_LOG(LogMedComPlayerState, Warning, TEXT("=== WireEquipmentModule START ==="));

    if (!CheckAuthority(TEXT("WireEquipmentModule")))
    {
        UE_LOG(LogMedComPlayerState, Verbose, TEXT("WireEquipmentModule: non-authority, skip wiring on this instance"));
    ...
    if (TryWireEquipmentModuleOnce())
    {
        UE_LOG(LogMedComPlayerState, Warning, TEXT("=== WireEquipmentModule COMPLETE (immediate) ==="));
        return true;
    }
    ...
    GetWorldTimerManager().SetTimer(
        ...
        [this]()
        {
            ...
            if (TryWireEquipmentModuleOnce())
            {
                ...
                UE_LOG(LogMedComPlayerState, Warning, TEXT("WireEquipmentModule: succeeded on retry %d"), EquipmentWireRetryCount);
                return;
            }
    ...
    UE_LOG(LogMedComPlayerState, Warning, TEXT("=== WireEquipmentModule DEFERRED (waiting for global services) ==="));
    return false;
}
