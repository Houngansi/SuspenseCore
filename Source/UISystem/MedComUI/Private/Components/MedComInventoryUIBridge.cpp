...
static TWeakInterfacePtr<IMedComInventoryUIBridgeWidget> GInventoryUIBridge;

// =====================================================
// Constructor & Core Lifecycle
// =====================================================

UMedComInventoryUIBridge::UMedComInventoryUIBridge()
{
    ...
    static ConstructorHelpers::FClassFinder<UMedComInventoryWidget> DefaultWidgetFinder(
    ...
    else
    {
        ...
        InventoryWidgetClass = UMedComInventoryWidget::StaticClass();
        UE_LOG(LogTemp, Warning, TEXT("[InventoryUIBridge] Using C++ base class for inventory widget"));
    }
}

bool UMedComInventoryUIBridge::Initialize(APlayerController* InPlayerController)
{
    if (bIsInitialized)
    {
    ...
    UIManager = UMedComUIManager::Get(InPlayerController);
    if (!UIManager)
    {
...
void UMedComInventoryUIBridge::SetInventoryInterface(TScriptInterface<IMedComInventoryInterface> InInventory)
{
    if (!bIsInitialized)
    {
    ...
    {
        // Get event manager
        EventManager = IMedComInventoryInterface::GetDelegateManagerStatic(GameInventory.GetObject());
        if (!EventManager)
        {
...
void UMedComInventoryUIBridge::SetInventoryWidgetClass(TSubclassOf<UMedComInventoryWidget> WidgetClass)
{
    if (WidgetClass)
    {
...
void UMedComInventoryUIBridge::Shutdown()
{
    ...
    if (UMedComInventoryWidget* InventoryWidget = GetCachedInventoryWidget())
    {
        // Unsubscribe widget from events through interface
        if (InventoryWidget->GetClass()->ImplementsInterface(UMedComUIWidgetInterface::StaticClass()))
        {
            IMedComUIWidgetInterface::Execute_UninitializeWidget(InventoryWidget);
        }
...
bool UMedComInventoryUIBridge::RemoveItemFromInventorySlot(int32 SlotIndex, FInventoryItemInstance& OutRemovedInstance)
{
    ...
    IMedComInventoryInterface* Inventory = GameInventory.GetInterface();
...
bool UMedComInventoryUIBridge::RestoreItemToInventory(const FInventoryItemInstance& ItemInstance)
{
    ...
    bool bFallbackSuccess = IMedComInventoryInterface::Execute_AddItemByID(
...
// =====================================================
// IMedComInventoryUIBridgeWidget Interface Implementation
// =====================================================

void UMedComInventoryUIBridge::ShowInventoryUI_Implementation()
{
...
void UMedComInventoryUIBridge::HideInventoryUI_Implementation()
{
...
void UMedComInventoryUIBridge::ToggleInventoryUI_Implementation()
{
...
bool UMedComInventoryUIBridge::IsInventoryUIVisible_Implementation() const
{
    // Check Character Screen visibility
    return IsCharacterScreenVisible_Implementation();
}

void UMedComInventoryUIBridge::RefreshInventoryUI_Implementation()
{
    ...
    UMedComInventoryWidget* InventoryWidget = GetCachedInventoryWidget();
    if (!InventoryWidget)
    {
    ...
    if (!InventoryWidget->IsFullyInitialized())
    {
        ...
        IMedComContainerUIInterface::Execute_InitializeContainer(InventoryWidget, ContainerData);
    }
    else
    {
        ...
        IMedComContainerUIInterface::Execute_UpdateContainer(InventoryWidget, ContainerData);
    }
    
    UE_LOG(LogTemp, Log, TEXT("[InventoryUIBridge] === RefreshInventoryUI END ==="));
}

void UMedComInventoryUIBridge::OnInventoryDataChanged_Implementation(const FGameplayTag& ChangeType)
{
    // Log change type
...
bool UMedComInventoryUIBridge::IsInventoryConnected_Implementation() const
{
    return GameInventory.GetInterface() != nullptr;
}

bool UMedComInventoryUIBridge::GetInventoryGridSize_Implementation(int32& OutColumns, int32& OutRows) const
{
    if (GameInventory.GetInterface())
    {
...
int32 UMedComInventoryUIBridge::GetInventorySlotCount_Implementation() const
{
    int32 Columns, Rows;
...
void UMedComInventoryUIBridge::ShowCharacterScreenWithTab_Implementation(const FGameplayTag& TabTag)
{
...
void UMedComInventoryUIBridge::HideCharacterScreen_Implementation()
{
...
void UMedComInventoryUIBridge::ToggleCharacterScreen_Implementation()
{
    if (IsCharacterScreenVisible_Implementation())
    {
        HideCharacterScreen_Implementation();
...
bool UMedComInventoryUIBridge::IsCharacterScreenVisible_Implementation() const
{
    if (!bIsInitialized || !UIManager)
    {
        return false;
...
void UMedComInventoryUIBridge::RegisterBridge(UMedComInventoryUIBridge* Bridge)
{
    if (Bridge)
    {
        // Set global bridge using interface method
        GInventoryUIBridge = TWeakInterfacePtr<IMedComInventoryUIBridgeWidget>(Bridge);
        IMedComInventoryUIBridgeWidget::SetGlobalBridge(Bridge);
        UE_LOG(LogTemp, Log, TEXT("[InventoryUIBridge] Bridge registered globally"));
    }
}

void UMedComInventoryUIBridge::UnregisterBridge()
{
    GInventoryUIBridge.Reset();
    IMedComInventoryUIBridgeWidget::ClearGlobalBridge();
...
UMedComInventoryWidget* UMedComInventoryUIBridge::FindInventoryWidgetInCharacterScreen() const
{
    if (!UIManager)
    {
    ...
    {
        if (UMedComInventoryWidget* InventoryWidget = Cast<UMedComInventoryWidget>(Widget))
        {
    ...
    for (UWidget* Widget : AllWidgets)
    {
        if (UMedComUpperTabBar* TabBar = Cast<UMedComUpperTabBar>(Widget))
        {
            ...
            for (int32 i = 0; i < TabCount; i++)
            {
                    ...
                    Config.TabTag.MatchesTagExact(FGameplayTag::RequestGameplayTag(TEXT("UI.Tab.Character"))))
                {
                    ...
                    if (UMedComInventoryWidget* InventoryWidget = Cast<UMedComInventoryWidget>(TabContent))
                    {
                    ...
                    if (UMedComBaseLayoutWidget* LayoutWidget = Cast<UMedComBaseLayoutWidget>(TabContent))
                    {
                        ...
                        if (UMedComInventoryWidget* InventoryInLayout = 
                            Cast<UMedComInventoryWidget>(LayoutWidget->GetWidgetByTag(InventoryTag)))
                        {
            ...
            if (CurrentTabIndex >= 0 && CurrentTabIndex < TabCount)
            {
                ...
                if (UMedComInventoryWidget* InventoryWidget = Cast<UMedComInventoryWidget>(CurrentContent))
                {
                ...
                if (UMedComBaseLayoutWidget* LayoutWidget = Cast<UMedComBaseLayoutWidget>(CurrentContent))
                {
                    FGameplayTag InventoryTag = FGameplayTag::RequestGameplayTag(TEXT("UI.Widget.Inventory"));
                    if (UMedComInventoryWidget* InventoryInLayout = 
                        Cast<UMedComInventoryWidget>(LayoutWidget->GetWidgetByTag(InventoryTag)))
    ...
    FGameplayTag InventoryTag = FGameplayTag::RequestGameplayTag(TEXT("UI.Widget.Inventory"));
    if (UMedComInventoryWidget* InventoryWidget = Cast<UMedComInventoryWidget>(UIManager->FindWidgetInLayouts(InventoryTag)))
    {
...
UMedComInventoryWidget* UMedComInventoryUIBridge::GetCachedInventoryWidget() const
{
    ...
    UMedComInventoryWidget* Widget = FindInventoryWidgetInCharacterScreen();
    if (Widget)
    {
...
void UMedComInventoryUIBridge::InvalidateWidgetCache()
{
    CachedInventoryWidget.Reset();
    LastWidgetCacheValidationTime = 0.0f;
}

void UMedComInventoryUIBridge::InitializeInventoryWidgetWithData(UMedComInventoryWidget* Widget)
{
    if (!Widget)
    {
    ...
    if (!ValidateInventoryConnection())
    {
        ...
        if (OwningPlayerController && OwningPlayerController->PlayerState)
        {
            ...
            {
                if (Component && Component->GetClass()->ImplementsInterface(UMedComInventoryInterface::StaticClass()))
                {
                    TScriptInterface<IMedComInventoryInterface> InventoryInterface;
                    InventoryInterface.SetObject(Component);
                    InventoryInterface.SetInterface(Cast<IMedComInventoryInterface>(Component));
    ...
    if (ContainerData.Slots.Num() > 0)
    {
        ...
        if (Widget->GetClass()->ImplementsInterface(UMedComContainerUIInterface::StaticClass()))
        {
            ...
            IMedComContainerUIInterface::Execute_InitializeContainer(Widget, ContainerData);
...
void UMedComInventoryUIBridge::UpdateInventoryWidgetData(UMedComInventoryWidget* Widget)
{
    if (!Widget)
    {
        return;
    ...
    if (Widget->GetClass()->ImplementsInterface(UMedComContainerUIInterface::StaticClass()))
    {
        ...
        {
            IMedComContainerUIInterface::Execute_UpdateContainer(Widget, ContainerData);
...
bool UMedComInventoryUIBridge::ConvertInventoryToUIData(FContainerUIData& OutContainerData) const
{
    ...
    IMedComInventoryInterface* InventoryInterface = GameInventory.GetInterface();
    
    // Get weight parameters
    OutContainerData.CurrentWeight = IMedComInventoryInterface::Execute_GetCurrentWeight(GameInventory.GetObject());
    OutContainerData.MaxWeight = IMedComInventoryInterface::Execute_GetMaxWeight(GameInventory.GetObject());
    OutContainerData.bHasWeightLimit = true;
    
    // Get allowed item types
    OutContainerData.AllowedItemTypes = IMedComInventoryInterface::Execute_GetAllowedItemTypes(GameInventory.GetObject());
...
bool UMedComInventoryUIBridge::ConvertItemInstanceToUIData(const FInventoryItemInstance& Instance, int32 SlotIndex, FItemUIData& OutItemData) const
{
    ...
    if (UWorld* World = GetWorld())
    {
        ...
        {
            if (UMedComItemManager* ItemManager = GameInstance->GetSubsystem<UMedComItemManager>())
            {
...
void UMedComInventoryUIBridge::SubscribeToEvents()
{
    ...
    EventManager->OnUIContainerUpdateRequested.AddDynamic(this, &UMedComInventoryUIBridge::OnUIRequestingUpdate);
    EventManager->OnUISlotInteraction.AddDynamic(this, &UMedComInventoryUIBridge::OnUISlotInteraction);
    EventManager->OnUIDragStarted.AddDynamic(this, &UMedComInventoryUIBridge::OnUIDragStarted);
    EventManager->OnUIDragCompleted.AddDynamic(this, &UMedComInventoryUIBridge::OnUIDragCompleted);
    EventManager->OnUIItemDropped.AddDynamic(this, &UMedComInventoryUIBridge::OnUIItemDropped);
    
    // КРИТИЧЕСКОЕ ДОБАВЛЕНИЕ: Подписка на завершение операций с экипировкой
    EventManager->OnEquipmentOperationCompleted.AddDynamic(this, &UMedComInventoryUIBridge::OnEquipmentOperationCompleted);
    ...
    {
        EventManager->OnInventoryItemMoved.AddDynamic(this, &UMedComInventoryUIBridge::OnInventoryItemMoved);
    }
    
    UE_LOG(LogTemp, Log, TEXT("[InventoryUIBridge] Subscribed to all events successfully"));
}

void UMedComInventoryUIBridge::UnsubscribeFromEvents()
{
    if (EventManager)
    {
        // Unsubscribe from dynamic delegates
        EventManager->OnUIContainerUpdateRequested.RemoveDynamic(this, &UMedComInventoryUIBridge::OnUIRequestingUpdate);
        EventManager->OnUISlotInteraction.RemoveDynamic(this, &UMedComInventoryUIBridge::OnUISlotInteraction);
        EventManager->OnUIDragStarted.RemoveDynamic(this, &UMedComInventoryUIBridge::OnUIDragStarted);
        EventManager->OnUIDragCompleted.RemoveDynamic(this, &UMedComInventoryUIBridge::OnUIDragCompleted);
        EventManager->OnUIItemDropped.RemoveDynamic(this, &UMedComInventoryUIBridge::OnUIItemDropped);
        EventManager->OnInventoryItemMoved.RemoveDynamic(this, &UMedComInventoryUIBridge::OnInventoryItemMoved);
        
        // ВАЖНО: Отписываемся от событий экипировки
        EventManager->OnEquipmentOperationCompleted.RemoveDynamic(this, &UMedComInventoryUIBridge::OnEquipmentOperationCompleted);
...
void UMedComInventoryUIBridge::OnGameInventoryUpdated()
{
    // Schedule UI update instead of immediate refresh
...
void UMedComInventoryUIBridge::OnUIRequestingUpdate(UUserWidget* Widget, const FGameplayTag& ContainerType)
{
...
void UMedComInventoryUIBridge::OnUISlotInteraction(UUserWidget* Widget, int32 SlotIndex, const FGameplayTag& InteractionType)
{
    ...
    {
        if (UMedComInventoryWidget* InvWidget = Cast<UMedComInventoryWidget>(Widget))
        {
...
void UMedComInventoryUIBridge::OnUIDragStarted(UUserWidget* SourceWidget, const FDragDropUIData& DragData)
{
...
void UMedComInventoryUIBridge::OnUIDragCompleted(UUserWidget* SourceWidget, UUserWidget* TargetWidget, bool bSuccess)
{
    if (!bSuccess)
    {
...
void UMedComInventoryUIBridge::OnUIItemDropped(UUserWidget* ContainerWidget, const FDragDropUIData& DragData, int32 TargetSlot)
{
    ...
    if (ContainerWidget && ContainerWidget->GetClass()->ImplementsInterface(UMedComContainerUIInterface::StaticClass()))
    {
        TargetContainerType = IMedComContainerUIInterface::Execute_GetContainerType(ContainerWidget);
    ...
    if (UWorld* World = GetWorld())
    {
        ...
        World->GetTimerManager().SetTimerForNextTick([this, TargetContainerType, EquipTag]()
        {
            ...
            {
                if (InvWidget->GetClass()->ImplementsInterface(UMedComInventoryUIBridgeWidget::StaticClass()))
                {
                    IMedComInventoryUIBridgeWidget::Execute_RefreshInventoryUI(InvWidget);
                }
            }
...
void UMedComInventoryUIBridge::HandleInvalidDrop(UUserWidget* ContainerWidget, const FDragDropUIData& DragData, int32 TargetSlot)
{
...
void UMedComInventoryUIBridge::OnEquipmentOperationCompleted(const FEquipmentOperationResult& Result)
{
...
void UMedComInventoryUIBridge::OnInventoryItemMoved(const FGuid& ItemID, int32 FromSlot, int32 ToSlot, bool bSuccess)
{
...
void UMedComInventoryUIBridge::OnInventoryUIClosed()
{
    IMedComInventoryUIBridgeWidget::Execute_HideCharacterScreen(this);
...
void UMedComInventoryUIBridge::ScheduleUIUpdate(const FGameplayTag& UpdateType)
{
    ...
    if (UWorld* World = GetWorld())
    {
        if (!BatchedUpdateTimerHandle.IsValid())
        {
            World->GetTimerManager().SetTimer(
                ...
                this,
                &UMedComInventoryUIBridge::ProcessBatchedUIUpdates,
                UPDATE_BATCH_DELAY,
                false
            );
        }
...
void UMedComInventoryUIBridge::ProcessBatchedUIUpdates()
{
...
void UMedComInventoryUIBridge::RefreshAllWidgetsInActiveLayout()
{
...
void UMedComInventoryUIBridge::ForceFullInventoryRefresh()
{
...
void UMedComInventoryUIBridge::HandleInventoryToInventoryDrop(const FDragDropUIData& DragData, int32 TargetSlot)
{
...
void UMedComInventoryUIBridge::HandleExternalToInventoryDrop(
    ...
    int32 TargetSlot)
{
    ...
    if (DragData.SourceContainerType.MatchesTag(FGameplayTag::RequestGameplayTag(TEXT("Container.Equipment"))))
    {
        ...
        if (ValidateInventoryConnection())
        {
            if (IMedComInventoryInterface* InventoryInterface = GameInventory.GetInterface())
            {
...
void UMedComInventoryUIBridge::HandleInventoryToEquipmentDrop(
    UUserWidget* ContainerWidget,
    const FDragDropUIData& DragData,
    int32 TargetSlot)
{
    ...
    UMedComEquipmentUIBridge* EquipmentBridge = UIManager->GetEquipmentUIBridge();
    if (!EquipmentBridge)
    {
...
FInventoryOperationResult UMedComInventoryUIBridge::ProcessInventoryDrop(
    const FDragDropUIData& DragData,
    ...
    UUserWidget* TargetWidget)
{
        ...
        if (DragData.SourceContainerType.MatchesTag(FGameplayTag::RequestGameplayTag(TEXT("Container.Equipment"))))
        {
            if (IMedComInventoryInterface* InventoryInterface = GameInventory.GetInterface())
            {
                ...
                if (!bCanPlace)
                {
                    ...
                    FInventoryItemInstance ExistingItem;
    ...
    const bool bSuccess = IMedComInventoryInterface::Execute_MoveItemBySlots(
        GameInventory.GetObject(),
...
int32 UMedComInventoryUIBridge::CalculateDropTargetSlot(
   const FVector2D& ScreenPosition,
...
bool UMedComInventoryUIBridge::ValidateDropPlacement(
   int32 TargetSlot,
   const FIntPoint& ItemSize,
   bool bIsRotated,
...
int32 UMedComInventoryUIBridge::FindNearestValidSlot(
   int32 PreferredSlot,
   const FIntPoint& ItemSize,
   bool bIsRotated,
...
void UMedComInventoryUIBridge::ProcessItemMoveRequest(const FGuid& ItemInstanceID, int32 TargetSlotIndex, bool bIsRotated)
{
   if (!GameInventory.GetInterface())
   {
       return;
...
void UMedComInventoryUIBridge::ProcessItemRotationRequest(int32 SlotIndex)
{
   if (!ValidateInventoryConnection())
   {
       return;
...
void UMedComInventoryUIBridge::ProcessSortRequest()
{
...
void UMedComInventoryUIBridge::ProcessItemDoubleClick(int32 SlotIndex, const FGuid& ItemInstanceID)
{
   if (!ValidateInventoryConnection())
   {
       return;
...
bool UMedComInventoryUIBridge::ValidateInventoryConnection() const
{
   if (!GameInventory.GetInterface())
   {
...
UMedComUIManager* UMedComInventoryUIBridge::GetUIManager() const
{
   return UIManager;
}

FString UMedComInventoryUIBridge::GetErrorCodeString(EInventoryErrorCode ErrorCode) const
{
   switch (ErrorCode)
   {
...
bool UMedComInventoryUIBridge::GetInventoryGridParams(
   int32& OutColumns,
   int32& OutRows,
   float& OutCellSize,
...
FVector2D UMedComInventoryUIBridge::ScreenToGridCoordinates(
...
bool UMedComInventoryUIBridge::AreRequiredSlotsFree(
   int32 AnchorSlot,
   const FIntPoint& ItemSize,
   int32 GridColumns,
...
void UMedComInventoryUIBridge::DiagnoseDropPosition(const FVector2D& ScreenPosition) const
{
...
void UMedComInventoryUIBridge::DiagnoseInventoryState() const
{
   if (!ValidateInventoryConnection())
   {
   ...
   float CurrentWeight = IMedComInventoryInterface::Execute_GetCurrentWeight(GameInventory.GetObject());
   float MaxWeight = IMedComInventoryInterface::Execute_GetMaxWeight(GameInventory.GetObject());
   ...
   {
       FInventoryItemInstance InstanceAtSlot;
   ...
   FGameplayTagContainer AllowedTypes = IMedComInventoryInterface::Execute_GetAllowedItemTypes(GameInventory.GetObject());
