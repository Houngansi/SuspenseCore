...
UMedComUIManager::UMedComUIManager()
{
...
void UMedComUIManager::Initialize(FSubsystemCollectionBase& Collection)
{
...
void UMedComUIManager::Deinitialize()
{
...
void UMedComUIManager::CleanupPreviousSession()
{
...
void UMedComUIManager::DestroyAllWidgets()
{
...
UMedComUIManager* UMedComUIManager::Get(const UObject* WorldContext)
{
    if (!WorldContext)
    {
        return nullptr;
    ...
    return GameInstance->GetSubsystem<UMedComUIManager>();
}

UUserWidget* UMedComUIManager::CreateWidget(
    ...
    bool bForceAddToViewport)
{
    ...
    if (NewWidget->GetClass()->ImplementsInterface(UMedComUIWidgetInterface::StaticClass()))
    {
        IMedComUIWidgetInterface::Execute_SetWidgetTag(NewWidget, WidgetTag);
    }
    else if (UMedComBaseWidget* BaseWidget = Cast<UMedComBaseWidget>(NewWidget))
    {
    ...
    if (NewWidget->GetClass()->ImplementsInterface(UMedComUIWidgetInterface::StaticClass()))
    {
        IMedComUIWidgetInterface::Execute_InitializeWidget(NewWidget);
        
        // If HUD widget, setup with owner
        if (NewWidget->GetClass()->ImplementsInterface(UMedComHUDWidgetInterface::StaticClass()))
        {
            if (APawn* Pawn = PC->GetPawn())
            {
                IMedComHUDWidgetInterface::Execute_SetupForPlayer(NewWidget, Pawn);
            }
        }
    }
...
bool UMedComUIManager::RegisterLayoutWidget(UUserWidget* Widget, FGameplayTag WidgetTag, UUserWidget* ParentLayout)
{
    ...
    // Set widget tag through interface
    if (Widget->GetClass()->ImplementsInterface(UMedComUIWidgetInterface::StaticClass()))
    {
        IMedComUIWidgetInterface::Execute_SetWidgetTag(Widget, WidgetTag);
    }
    else if (UMedComBaseWidget* BaseWidget = Cast<UMedComBaseWidget>(Widget))
    {
        BaseWidget->WidgetTag = WidgetTag;
    }
...
bool UMedComUIManager::UnregisterLayoutWidget(FGameplayTag WidgetTag)
{
    if (!WidgetTag.IsValid())
    {
        return false;
...
UUserWidget* UMedComUIManager::FindWidgetInLayouts(FGameplayTag WidgetTag) const
{
    if (!WidgetTag.IsValid())
    {
        return nullptr;
    ...
    for (const auto& Pair : ActiveWidgets)
    {
        ...
        if (UMedComBaseLayoutWidget* LayoutWidget = Cast<UMedComBaseLayoutWidget>(Pair.Value))
        {
        ...
        if (UMedComUpperTabBar* TabBar = Cast<UMedComUpperTabBar>(Pair.Value))
        {
            int32 CurrentIndex = TabBar->GetCurrentTabIndex();
            if (CurrentIndex >= 0)
            {
                if (UMedComBaseLayoutWidget* ActiveLayout = TabBar->GetTabLayoutWidget(CurrentIndex))
                {
...
UUserWidget* UMedComUIManager::GetWidgetFromLayout(UMedComBaseLayoutWidget* LayoutWidget, FGameplayTag WidgetTag) const
{
...
TArray<FGameplayTag> UMedComUIManager::GetAllWidgetTags() const
{
    TArray<FGameplayTag> AllTags;
    ActiveWidgets.GetKeys(AllTags);
    return AllTags;
...
bool UMedComUIManager::ShowWidget(FGameplayTag WidgetTag, bool bAddToViewport)
{
    ...
    if (Widget->GetClass()->ImplementsInterface(UMedComUIWidgetInterface::StaticClass()))
    {
        IMedComUIWidgetInterface::Execute_ShowWidget(Widget, true);
    }
...
bool UMedComUIManager::HideWidget(FGameplayTag WidgetTag, bool bRemoveFromParent)
{
    UUserWidget* Widget = GetWidget(WidgetTag);
    if (!Widget)
    {
    ...
    if (Widget->GetClass()->ImplementsInterface(UMedComUIWidgetInterface::StaticClass()))
    {
        IMedComUIWidgetInterface::Execute_HideWidget(Widget, true);
    }
    
    return true;
}

bool UMedComUIManager::DestroyWidget(FGameplayTag WidgetTag)
{
...
UUserWidget* UMedComUIManager::GetWidget(FGameplayTag WidgetTag) const
{
...
bool UMedComUIManager::WidgetExists(FGameplayTag WidgetTag) const
{
    return ActiveWidgets.Contains(WidgetTag);
}

UUserWidget* UMedComUIManager::ShowCharacterScreen(UObject* OwningObject, FGameplayTag TabTag)
{
    ...
    GetWorld()->GetTimerManager().SetTimerForNextTick([this, CharacterScreen, TabTag, OwningObject]()
    {
        ...
        UMedComUpperTabBar* TabBar = FindTabBarInCharacterScreen(CharacterScreen);
        if (!TabBar)
        {
        ...
        UMedComBaseLayoutWidget* ActiveLayout = TabBar->GetTabLayoutWidget(CurrentTabIndex);
        ...
        else
        {
            ...
            if (ContentWidget)
            {
                ...
                ActiveLayout = Cast<UMedComBaseLayoutWidget>(ContentWidget);
                if (ActiveLayout)
                {
...
void UMedComUIManager::InitializeBridgesByTabTag(APlayerController* PlayerController, FGameplayTag TabTag)
{
...
void UMedComUIManager::AnalyzeLayoutAndCreateBridges(APlayerController* PlayerController, UMedComBaseLayoutWidget* LayoutWidget)
{
    ...
    for (UUserWidget* Widget : LayoutWidgets)
    {
        ...
        // Инвентарь
        if (Widget->IsA<UMedComInventoryWidget>() || WidgetClassName.Contains(TEXT("Inventory")))
        {
            bNeedsInventoryBridge = true;
        ...
        // Экип
        if (Widget->IsA<UMedComEquipmentContainerWidget>() || WidgetClassName.Contains(TEXT("Equipment")))
        {
            bNeedsEquipmentBridge = true;
    ...
    if (bNeedsInventoryBridge && !InventoryUIBridge)
    {
        ...
        if (InventoryUIBridge)
        {
            ...
            GetWorld()->GetTimerManager().SetTimerForNextTick([this]()
            {
                if (InventoryUIBridge)
                {
                    IMedComInventoryUIBridgeWidget::Execute_RefreshInventoryUI(InventoryUIBridge);
                }
            });
        }
    }
    ...
    if (bNeedsEquipmentBridge && !EquipmentUIBridge)
    {
        ...
        if (EquipmentUIBridge)
        {
            ...
            GetWorld()->GetTimerManager().SetTimerForNextTick([this]()
            {
                if (EquipmentUIBridge)
                {
                    IMedComEquipmentUIBridgeWidget::Execute_RefreshEquipmentUI(EquipmentUIBridge);
                }
            });
        }
    }
...
void UMedComUIManager::SetupUniversalLayoutRefresh(UMedComBaseLayoutWidget* LayoutWidget)
{
    if (!LayoutWidget)
    {
        return;
...
void UMedComUIManager::RefreshAllWidgetsInLayout(UMedComBaseLayoutWidget* LayoutWidget)
{
    if (!LayoutWidget)
    {
        return;
    ...
    for (const FGameplayTag& Tag : WidgetTags)
    {
        ...
        {
            if (Widget->GetClass()->ImplementsInterface(UMedComScreenInterface::StaticClass()))
            {
                IMedComScreenInterface::Execute_RefreshScreenContent(Widget);
            }
        }
    }
...
UMedComUpperTabBar* UMedComUIManager::FindTabBarInCharacterScreen(UUserWidget* CharacterScreen) const
{
    if (!CharacterScreen)
    {
        return nullptr;
    ...
    if (UMedComUpperTabBar* DirectTabBar = Cast<UMedComUpperTabBar>(CharacterScreen))
    {
...
void UMedComUIManager::ConnectEquipmentBridgeToGameComponent(
    UMedComEquipmentUIBridge* Bridge, 
    APlayerController* PlayerController)
{
    ...
    if (APawn* Pawn = PlayerController->GetPawn())
    {
        ...
        {
            // Check for IMedComEquipmentDataProvider interface (new architecture)
            if (Component && Component->GetClass()->ImplementsInterface(UMedComEquipmentDataProvider::StaticClass()))
            {
                TScriptInterface<IMedComEquipmentDataProvider> DataProvider;
                DataProvider.SetObject(Component);
                DataProvider.SetInterface(Cast<IMedComEquipmentDataProvider>(Component));
            ...
            // Fallback: Check for legacy IMedComEquipmentInterface
            if (Component && Component->GetClass()->ImplementsInterface(UMedComEquipmentInterface::StaticClass()))
            {
                TScriptInterface<IMedComEquipmentInterface> EquipInterface;
                EquipInterface.SetObject(Component);
                EquipInterface.SetInterface(Cast<IMedComEquipmentInterface>(Component));
                
                IMedComEquipmentUIBridgeWidget::Execute_SetEquipmentInterface(Bridge, EquipInterface);
...
void UMedComUIManager::ConnectInventoryBridgeToGameComponent(UMedComInventoryUIBridge* Bridge, APlayerController* PlayerController)
{
    if (!Bridge || !PlayerController)
    {
...
void UMedComUIManager::InitializeInventoryBridgeForLayout(
    APlayerController* PlayerController, 
    UMedComBaseLayoutWidget* LayoutWidget)
{
    ...
    GetWorld()->GetTimerManager().SetTimer(InitHandle, [this, PlayerController]()
    {
        ...
        if (UMedComInventoryWidget* InventoryWidget = Cast<UMedComInventoryWidget>(
...
void UMedComUIManager::InitializeEquipmentBridgeForLayout(APlayerController* PlayerController, UMedComBaseLayoutWidget* LayoutWidget)
{
    if (!PlayerController || !LayoutWidget)
    {
...
bool UMedComUIManager::HideCharacterScreen()
{
    return HideWidget(CharacterScreenTag, false);
}

bool UMedComUIManager::IsCharacterScreenVisible() const
{
    UUserWidget* CharacterScreen = GetWidget(CharacterScreenTag);
    return CharacterScreen && CharacterScreen->IsVisible();
}

void UMedComUIManager::CreateAutoCreateWidgets(UObject* OwningObject)
{
    if (!OwningObject)
    {
        return;
    ...
    for (const FMedComWidgetInfo& Config : WidgetConfigurations)
    {
...
void UMedComUIManager::DestroyNonPersistentWidgets()
{
    ...
    for (const auto& Pair : ActiveWidgets)
    {
        if (FMedComWidgetInfo* Config = ConfigurationCache.Find(Pair.Key))
        {
...
UUserWidget* UMedComUIManager::InitializeMainHUD(UObject* OwningObject)
{
...
void UMedComUIManager::RequestHUDUpdate()
{
    // Get main HUD
    ...
    if (HUDWidget->GetClass()->ImplementsInterface(UMedComUIWidgetInterface::StaticClass()))
    {
        IMedComUIWidgetInterface::Execute_UpdateWidget(HUDWidget, 0.0f);
    }
...
bool UMedComUIManager::RegisterExternalWidget(UUserWidget* Widget, FGameplayTag WidgetTag)
{
    if (!Widget || !WidgetTag.IsValid())
    {
        return false;
...
UUserWidget* UMedComUIManager::UnregisterWidget(FGameplayTag WidgetTag)
{
    UUserWidget** WidgetPtr = ActiveWidgets.Find(WidgetTag);
    if (!WidgetPtr || !*WidgetPtr)
    {
...
void UMedComUIManager::BuildConfigurationCache()
{
    ConfigurationCache.Empty();
...
bool UMedComUIManager::IsWidgetValid(UUserWidget* Widget) const
{
    return Widget && IsValid(Widget);
}

void UMedComUIManager::CleanupWidget(UUserWidget* Widget)
{
    ...
    if (Widget->GetClass()->ImplementsInterface(UMedComUIWidgetInterface::StaticClass()))
    {
        IMedComUIWidgetInterface::Execute_UninitializeWidget(Widget);
    }
...
void UMedComUIManager::NotifyWidgetCreated(UUserWidget* Widget, FGameplayTag WidgetTag)
{
    if (UEventDelegateManager* EventManager = GetEventManager())
    {
...
void UMedComUIManager::NotifyWidgetDestroyed(FGameplayTag WidgetTag)
{
    if (UEventDelegateManager* EventManager = GetEventManager())
    {
...
UEventDelegateManager* UMedComUIManager::GetEventManager() const
{
...
APlayerController* UMedComUIManager::GetPlayerControllerFromObject(UObject* Object) const
{
    if (!Object)
    {
        return nullptr;
    ...
    if (AActor* Actor = Cast<AActor>(Object))
    {
        ...
        if (Actor->GetClass()->ImplementsInterface(UMedComControllerInterface::StaticClass()))
        {
            if (APawn* ControlledPawn = IMedComControllerInterface::Execute_GetControlledPawn(Actor))
            {
...
int32 UMedComUIManager::GetZOrderForWidget(FGameplayTag WidgetTag) const
{
    ...
    // Check configuration
    if (const FMedComWidgetInfo* Config = ConfigurationCache.Find(WidgetTag))
    {
...
bool UMedComUIManager::ShouldAutoAddToViewport(FGameplayTag WidgetTag) const
{
...
bool UMedComUIManager::IsRootWidgetTag(FGameplayTag WidgetTag) const
{
...
void UMedComUIManager::SubscribeToLayoutEvents()
{
    if (UEventDelegateManager* EventManager = GetEventManager())
    {
...
void UMedComUIManager::UnsubscribeFromLayoutEvents()
{
    if (UEventDelegateManager* EventManager = GetEventManager())
    {
...
void UMedComUIManager::OnLayoutWidgetCreated(UObject* Source, const FString& EventData)
{
...
void UMedComUIManager::OnLayoutWidgetDestroyed(const FString& EventData)
{
...
UMedComInventoryUIBridge* UMedComUIManager::CreateInventoryUIBridge(APlayerController* PlayerController)
{
    if (!PlayerController)
    {
    ...
    UMedComInventoryUIBridge* Bridge = NewObject<UMedComInventoryUIBridge>(this, 
        UMedComInventoryUIBridge::StaticClass(), 
...
UMedComInventoryUIBridge* UMedComUIManager::GetInventoryUIBridge() const
{
    return InventoryUIBridge;
}

void UMedComUIManager::InitializeInventoryBridge()
{
...
void UMedComUIManager::InitializeEquipmentBridge()
{
    // DO NOT create bridges at startup
    // They will be created on-demand when specific tabs are opened
...
UMedComEquipmentUIBridge* UMedComUIManager::CreateEquipmentUIBridge(APlayerController* PlayerController)
{
    if (!PlayerController)
    {
    ...
    UMedComEquipmentUIBridge* Bridge = NewObject<UMedComEquipmentUIBridge>(
        this,
        UMedComEquipmentUIBridge::StaticClass(),
...
UMedComEquipmentUIBridge* UMedComUIManager::GetEquipmentUIBridge() const
{
    return EquipmentUIBridge;
}

void UMedComUIManager::ShowNotification(const FText& Message, float Duration, FLinearColor Color)
{
        ...
        if (NotificationWidgetClass)
        {
            ...
            if (PC)
            {
                ...
                if (NotificationWidget)
                {
                    ...
                    if (NotificationWidget->GetClass()->ImplementsInterface(UMedComUIWidgetInterface::StaticClass()))
                    {
                        // Передаем текст через специальный метод интерфейса
                        // Это потребует расширения интерфейса IMedComUIWidgetInterface
                    }
...
void UMedComUIManager::ShowNotificationWithIcon(const FText& Message, UTexture2D* Icon, float Duration, FLinearColor Color)
{
...
void UMedComUIManager::ClearAllNotifications()
{
...
FItemUIData UMedComUIManager::ConvertUnifiedItemDataToUI(const FMedComUnifiedItemData& UnifiedData, int32 Quantity) const
{
    FItemUIData UIData;
...
FText UMedComUIManager::GameplayTagToDisplayText(const FGameplayTag& Tag) const
{
    if (!Tag.IsValid())
    {
