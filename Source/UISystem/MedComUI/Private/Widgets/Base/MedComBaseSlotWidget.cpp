...
UMedComBaseSlotWidget::UMedComBaseSlotWidget(const FObjectInitializer& ObjectInitializer)
...
void UMedComBaseSlotWidget::NativeConstruct()
{
...
void UMedComBaseSlotWidget::NativeDestruct()
{
    // Clean up tooltip
    CancelTooltipTimer();
    HideTooltip();
...
void UMedComBaseSlotWidget::NativeTick(const FGeometry& MyGeometry, float InDeltaTime)
{
...
void UMedComBaseSlotWidget::NativeOnMouseEnter(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
...
void UMedComBaseSlotWidget::NativeOnMouseLeave(const FPointerEvent& InMouseEvent)
{
...
FReply UMedComBaseSlotWidget::NativeOnMouseButtonDown(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
    if (bIsLocked)
    {
...
FReply UMedComBaseSlotWidget::NativeOnMouseButtonDoubleClick(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
    if (bIsLocked)
    {
        return FReply::Handled();
...
void UMedComBaseSlotWidget::NativeOnDragDetected(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent, UDragDropOperation*& OutOperation)
{
    ...
    if (OutOperation)
    {
        ...
        UMedComDragDropOperation* MedComOp = Cast<UMedComDragDropOperation>(OutOperation);
...
bool UMedComBaseSlotWidget::NativeOnDrop(const FGeometry& InGeometry, const FDragDropEvent& InDragDropEvent, UDragDropOperation* InOperation)
{
    UMedComDragDropOperation* DragOp = Cast<UMedComDragDropOperation>(InOperation);
...
bool UMedComBaseSlotWidget::NativeOnDragOver(const FGeometry& InGeometry, const FDragDropEvent& InDragDropEvent, UDragDropOperation* InOperation)
{
    UMedComDragDropOperation* DragOp = Cast<UMedComDragDropOperation>(InOperation);
    if (!DragOp || !IsValid(OwningContainer))
    {
...
void UMedComBaseSlotWidget::NativeOnDragEnter(const FGeometry& InGeometry, const FDragDropEvent& InDragDropEvent, UDragDropOperation* InOperation)
{
    ...
    UMedComDragDropOperation* MedComDragOp = Cast<UMedComDragDropOperation>(InOperation);
    if (MedComDragOp)
    {
...
void UMedComBaseSlotWidget::NativeOnDragLeave(const FDragDropEvent& InDragDropEvent, UDragDropOperation* InOperation)
{
...
void UMedComBaseSlotWidget::NativeOnDragCancelled(const FDragDropEvent& InDragDropEvent, UDragDropOperation* InOperation)
{
...
void UMedComBaseSlotWidget::InitializeSlot_Implementation(const FSlotUIData& SlotData, const FItemUIData& ItemData)
{
...
void UMedComBaseSlotWidget::UpdateSlot_Implementation(const FSlotUIData& SlotData, const FItemUIData& ItemData)
{
...
void UMedComBaseSlotWidget::SetSelected_Implementation(bool bInIsSelected)
{
...
void UMedComBaseSlotWidget::SetHighlighted_Implementation(bool bInIsHighlighted, const FLinearColor& HighlightColor)
{
...
void UMedComBaseSlotWidget::SetLocked_Implementation(bool bInIsLocked)
{
...
bool UMedComBaseSlotWidget::CanBeDragged_Implementation() const
{
    return !bIsLocked && !bIsPooled && CurrentSlotData.bIsOccupied && CurrentSlotData.bIsAnchor;
}

FDragDropUIData UMedComBaseSlotWidget::GetDragData_Implementation() const
{
    FGameplayTag SourceContainerType = FGameplayTag::RequestGameplayTag(TEXT("Container.Inventory"));
    if (IsValid(OwningContainer))
    {
        SourceContainerType = IMedComContainerUIInterface::Execute_GetContainerType(OwningContainer);
    }
...
void UMedComBaseSlotWidget::OnDragStarted_Implementation()
{
    bIsDragging = true;
...
void UMedComBaseSlotWidget::OnDragEnded_Implementation(bool bWasDropped)
{
    bIsDragging = false;
...
void UMedComBaseSlotWidget::UpdateDragVisual_Implementation(bool bIsValidTarget)
{
    // Visual feedback handled by drag visual widget and handler
}

FSlotValidationResult UMedComBaseSlotWidget::CanAcceptDrop_Implementation(const UDragDropOperation* DragOperation) const
{
    ...
    return IMedComContainerUIInterface::Execute_CanAcceptDrop(
        OwningContainer,
        DragOperation,
        CurrentSlotData.SlotIndex
    );
...
bool UMedComBaseSlotWidget::HandleDrop_Implementation(UDragDropOperation* DragOperation)
{
    // This should not be called directly - handled by container
    return false;
}

void UMedComBaseSlotWidget::OnDragEnter_Implementation(UDragDropOperation* DragOperation)
{
    // Handled in NativeOnDragEnter
}

void UMedComBaseSlotWidget::OnDragLeave_Implementation()
{
    if (IsValid(OwningContainer))
    {
...
FItemUIData UMedComBaseSlotWidget::GetTooltipData_Implementation() const
{
    return CurrentItemData;
}

bool UMedComBaseSlotWidget::CanShowTooltip_Implementation() const
{
...
float UMedComBaseSlotWidget::GetTooltipDelay_Implementation() const
{
    return TooltipDelay;
}

void UMedComBaseSlotWidget::OnTooltipShown_Implementation()
{
    bIsTooltipActive = true;
}

void UMedComBaseSlotWidget::OnTooltipHidden_Implementation()
{
    bIsTooltipActive = false;
}
...
void UMedComBaseSlotWidget::StartTooltipTimer()
{
    ...
    if (UWorld* World = GetWorld())
    {
        World->GetTimerManager().SetTimer(
            TooltipShowTimer,
            this,
            &UMedComBaseSlotWidget::ShowTooltipDelayed,
            TooltipDelay,
            false
        );
    }
...
void UMedComBaseSlotWidget::CancelTooltipTimer()
{
    if (UWorld* World = GetWorld())
    {
...
void UMedComBaseSlotWidget::ShowTooltipDelayed()
{
...
void UMedComBaseSlotWidget::HideTooltip()
{
    CancelTooltipTimer();
...
void UMedComBaseSlotWidget::UpdateTooltipPosition()
{
...
void UMedComBaseSlotWidget::SetOwningContainer(UMedComBaseContainerWidget* Container)
{
    OwningContainer = Container;
}

void UMedComBaseSlotWidget::SetSnapTarget(bool bIsTarget, float SnapStrength)
{
...
void UMedComBaseSlotWidget::ResetForPool()
{
    // Reset all state
    bIsSelected = false;
    bIsHovered = false;
...
void UMedComBaseSlotWidget::ProcessPendingVisualUpdates()
{
...
void UMedComBaseSlotWidget::UpdateVisualState()
{
    UpdateBackgroundVisual();
    UpdateItemIcon();
    UpdateQuantityText();
...
void UMedComBaseSlotWidget::UpdateBackgroundVisual()
{
    if (!BackgroundBorder)
    {
        return;
...
void UMedComBaseSlotWidget::UpdateItemIcon()
{
    if (!ItemIcon)
    {
        return;
...
void UMedComBaseSlotWidget::UpdateQuantityText()
{
    if (!QuantityText)
    {
        return;
...
void UMedComBaseSlotWidget::UpdateHighlightVisual()
{
...
void UMedComBaseSlotWidget::UpdateSelectionVisual()
{
    if (!SelectionBorder)
    {
        return;
...
FLinearColor UMedComBaseSlotWidget::GetBackgroundColor() const
{
...
void UMedComBaseSlotWidget::HandleClick()
{
    if (IsValid(OwningContainer))
    {
        IMedComContainerUIInterface::Execute_OnSlotClicked(
            OwningContainer, 
            CurrentSlotData.SlotIndex, 
...
void UMedComBaseSlotWidget::HandleDoubleClick()
{
    if (IsValid(OwningContainer))
    {
        IMedComContainerUIInterface::Execute_OnSlotDoubleClicked(
            OwningContainer, 
            CurrentSlotData.SlotIndex, 
            CurrentItemData.ItemInstanceID
        );
...
void UMedComBaseSlotWidget::HandleRightClick()
{
    if (IsValid(OwningContainer))
    {
        IMedComContainerUIInterface::Execute_OnSlotRightClicked(
            OwningContainer, 
            CurrentSlotData.SlotIndex, 
            CurrentItemData.ItemInstanceID
        );
...
UMedComDragDropHandler* UMedComBaseSlotWidget::GetDragDropHandler() const
{
...
UEventDelegateManager* UMedComBaseSlotWidget::GetEventManager() const
{
    if (CachedEventManager)
    {
...
bool UMedComBaseSlotWidget::ValidateWidgetBindings() const
{
    bool bAllValid = true;
...
void UMedComBaseSlotWidget::LoadIconAsync(const FString& IconPath)
{
    ...
    IconStreamingHandle = StreamableManager.RequestAsyncLoad(
        SoftPath,
        FStreamableDelegate::CreateUObject(this, &UMedComBaseSlotWidget::OnIconLoaded),
        FStreamableManager::AsyncLoadHighPriority
    );
}

void UMedComBaseSlotWidget::OnIconLoaded()
{
    if (!IconStreamingHandle.IsValid())
    {
        return;
...
void UMedComBaseSlotWidget::UpdateCachedGeometry(const FGeometry& NewGeometry)
{
...
const FGeometry& UMedComBaseSlotWidget::GetCachedOrCurrentGeometry() const
{
    if (bGeometryCached)
    {
        float CurrentTime = GetWorld() ? GetWorld()->GetTimeSeconds() : 0.0f;
