...
UMedComDragDropOperation::UMedComDragDropOperation()
{
    bWasSuccessful = false;
}

bool UMedComDragDropOperation::InitializeOperation(
    const FDragDropUIData& InDragData, 
    UMedComBaseSlotWidget* InSourceWidget,
    const FVector2D& InDragOffset,
    UMedComDragDropHandler* InHandler)
{
...
bool UMedComDragDropOperation::IsValidOperation() const
{
...
void UMedComDragDropOperation::Drop_Implementation(const FPointerEvent& PointerEvent)
{
    ...
    if (SourceWidget.IsValid() && 
        SourceWidget->GetClass()->ImplementsInterface(UMedComDraggableInterface::StaticClass()))
    {
        IMedComDraggableInterface::Execute_OnDragEnded(SourceWidget.Get(), bWasSuccessful);
    }

    Super::Drop_Implementation(PointerEvent);
}

void UMedComDragDropOperation::DragCancelled_Implementation(const FPointerEvent& PointerEvent)
{
    ...
    if (SourceWidget.IsValid() && 
        SourceWidget->GetClass()->ImplementsInterface(UMedComDraggableInterface::StaticClass()))
    {
        IMedComDraggableInterface::Execute_OnDragEnded(SourceWidget.Get(), false);
    }
    
    Super::DragCancelled_Implementation(PointerEvent);
}

void UMedComDragDropOperation::Dragged_Implementation(const FPointerEvent& PointerEvent)
{
