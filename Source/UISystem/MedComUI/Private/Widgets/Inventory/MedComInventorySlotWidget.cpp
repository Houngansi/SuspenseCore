...
UMedComInventorySlotWidget::UMedComInventorySlotWidget(const FObjectInitializer& ObjectInitializer)
...
void UMedComInventorySlotWidget::NativePreConstruct()
{
...
void UMedComInventorySlotWidget::NativeConstruct()
{
...
void UMedComInventorySlotWidget::InitializeInventorySlot(float InCellSize)
{
    // Update cell size
    if (InCellSize > 0.0f)
    {
...
void UMedComInventorySlotWidget::SetGridCoordinates(int32 X, int32 Y)
{
    GridCoordinates.X = X;
    GridCoordinates.Y = Y;
...
FVector2D UMedComInventorySlotWidget::GetEffectiveIconSize() const
{
...
void UMedComInventorySlotWidget::UpdateItemRarity(const FGameplayTag& ItemRarity)
{
    // Find rarity color
...
void UMedComInventorySlotWidget::RequestVisualUpdate()
{
...
void UMedComInventorySlotWidget::UpdateVisualState()
{
...
void UMedComInventorySlotWidget::UpdateRarityDisplay()
{
    if (!RarityBorder)
    {
        return;
...
void UMedComInventorySlotWidget::UpdateItemIcon()
{
    if (!ItemIcon)
    {
...
void UMedComInventorySlotWidget::UpdateGridCoordinateDisplay()
{
...
void UMedComInventorySlotWidget::UpdateDurabilityDisplay()
{
    if (!DurabilityBar)
    {
        return;
...
void UMedComInventorySlotWidget::UpdateMultiSlotOverlay()
{
    if (!MultiSlotOverlay)
    {
        return;
...
FVector2D UMedComInventorySlotWidget::CalculateMultiSlotIconSize() const
{
...
FVector2D UMedComInventorySlotWidget::CalculateIconOffset(const FVector2D& IconSize) const
{
    // Center icon in slot
...
void UMedComInventorySlotWidget::UpdateCachedIconBrush()
{
    UTexture2D* IconTexture = CurrentItemData.GetIcon();
    if (!IconTexture)
    {
...
bool UMedComInventorySlotWidget::ShouldRenderAsMultiSlot() const
{
...
bool UMedComInventorySlotWidget::NeedsInventoryVisualUpdate() const
{
...
void UMedComInventorySlotWidget::ApplyInventoryVisualProperties(const FInventorySlotVisualProperties& Props)
{
...
void UMedComInventorySlotWidget::ClearInventoryCaches()
{
    CachedInventoryVisuals = FInventorySlotVisualProperties();
    PendingInventoryVisuals = FInventorySlotVisualProperties();
    bIconBrushDirty = true;
...
void UMedComInventorySlotWidget::InitializeSlot_Implementation(const FSlotUIData& SlotData, const FItemUIData& ItemData)
{
    // Call base implementation
...
void UMedComInventorySlotWidget::UpdateSlot_Implementation(const FSlotUIData& SlotData, const FItemUIData& ItemData)
{
    // Check what changed
...
bool UMedComInventorySlotWidget::CanBeDragged_Implementation() const
{
...
FDragDropUIData UMedComInventorySlotWidget::GetDragData_Implementation() const
{
...
FVector2D UMedComInventorySlotWidget::CalculateDragOffset(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent) const
{
    // For multi-slot items, calculate offset from anchor
    if (!CurrentSlotData.bIsAnchor && CurrentSlotData.bIsPartOfItem)
    {
        // Get inventory owner
        UMedComInventoryWidget* InventoryOwner = GetInventoryOwner();
        if (!InventoryOwner)
        {
        ...
        UMedComBaseSlotWidget* AnchorSlotWidget = InventoryOwner->GetSlotWidget(AnchorSlotIndex);
        if (!AnchorSlotWidget)
        {
            return FVector2D(0.5f, 0.5f);
        }
...
float UMedComInventorySlotWidget::CalculateSnapStrength(const FVector2D& DragPosition) const
{
...
bool UMedComInventorySlotWidget::IsWithinSnapRange(const FVector2D& Position) const
{
    if (!SnappingConfig.bEnableSnapping)
    {
        return false;
...
UMedComInventoryWidget* UMedComInventorySlotWidget::GetInventoryOwner() const
{
    return Cast<UMedComInventoryWidget>(OwningContainer);
}

FVector2D UMedComInventorySlotWidget::GridToScreenPosition(const FIntPoint& GridPos) const
{
...
TArray<int32> UMedComInventorySlotWidget::GetNeighboringSlots() const
{
...
bool UMedComInventorySlotWidget::IsAtGridEdge() const
{
    UMedComInventoryWidget* InventoryOwner = GetInventoryOwner();
    if (!InventoryOwner)
    {
