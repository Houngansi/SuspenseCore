...
#include "Widgets/Inventory/MedComInventorySlotWidget.h"
...
UMedComInventoryWidget::UMedComInventoryWidget(const FObjectInitializer& ObjectInitializer)
    : Super(ObjectInitializer)
{
    // Set default slot widget class
    static ConstructorHelpers::FClassFinder<UMedComBaseSlotWidget> SlotWidgetClassFinder(
...
void UMedComInventoryWidget::NativePreConstruct()
{
...
void UMedComInventoryWidget::NativeConstruct()
{
    ...
    if (CloseButton)
    {
        CloseButton->OnClicked.RemoveDynamic(this, &UMedComInventoryWidget::OnCloseButtonClicked);
        CloseButton->OnClicked.AddDynamic(this, &UMedComInventoryWidget::OnCloseButtonClicked);
    }
    
    if (SortButton)
    {
        SortButton->OnClicked.RemoveDynamic(this, &UMedComInventoryWidget::OnSortButtonClicked);
        SortButton->OnClicked.AddDynamic(this, &UMedComInventoryWidget::OnSortButtonClicked);
    }
...
void UMedComInventoryWidget::RequestInventoryRefresh()
{
...
bool UMedComInventoryWidget::ValidateSlotsPanel() const
{
...
FReply UMedComInventoryWidget::NativeOnKeyDown(const FGeometry& InGeometry, const FKeyEvent& InKeyEvent)
{
    // Handle rotation key
...
void UMedComInventoryWidget::InitializeContainer_Implementation(const FContainerUIData& ContainerData)
{
...
void UMedComInventoryWidget::UpdateContainer_Implementation(const FContainerUIData& ContainerData)
{
...
void UMedComInventoryWidget::SetGridDimensions(int32 Columns, int32 Rows)
{
    if (Columns > 0 && Rows > 0)
    {
        GridColumns = Columns;
...
void UMedComInventoryWidget::CreateSlots()
{
    // Redirect to optimized version
    CreateSlotsOptimized();
}

void UMedComInventoryWidget::CreateSlotsOptimized()
{
    ...
    // Validate slot class
    TSubclassOf<UMedComBaseSlotWidget> SlotClass = SlotWidgetClass;
    if (!SlotClass)
    {
        SlotClass = UMedComInventorySlotWidget::StaticClass();
    }
    ...
    for (int32 Y = 0; Y < GridRows; Y++)
    {
        for (int32 X = 0; X < GridColumns; X++)
        {
            ...
            // Create widget slot
            UMedComInventorySlotWidget* NewSlot = CreateWidget<UMedComInventorySlotWidget>(this, SlotClass);
            if (!NewSlot)
            {
            ...
            {
                IMedComSlotUIInterface::Execute_InitializeSlot(NewSlot, SlotData, FItemUIData());
            }
...
void UMedComInventoryWidget::ApplyDifferentialSlotUpdates(const FContainerUIData& ContainerData)
{
    ...
    for (const auto& SlotPair : SlotWidgets)
    {
        ...
        UMedComInventorySlotWidget* SlotWidget = Cast<UMedComInventorySlotWidget>(SlotPair.Value);
        ...
        else
        {
            ...
            if (SlotWidget->GetClass()->ImplementsInterface(UMedComSlotUIInterface::StaticClass()))
            {
                IMedComSlotUIInterface::Execute_UpdateSlot(SlotWidget, SlotData, FItemUIData());
            }
...
void UMedComInventoryWidget::UpdateGridLayoutForMultiSlotItems()
{
    ...
    for (const FItemUIData& Item : CurrentContainerData.Items)
    {
        ...
        if (EffectiveSize.X > 1 || EffectiveSize.Y > 1)
        {
            ...
            if (UMedComInventorySlotWidget* AnchorSlot = Cast<UMedComInventorySlotWidget>(GetSlotWidget(Item.AnchorSlotIndex)))
            {
...
void UMedComInventoryWidget::UpdateSlotWidget(int32 SlotIndex, const FSlotUIData& SlotData, const FItemUIData& ItemData)
{
    // Get slot widget
    UMedComInventorySlotWidget* SlotWidget = Cast<UMedComInventorySlotWidget>(GetSlotWidget(SlotIndex));
    if (!SlotWidget)
    {
...
void UMedComInventoryWidget::UpdateGridSlotSpan(UMedComInventorySlotWidget* SlotWidget, const FItemUIData& ItemData)
{
    if (!SlotWidget)
    {
        return;
    ...
    if (SlotWidget->GetClass()->ImplementsInterface(UMedComSlotUIInterface::StaticClass()))
    {
        SlotIndex = IMedComSlotUIInterface::Execute_GetSlotIndex(SlotWidget);
    }
...
void UMedComInventoryWidget::SetVisibility(ESlateVisibility InVisibility)
{
...
FSlotValidationResult UMedComInventoryWidget::CanAcceptDrop_Implementation(
    ...
    int32 TargetSlotIndex) const
{
    ...
    for (int32 Y = 0; Y < EffectiveSize.Y; Y++)
    {
        for (int32 X = 0; X < EffectiveSize.X; X++)
        {
            ...
            if (const UMedComBaseSlotWidget* SlotWidget = GetSlotWidget(CheckSlotIndex))
            {
                bool bIsOccupied = false;
                if (SlotWidget->GetClass()->ImplementsInterface(UMedComSlotUIInterface::StaticClass()))
                {
                    bIsOccupied = IMedComSlotUIInterface::Execute_IsOccupied(SlotWidget);
                }
                ...
                if (bIsOccupied)
                {
                    ...
                    if (SlotWidget->GetClass()->ImplementsInterface(UMedComSlotUIInterface::StaticClass()))
                    {
                        OccupyingItemID = IMedComSlotUIInterface::Execute_GetItemInstanceID(SlotWidget);
                    }
...
FSmartDropZone UMedComInventoryWidget::FindBestDropZone(
...
bool UMedComInventoryWidget::CalculateOccupiedSlots(
    int32 TargetSlot, 
    FIntPoint ItemSize, 
    bool bIsRotated, 
...
FGameplayTag UMedComInventoryWidget::GetContainerIdentifier_Implementation() const
{
    // Return specific tag for inventory container
    return FGameplayTag::RequestGameplayTag(TEXT("UI.Container.Inventory"));
}

int32 UMedComInventoryWidget::GetAnchorSlotForSlot(int32 SlotIndex) const
{
...
int32 UMedComInventoryWidget::GetSlotIndexFromGridCoords(int32 GridX, int32 GridY) const
{
...
bool UMedComInventoryWidget::GetGridCoordsFromSlotIndex(int32 SlotIndex, int32& OutGridX, int32& OutGridY) const
{
...
bool UMedComInventoryWidget::FindItemAtScreenPosition(const FVector2D& ScreenPosition, int32& OutAnchorSlot) const
{
...
FGridSnapPoint UMedComInventoryWidget::GetBestGridSnapPoint(const FVector2D& ScreenPosition, const FIntPoint& ItemSize) const
{
...
bool UMedComInventoryWidget::IsValidPlacementPosition(int32 GridX, int32 GridY, const FIntPoint& ItemSize) const
{
    // Check bounds
    ...
    for (int32 Y = 0; Y < ItemSize.Y; Y++)
    {
        for (int32 X = 0; X < ItemSize.X; X++)
        {
            ...
            if (const UMedComBaseSlotWidget* SlotWidget = GetSlotWidget(SlotIndex))
            {
                // FIXED: Use Execute_ for interface call
                if (SlotWidget->GetClass()->ImplementsInterface(UMedComSlotUIInterface::StaticClass()))
                {
                    bool bIsOccupied = IMedComSlotUIInterface::Execute_IsOccupied(SlotWidget);
                    if (bIsOccupied)
                    {
                        return false;
                    }
...
bool UMedComInventoryWidget::IsWithinGridBounds(int32 GridX, int32 GridY, int32 ItemWidth, int32 ItemHeight) const
{
    return GridX >= 0 && 
           GridY >= 0 && 
...
void UMedComInventoryWidget::UpdateWeightDisplay()
{
...
void UMedComInventoryWidget::OnCloseButtonClicked()
{
...
void UMedComInventoryWidget::OnSortButtonClicked()
{
    ...
    if (UEventDelegateManager* DelegateManager = GetDelegateManager())
    {
        ...
        IMedComContainerUIInterface::BroadcastSlotInteraction(this, INDEX_NONE, InteractionType);
    }
    
    UE_LOG(LogTemp, Log, TEXT("[%s] Sort requested by user"), *GetName());
}

void UMedComInventoryWidget::RequestRotateSelectedItem()
{
    // Get selected slot
    ...
    if (CurrentSelectedSlot != INDEX_NONE)
    {
        ...
        else if (UMedComBaseSlotWidget* SlotWidget = GetSlotWidget(CurrentSelectedSlot))
        {
            // FIXED: Use Execute_ for interface call
            bool bIsOccupied = false;
            if (SlotWidget->GetClass()->ImplementsInterface(UMedComSlotUIInterface::StaticClass()))
...
void UMedComInventoryWidget::OnInventoryDataUpdated(const FContainerUIData& NewData)
{
    // Use interface to update container
    IMedComContainerUIInterface::Execute_UpdateContainer(this, NewData);
}

void UMedComInventoryWidget::OnRotateItemRequested(int32 SlotIndex)
{
...
void UMedComInventoryWidget::UpdateSlotOccupancyMap()
{
...
void UMedComInventoryWidget::SubscribeToInventoryEvents()
{
   if (UEventDelegateManager* DelegateManager = GetDelegateManager())
   {
...
int32 UMedComInventoryWidget::FindBestFitSlot(int32 TargetSlot, FIntPoint ItemSize, bool bIsRotated) const
{
...
FIntPoint UMedComInventoryWidget::ScreenToGridCoordinates(const FVector2D& ScreenPos) const
{
   if (!InventoryGrid)
   {
...
FBox2D UMedComInventoryWidget::GetGridCellScreenBounds(int32 GridX, int32 GridY) const
{
   if (!InventoryGrid)
   {
...
bool UMedComInventoryWidget::ValidateCriticalComponents() const
{
   bool bValid = true;
...
void UMedComInventoryWidget::AutoBindComponents()
{
   if (!WidgetTree)
   {
       return;
...
float UMedComInventoryWidget::GetDragVisualCellSize() const
{
    // Return actual cell size from inventory grid
    return CellSize > 0.0f ? CellSize : DefaultDragVisualCellSize;
}

void UMedComInventoryWidget::DiagnoseWidget() const
{
   ...
   UPanelWidget* Panel = const_cast<UMedComInventoryWidget*>(this)->GetSlotsPanel();
...
void UMedComInventoryWidget::LogGridPerformanceMetrics() const
{
