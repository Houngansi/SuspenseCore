...
UMedComBaseLayoutWidget::UMedComBaseLayoutWidget(const FObjectInitializer& ObjectInitializer)
...
void UMedComBaseLayoutWidget::NativeConstruct()
{
    Super::NativeConstruct();
}

void UMedComBaseLayoutWidget::NativeDestruct()
{
    ClearCreatedWidgets();
    Super::NativeDestruct();
}

void UMedComBaseLayoutWidget::InitializeWidget_Implementation()
{
...
void UMedComBaseLayoutWidget::UninitializeWidget_Implementation()
{
    ClearCreatedWidgets();
...
bool UMedComBaseLayoutWidget::AddWidgetToLayout_Implementation(UUserWidget* Widget, FGameplayTag SlotTag)
{
    if (!Widget)
    {
...
bool UMedComBaseLayoutWidget::RemoveWidgetFromLayout_Implementation(UUserWidget* Widget)
{
    if (!Widget)
    {
        return false;
...
TArray<UUserWidget*> UMedComBaseLayoutWidget::GetLayoutWidgets_Implementation() const
{
...
void UMedComBaseLayoutWidget::ClearLayout_Implementation()
{
    K2_OnLayoutClearing();
    ClearCreatedWidgets();
}

void UMedComBaseLayoutWidget::RefreshLayout_Implementation()
{
    ...
    for (const auto& Pair : LayoutWidgets)
    {
        if (Pair.Value && Pair.Value->GetClass()->ImplementsInterface(UMedComScreenInterface::StaticClass()))
        {
            IMedComScreenInterface::Execute_RefreshScreenContent(Pair.Value);
        }
    }
...
void UMedComBaseLayoutWidget::InitializeFromConfig()
{
    CreateConfiguredWidgets();
}

UUserWidget* UMedComBaseLayoutWidget::GetWidgetByTag(FGameplayTag InWidgetTag) const
{
...
UUserWidget* UMedComBaseLayoutWidget::CreateWidgetByTag(FGameplayTag InWidgetTag)
{
    if (!InWidgetTag.IsValid())
    {
...
bool UMedComBaseLayoutWidget::HasWidget(FGameplayTag InWidgetTag) const
{
    return InWidgetTag.IsValid() && LayoutWidgets.Contains(InWidgetTag);
}

const FLayoutWidgetConfig* UMedComBaseLayoutWidget::GetWidgetConfig(FGameplayTag InWidgetTag) const
{
    return FindConfigByTag(InWidgetTag);
}

bool UMedComBaseLayoutWidget::ValidateConfiguration() const
{
    return ValidateConfigurationInternal();
}

TArray<FGameplayTag> UMedComBaseLayoutWidget::GetAllWidgetTags() const
{
...
UUserWidget* UMedComBaseLayoutWidget::CreateLayoutWidget(const FLayoutWidgetConfig& Config)
{
    if (!Config.IsValid())
    {
...
void UMedComBaseLayoutWidget::InitializeLayoutWidget(UUserWidget* Widget, const FLayoutWidgetConfig& Config)
{
    if (!Widget)
    {
        return;
    ...
    if (Widget->GetClass()->ImplementsInterface(UMedComUIWidgetInterface::StaticClass()))
    {
        IMedComUIWidgetInterface::Execute_SetWidgetTag(Widget, Config.WidgetTag);
...
void UMedComBaseLayoutWidget::CreateConfiguredWidgets()
{
...
void UMedComBaseLayoutWidget::ClearCreatedWidgets()
{
    for (const auto& Pair : LayoutWidgets)
    {
        if (Pair.Value)
        {
            ...
            // Uninitialize if supported
            if (Pair.Value->GetClass()->ImplementsInterface(UMedComUIWidgetInterface::StaticClass()))
            {
                IMedComUIWidgetInterface::Execute_UninitializeWidget(Pair.Value);
            }
...
bool UMedComBaseLayoutWidget::ValidateConfigurationInternal() const
{
    bool bIsValid = true;
...
bool UMedComBaseLayoutWidget::HasDuplicateTags() const
{
...
const FLayoutWidgetConfig* UMedComBaseLayoutWidget::FindConfigByTag(FGameplayTag InTag) const
{
    if (!InTag.IsValid())
    {
        return nullptr;
...
void UMedComBaseLayoutWidget::RegisterWidgetInUIManager(UUserWidget* Widget, const FGameplayTag& InWidgetTag)
{
...
void UMedComBaseLayoutWidget::UnregisterWidgetFromUIManager(const FGameplayTag& InWidgetTag)
{
    if (!InWidgetTag.IsValid())
    {
        return;
...
void UMedComBaseLayoutWidget::NotifyWidgetCreated(UUserWidget* Widget, const FGameplayTag& InWidgetTag)
{
    if (UEventDelegateManager* EventManager = GetEventManager())
    {
...
void UMedComBaseLayoutWidget::NotifyWidgetDestroyed(const FGameplayTag& InWidgetTag)
{
    if (UEventDelegateManager* EventManager = GetEventManager())
    {
        // Create event data string
...
UMedComUIManager* UMedComBaseLayoutWidget::GetUIManager() const
{
    return UMedComUIManager::Get(this);
}

UEventDelegateManager* UMedComBaseLayoutWidget::GetEventManager() const
{
