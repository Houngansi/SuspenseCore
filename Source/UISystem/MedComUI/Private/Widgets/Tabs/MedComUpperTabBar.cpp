...
UMedComUpperTabBar::UMedComUpperTabBar(const FObjectInitializer& ObjectInitializer)
...
void UMedComUpperTabBar::NativePreConstruct()
{
...
void UMedComUpperTabBar::InitializeWidget_Implementation()
{
    ...
    if (CloseButton)
    {
        CloseButton->OnClicked.AddDynamic(this, &UMedComUpperTabBar::OnCloseButtonClicked);
    }
...
void UMedComUpperTabBar::UninitializeWidget_Implementation()
{
    ...
    if (CloseButton)
    {
        CloseButton->OnClicked.RemoveDynamic(this, &UMedComUpperTabBar::OnCloseButtonClicked);
    }
    ...
    if (CurrentTabIndex >= 0 && ContentWidgets.IsValidIndex(CurrentTabIndex))
    {
        if (UUserWidget* Content = ContentWidgets[CurrentTabIndex])
        {
            ...
            if (UMedComBaseLayoutWidget* LayoutWidget = Cast<UMedComBaseLayoutWidget>(Content))
            {
                LayoutWidget->ClearLayout_Implementation();
            }
            else if (Content->GetClass()->ImplementsInterface(UMedComScreenInterface::StaticClass()))
            {
                IMedComScreenInterface::Execute_OnScreenDeactivated(Content);
            }
        }
    }
...
int32 UMedComUpperTabBar::GetTabCount_Implementation() const
{
    return TabConfigs.Num();
}

int32 UMedComUpperTabBar::GetSelectedTabIndex_Implementation() const
{
    return CurrentTabIndex;
}

bool UMedComUpperTabBar::SelectTabByIndex_Implementation(int32 TabIndex)
{
    ...
    if (OldIndex >= 0 && ContentWidgets.IsValidIndex(OldIndex))
    {
        ...
        {
            // Для layout виджетов особая обработка
            if (UMedComBaseLayoutWidget* OldLayoutWidget = Cast<UMedComBaseLayoutWidget>(OldContent))
            {
                ...
                {
                    if (ChildWidget && ChildWidget->GetClass()->ImplementsInterface(UMedComScreenInterface::StaticClass()))
                    {
                        IMedComScreenInterface::Execute_OnScreenDeactivated(ChildWidget);
                    }
                }
            }
            else if (OldContent->GetClass()->ImplementsInterface(UMedComScreenInterface::StaticClass()))
            {
                IMedComScreenInterface::Execute_OnScreenDeactivated(OldContent);
            }
        }
    }
    ...
    if (ContentWidgets.IsValidIndex(TabIndex) && ContentWidgets[TabIndex])
    {
        ...
        if (UMedComBaseLayoutWidget* LayoutWidget = Cast<UMedComBaseLayoutWidget>(ContentWidgets[TabIndex]))
        {
            ...
            for (UUserWidget* ChildWidget : LayoutWidget->GetLayoutWidgets_Implementation())
            {
                if (ChildWidget && ChildWidget->GetClass()->ImplementsInterface(UMedComScreenInterface::StaticClass()))
                {
                    IMedComScreenInterface::Execute_OnScreenActivated(ChildWidget);
                }
            }
        ...
        else
        {
            ...
            if (ContentWidgets[TabIndex]->GetClass()->ImplementsInterface(UMedComScreenInterface::StaticClass()))
            {
                IMedComScreenInterface::Execute_OnScreenActivated(ContentWidgets[TabIndex]);
            }
        }
        ...
        {
            const FMedComTabConfig& Config = TabConfigs[TabIndex];
            ...
            if (Config.TabTag.MatchesTagExact(FGameplayTag::RequestGameplayTag(TEXT("UI.Tab.Inventory"))))
            {
                ...
                GetWorld()->GetTimerManager().SetTimerForNextTick([this, TabIndex]()
                {
                    ...
                    if (ContentWidgets[TabIndex])
                    {
                        ...
                        else if (ContentWidgets[TabIndex]->GetClass()->ImplementsInterface(UMedComScreenInterface::StaticClass()))
                        {
                            IMedComScreenInterface::Execute_RefreshScreenContent(ContentWidgets[TabIndex]);
                        }
                    }
                });
            }
...
bool UMedComUpperTabBar::SelectTabByTag(FGameplayTag TabTag)
{
...
UUserWidget* UMedComUpperTabBar::GetTabContent_Implementation(int32 TabIndex) const
{
...
void UMedComUpperTabBar::SetTabEnabled_Implementation(int32 TabIndex, bool bEnabled)
{
...
bool UMedComUpperTabBar::IsTabEnabled_Implementation(int32 TabIndex) const
{
    if (TabButtons.IsValidIndex(TabIndex) && TabButtons[TabIndex])
    {
...
void UMedComUpperTabBar::UpdateCharacterLevel(int32 Level, float Experience, float MaxExperience)
{
    // Update level text
    if (LevelText)
    {
...
void UMedComUpperTabBar::RefreshActiveTabContent()
{
    if (CurrentTabIndex >= 0 && ContentWidgets.IsValidIndex(CurrentTabIndex))
    {
        if (UUserWidget* Content = ContentWidgets[CurrentTabIndex])
        {
            ...
            {
                IMedComScreenInterface::Execute_RefreshScreenContent(Content);
            }
        }
    }
}

FMedComTabConfig UMedComUpperTabBar::GetTabConfig(int32 TabIndex) const
{
    if (TabConfigs.IsValidIndex(TabIndex))
    {
        return TabConfigs[TabIndex];
    }
    return FMedComTabConfig();
}

UMedComBaseLayoutWidget* UMedComUpperTabBar::GetTabLayoutWidget(int32 TabIndex) const
{
    if (ContentWidgets.IsValidIndex(TabIndex))
    {
        return Cast<UMedComBaseLayoutWidget>(ContentWidgets[TabIndex]);
    }
    return nullptr;
}

void UMedComUpperTabBar::CreateTabButtons()
{
    TabButtons.Empty();
...
UUserWidget* UMedComUpperTabBar::CreateTabContent(int32 TabIndex)
{
    if (!TabConfigs.IsValidIndex(TabIndex))
    {
        return nullptr;
...
UUserWidget* UMedComUpperTabBar::CreateSingleWidgetContent(const FMedComTabConfig& Config)
{
    ...
    if (Widget)
    {
        ...
        {
            IMedComUIWidgetInterface::Execute_InitializeWidget(Widget);
        }
        ...
        {
            if (UMedComEquipmentContainerWidget* EquipWidget = Cast<UMedComEquipmentContainerWidget>(Widget))
            {
...
UUserWidget* UMedComUpperTabBar::CreateLayoutWidgetContent(const FMedComTabConfig& Config)
{
    ...
    UMedComBaseLayoutWidget* LayoutWidget = CreateWidget<UMedComBaseLayoutWidget>(this, Config.LayoutWidgetClass);
    
    if (LayoutWidget)
    {
        // Инициализируем layout
        if (LayoutWidget->GetClass()->ImplementsInterface(UMedComUIWidgetInterface::StaticClass()))
        {
            IMedComUIWidgetInterface::Execute_InitializeWidget(LayoutWidget);
        }
...
void UMedComUpperTabBar::UpdateTabVisuals()
{
    for (int32 i = 0; i < TabButtons.Num(); i++)
    {
        if (TabButtons[i])
...
void UMedComUpperTabBar::OnTabButtonClicked(int32 TabIndex)
{
    SelectTabByIndex_Implementation(TabIndex);
}

void UMedComUpperTabBar::OnCloseButtonClicked()
{
    K2_OnCloseClicked();
...
UButton* UMedComUpperTabBar::CreateTabButton(const FMedComTabConfig& Config, int32 Index)
{
    // Создаём кнопку
    ...
    Button->OnClicked.AddDynamic(this, &UMedComUpperTabBar::InternalOnTabButtonClicked);
...
void UMedComUpperTabBar::ApplyButtonStyle(UButton* Button, bool bSelected)
{
    if (!Button)
    {
        return;
...
void UMedComUpperTabBar::SubscribeToEvents()
{
    if (UEventDelegateManager* EventManager = GetDelegateManager())
    {
...
void UMedComUpperTabBar::UnsubscribeFromEvents()
{
    if (UEventDelegateManager* EventManager = GetDelegateManager())
    {
...
void UMedComUpperTabBar::BroadcastTabSelectionChanged(int32 OldIndex, int32 NewIndex)
{
...
void UMedComUpperTabBar::OnCharacterDataUpdated()
{
...
void UMedComUpperTabBar::OnScreenActivated(int32 TabIndex)
{
...
void UMedComUpperTabBar::InternalOnTabButtonClicked()
{
...
FGameplayTag UMedComUpperTabBar::GetTabBarTag_Implementation() const
{
    return TabBarTag;
}

FOnTabBarSelectionChanged* UMedComUpperTabBar::GetOnTabSelectionChanged()
{
    return &OnTabSelectionChanged;
}

FOnTabBarClosed* UMedComUpperTabBar::GetOnTabBarClosed()
{
