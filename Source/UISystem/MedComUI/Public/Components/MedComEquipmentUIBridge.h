...
#include "MedComEquipmentUIBridge.generated.h"
 ...
 *  - NO IMedComEquipmentUIPort implementation (removed middleware layer)
 *  - NO dependency on UIConnector component
 *  - Direct communication with DataStore and Widgets
 */
UCLASS(BlueprintType)
class MEDCOMUI_API UMedComEquipmentUIBridge
    : public UObject
    , public IMedComEquipmentUIBridgeWidget
{
    GENERATED_BODY()
...
public:
  ...
  */
 UFUNCTION(BlueprintCallable, Category="UI|Equipment")
 void BindToDataStore(const TScriptInterface<IMedComEquipmentDataProvider>& DataStore);
    ...
    virtual void SetEquipmentInterface_Implementation(const TScriptInterface<IMedComEquipmentInterface>& InEquipment) override;
    virtual TScriptInterface<IMedComEquipmentInterface> GetEquipmentInterface_Implementation() const override;
    virtual UMedComItemManager* GetItemManager_Implementation() const override;

    // Static helpers
    static void RegisterBridge(UMedComEquipmentUIBridge* Bridge);
    static void UnregisterBridge(UMedComEquipmentUIBridge* Bridge);

    /** Optional: operations interface can be injected */
    UFUNCTION(BlueprintCallable, Category="UI|Equipment")
    void SetOperationsInterface(const TScriptInterface<IMedComEquipmentOperations>& InOps) { Operations = InOps; }
...
protected:
     ...
     */
    void HandleDataStoreSlotChanged(int32 SlotIndex, const FInventoryItemInstance& NewItem);
     ...
     */
    void UpdateCachedSlot(int32 SlotIndex, const FInventoryItemInstance& NewItem);
    ...
    bool ConvertItemInstanceToUIData(const FInventoryItemInstance& ItemInstance, FItemUIData& OutUIData) const;
    
    /** Resolve operations interface lazily */
    IMedComEquipmentOperations* ResolveOperations() const;
...
private:
    ...
    UPROPERTY() UMedComUIManager* UIManager = nullptr;
    UPROPERTY() UEventDelegateManager* EventManager = nullptr;
    UPROPERTY() mutable TWeakObjectPtr<UMedComItemManager> CachedItemManager;
    ...
    UPROPERTY() TMap<int32, FInventoryItemInstance> CachedItems;
    ...
    UPROPERTY() TScriptInterface<IMedComEquipmentInterface> GameEquipment;
    UPROPERTY() TScriptInterface<IMedComEquipmentOperations> Operations;
