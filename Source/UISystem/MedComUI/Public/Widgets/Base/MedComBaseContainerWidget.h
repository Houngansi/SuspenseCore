...
#include "MedComBaseContainerWidget.generated.h"

// Forward declarations
class UPanelWidget;
class UMedComBaseSlotWidget;
class UMedComDragDropOperation;
class UMedComDragDropHandler;
...
struct FSlotWidgetPool
{
	...
	UPROPERTY()
	TArray<UMedComBaseSlotWidget*> AvailableSlots;
	...
	UMedComBaseSlotWidget* AcquireSlot(UUserWidget* Outer, TSubclassOf<UMedComBaseSlotWidget> SlotClass);

	/** Return slot to pool */
	void ReleaseSlot(UMedComBaseSlotWidget* Slot);
 ...
 */
UCLASS(Abstract)
class MEDCOMUI_API UMedComBaseContainerWidget : public UUserWidget,
    public IMedComContainerUIInterface,
    public IMedComUIWidgetInterface
{
    GENERATED_BODY()

public:
    UMedComBaseContainerWidget(const FObjectInitializer& ObjectInitializer);
    ...
    virtual TSubclassOf<UMedComDragVisualWidget> GetDragVisualWidgetClass() const;
    ...
    virtual UMedComDragVisualWidget* CreateDragVisualWidget(const FDragDropUIData& DragData);
...
protected:
    ...
    TSubclassOf<UMedComBaseSlotWidget> SlotWidgetClass;
    ...
    UPROPERTY()
    TMap<int32, UMedComBaseSlotWidget*> SlotWidgets;
    ...
    UPROPERTY()
    UMedComDragDropHandler* CachedDragDropHandler;
...
public:
    ...
    UMedComBaseSlotWidget* GetSlotWidget(int32 SlotIndex) const;
    
    /** Get all slot widgets */
    UFUNCTION(BlueprintPure, Category = "Container")
    TArray<UMedComBaseSlotWidget*> GetAllSlotWidgets() const;
    
    /** Get slot at screen position */
    UFUNCTION(BlueprintPure, Category = "Container")
    UMedComBaseSlotWidget* GetSlotAtScreenPosition(const FVector2D& ScreenPosition) const;
    
    /** Get slots in region */
    UFUNCTION(BlueprintCallable, Category = "Container")
    TArray<UMedComBaseSlotWidget*> GetSlotsInRegion(const FVector2D& Center, float Radius) const;
...
protected:
    ...
    UMedComDragDropHandler* GetDragDropHandler() const;
    ...
    TSubclassOf<UMedComDragVisualWidget> DragVisualWidgetClass;
...
private:
    ...
    UMedComBaseSlotWidget* CreateOrAcquireSlot();
    
	/** Return slot to pool */
	void ReleaseSlot(UMedComBaseSlotWidget* SlotWidget);
};